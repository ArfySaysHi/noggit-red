...
void use_program::uniform(GLint pos, GLint value) { gl.uniform1i(pos, value); }
...
void use_program::bind_uniform_block(std::string const &name, unsigned target) {
  gl.uniformBlockBinding(_program._handle.value(), uniform_block_location(name),
                         target);
}
...
void use_program::uniform(std::string const &name, int const *data,
                          std::size_t size) {
  GLuint loc = uniform_location(name);
  if (loc < 0)
    return;
...
void use_program::uniform(std::string const &name, glm::vec3 const *data,
                          std::size_t size) {
  GLuint loc = uniform_location(name);
  if (loc < 0)
    return;

  gl.uniform3fv(loc, static_cast<GLsizei>(size),
...
void use_program::uniform(std::string const &name,
                          std::vector<glm::vec3> const &value) {
  GLuint loc = uniform_location(name);
  if (loc < 0)
    return;

  gl.uniform3fv(loc, static_cast<GLsizei>(value.size()),
                glm::value_ptr(value[0]));
}
void use_program::uniform(std::string const &name,
                          std::vector<glm::vec4> const &value) {
  GLuint loc = uniform_location(name);
  if (loc < 0)
    return;

  gl.uniform4fv(loc, static_cast<GLsizei>(value.size()),
                glm::value_ptr(value[0]));
}
void use_program::uniform_chunk_textures(
    std::string const &name,
    std::array<std::array<std::array<int, 2>, 4>, 256> const &value) {
  GLuint loc = uniform_location(name);
  if (loc < 0)
    return;
...
void use_program::uniform(GLint pos, std::vector<glm::vec3> const &value) {
  gl.uniform3fv(pos, static_cast<GLsizei>(value.size()),
                glm::value_ptr(value[0]));
}
void use_program::uniform(GLint pos, std::vector<glm::vec4> const &value) {
  gl.uniform4fv(pos, static_cast<GLsizei>(value.size()),
                glm::value_ptr(value[0]));
}
...
void use_program::attrib(std::string const &name,
                         std::vector<glm::vec2> const &data) {
  GLuint const location(attrib_location(name));
  gl.enableVertexAttribArray(location);
  _enabled_vertex_attrib_arrays.emplace(location);
...
void use_program::attrib(std::string const &name, glm::mat4x4 const *data,
                         GLuint divisor) {
  GLuint const location(attrib_location(name));
  ...
  for (GLuint i = 0; i < 4; ++i) {
    ...
    gl.vertexAttribPointer(location + i, 4, GL_FLOAT, GL_FALSE,
                           sizeof(glm::mat4x4), vec4_ptr + i);
    gl.vertexAttribDivisor(location + i, divisor);
  }
}
void use_program::attrib(std::string const &name, GLsizei size, GLenum type,
...
void use_program::attrib(std::string const &name, GLuint buffer, GLsizei size,
...
void use_program::attribi(std::string const &name, GLsizei size, GLenum type,
...
void use_program::attribi(std::string const &name, GLuint buffer, GLsizei size,
                          GLenum type, GLsizei stride, const GLvoid *data) {
  GLuint const location(attrib_location(name));
  gl.enableVertexAttribArray(location);
  _enabled_vertex_attrib_arrays.emplace(location);
  Scoped::buffer_binder<GL_ARRAY_BUFFER> const bind(buffer);
...
void use_program::attrib_divisor(std::string const &name, GLuint divisor,
                                 GLsizei range) {
  GLuint const location(attrib_location(name));
...
GLuint use_program::uniform_block_location(std::string const &name) {
  ...
  if (loc == -1) {
    throw std::invalid_argument("uniform block " + name +
