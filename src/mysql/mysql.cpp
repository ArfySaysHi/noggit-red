...
std::unique_ptr<sql::Connection> connect() {
  ...
  // if using release SQL binaries in debug mode it will crash
  // https://bugs.mysql.com/bug.php?id=91238 unless using sql strings
  // tcp://127.0.0.1:3306
  sql::SQLString hostname =
      "tcp://" +
      settings.value("project/mysql/server").toString().toStdString() + ":" +
      settings.value("project/mysql/port", "3306").toString().toStdString();
  ...
  sql::SQLString schema =
      settings.value("project/mysql/db").toString().toStdString();

  try {
    std::unique_ptr<sql::Connection> Con(
        get_driver_instance()->connect(hostname, userName, password));

    // crete database if it doesn't exist
    std::string createdb_statement = "CREATE DATABASE IF NOT EXISTS " + schema;
    std::unique_ptr<sql::PreparedStatement> dbpstmt(
        Con->prepareStatement(createdb_statement));
    std::unique_ptr<sql::ResultSet> res(dbpstmt->executeQuery());

    Con->setSchema(schema);

    // create table if it doesn't exist, querries from src/sql
    std::unique_ptr<sql::PreparedStatement> tablepstmt(
        Con->prepareStatement("CREATE TABLE IF NOT EXISTS `UIDs` ("
...
bool testConnection(bool report_only_err) {
  ...
  try {
    ...
    prompt.setText(
        "Failed to load MySQL database, check your settings. \nIf you did not "
...
bool hasMaxUIDStoredDB(std::size_t mapID) {
  ...
  std::unique_ptr<sql::PreparedStatement> pstmt(
      Con->prepareStatement("SELECT * FROM `UIDs` WHERE `_map_id`=(?)"));
...
std::uint32_t getGUIDFromDB(std::size_t mapID) {
  ...
  if (Con == nullptr)
    return 0;
  std::unique_ptr<sql::PreparedStatement> pstmt(
      Con->prepareStatement("SELECT `UID` FROM `UIDs` WHERE `_map_id`=(?)"));
  pstmt->setInt(1, mapID);
  std::unique_ptr<sql::ResultSet> res(pstmt->executeQuery());
...
void insertUIDinDB(std::size_t mapID, std::uint32_t NewUID) {
  ...
  if (Con == nullptr)
    return;
  std::unique_ptr<sql::PreparedStatement> pstmt(
      Con->prepareStatement("INSERT INTO `UIDs` SET `_map_id`=(?), `UID`=(?)"));
  pstmt->setInt(1, mapID);
...
void updateUIDinDB(std::size_t mapID, std::uint32_t NewUID) {
  ...
  if (Con == nullptr)
    return;
  std::unique_ptr<sql::PreparedStatement> pstmt(
      Con->prepareStatement("UPDATE `UIDs` SET `UID`=(?) WHERE `_map_id`=(?)"));
