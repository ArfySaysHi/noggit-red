...
WDBCTableReader::WDBCTableReader(
    ...
    : _streamReader(streamReader), _versionDefinition(versionDefinition) {
  _stringTable = std::map<long, std::string>();
}

WDBCTableReader::~WDBCTableReader() { _streamReader.reset(); }
...
void WDBCTableReader::LoadTableStructure() {
  ...
  if (length < headerSize) {
    std::cout << "Error Occured While Parsing WDC3 Header, Header to short."
              << std::endl;
    return;
  }
  ...
  if (magicNumber != Flag::TableFormatSignatures::WDBC_FMT_SIGNATURE) {
    std::cout << "Error Occured While Parsing WDC3 Header, Format Signature "
                 "doesnt Match."
              << std::endl;
    return;
  }
  ...
  _recordData =
      _streamReader->ReadBlock(Header.RecordsCount * Header.RecordSize);
...
Structures::BlizzardDatabaseRow WDBCTableReader::Record(unsigned int index) {
  ...
  memcpy(recordDataBlock.get(), _recordData.get() + recordDataOffset,
         Header.RecordSize);
  ...
  for (auto i = 1; i < _versionDefinition.versionDefinitions.definitions.size();
       i++) {
    auto definition = _versionDefinition.versionDefinitions.definitions[i];
    auto columnDefinition =
        _versionDefinition.columnDefinitions.at(definition.name);
...
std::size_t WDBCTableReader::RecordCount() { return Header.RecordsCount; }

std::vector<Structures::BlizzardDatabaseRowDefiniton>
WDBCTableReader::RecordDefinition() {
  auto recordDefinition =
      std::vector<Structures::BlizzardDatabaseRowDefiniton>();
  for (auto &columnInformation :
       _versionDefinition.versionDefinitions.definitions) {
    auto column = Structures::BlizzardDatabaseRowDefiniton();
    column.Type =
        _versionDefinition.columnDefinitions[columnInformation.name].type;
