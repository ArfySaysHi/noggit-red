...
void FlowView::setScene(FlowScene *scene) {
  ...
  connect(_scene, &FlowScene::changed, ([=, this] { Q_EMIT changed(); }));
}
...
void FlowView::contextMenuEvent(QContextMenuEvent *event) {
  ...
  auto depthSearch = [](QTreeWidgetItem *item, const QString &text) -> void {
    auto search = [](QTreeWidgetItem *item, const QString &text,
  ...
  // Setup filtering
  connect(txtBox, &QLineEdit::textChanged, this,
          [topLevelItems, depthSearch](const QString &text) {
