...
enum class StorageType : char { MPQ = 0, CASC = 1 };

enum class OpenMode { LOCAL, REMOTE };
...
public:
  /*
   * path - path to game directory for MPQ-based clients, path to storage
   * directory (the one containing .build.info) for CASC-based clients. CDN URL
   * for online CASC Storages. version - version of the game client. Currently
   * only WotLK and Shadowlands are supported. locale - prefered locale of the
   * client. Wotlk supports automatic detection, for that use Locale::AUTO
   * local_path - project directory, should also contain listfile.csv for
   * CASC-based projects.
   */
  explicit ClientData(std::string const &path, ClientVersion version,
  ...
  /* Methods used to universally request client file data in an archive type
   * agnostic way. */
...
public:
  inline static constexpr std::array<std::string_view, 10> Locales{
      "enGB", "enUS", "deDE", "koKR", "frFR",
