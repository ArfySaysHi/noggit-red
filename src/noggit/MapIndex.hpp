...
enum class uid_fix_status { done, done_with_errors, failed };
...
class MapIndex {
public:
  template <bool Load> struct tile_iterator {
  ...
  template <bool Load> struct TileRange {
  ...
  template <bool Load>
  auto tiles(std::function<bool(TileIndex const &, MapTile *)> pred =
                 [](TileIndex const &, MapTile *) { return true; }) {
    return TileRange<Load>(tile_iterator<Load>{this, {0, 0}, pred},
                           tile_iterator<Load>{});
  }
  ...
  auto tiles_in_rect(glm::vec3 const &pos, float radius) {
    ...
    return tiles<true>([this, pos, radius, l_chunk,
                        r_chunk](TileIndex const &index, MapTile *) {
  ...
  MapIndex(const std::string &pBasename, int map_id, World *,
           Noggit::NoggitRenderContext context, bool create_empty = false);

  void setBasename(const std::string &pBasename);

  void createEmptyWdl();

  void enterTile(const TileIndex &tile);
  MapTile *loadTile(const TileIndex &tile, bool reloading = false,
                    bool load_models = true, bool load_textures = true);

  void updateModelTile(const TileIndex &tile, ModelUpdate type,
  ...
  bool hasUnsavedChanges(const TileIndex &tile) const;
  ...
  void unloadTiles(const TileIndex &tile); // unloads all tiles more then x adts
                                           // away from given
  ...
  bool sortModelsBySizeClass() const { return _sortModelsBySizeClass; }
  void setSortModelsBySizeClass(bool state) {
    _sortModelsBySizeClass = state;
  }
  ...
  inline bool useMclqGreenLava() const { return _map_id == 530; }

  bool uidFixAllInProgress() const { return _uidFixAllInProgress; }
...
private:
  ...
  bool _uidFixAllInProgress = false;
