...
TextureSet::TextureSet(MapChunk *chunk, BlizzardArchive::ClientFile *f,
  ...
  std::copy(header.doodadMapping, header.doodadMapping + 8,
            _doodadMapping.begin());
  std::copy(header.doodadStencil, header.doodadStencil + 8,
            _doodadStencil.begin());
  ...
  if (nTextures) {
    ...
    for (unsigned int layer = 0; layer < nTextures; ++layer) {
      if (_layers_info[layer].flags & 0x100) {
        ...
        alphamaps[layer - 1].emplace(f, _layers_info[layer].flags,
                                     use_big_alphamaps, do_not_fix_alpha_map);
      }
    }
...
bool TextureSet::stampTexture(float xbase, float zbase, float x, float z,
                              Brush *brush, float strength, float pressure,
                              scoped_blp_texture_reference texture,
                              QImage *image, bool paint) {
  ...
  for (int j = 0; j < 64; j++) {
    xPos = xbase;
    for (int i = 0; i < 64; ++i) {
      if (std::abs(x - (xPos + TEXDETAILSIZE / 2.0)) > radius ||
          std::abs(z - (zPos + TEXDETAILSIZE / 2.0f)) > radius) {
        xPos += TEXDETAILSIZE;
        continue;
      }
      ...
      glm::vec3 const diff{glm::vec3{xPos + TEXDETAILSIZE / 2.0f, 0.f,
                                     zPos + TEXDETAILSIZE / 2.0f} -
                           glm::vec3{x, 0.f, z}};

      int pixel_x =
          std::round(((diff.x + radius) / (2.f * radius)) * image->width());
      int pixel_y =
          std::round(((diff.z + radius) / (2.f * radius)) * image->height());
      ...
      double alpha_change =
          image_factor * (strength - current_alpha) * pressure;
      ...
      if (sum_other_alphas < 1.) {
        // alpha is currently at 254/255 -> set it at 255 and clear the rest of
        // the values
        if (alpha_change > 0.f) {
          for (int layer = 0; layer < nTextures; ++layer) {
        ...
        // all the other textures amount for less an 1/255 -> add the
        // alpha_change (negative) to current texture and remove it from the
        // first non current texture, clear the rest
        else {
        ...
        for (int layer = 0; layer < nTextures; ++layer) {
          if (layer == tex_layer) {
            ...
            alpha_values[layer] -=
                alpha_change * alpha_values[layer] / sum_other_alphas;
      ...
      double total_final =
          std::accumulate(alpha_values.begin(), alpha_values.end(), 0.);
  ...
  for (int j = 0; j < 64; j++)
  {
    ...
    for (int i = 0; i < 64; ++i)
    {
      if(std::abs(x - (xPos + TEXDETAILSIZE / 2.0)) > radius || std::abs(z -
  (zPos + TEXDETAILSIZE / 2.0f)) > radius) continue;

      dist = misc::dist(x, z, xPos + TEXDETAILSIZE / 2.0f, zPos + TEXDETAILSIZE
  / 2.0f);

      glm::vec3 const diff{glm::vec3{xPos + TEXDETAILSIZE / 2.0f, 0.f, zPos +
  TEXDETAILSIZE / 2.0f} - glm::vec3{x, 0.f, z}};

      int pixel_x = std::floor(diff.x + radius);
      int pixel_y = std::floor(diff.z + radius);

      auto color = image->pixelColor(pixel_x, pixel_y);
      float image_factor = (color.redF() + color.greenF() + color.blueF())
  / 3.0f;
      ...
      float alpha_change = image_factor * ((strength - current_alpha) * pressure
  * brush->getValue(dist));
      ...
      for (int layer = 0; layer < nTextures; ++layer)
      {
        ...
        {
          amaps[layer][offset] -= alpha_change * (amaps[layer][offset] /
  sum_other_alphas);
        }
  ...
  for (int j = 0; j < 64; j++) {
    ...
    for (int i = 0; i < 64; ++i) {
      ...
      if (dist <= radius) {
        ...
        double sum_other_alphas = (total - current_alpha);
        double alpha_change =
            (strength - current_alpha) * pressure * brush->getValue(dist);
        ...
        if (!misc::float_equals(current_alpha, strength)) {
          if (sum_other_alphas < 1.) {
            // alpha is currently at 254/255 -> set it at 255 and clear the rest
            // of the values
            if (alpha_change > 0.f) {
              for (int layer = 0; layer < nTextures; ++layer) {
                alpha_values[layer] = layer == tex_layer ? 255. : 0.f;
...
bool TextureSet::replace_texture(
    ...
    scoped_blp_texture_reference const &texture_to_replace,
    scoped_blp_texture_reference replacement_texture, bool entire_chunk) {
...
void TextureSet::alphas_to_big_alpha(uint8_t *dest) {
  auto alpha([&](int layer, int pos = 0) { return dest + layer * 4096 + pos; });
...
void TextureSet::uploadAlphamapData() {
  // This method assumes tile's alphamap storage is currently bound to the
...
current_layer_values(std::uint8_t nTextures,
                     std::optional<Alphamap> const *alphamaps, std::size_t pz,
                     std::size_t px) {
...
void TextureSet::update_lod_texture_map() {
  ...
  for (int z = 0; z < 8; ++z) {
    for (int x = 0; x < 8; ++x) {
      misc::max_capacity_stack_vector<std::size_t, 4> dominant_square_count(
          nTextures);

      for (int pz = z * 8; pz < (z + 1) * 8; ++pz) {
        for (int px = x * 8; px < (x + 1) * 8; ++px) {
          ++dominant_square_count[max_element_index(current_layer_values(
              static_cast<std::uint8_t>(nTextures), alphamaps.data(), pz, px))];
        }
      }
...
bool TextureSet::apply_alpha_changes() {
  ...
  for (int alpha_layer = 0; alpha_layer < nTextures - 1; ++alpha_layer) {
    ...
    for (int i = 0; i < 64 * 64; ++i) {
      ...
      // max 2 if all 4 values round up so it won't change the layer's alpha
      // much
      if (totals[i] > 255) {
  ...
  // After applying alpha changes, update doodad mapping
  // For each of the 64 subchunks (8x8 grid), find the dominant layer
  for (int row = 0; row < 8; ++row) {
    _doodadMapping[row] = 0;
    for (int col = 0; col < 8; ++col) {
      // Average alpha over the 8x8 texels for this subchunk
      float best = -1.f;
      int best_layer = 0;
      for (int layer = 0; layer < (int)nTextures; ++layer) {
        float sum = 0.f;
        for (int dy = 0; dy < 8; ++dy)
          for (int dx = 0; dx < 8; ++dx)
            sum += tmp_edit_values
                       .value()[layer][(row * 8 + dy) * 64 + (col * 8 + dx)];
        if (sum > best) {
          best = sum;
          best_layer = layer;
        }
      }
      // Pack 2-bit layer index into the uint16_t
      _doodadMapping[row] |= (uint16_t)(best_layer & 3) << (col * 2);
    }
  }
