...
bool World::IsEditableWorld(
  ...
  auto wdt_path =
      BlizzardArchive::ClientData::normalizeFilenameInternal(ssfilename.str());

  if (!Noggit::Application::NoggitApplication::instance()->clientData()->exists(
          wdt_path)) {
    Log << "World " << record.RecordId << ": " << lMapName
        << " has no WDT file!" << std::endl;
    return false;
  }

  BlizzardArchive::ClientFile mf(
      wdt_path,
...
World::World(const std::string &name, int map_id,
    ...
    : _renderer(Noggit::Rendering::WorldRender(this)),
      _model_instance_storage(this), _tile_update_queue(this),
      mapIndex(name, map_id, this, context, create_empty),
      horizon(name, &mapIndex), mWmoFilename(mapIndex.globalWMOName),
      mWmoEntry(mapIndex.wmoEntry), animtime(0), time(1450), basename(name),
      _current_selection(), _settings(new QSettings()), _context(context) {
  LogDebug << "Loading world \"" << name << "\"." << std::endl;
  _loaded_tiles_buffer[0] = std::make_pair<std::pair<int, int>, MapTile *>(
      std::make_pair(0, 0), nullptr);
}

void World::LoadSavedSelectionGroups() {
  _selection_groups.clear();

  auto &saved_map_groups =
      Noggit::Project::CurrentProject::get()->ObjectSelectionGroups;
...
void World::saveSelectionGroups() {
  auto proj_selection_map_group =
      Noggit::Project::NoggitProjectSelectionGroups();
  proj_selection_map_group.MapId = mapIndex._map_id;
  for (auto &selection_group : _selection_groups) {
    proj_selection_map_group.SelectionGroups.push_back(
        selection_group.getMembers());
  }

  Noggit::Project::CurrentProject::get()->saveObjectSelectionGroups(
      proj_selection_map_group);
}
...
bool World::is_selected(selection_type selection) const {
  ...
  if (which == eMODEL) {
    uint uid =
        static_cast<ModelInstance *>(std::get<selected_object_type>(selection))
            ->uid;
    auto const &it = std::find_if(
        ...
        [uid](selection_type type) {
          return var_type(type) == typeid(selected_object_type) &&
                 ...
                 static_cast<ModelInstance *>(
                     std::get<selected_object_type>(type))
                         ->uid == uid;
        });
    ...
    uint uid =
        static_cast<WMOInstance *>(std::get<selected_object_type>(selection))
            ->uid;
    auto const &it = std::find_if(
        _current_selection.begin(), _current_selection.end(),
        [uid](selection_type type) {
...
std::optional<selection_type> World::get_last_selected_model() const {
  ...
  return it == _current_selection.rend() ? std::optional<selection_type>()
...
glm::vec3 getBarycentricCoordinatesAt(const glm::vec3 &a, const glm::vec3 &b,
                                      const glm::vec3 &c,
...
void World::rotate_selected_models_randomly(float minX, float maxX, float minY,
...
void World::rotate_selected_models_to_ground_normal(bool smoothNormals) {
  ...
  for (auto &entry : _current_selection) {
    ...
    // !\ todo We shouldn't end up with empty ever (but we do, on completely
    // flat ground)
    if (results.empty()) {
    ...
    // We hit the terrain, now we take the normal of this position and use it to
    // get the rotation we want.
    auto const &hitChunkInfo =
        std::get<selected_chunk_type>(results.front().second);
    ...
    // Surface Normal
    auto &p0 =
        hitChunkInfo.chunk->mVertices[std::get<0>(hitChunkInfo.triangle)];
    auto &p1 =
        hitChunkInfo.chunk->mVertices[std::get<1>(hitChunkInfo.triangle)];
    auto &p2 =
        hitChunkInfo.chunk->mVertices[std::get<2>(hitChunkInfo.triangle)];
    ...
    {
      auto normalWeights = getBarycentricCoordinatesAt(
          p0, p1, p2, hitChunkInfo.position, varnormal);

      auto &tile_buffer = hitChunkInfo.chunk->mt->getChunkHeightmapBuffer();
      int chunk_start = (hitChunkInfo.chunk->px * 16 + hitChunkInfo.chunk->py) *
                        mapbufsize * 4;

      const auto &vNormal0 = *reinterpret_cast<glm::vec3 *>(
          &tile_buffer[chunk_start + std::get<0>(hitChunkInfo.triangle) * 4]);
      const auto &vNormal1 = *reinterpret_cast<glm::vec3 *>(
          &tile_buffer[chunk_start + std::get<1>(hitChunkInfo.triangle) * 4]);
      const auto &vNormal2 = *reinterpret_cast<glm::vec3 *>(
          &tile_buffer[chunk_start + std::get<2>(hitChunkInfo.triangle) * 4]);

      varnormal.x = vNormal0.x * normalWeights.x +
                    vNormal1.x * normalWeights.y + vNormal2.x * normalWeights.z;

      varnormal.y = vNormal0.y * normalWeights.x +
                    vNormal1.y * normalWeights.y + vNormal2.y * normalWeights.z;

      varnormal.z = vNormal0.z * normalWeights.x +
                    vNormal1.z * normalWeights.y + vNormal2.z * normalWeights.z;
    }
    ...
    if (std::abs(sinp) >= 1)
      new_dir.y = std::copysign(math::constants::pi / 2, sinp) * 180.0f /
   math::constants::pi; // use 90 degrees if out of range else new_dir.y =
   std::asin(sinp) * 180.0f / math::constants::pi;
    ...
    if (!skip_group) {
      ...
      for (auto &group : _selection_groups) {
        if (group.contains_object(obj)) {
          // this then calls add_to_selection() with skip_group = true to avoid
          // repetition
          group.select_group();
          break;
        }
  ...
  if (position != _current_selection.end()) {
    if (entry.index() == eEntry_Object) {
      ...
      if (!skip_group) {
        ...
        for (auto &group : _selection_groups) {
          if (group.contains_object(obj)) {
            // this then calls remove_from_selection() with skip_group = true to
            // avoid repetition
...
void World::set_model_pos(selection_type entry, glm::vec3 const &pos,
                          bool change_height) {
  ZoneScoped;
  auto type = entry.index();
  if (type == eEntry_MapChunk) {
...
void World::rotate_selected_models(math::degrees rx, math::degrees ry,
  ...
  for (auto &entry : _current_selection) {
    ...
    if (use_pivot && has_multi_select) {
      ...
      glm::quat rotationQuat = glm::quat(glm::vec3(
          glm::radians(rx._), glm::radians(ry._), glm::radians(rz._)));
...
MapChunk *World::getChunkAt(glm::vec3 const &pos) {
  ...
  if (tile && tile->finishedLoading()) {
    return tile->getChunk((pos.x - tile->xbase) / CHUNKSIZE,
...
bool World::isInIndoorWmoGroup(std::array<glm::vec3, 2> obj_bounds,
                               glm::mat4x4 obj_transform) {
  bool is_indoor = false;
  // check if model bounds is within wmo bounds then check each indor wmo group
  // bounds
...

    {
      for (int i = 0; i < (int)wmo_instance.wmo->groups.size(); ++i) {
        ...
        if (group->is_indoor()) {
          ...
          // TODO : do a precise calculation instead of using axis aligned
          // bounding boxes.
...
unsigned int World::getAreaID(glm::vec3 const &pos) {
  ZoneScoped;
  return for_maybe_chunk_at(pos,
                            [&](MapChunk *chunk) { return chunk->getAreaID(); })
      .value_or(-1);
}
...
void World::clearHeight(glm::vec3 const &pos) {
  ...
  for_all_chunks_on_tile(pos, [this](MapChunk *chunk) { recalc_norms(chunk); });
}
...
void World::changeShader(glm::vec3 const &pos, glm::vec4 const &color,
...
void World::stampShader(glm::vec3 const &pos, glm::vec4 const &color,
                        float change, float radius, bool editMode, QImage *img,
                        bool paint, bool use_image_colors) {
  ZoneScoped;
  for_all_chunks_in_rect(pos, radius, [&](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkVertexColorChange(chunk);
...
auto World::stamp(glm::vec3 const &pos, float dt, QImage const *img,
  ...
  for_all_chunks_in_rect(
      ...
      [this](MapChunk *chunk) -> void {
        ...
        auto get_neighbor = [this, chunk](int px, int py) -> MapChunk * {
          MapChunk *neighbor{};

          int new_chunk_x = px + chunk->px;
          int new_chunk_z = py + chunk->py;

          if (new_chunk_x < 0 || new_chunk_z < 0 || new_chunk_x == 16 ||
              new_chunk_z == 16) {
...
void World::changeObjectsWithTerrain(glm::vec3 const &pos, float change,
                                     float radius, int BrushType,
                                     float inner_radius, bool iter_wmos_,
                                     bool iter_m2s) {
  // applies the terrain brush to the terrain objects hit
  ZoneScoped;

  // Identical code to chunk->changeTerrain()
  //    if (_snap_m2_objects_chkbox->isChecked() ||
    ...
    if (BrushType == eTerrainType_Quadra) {
      ...
      if (dist < radius) {
        ...
        case eTerrainType_Gaussian:
          dt = dist < radius * inner_radius
                   ? dt * std::exp(
                              -(std::pow(radius * inner_radius / radius, 2) /
                                (2 * std::pow(0.39f, 2))))
                   : dt * std::exp(-(std::pow(dist / radius, 2) /
                                     (2 * std::pow(0.39f, 2))));

          break;
        default:
          LogError << "Invalid terrain edit type (" << inner_radius << ")"
                   << std::endl;
          changed = false;
          break;
        }
...
void World::changeTerrain(glm::vec3 const &pos, float change, float radius,
...
std::vector<selected_object_type>
World::getObjectsInRange(glm::vec3 const &pos, float radius, bool ignore_height,
                         bool iter_wmos_, bool iter_m2s) {
  ...
  if (iter_m2s) {
    _model_instance_storage.for_each_m2_instance(
        [&](ModelInstance &model_instance) {
          ...
          if (glm::distance(obj_pos, pos) <=
              radius) // this is just origin point
          {
...
void World::flattenTerrain(glm::vec3 const &pos, float remain, float radius,
                           int BrushType, flatten_mode const &mode,
                           const glm::vec3 &origin, math::degrees angle,
  ...
  for_all_chunks_in_range(
      ...
      [&](MapChunk *chunk) {
        NOGGIT_CUR_ACTION->registerChunkTerrainChange(chunk);
        return chunk->flattenTerrain(pos, remain, radius, BrushType, mode,
                                     origin, angle, orientation);
      },
      [this](MapChunk *chunk) { recalc_norms(chunk); });
}

std::vector<std::pair<SceneObject *, float>>
World::getObjectsGroundDistance(glm::vec3 const &pos, float radius,
                                bool iter_wmos_, bool iter_m2s) {
  ...
  for (auto obj : objects_hit) {
    if ((obj->which() == eMODEL && !iter_m2s) ||
        (obj->which() == eWMO && !iter_wmos_))
      continue;
    float height_diff = obj->pos.y - get_ground_height(obj->pos).y;
    objects_ground_distance.push_back(
        std::pair<SceneObject *, float>(obj, height_diff));
  }

  return objects_ground_distance;
}

void World::blurTerrain(glm::vec3 const &pos, float remain, float radius,
                        int BrushType, flatten_mode const &mode) {
  ZoneScoped;
  for_all_chunks_in_range(
      pos, radius,
...
void World::set_texture_effect(glm::vec3 const &pos, Brush *brush,
                               scoped_blp_texture_reference const &tex,
                               int effectID) {
  for_all_chunks_in_range(pos, brush->getRadius(), [&](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkTextureChange(chunk);
    chunk->set_texture_effect(tex, effectID);
    return true;
  });
}

bool World::paintTexture(glm::vec3 const &pos, Brush *brush, float strength,
                         float pressure, scoped_blp_texture_reference texture) {
  ZoneScoped;
  return for_all_chunks_in_range(pos, brush->getRadius(), [&](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkTextureChange(chunk);
    return chunk->paintTexture(pos, brush, strength, pressure, texture);
  });
}

bool World::stampTexture(glm::vec3 const &pos, Brush *brush, float strength,
                         float pressure, scoped_blp_texture_reference texture,
                         QImage *img, bool paint) {
  ZoneScoped;
  return for_all_chunks_in_rect(pos, brush->getRadius(), [&](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkTextureChange(chunk);
    return chunk->stampTexture(pos, brush, strength, pressure, texture, img,
                               paint);
  });
}

bool World::sprayTexture(glm::vec3 const &pos, Brush *brush, float strength,
  ...
  for (float pz = pos.z - spraySize; pz < pos.z + spraySize; pz += inc) {
    for (float px = pos.x - spraySize; px < pos.x + spraySize; px += inc) {
      if ((sqrt(pow(px - pos.x, 2) + pow(pz - pos.z, 2)) <= spraySize) &&
        ...
        succ |=
            paintTexture({px, pos.y, pz}, brush, strength, pressure, texture);
      }
    }
  }
...
bool World::replaceTexture(glm::vec3 const &pos, float radius,
...
void World::overwriteTextureAtCurrentChunk(
    glm::vec3 const &pos, scoped_blp_texture_reference const &oldTexture,
...
void World::convertAlphamap(bool to_big_alpha) {
  ...
  for (size_t z = 0; z < 64; z++) {
    for (size_t x = 0; x < 64; x++) {
      ...
      bool unload =
          !mapIndex.tileLoaded(tile) && !mapIndex.tileAwaitingLoading(tile);
      MapTile *mTile = mapIndex.loadTile(tile);
...
void World::addM2(BlizzardArchive::Listfile::FileKey const &file_key,
                  glm::vec3 newPos, float scale, glm::vec3 rotation,
...
WMOInstance *
World::addWMOAndGetInstance(BlizzardArchive::Listfile::FileKey const &file_key,
                            glm::vec3 newPos, math::degrees::vec3 rotation) {
  ZoneScoped;
  WMOInstance wmo_instance(file_key, _context);
...
void World::remove_models_if_needed(std::vector<uint32_t> const &uids) {
  ...
  if (_current_selection.size() == 1 &&
      _current_selection.at(0).index() == eEntry_MapChunk) {
    reset_selection();
  }
  /*
  if (uids.size())
...
void World::exportADTAlphamap(glm::vec3 const &pos) {
  ...
  for_tile_at(pos, [&](MapTile *tile) {
    ...
    for (int i = 1; i < 4; ++i) {
      ...
      img.save(path + "/world/maps/" + basename.c_str() + "/" +
                   basename.c_str() + "_" +
                   std::to_string(tile->index.x).c_str() + "_" +
                   std::to_string(tile->index.z).c_str() + "_layer" +
...
void World::exportADTNormalmap(glm::vec3 const &pos) {
  ...
  for_tile_at(pos, [&](MapTile *tile) {
    ...
    img.save(path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
                 "_" + std::to_string(tile->index.x).c_str() + "_" +
                 std::to_string(tile->index.z).c_str() + "_normal.png",
             "PNG");
  });
}
...
void World::exportADTAlphamap(glm::vec3 const &pos,
  ...
  for_tile_at(pos, [&](MapTile *tile) {
    ...
    img.save(path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
                 "_" + std::to_string(tile->index.x).c_str() + "_" +
                 std::to_string(tile->index.z).c_str() + "_" +
                 tex.replace("/", "-") + ".png",
             "PNG");
  });
}

void World::exportADTHeightmap(glm::vec3 const &pos, float min_height,
                               float max_height) {
  ZoneScoped;
  for_tile_at(pos, [&](MapTile *tile) {
    QString path =
    ...
    img.save(path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
                 "_" + std::to_string(tile->index.x).c_str() + "_" +
                 std::to_string(tile->index.z).c_str() + "_height.png",
             "PNG");
  });
}
...
void World::exportADTVertexColorMap(glm::vec3 const &pos) {
  ...
  for_tile_at(pos, [&](MapTile *tile) {
    ...
    img.save(path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
                 "_" + std::to_string(tile->index.x).c_str() + "_" +
                 std::to_string(tile->index.z).c_str() + "_vcol.png",
             "PNG");
  });
}

void World::importADTAlphamap(glm::vec3 const &pos, QImage const &image,
                              unsigned layer) {
  ZoneScoped;
  for_all_chunks_on_tile(pos, [](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkTextureChange(chunk);
  ...
  if (image.width() != 1024 || image.height() != 1024) {
    QImage scaled =
        image.scaled(1024, 1024, Qt::AspectRatioMode::IgnoreAspectRatio);

    for_tile_at(pos,
                [&](MapTile *tile) { tile->setAlphaImage(scaled, layer); });

  } else {
    for_tile_at(pos, [&](MapTile *tile) { tile->setAlphaImage(image, layer); });
  }
}
...
void World::importADTAlphamap(glm::vec3 const &pos) {
  ...
  for_tile_at(pos, [&](MapTile *tile) {
    for (int i = 1; i < 4; ++i) {
      QString filename = path + "/world/maps/" + basename.c_str() + "/" +
                         basename.c_str() + "_" +
                         std::to_string(tile->index.x).c_str() + "_" +
                         std::to_string(tile->index.z).c_str() + "_layer" +
                         std::to_string(i).c_str() + ".png";
...
void World::importADTHeightmap(glm::vec3 const &pos, QImage const &image,
                               float multiplier, unsigned mode,
                               bool tiledEdges) {
  ZoneScoped;
  ...
  if (image.width() != desired_dimensions ||
      image.height() != desired_dimensions) {
    QImage scaled = image.scaled(desired_dimensions, desired_dimensions,
...
void World::importADTHeightmap(glm::vec3 const &pos, float multiplier,
                               unsigned mode, bool tiledEdges) {
  ZoneScoped;
  for_tile_at(pos, [&](MapTile *tile) {
    QString path =
        QString(Noggit::Project::CurrentProject::get()->ProjectPath.c_str());
    ...
    QString filename =
        path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
        "_" + std::to_string(tile->index.x).c_str() + "_" +
        std::to_string(tile->index.z).c_str() + "_height" + ".png";
    ...
    if (img.width() != desiredSize || img.height() != desiredSize)
      img = img.scaled(static_cast<int>(desiredSize),
  ...
  for_all_chunks_on_tile(pos, [=](MapChunk *chunk) {
    ...
    for (int i = 0; i < 4; ++i) {
      if (chunk->texture_set->num() <= i) {
        scoped_blp_texture_reference tex{textures[i].toStdString(),
...
void World::importADTVertexColorMap(glm::vec3 const &pos, QImage const &image,
                                    int mode, bool tiledEdges) {
  ZoneScoped;
  for_all_chunks_on_tile(pos, [](MapChunk *chunk) {
    NOGGIT_CUR_ACTION->registerChunkVertexColorChange(chunk);
  });
  ...
  if (image.width() != desiredDimensions ||
      image.height() != desiredDimensions) {
    QImage scaled = image.scaled(static_cast<int>(desiredDimensions),
                                 static_cast<int>(desiredDimensions),
                                 Qt::AspectRatioMode::IgnoreAspectRatio);
...
void World::swapTexture(glm::vec3 const &pos,
...
void World::removeTexture(glm::vec3 const &pos,
                          scoped_blp_texture_reference tex) {
  ZoneScoped;
  if (!!Noggit::Ui::selected_texture::get()) {
    for_all_chunks_on_tile(pos, [&](MapChunk *chunk) {
...
void World::change_texture_flag(glm::vec3 const &pos,
                                scoped_blp_texture_reference const &tex,
...
void World::paintLiquid(glm::vec3 const &pos, float radius, int liquid_id,
...
bool World::isUnderMap(glm::vec3 const &pos) {
  ...
  if (mapIndex.tileLoaded(tile)) {
    ...
    return (mapIndex.getTile(tile)->getChunk(chnkX, chnkZ)->getMinHeight()) >
           pos.y + 2.0f;
  }
...
void World::orientVertices(glm::vec3 const &ref_pos, math::degrees vertex_angle,
...
float World::getMaxTileHeight(const TileIndex &tile) {
  ...
  for (uint32_t uid : *uids) {
    ...
    if (instance.value().index() == eEntry_Object) {
      ...
      obj->ensureExtents();
      max_height =
          std::max(max_height, std::max(obj->extents[0].y, obj->extents[1].y));
    }
  }
...
Noggit::VertexSelectionCache World::getVertexSelectionCache() {
  ZoneScoped;
  return std::move(Noggit::VertexSelectionCache{
      _vertex_tiles, _vertex_chunks, _vertex_border_chunks, _vertices_selected,
      _vertex_center});
}
    ...
    for (size_t x = 0; x < 64; x++) {
      ...
      if (mTile) {
        ...
        for (int i = 1; i < 4; ++i) {
          ...
          img.save(path + "/world/maps/" + basename.c_str() + "/" +
                       basename.c_str() + "_" +
                       std::to_string(mTile->index.x).c_str() + "_" +
                       std::to_string(mTile->index.z).c_str() + "_layer" +
                       std::to_string(i).c_str() + ".png",
                   "PNG");
        }
    ...
    for (size_t x = 0; x < 64; x++) {
      ...
      if (mTile) {
        ...
        img.save(path + "/world/maps/" + basename.c_str() + "/" +
                     basename.c_str() + "_" +
                     std::to_string(mTile->index.x).c_str() + "_" +
                     std::to_string(mTile->index.z).c_str() + "_" +
                     tex.replace("/", "-") + ".png",
                 "PNG");
    ...
    for (size_t x = 0; x < 64; x++) {
      ...
      if (mTile) {
        ...
        img.save(path + "/world/maps/" + basename.c_str() + "/" +
                     basename.c_str() + "_" +
                     std::to_string(mTile->index.x).c_str() + "_" +
                     std::to_string(mTile->index.z).c_str() + "_height.png",
                 "PNG");
    ...
    for (size_t x = 0; x < 64; x++) {
      ...
      if (mTile) {
        ...
        img.save(path + "/world/maps/" + basename.c_str() + "/" +
                     basename.c_str() + "_" +
                     std::to_string(mTile->index.x).c_str() + "_" +
                     std::to_string(mTile->index.z).c_str() + "_vcol.png",
                 "PNG");
  ...
  for (size_t z = 0; z < 64; z++) {
    for (size_t x = 0; x < 64; x++) {
      ...
      if (mTile) {
        ...
        QString filename =
            path + "/world/maps/" + basename.c_str() + "/" + basename.c_str() +
            "_" + std::to_string(mTile->index.x).c_str() + "_" +
            std::to_string(mTile->index.z).c_str() + "_height.png";
...
void World::select_objects_in_area(const std::array<glm::vec2, 2> selection_box,
                                   bool reset_selection, glm::mat4x4 view,
                                   glm::mat4x4 projection, int viewport_width,
                                   int viewport_height, float user_depth,
        ...
        for (auto &instance : pair.second) {
          ...
          if (depth <= user_depth) {
            ...
            if (misc::pointInside(screenPos2D, selection_box)) {
              ...
              if (modelInstance &&
                  modelInstance.value().index() == eEntry_Object) {
                auto obj =
                    std::get<selected_object_type>(modelInstance.value());
                auto which =
                    std::get<selected_object_type>(modelInstance.value())
                        ->which();
                if (which == eWMO) {
...
void World::remove_selection_group(selection_group *group) {
  // std::vector<selection_type>::iterator position =
  // std::find(_selection_groups.begin(), _selection_groups.end(), group); if
  // (position != _selection_groups.end())
  // {
  //     _selection_groups.erase(position);
  // }

  // for (auto it = _selection_groups.begin(); it != _selection_groups.end();
  // ++it)
  // {
  //     auto it_group = *it;
  //     if (it_group.getMembers().size() == group->getMembers().size() &&
  //     it_group.getExtents() == group->getExtents())
