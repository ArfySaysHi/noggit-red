...
MapHorizon::MapHorizon(const std::string &basename,
  ...
  BlizzardArchive::ClientFile wdl_file(
      _filename, Application::NoggitApplication::instance()->clientData());
  ...
  do {
    ...
    case 'MWMO': {
      {
        char const *lCurPos =
            reinterpret_cast<char const *>(wdl_file.getPointer());
    ...
    case 'MODF': {
      ...
      break;
      // {
      //     ENTRY_MODF const* modf_ptr = reinterpret_cast<ENTRY_MODF
      //     const*>(wdl_file.getPointer()); for (unsigned int i = 0; i < size /
      //     sizeof(ENTRY_MODF); ++i)
      //     {
    ...
    case 'MAOF': {
      ...
      for (size_t y(0); y < 64; ++y) {
        for (size_t x(0); x < 64; ++x) {
          ...
          _tiles[y][x] = std::make_unique<MapHorizon_tile>();
...
void MapHorizon::update_minimap_tile(int y, int x, bool has_data = false) {
  if (_tiles[y][x]) {
    //! \todo There also is a second heightmap appended which has additional
    //! 16*16 pixels.
    //! \todo There also is MAHO giving holes into this heightmap.
    ...
    for (int j(0); j < 16; ++j) {
      for (int i(0); i < 16; ++i) {
        ...
        _qt_minimap.setPixel(x * 16 + i, y * 16 + j,
                             color_for_height(_tiles[y][x]->height_17[j][i]));
      }
    }
  }
...
void MapHorizon::set_minimap(const MapIndex *const index) {
  _qt_minimap = QImage(16 * 64, 16 * 64, QImage::Format_ARGB32);
  _qt_minimap.fill(Qt::transparent);
...
void MapHorizon::remove_horizon_tile(int y, int x) {
  _tiles[y][x].reset();
...
Noggit::MapHorizon_tile *MapHorizon::get_horizon_tile(int y, int x) {
  return _tiles[y][x].get();
}

int16_t MapHorizon::getWdlheight(MapTile *tile, float x, float y) {
  ...
  int col =
      static_cast<int>((x - UNITSIZE * 0.5f * (row % 2)) / UNITSIZE + 0.5f);
  bool inner = (row % 2) == 1;
  ...
  // float height = heights[cy * 16 + cx][17 * (row / 2) + (inner ? 9 : 0) +
  // col];
  if (!chunk)
    return 0.0f;

  float height =
      chunk->getHeightmap()[17 * (row / 2) + (inner ? 9 : 0) + col].y;
  return std::min(
      std::max(static_cast<int16_t>(height), static_cast<int16_t>(SHRT_MIN)),
      static_cast<int16_t>(SHRT_MAX));
}

void MapHorizon::update_horizon_tile(MapTile *mTile) {
  ...
  for (int i = 0; i < 17; i++) {
    for (int j = 0; j < 17; j++) {
      ...
      if (!_tiles[tile_index.z][tile_index.x]
               .get()) // tile has not been initialised
                       //     continue;
      {
        _tiles[tile_index.z][tile_index.x] =
            std::make_unique<MapHorizon_tile>();
        // do we need to use memcpy as well ?
      }
      // only works for initialised
      _tiles[tile_index.z][tile_index.x].get()->height_17[i][j] =
          getWdlheight(mTile, x, y);

      // inner - close enough; correct values appear to use some form of
      // averaging
      if (i < 16 && j < 16)
        _tiles[tile_index.z][tile_index.x].get()->height_16[i][j] =
            getWdlheight(mTile, x + CHUNKSIZE / 2.0f, y + CHUNKSIZE / 2.0f);
    }
  }
  // Holes
  for (int i = 0; i < 16; ++i) {
    ...
    for (int j = 0; j < 16; ++j) {
      ...
      if (!chunk)
        continue;
      // the ordering seems to be : short array = Y axis, flags values = X axis
      // and the values are for a whole chunk.
    ...
    _tiles[tile_index.z][tile_index.x].get()->holes[i] =
        static_cast<int16_t>(wdlHoleMask.to_ulong());
  }

  update_minimap_tile(tile_index.z, tile_index.x, true);
}

void MapHorizon::save_wdl(World *world, bool regenerate) {
  world->waitForAllTileUpdates();

  std::stringstream filename;
  filename << "World/Maps/" << world->basename << "/" << world->basename
           << ".wdl";
    ...
    for (int x = 0; x < 64; ++x) {
      ...
      *(wdlFile.GetPointer<uint>(curPos)) = has_tile ? mareoffset : 0;
      ...
      if (has_tile) {
        ...
        //  {
        wdlFile.Extend(8);
        SetChunkHeader(wdlFile, mareoffset, 'MARE',
                       (2 * (17 * 17)) +
                           (2 * (16 * 16))); // outer heights+inner heights
        mareoffset += 8;

        // this might be invalid if map had no WDL
        Noggit::MapHorizon_tile *horizon_tile = get_horizon_tile(y, x);

        // laod tile and extract WDL data
        if (!horizon_tile || regenerate) {
          bool unload = !world->mapIndex.tileLoaded(index) &&
                        !world->mapIndex.tileAwaitingLoading(index);
        ...
        wdlFile.Insert(mareoffset, sizeof(Noggit::MapHorizon_tile::height_17),
                       reinterpret_cast<char *>(&horizon_tile->height_17));
        mareoffset += sizeof(Noggit::MapHorizon_tile::height_17);
        wdlFile.Insert(mareoffset, sizeof(Noggit::MapHorizon_tile::height_16),
                       reinterpret_cast<char *>(&horizon_tile->height_16));
        mareoffset += sizeof(Noggit::MapHorizon_tile::height_16);

        // MAHO (maparea holes) MAHO was added in WOTLK ?
        //  {
        wdlFile.Extend(8);
        SetChunkHeader(wdlFile, mareoffset, 'MAHO',
                       (2 * 16)); // 1 hole mask for each chunk
        mareoffset += 8;
        wdlFile.Extend(32);
        for (int i = 0; i < 16; ++i) {
...
MapHorizon::minimap::minimap(const MapHorizon &horizon) {
      ...
      for (size_t j(0); j < 16; ++j) {
        for (size_t i(0); i < 16; ++i) {
          texture[(y * 16 + j) * 1024 + x * 16 + i] =
              color_for_height(horizon._tiles[y][x]->height_17[j][i]);
        }
      }
    }
  }
...
MapHorizon::render::render(const MapHorizon &horizon) {
  ...
  for (size_t y(0); y < 64; ++y) {
    for (size_t x(0); x < 64; ++x) {
      ...
      _batches[y][x] = MapHorizon_batch(
          static_cast<std::uint32_t>(vertices.size()), 17 * 17 + 16 * 16);
  ...
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec3>(_vertex_buffer, vertices,
                                            GL_STATIC_DRAW);
}

static inline uint32_t outer_index(const MapHorizon_batch &batch, int y,
                                   int x) {
  return batch.vertex_start + y * 17 + x;
};

static inline uint32_t inner_index(const MapHorizon_batch &batch, int y,
                                   int x) {
  return batch.vertex_start + 17 * 17 + y * 16 + x;
};

void MapHorizon::render::draw(glm::mat4x4 const &model_view,
                              glm::mat4x4 const &projection, MapIndex *index,
  ...
  for (size_t y(current_index.z - lrr); y <= current_index.z + lrr; ++y) {
    for (size_t x(current_index.x - lrr); x < current_index.x + lrr; ++x) {
      ...
      MapHorizon_batch const &batch = _batches[y][x];
      ...
      for (int j(0); j < 16; ++j) {
        for (int i(0); i < 16; ++i) {
          ...
          /* TODO: when this optimization is turned off, we end up with
          inconsistent rendering between chunks and horizon batches.
           * Potentially it is caused by inconsistent coordinate space in
          visibility checking or chunk update system. if (index->tileLoaded({y,
          x}) && index->getTile({y, x})->getChunk(j,
          i)->is_visible(cull_distance, frustum, camera, display))
          {
            //continue;
          }
          */
  ...
  if (_MapHorizon_program) {
    gl.bufferSubData<GL_ELEMENT_ARRAY_BUFFER, std::uint32_t>(_index_buffer, 0,
                                                             indices);
  } else {
    gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint32_t>(
        _index_buffer, indices, GL_DYNAMIC_DRAW);

    _MapHorizon_program.reset(new OpenGL::program{
  ...
  OpenGL::Scoped::use_program shader{*_MapHorizon_program.get()};
