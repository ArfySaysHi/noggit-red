...
MapIndex::TileRange<false> MapIndex::loaded_tiles() {
  return tiles<false>([](TileIndex const &, MapTile *tile) {
    return !!tile && tile->finishedLoading();
  });
}

MapIndex::TileRange<true> MapIndex::tiles_in_range(glm::vec3 const &pos,
                                                   float radius) {
  return tiles<true>([this, pos, radius](TileIndex const &index, MapTile *) {
    return hasTile(index) &&
           misc::getShortestDist(pos.x, pos.z, index.x * TILESIZE,
                                 index.z * TILESIZE, TILESIZE) <= radius;
  });
}

MapIndex::TileRange<true> MapIndex::tiles_in_rect(glm::vec3 const &pos,
                                                  float radius) {
  ...
  return tiles<true>(
      [this, pos, radius, l_chunk, r_chunk](TileIndex const &index, MapTile *) {
        ...
        glm::vec2 r_tile{index.x * TILESIZE + TILESIZE,
                         index.z * TILESIZE + TILESIZE};

        return ((l_chunk.x < r_tile.x) && (r_chunk.x >= l_tile.x) &&
                (l_chunk.y < r_tile.y) && (r_chunk.y >= l_tile.y));
      });
}
...
MapIndex::MapIndex(const std::string &pBasename, int map_id, World *world,
    ...
    : basename(pBasename), _map_id(map_id),
      ...
      ,
      mBigAlpha(false), mHasAGlobalWMO(false), changed(false),
  ...
  for (int j = 0; j < 64; ++j) {
    for (int i = 0; i < 64; ++i) {
      ...
      adt_filename << "World\\Maps\\" << basename << "\\" << basename << "_"
                   << i << "_" << j << ".adt";

      mTiles[j][i].onDisc = Noggit::Application::NoggitApplication::instance()
                                ->clientData()
                                ->existsOnDisk(adt_filename.str());
  ...
  if (!theFile.isEof() && mHasAGlobalWMO) {
    //! \note We actually don't load WMO only worlds, so we just stop reading
    //! here, k?
    //! \bug MODF reads wrong. The assertion fails every time. Somehow, it keeps
    //! being MWMO. Or are there two blocks?
    //! \nofuckingbug  on eof read returns just without doing sth to the var and
    //! some wdts have a MWMO without having a MODF so only checking for eof
    //! above is not enough
...
void MapIndex::save() {
  ...
  *(wdtFile.GetPointer<int>(8)) = 18;
  ...
  if (mHasAGlobalWMO) {
    ...
    wdtFile.Extend(8);
    SetChunkHeader(wdtFile, curPos, 'MWMO',
                   static_cast<int>(globalWMOName.size()));
    curPos += 8;

    wdtFile.Insert(curPos, static_cast<unsigned long>(globalWMOName.size()),
                   globalWMOName.data());
...
void MapIndex::enterTile(const TileIndex &tile) {
  ...
  for (int pz = std::max(cz - _loading_radius, 0);
       pz <= std::min(cz + _loading_radius, 63); ++pz) {
    for (int px = std::max(cx - _loading_radius, 0);
         px <= std::min(cx + _loading_radius, 63); ++px) {
...
MapTile *MapIndex::loadTile(const TileIndex &tile, bool reloading,
                            bool load_models, bool load_textures) {
  if (!hasTile(tile)) {
    return nullptr;
  }
...
void MapIndex::unloadTile(const TileIndex &tile) {
  ...
  if (tileLoaded(tile)) {
    ...
    // otherwise it can be deleted before the log because it comes from the adt
    // itself (see unloadTiles)
  ...
  if (save_unloaded) {
    auto filepath = std::filesystem::path(
                    ...
                    BlizzardArchive::ClientData::normalizeFilenameInternal(
                        mTiles[tile.z][tile.x].tile->file_key().filepath());
      ...
      for (int j = 0; j < 64; ++j) {
        ...
        auto filepath =
            std::filesystem::path(
                Noggit::Project::CurrentProject::get()->ProjectPath) /
            BlizzardArchive::ClientData::normalizeFilenameInternal(
                mTiles[i][j].tile->file_key().filepath());
...
bool MapIndex::hasAGlobalWMO() const { return mHasAGlobalWMO; }
...
bool MapIndex::hasBigAlpha() const { return mBigAlpha; }

void MapIndex::setBigAlpha(bool state) { mBigAlpha = state; }

unsigned MapIndex::getNLoadedTiles() const { return _n_loaded_tiles; }
...
void MapIndex::setUnloadInterval(int value) { _unload_interval = value; }
...
uint32_t MapIndex::newGUID() {
  ...
  if (settings.value("project/mysql/enabled", false).toBool()) {
    // update the highest uid in db, note that if the user don't save these uid
    // won't be used (not really a problem tho)
...
uid_fix_status MapIndex::fixUIDs(World *world,
  ...
  for (int z = 0; z < 64; ++z) {
    for (int x = 0; x < 64; ++x) {
      ...
      std::stringstream filename;
      filename << "World\\Maps\\" << basename << "\\" << basename << "_" << x
               << "_" << z << ".adt";
  ...
  auto uids_per_tile = std::make_unique<std::map<
      std::size_t, std::map<std::size_t, std::forward_list<std::uint32_t>>>>();
  ...
  for (ModelInstance &instance : *models) {
    ...
    std::size_t sx = std::max(
        (std::size_t)(instance.getExtents()[0].x / TILESIZE), (std::size_t)0);
    std::size_t sz = std::max(
        (std::size_t)(instance.getExtents()[0].z / TILESIZE), (std::size_t)0);
    std::size_t ex = std::min(
        (std::size_t)(instance.getExtents()[1].x / TILESIZE), (std::size_t)63);
    std::size_t ez = std::min(
        (std::size_t)(instance.getExtents()[1].z / TILESIZE), (std::size_t)63);

    auto const real_uid(
        world->add_model_instance(std::move(instance), false, false));
  ...
  for (WMOInstance &instance : *wmos) {
    ...
    instance.recalcExtents();
    // no need to check if the loading is finished since the extents are stored
    // inside the adt to avoid going outside of bound
    std::size_t sx = std::max(
        (std::size_t)(instance.getExtents()[0].x / TILESIZE), (std::size_t)0);
    std::size_t sz = std::max(
        (std::size_t)(instance.getExtents()[0].z / TILESIZE), (std::size_t)0);
    ...
    auto const real_uid(
        world->add_wmo_instance(std::move(instance), false, false));
  ...
  for (int z = 0; z < 64; ++z) {
    for (int x = 0; x < 64; ++x) {
      ...
      if (!Noggit::Application::NoggitApplication::instance()
               ->clientData()
               ->exists(filename.str())) {
        LogError << "searchMaxUID(): ADT(" << x << "_" << z
                 << ") referenced by WDT does not exist. Skipping."
                 << std::endl;
        continue;
      }
...
void MapIndex::loadMaxUID() {
  ...
  if (settings.value("project/mysql/enabled", false).toBool()) {
    highestGUID = std::max(Noggit::Sql::SqlUIDStorage::getGUIDFromDB(_map_id),
                           highestGUID);
...
void MapIndex::loadMinimapMD5translate() {
  auto &minimap_md5translate = this->_minimap_md5translate;
  ...
  if (!Noggit::Application::NoggitApplication::instance()->clientData()->exists(
    ...
    LogError << "md5translate.trs was not found. "
                "Noggit will generate a new one in the project directory on "
                "minimap save."
             << std::endl;
    return;
  }

  BlizzardArchive::ClientFile md5trs_file(
      "textures/minimap/md5translate.trs",
      Noggit::Application::NoggitApplication::instance()->clientData());
  ...
  QByteArray md5trs_bytes(static_cast<char *>(buffer_raw),
  ...
  while (!md5trs_stream.atEnd()) {
    ...
    if (line_split.length() < 2) {
      std::string text = "Failed to read md5translate.trs.\nLine \"" +
                         line.toStdString() +
                         "\n has no tab spacing. Spacing must be only a tab "
    ...
    if (cur_dir.length()) {
      minimap_md5translate[cur_dir.toStdString()][line_split[0].toStdString()] =
          line_split[1].toStdString();
    }
  }
}
...
void MapIndex::saveMinimapMD5translate() {
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Text | QFile::Truncate)) {
    ...
    auto const &minimap_md5translate = this->_minimap_md5translate;

    for (auto it = minimap_md5translate.begin();
         it != minimap_md5translate.end(); ++it) {
    ...
    LogError << "Failed saving md5translate.trs. File can't be opened."
             << std::endl;
  }
}
...
bool MapIndex::use_mclq_green_lava() const { return _map_id == 530; }
...
void MapIndex::create_empty_wdl() const {
  ...
  *(wdlFile.GetPointer<int>(8)) = 18; // write version 18
  ...
  for (int i = 0; i < 4096; ++i) {
    ...
    //  {
    wdlFile.Extend(8);
    SetChunkHeader(wdlFile, curPos, 'MARE',
                   (2 * (17 * 17)) +
    ...
    //  {
    wdlFile.Extend(8);
    SetChunkHeader(wdlFile, curPos, 'MAHO',
...
MapTileEntry::MapTileEntry() : flags(0), tile(nullptr), onDisc(false) {}
