...
MapIndex::MapIndex(const std::string &pBasename, int map_id, World *world,
  ...
  for (int j = 0; j < 64; ++j) {
    for (int i = 0; i < 64; ++i) {
      ...
      adt_filename << "World/Maps/" << basename << "/" << basename << "_" << i
                   << "_" << j << ".adt";
...
uint32_t MapIndex::newGUID() {
  ...
  if (settings.value("project/mysql/enabled", false).toBool()) {
    mysql::updateUIDinDB(
        _map_id,
        highestGUID +
            1); // update the highest uid in db, note that if the user don't
                // save these uid won't be used (not really a problem tho)
  }
#endif
  return ++highestGUID;
}
...
uid_fix_status MapIndex::fixUIDs(World *world,
  ...
  std::map<std::size_t, std::map<std::size_t, std::forward_list<std::uint32_t>>>
      uids_per_tile;
  ...
  for (ModelInstance &instance : models) {
    ...
    std::size_t sx = std::max((std::size_t)(instance.extents[0].x / TILESIZE),
                              (std::size_t)0);
    std::size_t sz = std::max((std::size_t)(instance.extents[0].z / TILESIZE),
                              (std::size_t)0);
    std::size_t ex = std::min((std::size_t)(instance.extents[1].x / TILESIZE),
                              (std::size_t)63);
    std::size_t ez = std::min((std::size_t)(instance.extents[1].z / TILESIZE),
                              (std::size_t)63);
...
void MapIndex::loadMinimapMD5translate() {
  ...
  while (!md5trs_stream.atEnd()) {
    ...
    QStringList line_split = line.split(QRegularExpression("[\t]"));
    ...
    if (cur_dir.length()) {
      _minimap_md5translate[cur_dir.toStdString()]
                           [line_split[0].toStdString()] =
...
void MapIndex::saveMinimapMD5translate() {
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Text | QFile::Truncate)) {
    ...
    for (auto it = _minimap_md5translate.begin();
         it != _minimap_md5translate.end(); ++it) {
