...
ParticleSystem::ParticleSystem(Model *model_,
    ...
    : model(model_), emitter_type(mta.EmitterType),
      emitter(mta.EmitterType == 1
      ...
      lat(mta.HorizontalRange, f, globals), gravity(mta.Gravity, f, globals),
      lifespan(mta.Lifespan, f, globals), rate(mta.EmissionRate, f, globals),
      areal(mta.EmissionAreaLength, f, globals),
      areaw(mta.EmissionAreaWidth, f, globals),
      deacceleration(mta.Gravity2, f, globals), enabled(mta.en, f, globals),
      mid(0.5), slowdown(mta.p.slowdown), pos(fixCoordSystem(mta.pos)),
      _texture_id(mta.texture), blend(mta.blend),
      order(mta.ParticleType > 0 ? -1 : 0), type(mta.ParticleType), manim(0),
      mtime(0), rows(mta.rows), cols(mta.cols), billboard(!(mta.flags & 4096)),
      rem(0), parent(&model->bones[mta.bone]), flags(mta.flags),
      tofs(misc::frand()), _context(context) {
  glm::vec3 colors2[3];
  ...
  for (size_t i = 0; i < 3; ++i) {
    ...
    sizes[i] = (*reinterpret_cast<float const *>(f.getBuffer() +
                                                 mta.p.sizes.ofsKeys + i * 4)) *
               mta.p.scales[i];
  }
...
ParticleSystem::ParticleSystem(ParticleSystem const &other)
    : model(other.model), emitter_type(other.emitter_type),
      emitter(emitter_type == 1 ? std::unique_ptr<ParticleEmitter>(
                                      std::make_unique<PlaneParticleEmitter>())
              : emitter_type == 2
                  ? std::unique_ptr<ParticleEmitter>(
                        std::make_unique<SphereParticleEmitter>())
                  : std::unique_ptr<ParticleEmitter>(
                        std::make_unique<PlaneParticleEmitter>())),
      speed(other.speed), variation(other.variation), spread(other.spread),
      lat(other.lat), gravity(other.gravity), lifespan(other.lifespan),
      rate(other.rate), areal(other.areal), areaw(other.areaw),
      deacceleration(other.deacceleration), enabled(other.enabled),
      colors(other.colors), sizes(other.sizes), mid(other.mid),
      slowdown(other.slowdown), pos(other.pos), _texture_id(other._texture_id),
      particles(other.particles), blend(other.blend), order(other.order),
      type(other.type), manim(other.manim), mtime(other.mtime),
      manimtime(other.manimtime), rows(other.rows), cols(other.cols),
      tiles(other.tiles), billboard(other.billboard), rem(other.rem),
      parent(other.parent), flags(other.flags), tofs(other.tofs),
      _context(other._context) {}

ParticleSystem::ParticleSystem(ParticleSystem &&other)
    : model(other.model), emitter_type(other.emitter_type),
      emitter(std::move(other.emitter)), speed(other.speed),
      variation(other.variation), spread(other.spread), lat(other.lat),
      gravity(other.gravity), lifespan(other.lifespan), rate(other.rate),
      areal(other.areal), areaw(other.areaw),
      deacceleration(other.deacceleration), enabled(other.enabled),
      colors(other.colors), sizes(other.sizes), mid(other.mid),
      slowdown(other.slowdown), pos(other.pos), _texture_id(other._texture_id),
      particles(other.particles), blend(other.blend), order(other.order),
  ...
  if (emitter) {
    ...
    if (ftospawn < 1.0f) {
      ...
      if ((tospawn + particles.size()) >
          MAX_PARTICLES) // Error check to prevent the program from trying to
                         // load insane amounts of particles.
      ...
      if (en) {
        for (int i = 0; i < tospawn; ++i) {
          Particle p = emitter->newParticle(this, manim, mtime, manimtime, w, l,
                                            spd, var, spr, spr2);
          // sanity check:
          // if (particles.size() < MAX_PARTICLES) // No need to check this
          // every loop iteration. Already checked above.
...
void ParticleSystem::draw(glm::mat4x4 const &model_view,
  ...
  if (billboard) {
    ...
    // vUp = glm::vec3(model_view[0][1], model_view[1][1], model_view[2][1]); //
    // Spherical billboarding vUp = glm::vec3(0,1,0); // Cylindrical
    // billboarding
  }
   ...
   * type:
   * 0   "normal" particle
   * 1  large quad from the particle's origin to its position (used in Moonwell
   * water effects) 2  seems to be the same as 0 (found some in the Deeprun Tram
   * blinky-lights-sign thing)
   */
  if (type == 0 || type == 2) {
  ...
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec3>(_vertices_vbo, vertices,
                                            GL_STREAM_DRAW);
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec4>(_colors_vbo, colors_data,
                                            GL_STREAM_DRAW);
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec2>(_texcoord_vbo, texcoords,
                                            GL_STREAM_DRAW);
  ...
  if (billboard) {
    gl.bufferData<GL_ARRAY_BUFFER, glm::vec3>(_offsets_vbo, offsets,
                                              GL_STREAM_DRAW);
  ...
  OpenGL::Scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const indices_binder(
      _indices_vbo);
  gl.drawElementsInstanced(GL_TRIANGLES, static_cast<GLsizei>(indices.size()),
                           GL_UNSIGNED_SHORT, nullptr, instances_count);
}
...
Particle PlaneParticleEmitter::newParticle(ParticleSystem *sys, int anim,
  ...
  // 0x00 / 41 = [1,8,32] Blood elf broom, dust spread out on the ground (X, Z
  // axis) 0x1D / 29 = [1,4,8,16] particle being static 0x19 / 25 = [1,8,16]
  // flame on weapon - move up/along the weapon 17 = 0x11 = [1,16] glow on
  // weapon - static, random direction.  - Aurastone Hammer 1 = 0x1 = perdition
  // blade 4121 = water ele 4097 = water elemental 1041 = Transcendance Halo
  // 1039 = water ele
  ...
  if (sys->flags == 1041) { // Trans Halo
    p.pos = sys->parent->mat *
            (glm::vec4(sys->pos, 0) +
             glm::vec4(misc::randfloat(-l, l), 0, misc::randfloat(-w, w), 0));

    const float t = misc::randfloat(0.0f, 2.0f * glm::pi<float>());

    p.pos =
        glm::vec3(0.0f, sys->pos.y + 0.15f, sys->pos.z) +
        glm::vec3(cos(t) / 8, 0.0f,
    ...
    p.pos = sys->parent->pivot + (sys->pos + glm::vec3(misc::randfloat(-l, l),
                                                       misc::randfloat(-l, l),
                                                       misc::randfloat(-w, w)));
    glm::vec3 dir = mrot * glm::vec4(0.0f, 1.0f, 0.0f, 0.0f);
    p.dir = glm::normalize(dir);
    // glm::vec3 dir = sys->model->bones[sys->parent->parent].mrot *
    // sys->parent->mrot * glm::vec3(0.0f, 1.0f, 0.0f); p.speed =
    // dir.normalize() * spd;

  } else if (sys->flags == 25 &&
             sys->parent->parent >
                 0) { // Weapon with built-in Flame (Avenger lightsaber!)
    p.pos = sys->parent->mat *
            (glm::vec4(sys->pos, 0) + glm::vec4(misc::randfloat(-l, l),
                                                misc::randfloat(-l, l),
                                                misc::randfloat(-w, w), 0));
    glm::vec3 dir = glm::vec4(sys->parent->mat[1][0], sys->parent->mat[1][1],
                              sys->parent->mat[1][2], 0.0f) +
    ...
    p.pos =
        sys->pos + glm::vec3(misc::randfloat(-l, l), 0, misc::randfloat(-w, w));
  ...
  */
  ...
  if (sys->flags == 57 || sys->flags == 313) { // Faith Halo
    ...
    if (glm::length(bdir) * glm::length(bdir) == 0)
      ...
      dir = sys->parent->mrot * glm::vec4((glm::normalize(bdir)),
                                          0); // mrot * glm::vec3(0, 1.0f,0);
      p.speed =
          glm::normalize(dir) * spd * (1.0f + misc::randfloat(-var, var)); // ?
    }
...
RibbonEmitter::RibbonEmitter(Model *model_,
                             ...
                             Noggit::NoggitRenderContext context)
    : model(model_), color(mta.color, f, globals),
      opacity(mta.opacity, f, globals), above(mta.above, f, globals),
      below(mta.below, f, globals), parent(&model->bones[mta.bone]),
      pos(fixCoordSystem(mta.pos)), seglen(mta.length), length(mta.res * seglen)
      ...
      // in BFD, res is 60 and len is 0.6, the trails are very short (too long
      // here) in CoT, res and len are like 10 but the trails are supposed to be
      // much longer (too short here)
      ,
      _context(context) {
...
RibbonEmitter::RibbonEmitter(RibbonEmitter const &other)
    : model(other.model), color(other.color), opacity(other.opacity),
      above(other.above), below(other.below), parent(other.parent),
      pos(other.pos), manim(other.manim), mtime(other.mtime),
      seglen(other.seglen), length(other.length), tpos(other.tpos),
      tcolor(other.tcolor), tabove(other.tabove), tbelow(other.tbelow),
      _texture_ids(other._texture_ids), _material_ids(other._material_ids),
      segs(other.segs), _context(other._context) {}
...
void RibbonEmitter::setup(int anim, int time, int animtime) {
  ...
  tcolor =
      glm::vec4(col.x, col.y, col.z, opacity.getValue(anim, time, animtime));
