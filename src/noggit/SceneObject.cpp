...
SceneObject::SceneObject(SceneObjectTypes type,
                         Noggit::NoggitRenderContext context)
    : _type(type), _context(context), pos(0.f, 0.f, 0.f), dir(0.f, 0.f, 0.f) {
...
bool SceneObject::isDuplicateOf(SceneObject const &other) {
  ...
  if ((a_obj_this && a_obj_other) &&
      a_obj_this->file_key() != a_obj_other->file_key()) {
    return false;
  }
...
void SceneObject::updateTransformMatrix() {
  ...
  matrix =
      matrix * glm::eulerAngleYZX(glm::radians(dir.y - math::degrees(90.0)._),
