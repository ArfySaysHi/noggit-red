...
MapTile::MapTile(int pX, int pZ, std::string const &pFilename, bool pBigAlpha,
                 ...
                 bool reloading_tile, World *world,
                 Noggit::NoggitRenderContext context, TileMode mode,
                 bool pLoadTextures)
    : AsyncObject(pFilename), _renderer(this), _fl_bounds_render(this),
      index(TileIndex(pX, pZ)), xbase(pX * TILESIZE), zbase(pZ * TILESIZE),
      changed(false), Water(this, xbase, zbase, use_mclq_green_lava),
      _mode(mode), _tile_is_being_reloaded(reloading_tile),
      mBigAlpha(pBigAlpha), _load_models(pLoadModels),
      _load_textures(pLoadTextures), _world(world), _context(context),
      ...
      _extents{glm::vec3{pX * TILESIZE, std::numeric_limits<float>::max(),
                         pZ * TILESIZE},
               glm::vec3{pX * TILESIZE + TILESIZE,
                         std::numeric_limits<float>::lowest(),
                         pZ * TILESIZE + TILESIZE}},
      _combined_extents{glm::vec3{pX * TILESIZE,
                                  std::numeric_limits<float>::max(),
                                  pZ * TILESIZE},
                        glm::vec3{pX * TILESIZE + TILESIZE,
                                  std::numeric_limits<float>::lowest(),
                                  pZ * TILESIZE + TILESIZE}},
      _object_instance_extents{glm::vec3{std::numeric_limits<float>::max()},
                               glm::vec3{std::numeric_limits<float>::lowest()}},
      _center{pX * TILESIZE + TILESIZE / 2.f, 0.f,
              pZ * TILESIZE + TILESIZE / 2.f} {}
  ...
  for (int i = 0; i < 16; ++i) {
    for (int j = 0; j < 16; ++j) {
      for (int k = 0; k < mChunks[i][j].get()->texture_set->num(); ++k) {
        (*mChunks[i][j].get()->texture_set->getTextures())[k]
            .get()
            ->wait_until_loaded();
      }
    }
...
void MapTile::finishLoading() {
  ...
  BlizzardArchive::ClientFile theFile(
      _file_key,
      Noggit::Application::NoggitApplication::instance()->clientData());

  Log << "Opening tile " << index.x << ", " << index.z << " (\""
      << _file_key.stringRepr() << "\") from "
  ...
  if (mFlags & 1) {
    ...
    for (int y = 0; y < 3; y++) {
      for (int x = 0; x < 3; x++) {
        ...
        mMinimumValues[pos] = {xPositions[x], static_cast<float>(z.first),
                               yPositions[y]};
        mMaximumValues[pos] = {xPositions[x], static_cast<float>(z.second),
                               yPositions[y]};
      }
    }
  }
  ...
  << temp << std::endl; mTextureFilenames[tCount++].second = temp; lCurPos += 4;
  }
  }
  ...
  if (_load_models) {
    ...
    for (auto const &object : lWMOInstances) {
      add_model(_world->add_wmo_instance(
          WMOInstance(mWMOFilenames[object.nameID], &object, _context),
    ...
    for (auto const &model : lModelInstances) {
      add_model(_world->add_model_instance(
          ModelInstance(mModelFilenames[model.nameID], &model, _context),
          _tile_is_being_reloaded));
    }

    _world->need_ModelUpdates = true;
  }
  ...
  for (int nextChunk = 0; nextChunk < 256; ++nextChunk) {
    ...
    mChunks[x][z] = std::make_unique<MapChunk>(
        this, &theFile, mBigAlpha, _mode, _context, false, 0, _load_textures);
...
float MapTile::getMaxHeight() { return _extents[1].y; }

float MapTile::getMinHeight() { return _extents[0].y; }
...
void MapTile::convertAlphamap(bool to_big_alpha) {
  mBigAlpha = true;
...
std::vector<MapChunk *> MapTile::chunks_in_range(glm::vec3 const &pos,
                                                 float radius) const {
  ...
  for (size_t ty(0); ty < 16; ++ty) {
    for (size_t tx(0); tx < 16; ++tx) {
      if (misc::getShortestDist(pos.x, pos.z, mChunks[ty][tx]->xbase,
                                mChunks[ty][tx]->zbase, CHUNKSIZE) <= radius) {
...
std::vector<MapChunk *> MapTile::chunks_in_rect(glm::vec3 const &pos,
                                                float radius) const {
  std::vector<MapChunk *> chunks;
  ...
  for (size_t ty(0); ty < 16; ++ty) {
    for (size_t tx(0); tx < 16; ++tx) {
      ...
      if ((l_rect.x < r_chunk.x) && (r_rect.x >= l_chunk.x) &&
          (l_rect.y < r_chunk.y) && (r_rect.y >= l_chunk.y)) {
...
void MapTile::saveTile(World *world) {
  ...
  int lID; // This is a global counting variable. Do not store something in here
           // you need later.
  ...
  for (std::uint32_t uid : uids) {
    ...
    if (!model) {
      ...
      LogError << "Could not find model with uid=" << uid << " when saving "
               << _file_key.stringRepr() << std::endl;
    } else {
      if (model.value().index() == eEntry_Object) {
        auto which = std::get<selected_object_type>(model.value())->which();
        if (which == eWMO) {
          lObjectInstances.emplace_back(static_cast<WMOInstance *>(
              std::get<selected_object_type>(model.value())));
        } else if (which == eMODEL) {
          lModelInstances.emplace_back(static_cast<ModelInstance *>(
              std::get<selected_object_type>(model.value())));
        }
      }
    }
  ...
  *(lADTFile.GetPointer<int>(8)) = 18;
  ...
  for (auto const &texture : lTextures) {
    lADTFile.Insert(lCurrentPosition,
                    static_cast<unsigned long>(texture.first.size() + 1),
                    texture.first.c_str());

    lCurrentPosition += static_cast<int>(texture.first.size() + 1);
    lADTFile.GetPointer<sChunkHeader>(lMTEX_Position)->mSize +=
        static_cast<int>(texture.first.size() + 1);
  ...
  if (world->mapIndex.sortModelsBySizeClass()) {
    std::sort(lModelInstances.begin(), lModelInstances.end(),
              [](ModelInstance *m1, ModelInstance *m2) {
  ...
  lID = 0;
  for (auto const &model : lModelInstances) {
    auto filename_to_offset_and_name =
        lModels.find(model->model->file_key().filepath());
    if (filename_to_offset_and_name == lModels.end()) {
      LogError << "There is a problem with saving the doodads. We have a "
                  "doodad that somehow changed the name during the saving "
                  "function. However this got produced, you can get a reward "
                  "from schlumpf by pasting him this line."
               << std::endl;
      return;
    }
  ...
  for (auto const &object : lObjectInstances) {
    auto filename_to_offset_and_name =
        lObjects.find(object->wmo->file_key().filepath());
    if (filename_to_offset_and_name == lObjects.end()) {
      LogError << "There is a problem with saving the objects. We have an "
                  "object that somehow changed the name during the saving "
                  "function. However this got produced, you can get a reward "
                  "from schlumpf by pasting him this line."
               << std::endl;
      return;
  ...
  for (int y = 0; y < 16; ++y) {
    for (int x = 0; x < 16; ++x) {
      mChunks[y][x]->save(lADTFile, lCurrentPosition, lMCIN_Position, lTextures,
                          lObjectInstances, lModelInstances);
    }
  }
  ...
  if (mFlags & 1) {
    ...
    SetChunkHeader(lADTFile, lCurrentPosition, 'MFBO',
  ...
  lADTFile.Extend(static_cast<long>(
      lCurrentPosition -
...
void MapTile::add_model(uint32_t uid) {
  ...
  if (std::find(uids.begin(), uids.end(), uid) == uids.end()) {
    ...
    if (instance->finishedLoading()) {
      ...
      _object_instance_extents[0].x =
          std::min(_object_instance_extents[0].x, instance->extents[0].x);
      _object_instance_extents[0].y =
          std::min(_object_instance_extents[0].y, instance->extents[0].y);
      _object_instance_extents[0].z =
          std::min(_object_instance_extents[0].z, instance->extents[0].z);

      _object_instance_extents[1].x =
          std::max(_object_instance_extents[1].x, instance->extents[1].x);
      _object_instance_extents[1].y =
          std::max(_object_instance_extents[1].y, instance->extents[1].y);
      _object_instance_extents[1].z =
          std::max(_object_instance_extents[1].z, instance->extents[1].z);
...
void MapTile::initEmptyChunks() {
  for (int nextChunk = 0; nextChunk < 256; ++nextChunk) {
    mChunks[nextChunk / 16][nextChunk % 16] = std::make_unique<MapChunk>(
        this, nullptr, mBigAlpha, _mode, _context, true, nextChunk);
  }
}
    ...
    for (int l = 0; l < 16; ++l) {
      ...
      for (unsigned y = 0; y < SUM; ++y) {
        for (unsigned x = 0; x < SUM; ++x) {
          ...
          float value =
              is_virtual
                  ? (heightmap[idx].y + heightmap[idx + (erp ? SUM : 1)].y) /
                        2.f
                  : heightmap[idx].y;
          value = std::min(1.0f, std::max(0.0f, ((value - min_height) /
                                                 (max_height - min_height))));
          image.setPixelColor((k * 16) + x, (l * 16) + y,
                              QColor::fromRgbF(value, value, value, 1.0));
        }
      }
    }
  }
    ...
    for (int l = 0; l < 16; ++l) {
      ...
      for (unsigned y = 0; y < SUM; ++y) {
        for (unsigned x = 0; x < SUM; ++x) {
          ...
          float value_r =
              is_virtual ? (normal.x + normal_inner.x) / 2.f : normal.x;
          float value_g =
              is_virtual ? (normal.y + normal_inner.y) / 2.f : normal.y;
          float value_b =
              is_virtual ? (normal.z + normal_inner.z) / 2.f : normal.z;

          image.setPixelColor((k * 16) + x, (l * 16) + y,
                              QColor::fromRgbF(value_r, value_g, value_b, 1.0));
        }
      }
    }
  }
    ...
    for (int j = 0; j < 16; ++j) {
      ...
      for (int k = 0; k < 64; ++k) {
        for (int l = 0; l < 64; ++l) {
          ...
          image.setPixelColor((i * 64) + k, (j * 64) + l,
                              QColor(value, value, value, 255));
        }
      }
    }
  }
...
void MapTile::setHeightmapImage(QImage const &baseimage, float multiplier,
                                int mode, bool tiledEdges) // image
{
      ...
      for (unsigned y = 0; y < SUM; ++y)
        for (unsigned x = 0; x < SUM; ++x) {
          ...
          case 32: {
            ...
            case 0: // Set
              heightmap[idx].y =
                  qGray(image.pixel((k * 16) + x, (l * 16) + y)) / 255.0f *
                  multiplier;
              break;
          ...
          case 64: {
            ...
            case 0: // Set
              heightmap[idx].y =
                  image.pixelColor((k * 16) + x, (l * 16) + y).redF() *
                  multiplier;
              break;
  ...
  {
    if (index.z > 0) {
      getWorld()->for_tile_at_force(
          TileIndex{index.x, index.z - 1}, [&](MapTile *tile) {
            for (int chunk_x = 0; chunk_x < 16; ++chunk_x) {
              ...
              for (int vert_x = 0; vert_x < 9; ++vert_x) {
                ...
                targetChunk->getHeightmap()[target_vert].y =
                    sourceChunk->getHeightmap()[source_vert].y;
              }
            }
    ...
    if (index.x > 0) {
      getWorld()->for_tile_at_force(
          TileIndex{index.x - 1, index.z}, [&](MapTile *tile) {
    ...
    if (index.x > 0 && index.z > 0) {
      getWorld()->for_tile_at_force(
          TileIndex{index.x - 1, index.z - 1}, [&](MapTile *tile) {
            MapChunk *targetChunk = tile->getChunk(15, 15);
            targetChunk->registerChunkUpdate(ChunkUpdateFlags::VERTEX);
            tile->getChunk(15, 15)->getHeightmap()[144].y =
                this->getChunk(0, 0)->getHeightmap()[0].y;
            tile->registerChunkUpdate(ChunkUpdateFlags::VERTEX);
          });
    }
  }
...
void MapTile::setAlphaImage(QImage const &baseimage, unsigned layer) {
  ...
  for (int k = 0; k < 16; ++k) {
    for (int l = 0; l < 16; ++l) {
      ...
      for (int i = 0; i < 64; ++i) {
        for (int j = 0; j < 64; ++j) {
          temp_alphamaps[layer][64 * j + i] = static_cast<float>(
              qGray(image.pixel((k * 64) + i, (l * 64) + j)));
        }
      }
    ...
    for (int l = 0; l < 16; ++l) {
      ...
      for (unsigned y = 0; y < SUM; ++y)
        for (unsigned x = 0; x < SUM; ++x) {
          ...
          {
            auto color = image.pixelColor((k * 16) + x, (l * 16) + y);
            colors[idx].x = std::min(
                2.0, std::max(0.0, colors[idx].x + color.redF() * 2.f));
            colors[idx].y = std::min(
                2.0, std::max(0.0, colors[idx].y + color.greenF() * 2.f));
            colors[idx].z = std::min(
                2.0, std::max(0.0, colors[idx].z + color.blueF() * 2.f));
            break;
          }
          ...
          TileIndex{index.x, index.z - 1}, [&](MapTile *tile) {
            for (int chunk_x = 0; chunk_x < 16; ++chunk_x) {
              ...
              for (int vert_x = 0; vert_x < 9; ++vert_x) {
                ...
                targetChunk->getVertexColors()[target_vert] =
                    sourceChunk->getVertexColors()[source_vert];
              }
            }
    ...
    if (index.x > 0 && index.z > 0) {
      getWorld()->for_tile_at_force(
          TileIndex{index.x - 1, index.z - 1}, [&](MapTile *tile) {
            MapChunk *targetChunk = tile->getChunk(15, 15);
            targetChunk->registerChunkUpdate(ChunkUpdateFlags::MCCV);
            tile->getChunk(15, 15)->getVertexColors()[144] =
                this->getChunk(0, 0)->getVertexColors()[0];
            tile->registerChunkUpdate(ChunkUpdateFlags::MCCV);
          });
    }
  }
...
void MapTile::recalcObjectInstanceExtents() {
  ...
  for (auto &pair : object_instances) {
    for (auto &instance : pair.second) {
      ...
      _object_instance_extents[0].x =
          std::min(_object_instance_extents[0].x, min.x);
      _object_instance_extents[0].y =
          std::min(_object_instance_extents[0].y, min.y);
      _object_instance_extents[0].z =
          std::min(_object_instance_extents[0].z, min.z);

      _object_instance_extents[1].x =
          std::max(_object_instance_extents[1].x, max.x);
      _object_instance_extents[1].y =
          std::max(_object_instance_extents[1].y, max.y);
      _object_instance_extents[1].z =
          std::max(_object_instance_extents[1].z, max.z);
    }
  }
...
void MapTile::recalcCombinedExtents() {
  ...
  if (!object_instances.empty()) {
    for (int i = 0; i < 3; ++i) {
      _combined_extents[0][i] =
          std::min(_combined_extents[0][i], _object_instance_extents[0][i]);
    }

    for (int i = 0; i < 3; ++i) {
      _combined_extents[1][i] =
          std::max(_combined_extents[1][i], _object_instance_extents[1][i]);
    }
  }
