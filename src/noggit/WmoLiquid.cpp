...
wmo_liquid::wmo_liquid(BlizzardArchive::ClientFile *f,
                       WMOLiquidHeader const &header, int group_liquid,
                       bool use_dbc_type, bool is_ocean)
    : pos(glm::vec3(header.pos.x, header.pos.z, -header.pos.y)),
      xtiles(header.A), ytiles(header.B) {
  ...
  if (use_dbc_type) {
    ...
    if (group_liquid == LIQUID_Green_Lava) {
      // todo: investigage
      // This method is most likely wrong since "liquid" is the last SMOLTile's
      // liquid value and it can vary from one liquid tile to another.
...
wmo_liquid::wmo_liquid(wmo_liquid const &other)
    : pos(other.pos), mTransparency(other.mTransparency), xtiles(other.xtiles),
      ytiles(other.ytiles), _liquid_id(other._liquid_id), depths(other.depths),
      tex_coords(other.tex_coords), vertices(other.vertices),
      indices(other.indices), _uploaded(false) {}
...
int wmo_liquid::initGeometry(BlizzardArchive::ClientFile *f) {
  ...
  for (int j = 0; j < ytiles + 1; j++) {
    for (int i = 0; i < xtiles + 1; ++i) {
      ...
      lVertices[p] =
          glm::vec3(pos.x + UNITSIZE * i, map[p].height, pos.z - UNITSIZE * j);
    }
  }
  ...
  for (int j = 0; j < ytiles; j++) {
    for (int i = 0; i < xtiles; ++i) {
      ...
      if (!(tile.liquid & 0x8)) {
        ...
        if (!(tile.liquid & 2)) {
          depths.emplace_back(static_cast<float>(map[p].water_vertex.flow1) /
                              255.0f);
          tex_coords.emplace_back(i, j);

          depths.emplace_back(
              static_cast<float>(map[p + 1].water_vertex.flow1) / 255.0f);
          tex_coords.emplace_back(i + 1, j);

          depths.emplace_back(
              static_cast<float>(map[p + xtiles + 1 + 1].water_vertex.flow1) /
              255.0f);
          tex_coords.emplace_back(i + 1, j + 1);

          depths.emplace_back(
              static_cast<float>(map[p + xtiles + 1].water_vertex.flow1) /
              255.0f);
          ...
          tex_coords.emplace_back(
              static_cast<float>(map[p + xtiles + 1 + 1].magma_vertex.s) /
                  255.f,
              static_cast<float>(map[p + xtiles + 1 + 1].magma_vertex.t) /
                  255.f);
          tex_coords.emplace_back(
              static_cast<float>(map[p + xtiles + 1].magma_vertex.s) / 255.f,
...
void wmo_liquid::upload(OpenGL::Scoped::use_program &water_shader) {
  ...
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec3>(_vertices_buffer, vertices,
                                            GL_STATIC_DRAW);
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec2>(_tex_coord_buffer, tex_coords,
                                            GL_STATIC_DRAW);
