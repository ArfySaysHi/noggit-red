...
// This can be moved somewhere lated, replaces Boost::variant::type() which
// returnes typeid of active indexed variant
// https://stackoverflow.com/questions/53696720/get-currently-held-typeid-of-stdvariant-like-boostvariant-type
template <class V> std::type_info const &var_type(V const &v) {
...
float getShortestDist(float x, float z, float squareX, float squareZ,
                      float unitSize);
float getShortestDist(glm::vec3 const &pos, glm::vec3 const &square_pos,
                      float unitSize);
...
float angledHeight(glm::vec3 const &origin, glm::vec3 const &pos,
                   math::radians const &angle,
                   math::radians const &orientation);
void extract_v3d_min_max(glm::vec3 const &point, glm::vec3 &min,
                         glm::vec3 &max);
std::vector<glm::vec3> intersection_points(glm::vec3 const &vmin,
                                           glm::vec3 const &vmax);
...
bool deg_vec3d_equals(math::degrees::vec3 const &v1,
                      math::degrees::vec3 const &v2);

glm::vec4 projectPointToScreen(const glm::vec3 &point,
                               const glm::mat4 &VPmatrix, float viewport_width,
                               float viewport_height, bool &valid);

std::array<glm::vec2, 2>
getBoundingBoxScreenBounds(const std::array<glm::vec3, 2> &local_extents,
                           const glm::mat4 &VPmatrix, float viewport_width,
                           float viewport_height, int &valid_points,
                           glm::mat4x4 const &obj_matrix, float scale = 1.0f);
...
glm::vec4 normalized_device_coords(int x, int y, int screen_width,
                                   int screen_height);
...
struct random_color : glm::vec4 {
  random_color()
      : glm::vec4(misc::randfloat(0.0f, 1.0f), misc::randfloat(0.0f, 1.0f),
...
template <typename T, std::size_t Capacity> struct max_capacity_stack_vector {
...
void SetChunkHeader(util::sExtendableArray &pArray, int pPosition, int pMagix,
                    int pSize = 0);
