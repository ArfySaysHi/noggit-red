...
LiquidLayer::LiquidLayer(ChunkWater *chunk, glm::vec3 const &base, float height,
                         int liquid_id)
    : _liquid_id(liquid_id), _liquid_vertex_format(0), _minimum(height),
      _maximum(height), _subchunks(0), pos(base), _chunk(chunk) {
  ...
  for (int z = 0; z < 9; ++z) {
    for (int x = 0; x < 9; ++x) {
      ...
      _depth[v_index] = 1.0f;
      _vertices[v_index] =
          glm::vec3(pos.x + UNITSIZE * x, height, pos.z + UNITSIZE * z);
    }
  }
...
LiquidLayer::LiquidLayer(ChunkWater *chunk, glm::vec3 const &base, mclq &liquid,
                         int liquid_id)
    : _liquid_id(liquid_id), _minimum(liquid.min_height),
      _maximum(liquid.max_height), _subchunks(0), pos(base), _chunk(chunk) {
  if (!gLiquidTypeDB.CheckIfIdExists(_liquid_id))
    _liquid_id = 1;
  ...
  for (int z = 0; z < 9; ++z) {
    for (int x = 0; x < 9; ++x) {
      ...
      if (_liquid_vertex_format == 1) {
        _depth[v_index] = 1.0f;
        _tex_coords[v_index] = glm::vec2(static_cast<float>(v.magma.x) / 255.f,
...
LiquidLayer::LiquidLayer(ChunkWater *chunk, BlizzardArchive::ClientFile &f,
                         std::size_t base_pos, glm::vec3 const &base,
    ...
    : _liquid_id(info.liquid_id),
      ...
      _minimum(info.minHeight), _maximum(info.maxHeight), _subchunks(0),
      pos(base), _chunk(chunk) {
  ...
  for (int z = 0; z < 9; ++z) {
    for (int x = 0; x < 9; ++x) {
      ...
      _depth[v_index] = 1.0f;
      _vertices[v_index] =
          glm::vec3(pos.x + UNITSIZE * x, _minimum, pos.z + UNITSIZE * z);
    }
  }
...
LiquidLayer::LiquidLayer(LiquidLayer &&other)
    : _liquid_id(other._liquid_id),
      ...
      _subchunks(other._subchunks), _vertices(other._vertices),
      _depth(other._depth), _tex_coords(other._tex_coords),
      _indices_by_lod(other._indices_by_lod), pos(other.pos),
      _chunk(other._chunk) {
  changeLiquidID(_liquid_id);
}

LiquidLayer::LiquidLayer(LiquidLayer const &other)
    : _liquid_id(other._liquid_id),
      _liquid_vertex_format(other._liquid_vertex_format),
      _minimum(other._minimum), _maximum(other._maximum),
      _subchunks(other._subchunks), _vertices(other._vertices),
...
LiquidLayer &LiquidLayer::operator=(LiquidLayer &&other) {
...
LiquidLayer &LiquidLayer::operator=(LiquidLayer const &other) {
...
void LiquidLayer::save(sExtendableArray &adt, int base_pos, int &info_pos,
  ...
  if (_liquid_vertex_format == 0 || _liquid_vertex_format == 1) {
    ...
    for (int z = info.yOffset; z <= info.yOffset + info.height; ++z) {
      for (int x = info.xOffset; x <= info.xOffset + info.width; ++x) {
        memcpy(adt.GetPointer<char>(current_pos), &_vertices[z * 9 + x].y,
               sizeof(float));
...
void LiquidLayer::changeLiquidID(int id) {
  _liquid_id = id;
...
void LiquidLayer::crop(MapChunk *chunk) {
...
void LiquidLayer::update_opacity(MapChunk *chunk, float factor) {
  for (int z = 0; z < 9; ++z) {
    for (int x = 0; x < 9; ++x) {
...
bool LiquidLayer::hasSubchunk(int x, int z, int size) const {
...
void LiquidLayer::setSubchunk(int x, int z, bool water) {
  misc::set_bit(_subchunks, x, z, water);
}

void LiquidLayer::paintLiquid(glm::vec3 const &cursor_pos, float radius,
        ...
        if (add) {
          for (int index : {id, id + 1, id + 9, id + 10}) {
            ...
            if (no_subchunk || (in_range && override_height)) {
              _vertices[index].y =
                  misc::angledHeight(ref, _vertices[index], angle, orientation);
            }
...
void LiquidLayer::update_min_max() {
...
void LiquidLayer::copy_subchunk_height(int x, int z, LiquidLayer const &from) {
  int id = 9 * z + x;
...
void LiquidLayer::update_vertex_opacity(int x, int z, MapChunk *chunk,
                                        float factor) {
...
int LiquidLayer::get_lod_level(glm::vec3 const &camera_pos) const {
