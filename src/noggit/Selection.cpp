...
void selected_chunk_type::updateDetails(
    Noggit::Ui::detail_infos *detail_widget) {
  std::stringstream select_info;

  mcnk_flags const &flags = chunk->header_flags;

  select_info << "<b>Chunk</b> (" << chunk->px << ", " << chunk->py
              << ") flat index: (" << chunk->py * 16 + chunk->px
              << ") of <b>tile</b> (" << chunk->mt->index.x << " , "
              << chunk->mt->index.z << ")"
              << "<br><b>area ID:</b> " << chunk->getAreaID() << " (\""
  ...
  if (chunk->mt->Water.hasData(0)) {
    ...
    MH2O_Render liquid_render = waterchunk->Render.value_or(
        MH2O_Render{0xffffffffffffffff, 0xffffffffffffffff});

    if (waterchunk->hasData(0)) {

      LiquidLayer liquid = waterchunk->getLayers()->at(
          0); // only getting data from layer 0, maybe loop them ?
      int liquid_flags = liquid.getSubchunks();

      select_info << "\nliquid type: " << liquid.liquidID() << " (\""
                  << gLiquidTypeDB.getLiquidName(liquid.liquidID()) << "\")"
                  << "\nliquid flags: "
  ...
  for (auto &tex : *(chunk->texture_set->getTextures())) {
    ...
    select_info << "<b>" << (counter + 1) << ":</b> "
                << tex->file_key().stringRepr();
...
selection_group::selection_group(std::vector<SceneObject *> selected_objects,
                                 World *world)
    : _world(world) {
...
selection_group::selection_group(std::vector<unsigned int> objects_uids,
                                 World *world)
    : _world(world) {
...
void selection_group::save_json() { _world->saveSelectionGroups(); }
...
void selection_group::select_group() {
  for (unsigned int obj_uid : _members_uid) {
    ...
    instance->_grouped = true; // ensure grouped attribute, some models could
                               // still be unloaded when creating the group
...
void selection_group::remove_group(bool save) {
  ...
  for (auto it = _world->_selection_groups.begin();
       it != _world->_selection_groups.end(); ++it) {
    auto it_group = *it;
    if (it_group.getMembers().size() == _members_uid.size() &&
        it_group.getExtents() == _group_extents)
