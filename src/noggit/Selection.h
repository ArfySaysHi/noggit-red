...
struct selected_chunk_type : Selectable {
  selected_chunk_type(MapChunk *_chunk, std::tuple<int, int, int> _triangle,
                      glm::vec3 _position)
...
enum eSelectionEntryTypes { eEntry_Object, eEntry_MapChunk };
...
public:
  ...
  void recalcExtents();
  // void copy_group(); // create and save a new selection group from copied
  // objects
