...
Noggit::Action::Action(MapView *map_view) : QObject(), _map_view(map_view) {}

void Noggit::Action::setFlags(int flags) { _flags = flags; }

void Noggit::Action::addFlags(int flags) { _flags |= flags; }

int Noggit::Action::getFlags() { return _flags; }
...
int Noggit::Action::getModalityControllers() { return _modality_controls; }
...
void Noggit::Action::undo(bool redo) {
  ...
  if (_flags & ActionFlags::eCHUNKS_TERRAIN) {
    for (auto &pair : redo ? _chunk_terrain_post : _chunk_terrain_pre) {
      std::memcpy(&pair.first->mVertices, pair.second.data(),
                  145 * 3 * sizeof(float));
  ...
  if (_flags & ActionFlags::eCHUNKS_TEXTURE) {
    for (auto &pair : redo ? _chunk_texture_post : _chunk_texture_pre) {
      ...
      *texture_set->getTempAlphamaps() = pair.second.tmp_edit_values;
      std::memcpy(texture_set->getMCLYEntries(), &pair.second.layers_info,
                  sizeof(ENTRY_MCLY) * 4);
  ...
  if (_flags & ActionFlags::eCHUNKS_VERTEX_COLOR) {
    for (auto &pair :
         redo ? _chunk_vertex_color_post : _chunk_vertex_color_pre) {
      std::memcpy(&pair.first->mccv, pair.second.data(),
                  145 * 3 * sizeof(float));
  ...
  if (_flags & ActionFlags::eVERTEX_SELECTION) {
    _map_view->getWorld()->setVertexSelectionCache(
        redo ? _vertex_selection_post : _vertex_selection_pre);
  }
  if (_flags & ActionFlags::eCHUNK_SHADOWS) {
    for (auto &pair : redo ? _chunk_shadow_map_post : _chunk_shadow_map_pre) {
      std::memcpy(&pair.first->_shadow_map, pair.second.data(),
  ...
  for (auto &pair : _added_objects_pre) {
    ...
    if (redo) {
      ...
      if (pair.second.type == ActionObjectTypes::WMO)
        obj = _map_view->getWorld()->addWMOAndGetInstance(
            pair.second.file_key, pair.second.pos, pair.second.dir);
      else
        obj = _map_view->getWorld()->addM2AndGetInstance(
            pair.second.file_key, pair.second.pos, pair.second.scale,
            pair.second.dir, nullptr);
...
unsigned Noggit::Action::handleObjectTransformed(unsigned uid, bool redo) {
  for (auto &pair :
       redo ? _transformed_objects_post : _transformed_objects_pre) {
    if (pair.first != uid)
      continue;
    ...
    _map_view->getWorld()->updateTilesEntry(obj, ModelUpdate::remove);
    ...
    _map_view->getWorld()->updateTilesEntry(obj, ModelUpdate::add);
...
void Noggit::Action::finish() {
  if (_flags & ActionFlags::eCHUNKS_TERRAIN) {
    ...
    for (size_t i = 0; i < _chunk_terrain_pre.size(); ++i) {
      ...
      std::memcpy(post.second.data(), &post.first->mVertices,
                  145 * 3 * sizeof(float));
    }
  }
  if (_flags & ActionFlags::eCHUNKS_TEXTURE) {
    ...
    for (size_t i = 0; i < _chunk_texture_pre.size(); ++i) {
      ...
      _chunk_texture_post.emplace_back(
          std::make_pair(pre.first, std::move(cache)));
    }
  }
  if (_flags & ActionFlags::eCHUNKS_VERTEX_COLOR) {
    ...
    for (size_t i = 0; i < _chunk_vertex_color_pre.size(); ++i) {
      ...
      post.first = pre.first;
      std::memcpy(post.second.data(), &post.first->mccv,
                  145 * 3 * sizeof(float));
    }
  }
  ...
  if (_flags & ActionFlags::eCHUNK_SHADOWS) {
    ...
    for (size_t i = 0; i < _chunk_shadow_map_pre.size(); ++i) {
      ...
      post.first = pre.first;
      std::memcpy(post.second.data(), &post.first->_shadow_map,
                  64 * 64 * sizeof(std::uint8_t));
    }
  }
...
void Noggit::Action::setDelta(float delta) { _delta = delta; }

float Noggit::Action::getDelta() const { return _delta; }

void Noggit::Action::setBlockCursor(bool state) { _block_cursor = state; }

bool Noggit::Action::getBlockCursor() const { return _block_cursor; }
...
void Noggit::Action::registerChunkTerrainChange(MapChunk *chunk) {
  ...
  // LogDebug << "Chunk: " << chunk->px << "_" << chunk->py << "on tile: " <<
  // chunk->mt->index.x << "_" << chunk->mt->index.z << std::endl;
}
...
void Noggit::Action::registerObjectTransformed(SceneObject *obj) {
  ...
  ActionObjectTypes type =
      obj->which() == eWMO ? ActionObjectTypes::WMO : ActionObjectTypes::M2;
  _transformed_objects_pre.emplace_back(std::make_pair(
      obj->uid, ObjectInstanceCache{obj->instance_model()->file_key(), type,
                                    obj->pos, obj->dir, obj->scale}));
}
...
void Noggit::Action::registerObjectAdded(SceneObject *obj) {
  ...
  ActionObjectTypes type =
      obj->which() == eWMO ? ActionObjectTypes::WMO : ActionObjectTypes::M2;
  _added_objects_pre.emplace_back(std::make_pair(
      obj->uid, ObjectInstanceCache{obj->instance_model()->file_key(), type,
                                    obj->pos, obj->dir, obj->scale}));
}
...
void Noggit::Action::registerObjectRemoved(SceneObject *obj) {
  ...
  ActionObjectTypes type =
      obj->which() == eWMO ? ActionObjectTypes::WMO : ActionObjectTypes::M2;
  _removed_objects_pre.emplace_back(std::make_pair(
      obj->uid, ObjectInstanceCache{obj->instance_model()->file_key(), type,
                                    obj->pos, obj->dir, obj->scale}));
}
...
void Noggit::Action::registerChunkLiquidChange(MapChunk *chunk) {
  ...
  _chunk_liquid_pre.emplace_back(
      std::make_pair(chunk, *chunk->liquid_chunk()->getLayers()));
}
