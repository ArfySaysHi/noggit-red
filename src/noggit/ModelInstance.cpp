...
ModelInstance::ModelInstance(BlizzardArchive::Listfile::FileKey const &file_key,
    ...
    : SceneObject(SceneObjectTypes::eMODEL, context), model(file_key, context) {
}
...
ModelInstance::ModelInstance(BlizzardArchive::Listfile::FileKey const &file_key,
  ...
  // scale factor - divide by 1024. blizzard devs must be on crack, why not just
  // use a float?
...
void ModelInstance::draw_box(glm::mat4x4 const &model_view,
  ...
  if (is_current_selection) {
    Noggit::Rendering::Primitives::WireBox::getInstance(_context).draw(
        model_view, projection, transformMatrix(), {1.0f, 1.0f, 0.0f, 1.0f},
    ...
    Noggit::Rendering::Primitives::WireBox::getInstance(_context).draw(
        model_view, projection, transformMatrix(), {1.0f, 1.0f, 1.0f, 1.0f},
    ...
    Noggit::Rendering::Primitives::WireBox::getInstance(_context).draw(
        model_view, projection, glm::mat4x4(1), {0.0f, 1.0f, 0.0f, 1.0f},
        extents[0], extents[1]);
  } else {
    const glm::vec4 color = _grouped ? glm::vec4(0.5f, 0.5f, 1.0f, 0.5f)
                                     : glm::vec4(1.0f, 1.0f, 1.0f, 1.0f);
    Noggit::Rendering::Primitives::WireBox::getInstance(_context).draw(
        model_view, projection, transformMatrix(), color,
        misc::transform_model_box_coords(model->header.bounding_box_min),
        misc::transform_model_box_coords(model->header.bounding_box_max));
  }
}
...
ModelInstance::intersect(glm::mat4x4 const &model_view, math::ray const &ray,
                         selection_result *results, int animtime) {
...
bool ModelInstance::isInRenderDist(const float &cull_distance,
...
void ModelInstance::recalcExtents() {
  ...
  math::aabb const relative_to_model(
      ...
      glm::max(model->header.collision_box_max,
  ...
  for (auto const &point : corners_in_world) {
    rotated_corners_in_world.emplace_back(transposedMat *
                                          glm::vec4(point, 1.f));
  }
  ...
  size_cat = glm::distance(bounding_of_rotated_points.max,
                           bounding_of_rotated_points.min);
...
void ModelInstance::updateDetails(Noggit::Ui::detail_infos *detail_widget) {
  ...
  for (unsigned j = 0; j < model->header.nTextures; j++) {
    ...
    bool error = model->_textures[j]->finishedLoading() &&
                 !model->_textures[j]->is_uploaded();
    ...
    select_info << "<b>" << (j + 1) << ":</b> "
                << model->_textures[j]->file_key().stringRepr();
...
void wmo_doodad_instance::update_transform_matrix_wmo(WMOInstance *wmo) {
  ...
  glm::mat4x4 mat(wmo->transformMatrix() * m2_mat);
