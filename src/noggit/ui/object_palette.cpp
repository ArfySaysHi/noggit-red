...
ObjectPalette::ObjectPalette(
    MapView *map_view, std::shared_ptr<Noggit::Project::NoggitProject> Project,
    QWidget *parent)
  ...
  _preview_renderer = new Noggit::Ui::Tools::PreviewRenderer(
      _object_list->iconSize().width(), _object_list->iconSize().height(),
  ...
  connect(_add_button, &QAbstractButton::clicked, this,
          &ObjectPalette::addObjectFromAssetBrowser);
  ...
  connect(_remove_button, &QAbstractButton::clicked, this,
          &ObjectPalette::removeSelectedTexture);
...
void ObjectPalette::addObjectFromAssetBrowser() {
  ...
  std::string const &display_name =
      reinterpret_cast<
          ...
          _map_view->getAssetBrowser()->widget())
          ->getFilename();
...
void ObjectPalette::removeObject(QString filename) {
  ...
  QList<QListWidgetItem *> objects =
      _object_list->findItems(filename, Qt::MatchExactly);
...
void ObjectPalette::dragEnterEvent(QDragEnterEvent *event) {
  if (event->mimeData()->hasText() &&
      (_object_paths.find(event->mimeData()->text().toStdString()) ==
       _object_paths.end()))
    event->accept();
}
...
ObjectPalette::~ObjectPalette() { delete _preview_renderer; }
