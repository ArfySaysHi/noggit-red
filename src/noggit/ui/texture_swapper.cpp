...
namespace Noggit {
namespace Ui {
texture_swapper::texture_swapper(QWidget *parent, const glm::vec3 *camera_pos,
                                 MapView *map_view)
  ...
  QPushButton *remove_text_adt =
      new QPushButton(tr("Remove this texture from ADT"), this);
  ...
  connect(select, &QPushButton::clicked, [&, map_view]() {
    ...
    if (_texture_to_swap) {
      _texture_to_swap_display->set_texture(
          _texture_to_swap.value()->file_key().filepath());
    }
  });
  ...
  connect(remove_text_adt, &QPushButton::clicked,
          [this, camera_pos, map_view]() {
            if (_texture_to_swap) {
              ActionManager::instance()->beginAction(
                  map_view, ActionFlags::eCHUNKS_TEXTURE);
...
void texture_swapper::set_texture(std::string const &filename) {
  _texture_to_swap = std::move(
      scoped_blp_texture_reference(filename, _world->getRenderContext()));
}
} // namespace Ui
} // namespace Noggit
