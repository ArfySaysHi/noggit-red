...
object_editor::object_editor(MapView *mapView, World *world,
  ...
  */
  ...
  connect(toTxt, &QPushButton::clicked,
          ([=, this]() { SaveObjecttoTXT(world); }));

  connect(fromTxt, &QPushButton::clicked,
          ([=, this]() { showImportModels(); }));
  ...
  connect(asset_browser_btn, &QPushButton::clicked, ([=, this]() {
            mapView->getAssetBrowser()->setVisible(
                mapView->getAssetBrowser()->isHidden());
          }));

  connect(object_palette_btn, &QPushButton::clicked, ([=, this]() {
            mapView->getObjectPalette()->setVisible(
                mapView->getObjectPalette()->isHidden());
          }));
...
void object_editor::pasteObject(glm::vec3 cursor_pos, glm::vec3 camera_pos,
  ...
  for (auto &selection : _model_instance_created) {
    ...
    if (obj->which() == eMODEL) {
      ...
      auto new_obj =
          world->addM2AndGetInstance(obj->instance_model()->file_key(), pos,
                                     scale, rotation, paste_params);
      ...
      auto new_obj = world->addWMOAndGetInstance(
          obj->instance_model()->file_key(), pos, rotation);
