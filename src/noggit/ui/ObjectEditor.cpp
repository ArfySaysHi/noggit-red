...
object_editor::object_editor(MapView *mapView, World *world,
                             ...
                             QWidget *parent)
    : QWidget(parent), modelImport(new model_import(this)),
      rotationEditor(new rotation_editor(mapView, world)),
      helper_models_widget(new helper_models(this)),
      _settings(new QSettings(this)), _copy_model_stats(true),
      _model_instance_created(), pasteMode(PASTE_ON_TERRAIN),
      _map_view(mapView) {
  setMinimumWidth(250);
  // setMaximumWidth(250);
  ...
  auto drag_selection_depth_layout = new
  QFormLayout(drag_selection_depth_group);
  ...
  asset_browser_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::file));
  layout->addWidget(asset_browser_btn);
  QPushButton *object_palette_btn = new QPushButton("Object palette", this);
  object_palette_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::palette));
  ...
  _horizonModel->setToolTip(
      "Sets a model that will be rendered in the horizon fog, allowing to "
      "render further than normal render distance."
      "Used to show landmarks from far away.");
  horizon_layout->addWidget(_horizonModel, 1);

  QToolButton *validateButton = new QToolButton;
  validateButton->setIcon(
      QApplication::style()->standardIcon(QStyle::SP_DialogApplyButton));
  // validateButton->setIconSize(QSize(24, 24));
  validateButton->setAutoRaise(
      true); // Gives it a flat toolbar-style appearance
  ...
  _previewButton->setIcon(Noggit::Ui::FontNoggitIcon(
      Noggit::Ui::FontNoggit::Icons::VISIBILITY_HIDDEN_MODELS));
  ...
  copyBox->setExpanded(
      _settings->value("object_editor/copy_options", false).toBool());
  ...
  pasteBox->setExpanded(
      _settings->value("object_editor/paste_options", false).toBool());
  ...
  QCheckBox *paste_override_rotate_cb =
      new QCheckBox("Rotate to Terrain", this);
  ...
  auto object_movement_snap_cb(new CheckBox("Mouse move snaps\nto objects",
                                            move_model_snap_to_objects, this));

  auto object_rotateground_cb(
      new CheckBox("Rotate following cursor", rotate_along_ground, this));

  auto object_rotategroundsmooth_cb(
      new CheckBox("Smooth follow rotation", rotate_along_ground_smooth, this));

  auto object_rotategroundrandom_cb(new CheckBox(
      "Random rot/tilt/scale\n on rotate", rotate_along_ground_random, this));
  ...
  auto object_median_pivot_point(
      new CheckBox("Rotate around pivot point", use_median_pivot_point, this));
  ...
  selectionOptionsBox->setExpanded(
      _settings->value("object_editor/movement_options", false).toBool());
  ...
  // QPushButton *visToggleButton = new QPushButton("Toggle Hidden Models
  // Visibility", this);
  QPushButton *clearListButton =
      new QPushButton("Clear Hidden Models List", this);
  QPushButton *clearGroupsButton =
      new QPushButton("Clear Selection Groups List", this);
  ...
  importBox->setExpanded(
      _settings->value("object_editor/import_box", false).toBool());
  ...
  connect(selectionOptionsBox, &ExpanderWidget::expanderChanged,
          [&](bool flag) {
  ...
  /*
  connect(_drag_selection_depth_spin,
  qOverload<double>(&QDoubleSpinBox::valueChanged) , [&] (double v)
          {
  ...
  */
  ...
  connect(_doodadSetSelector, qOverload<int>(&QComboBox::currentIndexChanged),
          [this](int index) {
            NOGGIT_ACTION_MGR->beginAction(
                _map_view, Noggit::ActionFlags::eOBJECTS_TRANSFORMED);
            _map_view->change_selected_wmo_doodadset(index);
            NOGGIT_ACTION_MGR->endAction();
          });

  connect(_nameSetSelector, qOverload<int>(&QComboBox::currentIndexChanged),
          [this](int index) {
            NOGGIT_ACTION_MGR->beginAction(
                _map_view, Noggit::ActionFlags::eOBJECTS_TRANSFORMED);
      ...
      if (obj->which() == eWMO) {
        ...
        if (!_horizonModel->text().isEmpty()) {
          if (!Noggit::Application::NoggitApplication::instance()
                   ->clientData()
                   ->exists(_horizonModel->text().toStdString())) {
            QMessageBox::warning(
                nullptr, "Warning",
                QString::fromStdString(_horizonModel->text().toStdString() +
                                       " not found."));
            return;
          }
        }
        ...
        {
          // same model
          if (lowresmodel.value()->get()->file_key().filepath() ==
              _horizonModel->text().toStdString())
            return;
          ...
          {
            // add a model instead of replacing, because a filename can be
            // referenced by multiple.
            // TODO store instances filenames directly for this reason instead
            // of using raw MWMO/MODF
            world->horizon.mWMOFilenames.push_back(
                _horizonModel->text().toStdString());
            world->horizon.wmos.emplace_back(
                scoped_wmo_reference(_horizonModel->text().toStdString(),
                                     _map_view->getRenderContext()));

            wi->lowResInstance->nameID =
                world->horizon.mWMOFilenames.size() - 1;
  ...
  QObject::connect(
      _previewButton, &QToolButton::toggled, this, [&](bool checked) {
        auto last_entry = _map_view->getWorld()->get_last_selected_model();
        // for (auto& selection : selected)
        if (last_entry) {
          if (last_entry.value().index() != eEntry_Object) {
...
float object_editor::brushRadius() const { return _radius; }
...
void object_editor::pasteObject(glm::vec3 cursor_pos, glm::vec3 camera_pos,
                                World *world,
                                object_paste_params *paste_params) {
  ...
  for (auto &selection : _model_instance_created) {
    ...
    case PASTE_ON_SELECTION:
      if (last_entry) {
        glm::vec3 last_entry_pos =
            std::get<selected_object_type>(last_entry.value())->pos;
    ...
    case PASTE_ON_CAMERA:
      ...
      break;
    default:
      LogDebug << "object_editor::pasteObject: unknown paste mode " << pasteMode
               << std::endl;
      break;
    }
    ...
    if (obj->which() == eMODEL) {
      ...
      float scale(1.f);
      math::degrees::vec3 rotation(math::degrees(0)._, math::degrees(0)._,
                                   math::degrees(0)._);
      ...
      auto new_obj = world->addM2AndGetInstance(
          obj->instance_model()->file_key(), pos, scale, rotation, paste_params,
          false, true);
      ...
      if (paste_params->rotate_on_terrain) {
        // new_obj->pos.y = world->get_ground_height(new_obj->pos).y;// in multi
        // select, objects aren't on the ground
      ...
      // new_obj->transformMatrix());
      bool is_indoor = false; // TODO Disabled the indoor check until non axis
                              // aligned boxes check is implemented
      if (is_indoor) {
        QMessageBox::warning(
            nullptr, "Warning",
            "You can't place M2 models inside WMO models interiors, they will "
            "not render."
            "\nTo place objects inside WMOs, use server side gameobjects or "
            "modify the WMO doodads(with wow blender studio).");
      }
      ...
      auto new_obj = world->addWMOAndGetInstance(
          obj->instance_model()->file_key(), pos, rotation, scale, true);
...
void object_editor::update_clipboard() {
  ...
  if (_model_instance_created.size() == 1) {
    ...
    if (selectedObject.index() == eEntry_Object) {
      ss << std::get<selected_object_type>(selectedObject)
                ->instance_model()
                ->file_key()
                .filepath();
    } else {
...
void object_editor::copy(std::string const &filename) {
  ...
  if (filename.ends_with(".m2")) {
    ModelInstance *mi =
        new ModelInstance(filename, _map_view->getRenderContext());
...
void object_editor::copy_current_selection(World *world) {
  ...
  for (auto &selection : current_selection) {
    ...
    if (obj->which() == eMODEL) {
      ...
      auto clone = new ModelInstance(original->model->file_key().filepath(),
                                     _map_view->getRenderContext());
      ...
      auto clone = new WMOInstance(original->wmo->file_key().filepath(),
                                   _map_view->getRenderContext());
...
void object_editor::SaveObjecttoTXT(World *world) {
  ...
  std::ofstream stream(_settings->value("project/import_file", "import.txt")
                           .toString()
                           .toStdString(),
                       std::ios_base::app);
  for (auto &selection : world->current_selection()) {
    ...
    std::string path = std::get<selected_object_type>(selection)
                           ->instance_model()
                           ->file_key()
                           .filepath();
...
void object_editor::import_last_model_from_wmv(int type) {
  std::string wmv_log_file(
      _settings->value("project/wmv_log_file").toString().toStdString());
    ...
    if (obj->which() == eMODEL) {
      ...
      for (auto &area_name : setnames) {
        if (area_name.empty()) {
          ...
          namesetnames.append(
              gAreaDB.getAreaFullName(chunk->getAreaID()).c_str());
        } else
      ...
      if (lowresmodel.has_value()) {
        _horizonModel->setText(
            lowresmodel.value()->get()->file_key().filepath().c_str());
