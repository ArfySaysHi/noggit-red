...
struct model_item : QStandardItem {
  model_item(QString const &display_role) : QStandardItem(display_role) {}
    ...
    if (role == Qt::DecorationRole) {
      if (!_rendered) {
        ...
        that->_pixmap = *BLPRenderer::getInstance().render_blp_to_pixmap(
            data(Qt::DisplayRole).toString().prepend("tileset/").toStdString(),
            256, 256);
      }
...
tileset_chooser::tileset_chooser(QWidget *parent) : widget(parent, Qt::Window) {
  ...
  for (auto const &entry_pair : Application::NoggitApplication::instance()
                                    ->clientData()
                                    ->listfile()
                                    ->pathToFileDataIDMap()) {
...

  {
    ...
    auto const prefix(std::filesystem::path(
  ...
  for (auto const &texture : tilesets) {
    auto item(
        new model_item(QString::fromStdString(texture).remove("tileset/")));
    item->setData(tilesets_with_specular_variant.count(texture) ? "true"
                                                                : "false",
                  has_specular_role);
  ...
  filter->addItems(
      {"",      "base",   "brick",  "brush",   "bush",       "cobblestone",
       "crack", "creep",  "crop",   "crystal", "dark",       "dead",
       "dirt",  "fern",   "floor",  "flower",  "footprints", "grass",
       "ice",   "ivy",    "lava",   "leaf",    "light",      "mineral",
       "moss",  "mud",    "needle", "pebbl",   "road",       "rock",
       "root",  "rubble", "sand",   "shore",   "slime",      "smooth",
       "snow",  "water",  "waves",  "web",     "weed"});
  connect(filter, &QComboBox::currentTextChanged,
          ([=, this](QString text) { search_filter->setFilterRegExp(text); }));

  auto only_specular(new QCheckBox("only with specular texture variant"));
  connect(only_specular, &QCheckBox::toggled, ([=, this](bool on) {
  ...
  connect(size_slider, &QSlider::valueChanged,
          ([=, this](int size) { list->setIconSize({size, size}); }));
...
std::optional<scoped_blp_texture_reference> selected_texture::get() {
  return selected_texture::texture; // TODO: something performance-hungry is
                                    // going on here
}
