...
TerrainTool::TerrainTool(MapView *map_view, QWidget *parent, bool stamp)
    : QWidget(parent), _edit_type(eTerrainType_Linear), _vertex_angle(0.0f),
      _vertex_orientation(0.0f), _cursor_pos(nullptr),
      _vertex_mode(eVertexMode_Center), _map_view(map_view) {
  setMinimumWidth(250);
  setMaximumWidth(250);
  ...
  _vertex_type_group->setSizePolicy(QSizePolicy::Preferred,
                                    QSizePolicy::Maximum);
  ...
  connect(_angle_slider, &QSlider::valueChanged, [this](int v) {
  ...
  connect(_orientation_dial, &QDial::valueChanged, [this](int v) {
    if (NOGGIT_CUR_ACTION) {
...
void TerrainTool::changeTerrain(World *world, glm::vec3 const &pos, float dt) {

  float radius = static_cast<float>(_radius_slider->value());
  if (_edit_type != eTerrainType_Vertex) {
    if (_image_mask_group->isEnabled()) {
      // store the ground height diff at center of all objects hit before
      // editing it
      std::vector<std::pair<SceneObject *, float>> objects_ground_distance =
          world->getObjectsGroundDistance(pos, radius,
      ...
      for (auto pair : objects_ground_distance) {
        ...
        world->set_model_pos(
            obj,
            glm::vec3(obj->pos.x, new_ground_height + pair.second, obj->pos.z));
      }
    } else {
      world->changeTerrain(pos, dt * _speed_slider->value(), radius, _edit_type,
                           _inner_radius_slider->value());

      world->changeObjectsWithTerrain(pos, dt * _speed_slider->value(), radius,
                                      _edit_type, _inner_radius_slider->value(),
                                      _snap_wmo_objects_chkbox->isChecked(),
                                      _snap_m2_objects_chkbox->isChecked());
    }
  } else {
...
void TerrainTool::nextType() {
  _edit_type = static_cast<eTerrainType>((static_cast<int>(_edit_type) + 1) %
                                         eTerrainType_Count);
...
void TerrainTool::setSpeed(float speed) { _speed_slider->setValue(speed); }
...
void TerrainTool::setOrientRelativeTo(World *world, glm::vec3 const &pos) {
  if (_edit_type == eTerrainType_Vertex) {
    ...
    _vertex_orientation =
        math::radians(std::atan2(center.z - pos.z, center.x - pos.x));
    emit updateVertices(_vertex_mode, _vertex_angle, _vertex_orientation);
  }
}
...
void TerrainTool::updateVertexGroup() {
  ...
  _image_mask_group->setVisible(_edit_type != eTerrainType_Vertex &&
                                _edit_type != eTerrainType_Script);
}

QSize TerrainTool::sizeHint() const { return QSize(250, height()); }
