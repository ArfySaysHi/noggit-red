...
helper_models::helper_models(object_editor *object_editor)
  ...
  icon->setPixmap(*BLPRenderer::getInstance().render_blp_to_pixmap(
  ...
  auto add_button([&](char const *label, std::string path, int pos_x,
                      int pos_y) {
    auto button(new QPushButton(label, this));

    connect(button, &QPushButton::clicked, [=] { object_editor->copy(path); });
