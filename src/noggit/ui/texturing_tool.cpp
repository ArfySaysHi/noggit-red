...
namespace Noggit {
namespace Ui {
texturing_tool::texturing_tool(const glm::vec3 *camera_pos, MapView *map_view,
                               BoolToggleProperty *show_quick_palette,
                               QWidget *parent)
    : QWidget(parent), _brush_level(255), _show_unpaintable_chunks(false),
      _spray_size(1.0f), _spray_pressure(2.0f), _anim_prop(true),
      _anim_speed_prop(1), _anim_rotation_prop(4), _overbright_prop(false),
      _texturing_mode(texturing_mode::paint), _map_view(map_view) {
  ...
  _hardness_slider =
      new Noggit::Ui::Tools::UiCommon::ExtendedSlider(tool_widget);
  ...
  _spray_mode_group->setSizePolicy(QSizePolicy::Preferred,
                                   QSizePolicy::Maximum);
  ...
  _image_mask_group->setSizePolicy(QSizePolicy::Preferred,
                                   QSizePolicy::Maximum);
  ...
  connect(_hardness_slider,
          &Noggit::Ui::Tools::UiCommon::ExtendedSlider::valueChanged,
          [&](double v) { update_brush_hardness(); });

  connect(_image_mask_group,
          &Noggit::Ui::Tools::ImageMaskSelector::rotationUpdated, this,
          &texturing_tool::updateMaskImage);
  connect(_radius_slider,
...
void texturing_tool::set_radius(float radius) {
  ...
  _inner_brush.setRadius(radius *
                         static_cast<float>(_hardness_slider->value()));
}
...
void texturing_tool::change_radius(float change) {
  if (_texturing_mode == texturing_mode::paint) {
    _radius_slider->setValue(static_cast<float>(_radius_slider->value()) +
                             change);
  } else if (_texturing_mode == texturing_mode::swap) {
...
void texturing_tool::change_hardness(float change) {
  if (_texturing_mode == texturing_mode::paint) {
    _hardness_slider->setValue(static_cast<float>(_hardness_slider->value()) +
                               change);
  }
}
...
void texturing_tool::change_pressure(float change) {
  if (_texturing_mode == texturing_mode::paint) {
    _pressure_slider->setValue(static_cast<float>(_pressure_slider->value()) +
                               change);
  }
}
...
float texturing_tool::brush_radius() const {
  ...
  case texturing_mode::swap:
    return (_texture_switcher->brush_mode() ? _texture_switcher->radius()
                                            : 0.f);
  default:
    return 0.f;
  }
...
float texturing_tool::hardness() const {
  switch (_texturing_mode) {
  case texturing_mode::paint:
    return static_cast<float>(_hardness_slider->value());
  default:
    return 0.f;
  }
}
...
void texturing_tool::paint(World *world, glm::vec3 const &pos, float dt,
  ...
  if (_texturing_mode == texturing_mode::swap) {
    ...
    if (to_swap) {
      if (_texture_switcher->brush_mode()) {
        ...
        world->replaceTexture(pos, _texture_switcher->radius(), to_swap.value(),
                              texture, _texture_switcher->entireChunk());
      } else {
    ...
    if (_spray_mode_group->isChecked()) {
      world->sprayTexture(pos, &_spray_brush, alpha_target(), strength,
                          static_cast<float>(_radius_slider->value()),
...
void texturing_tool::change_tex_flag(World *world, glm::vec3 const &pos,
                                     bool add,
                                     scoped_blp_texture_reference texture) {
  std::uint32_t flag = 0;
...
QJsonObject texturing_tool::toJSON() {
  ...
  if (_texture_switcher->texture_to_swap().has_value())
    json["texture_to_swap"] = _texture_switcher->texture_to_swap()
                                  .value()
                                  ->file_key()
                                  .filepath()
                                  .c_str();
...
void texturing_tool::fromJSON(QJsonObject const &json) {
  _current_texture->set_texture(
      json["current_texture"].toString().toStdString());
  ...
  _show_unpaintable_chunks_cb->setChecked(
      json["show_unpaintable_chunks"].toBool());
