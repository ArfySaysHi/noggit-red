...
namespace Noggit {
namespace Ui {
water::water(UnsignedIntProperty *current_layer,
             BoolToggleProperty *display_all_layers, QWidget *parent)
    : QWidget(parent), _liquid_id(5), _radius(10.0f), _angle(10.0f),
      _orientation(0.0f), _locked(false), _angled_mode(false),
      _override_liquid_id(true), _override_height(true),
      _opacity_mode(river_opacity), _custom_opacity_factor(0.0337f),
      _lock_pos(glm::vec3(0.0f, 0.0f, 0.0f)), tile(0, 0) {
  setMinimumWidth(250);
  setMaximumWidth(250);
  ...
  for (DBCFile::Iterator i = gLiquidTypeDB.begin(); i != gLiquidTypeDB.end();
    ...
    // filter WMO liquids
    if (liquid_id == 13 || liquid_id == 14 || liquid_id == 17 ||
        liquid_id == 19 || liquid_id == 20)
      continue;
  ...
  connect(&_angled_mode, &BoolToggleProperty::changed, angle_group,
          &QGroupBox::setChecked);
  connect(angle_group, &QGroupBox::toggled, &_angled_mode,
          &BoolToggleProperty::set);
  ...
  connect(&_locked, &BoolToggleProperty::changed, lock_group,
          &QGroupBox::setChecked);
  connect(lock_group, &QGroupBox::toggled, &_locked, &BoolToggleProperty::set);
  ...
  layout->addRow(new pushbutton("Crop water", [this] { emit crop_water(); }));
  ...
  connect(waterLayer, qOverload<int>(&QSpinBox::valueChanged), current_layer,
          &UnsignedIntProperty::set);
  connect(current_layer, &UnsignedIntProperty::changed, waterLayer,
          &QSpinBox::setValue);
...
void water::paintLiquid(World *world, glm::vec3 const &pos, bool add) {
  world->paintLiquid(pos, _radius, _liquid_id, add,
...
QSize water::sizeHint() const { return QSize(250, height()); }
} // namespace Ui
} // namespace Noggit
