...
void RecentProjectsComponent::buildRecentProjectsList(
  ...
  for (int i = 0; i < size; ++i) {
    ...
    std::filesystem::path project_path =
        settings.value("project_path").toString().toStdString().c_str();
    ...
    project_data.project_directory =
        QString::fromStdString(project_path.generic_string());
    project_data.project_name = QString::fromStdString(project->ProjectName);
    project_data.project_last_edited =
        QDateTime::currentDateTime().date().toString();

    auto project_list_item = new Noggit::Ui::Widget::ProjectListItem(
        project_data, parent->_ui->listView);

    item->setData(Qt::UserRole,
                  QVariant(QString(project_path.string().c_str())));
    item->setSizeHint(project_list_item->minimumSizeHint());

    QObject::connect(
        project_list_item, &QListWidget::customContextMenuRequested,
        [=](const QPoint &pos) {
          QMenu context_menu(project_list_item->tr("Context menu"),
                             project_list_item);

          // removing the option to delete project files due to people
          // accidentally removing their work...
          /*
          QAction action_1("Delete Project", project_list_item);
          action_1.setIcon(FontAwesomeIcon(FontAwesome::trash).pixmap(QSize(16,
          16)));
          ...
          */
          ...
          action_2.setIcon(
              FontAwesomeIcon(FontAwesome::cloud).pixmap(QSize(16, 16)));

          QObject::connect(&action_2, &QAction::triggered, [=]() {
            parent->handleContextMenuProjectListItemForget(
                project_data.project_directory.toStdString());
          });
          ...
          action_3.setIcon(
              FontAwesomeIcon(FontAwesome::folderopen).pixmap(QSize(16, 16)));
          ...
          action_4.setIcon(
              FontAwesomeIcon(FontAwesome::gamepad).pixmap(QSize(16, 16)));
...
void RecentProjectsComponent::registerProjectChange(
  ...
  // remove duplicates
  auto it = std::find(recent_projects.begin(), recent_projects.end(),
                      project_path.c_str());
...
void RecentProjectsComponent::registerProjectRemove(
    std::string const &project_path) {
  QSettings settings;
  settings.sync();
