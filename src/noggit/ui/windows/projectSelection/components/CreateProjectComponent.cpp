...
void CreateProjectComponent::createProject(
    ...
    for (const auto &entry : std::filesystem::directory_iterator(
      ...
      if (entry.path().extension() == ".noggitproj") {
        QMessageBox::critical(parent, "Error",
                              "Selected already contains an existing project.");
        return;
      }
    }
  }

  application_project_service.createProject(
      project_information.project_path, project_information.game_client_path,
      project_information.game_client_version,
      project_information.project_name);

  RecentProjectsComponent::registerProjectChange(
      project_information.project_path);
}