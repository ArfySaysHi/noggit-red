...
namespace Noggit {
namespace Ui {
CUpdater::CUpdater(QWidget *parent) : QDialog(parent) {
...
QByteArray CUpdater::FileMD5(const QString &filename,
...
void CUpdater::GenerateLocalMD5() {
  QVector<QString> ignore = {"md5", "listfile.csv", "log.txt"};

  QDirIterator it(QDir::currentPath(), QStringList() << "*", QDir::Files,
  ...
  for (const auto &e : OnlineMD5.toStdMap()) {
    if (LocalMD5[e.first] != e.second) {
      ...
      if (ui->FileList->toPlainText().isEmpty()) {
        ui->FileList->append(
            QString(tr("This is all files needed to update.")));
      }

      ui->FileList->append(QString(" - %1 [%2 => %3]")
                               .arg(FileNeeded.last())
                               .arg(LocalMD5[e.first])
                               .arg(e.second));
    }
  }
...
void CUpdater::GetOnlineFile() {
  ...
  if (index >= 0) {
    QDir dir;
    if (!dir.exists(FileURL.arg(QDir::currentPath(), TemporaryFolder,
                                FileNeeded[0].mid(0, index))))
      dir.mkpath(FileURL.arg(QDir::currentPath(), TemporaryFolder,
                             FileNeeded[0].mid(0, index)));
  }
