...
namespace Noggit {
namespace Ui {
SoundEntryPlayer::SoundEntryPlayer(QWidget *parent) : QWidget(parent) {
  ...
  connect(btn_pause, &QToolButton::clicked, _media_player,
          &QMediaPlayer::pause);
  ...
  // connect(btn_volume, &QToolButton::clicked, _media_player,
  // &QMediaPlayer::stop);
  ...
  connect(_files_listview, &QListWidget::itemClicked, this,
          ([=, this](QListWidgetItem *item) { play_selected_sound(); }));

  // connect volume
  connect(_volume_slider, &QSlider::valueChanged,
          [&](int v) { _media_player->setVolume(v); });

  // media doesn't start playnig imemdiatly when called, need to use this signal
  connect(_media_player, &QMediaPlayer::durationChanged, this,
          [&](qint64 dur) { _position_slider->setMaximum(dur); });

  // connect(_media_player, SIGNAL(positionChanged(qint64)), this,
...
void SoundEntryPlayer::LoadSoundsFromSoundEntry(int sound_entry_id) {
  ...
  _directory_lbl->setText(
      sound_entry_record.getString(SoundEntriesDB::FilePath));

  _volume_slider->setValue(sound_entry_record.getFloat(SoundEntriesDB::Volume) *
                           100);
  _media_player->setVolume(sound_entry_record.getFloat(SoundEntriesDB::Volume) *
                           100);

  for (int fileid = 0; fileid < 10; fileid++) {
    std::string filename =
        sound_entry_record.getString(SoundEntriesDB::Filenames + fileid);
    if (!filename.empty()) {
...
void SoundEntryPlayer::PlaySingleSoundFile(std::string filepath,
                                           std::string dir_path) {
...
void SoundEntryPlayer::play_selected_sound() {
  ...
  filename << _directory_lbl->text().toStdString() << "/"
           << item->text().toStdString();

  if (!Noggit::Application::NoggitApplication::instance()->clientData()->exists(
          filename.str())) {
    LogError << "The requested sound file \"" << filename.str()
    ...
    std::stringstream ss;
    ss << "The requested sound file \"" << filename.str()
       << "\" was not found in the client." << std::endl;
  ...
  auto temp_file =
      new QTemporaryFile(this); // must parent for the object to be destroyed
                                // properly(and file deleted)

  temp_file->open();
  temp_file->write(file.getBuffer(), file.getSize());
  temp_file->close();
  // default tempname is like User\AppData\Local\Temp\Noggit.qrRfsy ...We need
  // to add back the file extension or it won't be read by the player! must
  // rename after closing or it doesn't write correctly
  ...
  _media_player->setMedia(QUrl::fromLocalFile(
      temp_file
          ->fileName())); // QUrl::fromLocalFile("/Users/me/Music/coolsong.mp3")
  _media_player->play();
}
