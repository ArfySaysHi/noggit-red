...
SoundEntryPlayer::SoundEntryPlayer(QWidget *parent) : QWidget(parent) {
  ...
  QObject::connect(
      _files_listview, &QListWidget::itemSelectionChanged, [this]() {
...
void SoundEntryPlayer::LoadSoundsFromSoundEntry(int sound_entry_id) {
  ...
  // get files list
  try {
    DBCFile::Record sound_entry_record =
        gSoundEntriesDB.getByID(sound_entry_id);

    _directory_lbl->setText(
        sound_entry_record.getString(SoundEntriesDB::FilePath));

    _volume_slider->setValue(
        sound_entry_record.getFloat(SoundEntriesDB::Volume) * 100);
    _media_player->setVolume(
        sound_entry_record.getFloat(SoundEntriesDB::Volume) * 100);
