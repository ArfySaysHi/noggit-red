...
settings::settings(QWidget *parent)
    : QMainWindow(parent, Qt::Window), _settings(new QSettings(this)) {
  ...
  // dev QOL, selecting another tab when editing the UI file makes it the
  // default
  ui->tabWidget->setCurrentIndex(0);

  ui->mysql_client_db_checkbox
      ->hide(); // Currently unfinished feature, need all DBCs to use the new
                // database library
  ...
  connect(ui->importPathField_browse, &QPushButton::clicked, [=, this] {
    auto result(QFileDialog::getOpenFileName(nullptr, "Import File Path",
                                             ui->importPathField->text()));
  ...
  connect(ui->wmvLogPathField_browse, &QPushButton::clicked, [=, this] {
    auto result(QFileDialog::getOpenFileName(nullptr, "WMV Log Path",
                                             ui->wmvLogPathField->text()));
  ...
  if (theme_dir.exists()) {
    ...
    LogError << "Failed to load themes. The \"themes/\" folder does not exist "
                "in Noggit directory. Using system theme."
             << std::endl;
  }
  ...
  connect(ui->_theme, &QComboBox::currentTextChanged, [&](QString s) {
    ...
    if (file.open(QFile::ReadOnly)) {
      ...
      QString style_sheet_fixed =
          style_sheet.replace("@rpath", QCoreApplication::applicationDirPath());
  ...
  connect(ui->mysql_connect_test, &QPushButton::clicked, [this] {
    ...
    if (!db_manager.isDriverAvailable()) {
      ...
      prompt.setInformativeText(
          "Make sure Noggit/sqldrivers/qsqlmysql.dll exists."
          "If not, it can be downloaded at ");
      prompt.exec();
      return;
    }
    ...
    bool test1 = db_manager.initializeDbConnection(Sql::SQLDbType::Noggit, host,
                                                   port, db_name, user, pass);
    bool test2 =
        db_manager.testConnectionWithPopup(Sql::SQLDbType::Noggit, false);
    ...
    bool test3 = db_manager.initializeDbConnection(Sql::SQLDbType::Noggit, host,
                                                   port, db_name, user, pass);
...
void settings::discard_changes() {
  ui->importPathField->setText(
      _settings->value("project/import_file", "import.txt").toString());
  ui->wmvLogPathField->setText(
      _settings->value("project/wmv_log_file").toString());
  ui->_view_distance->setValue(
      _settings->value("view_distance", 2000.f).toFloat());
  ...
  ui->_anti_aliasing_cb->setChecked(
      _settings->value("anti_aliasing", false).toBool());
  ui->_fullscreen_cb->setChecked(
      _settings->value("fullscreen", false).toBool());
  ui->_background_fps_limit_cb->setChecked(
      _settings->value("background_fps_limit", true).toBool());
  ui->_directional_light_cb->setChecked(
      _settings->value("directional_lightning", true).toBool());
  ui->_local_lightning_cb->setChecked(
      _settings->value("local_lightning", true).toBool());
  ui->_adt_unload_dist->setValue(_settings->value("unload_dist", 5).toInt());
  ui->_adt_unload_check_interval->setValue(
      _settings->value("unload_interval", 30).toInt());
  ui->_adt_loading_radius->setValue(
      _settings->value("loading_radius", 2).toInt());
  ui->_uid_cb->setChecked(_settings->value("uid_startup_check", true).toBool());
  ui->_load_fav_cb->setChecked(
      _settings->value("auto_load_fav_project", true).toBool());
  ui->_systemWindowFrame->setChecked(
      _settings->value("systemWindowFrame", true).toBool());
  ui->_nativeMenubar->setChecked(
      _settings->value("nativeMenubar", true).toBool());
  ...
  ui->_keyboard_locale->setCurrentText(
      _settings->value("keyboard_locale", "QWERTY").toString());
  ui->_use_mclq_liquids_export->setChecked(
      _settings->value("use_mclq_liquids_export", false).toBool());
  ui->_theme->setCurrentText(_settings->value("theme", "Dark").toString());
  ui->_modern_features->setChecked(
      _settings->value("modern_features", false).toBool());

  ui->assetBrowserBgCol->setColor(
      _settings
          ->value("assetBrowser/background_color",
                  QVariant::fromValue(QColor(127, 127, 127)))
          .value<QColor>());
  ui->assetBrowserDiffuseLight->setColor(
      _settings
          ->value(
              "assetBrowser/diffuse_light",
              QVariant::fromValue(QColor::fromRgbF(1.0f, 0.532352924f, 0.0f)))
          .value<QColor>());

  ui->assetBrowserAmbientLight->setColor(
      _settings
          ->value("assetBrowser/ambient_light",
                  QVariant::fromValue(QColor::fromRgbF(
                      0.407770514f, 0.508424163f, 0.602650642f)))
          .value<QColor>());

  ui->assetBrowserCopyToClipboard->setChecked(
      _settings->value("assetBrowser/copy_to_clipboard", true).toBool());
  ui->assetBrowserDefaultModel->setText(
      _settings
          ->value("assetBrowser/default_model",
                  "world/wmo/azeroth/human/buildings/human_farm/farm.wmo")
          .toString());
  ui->assetBrowserMoveSensitivity->setValue(
      _settings->value("assetBrowser/move_sensitivity", 15.0f).toFloat());
  ui->assetBrowserRenderAssetPreview->setChecked(
      _settings->value("assetBrowser/render_asset_preview", false).toBool());

  ui->_m2_aabb_2->setChecked(
      _settings->value("render/m2_aabb", false).toBool());
  ui->_m2_coll_bb->setChecked(
      _settings->value("render/m2_coll_bb", false).toBool());
  ui->_wmo_aabb->setChecked(
      _settings->value("render/wmo_aabb", false).toBool());
  ui->_wmo_group_bounds->setChecked(
      _settings->value("render/wmo_groups_bounds", false).toBool());

  // ui->MySQL_box->setChecked (_settings->value
  // ("project/mysql/enabled").toBool());
  ui->mysql_uid_checkbox->setChecked(
      _settings->value("project/mysql/enabled").toBool());
  ui->mysql_client_db_checkbox->setChecked(
      _settings->value("project/mysql/client_db_enabled").toBool());

  auto server_str =
      _settings->value("project/mysql/server", "127.0.0.1").toString();
...
void settings::save_changes() {
  ...
  _settings->setValue("undock_tool_properties/enabled",
                      ui->_undock_tool_properties->isChecked());
  ...
  _settings->setValue("background_fps_limit",
                      ui->_background_fps_limit_cb->isChecked());
  _settings->setValue("directional_lightning",
                      ui->_directional_light_cb->isChecked());
  ...
  _settings->setValue("unload_interval",
                      ui->_adt_unload_check_interval->value());
  ...
  _settings->setValue("additional_file_loading_log",
                      ui->_additional_file_loading_log->isChecked());
  ...
  _settings->setValue("use_mclq_liquids_export",
                      ui->_use_mclq_liquids_export->isChecked());

  // _settings->setValue ("project/mysql/enabled", ui->MySQL_box->isChecked());
  _settings->setValue("project/mysql/enabled",
                      ui->mysql_uid_checkbox->isChecked());
  _settings->setValue("project/mysql/client_db_enabled",
                      ui->mysql_client_db_checkbox->isChecked());
  ...
  _settings->setValue("assetBrowser/background_color",
                      ui->assetBrowserBgCol->color());
  _settings->setValue("assetBrowser/diffuse_light",
                      ui->assetBrowserDiffuseLight->color());
  _settings->setValue("assetBrowser/ambient_light",
                      ui->assetBrowserAmbientLight->color());
  _settings->setValue("assetBrowser/copy_to_clipboard",
                      ui->assetBrowserCopyToClipboard->isChecked());
  _settings->setValue("assetBrowser/default_model",
                      ui->assetBrowserDefaultModel->text());
  _settings->setValue("assetBrowser/move_sensitivity",
                      ui->assetBrowserMoveSensitivity->value());
  _settings->setValue("assetBrowser/render_asset_preview",
                      ui->assetBrowserRenderAssetPreview->isChecked());
  ...
  _settings->setValue("render/wmo_groups_bounds",
                      ui->_wmo_group_bounds->isChecked());
