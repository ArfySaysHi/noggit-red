...
MapListBookmarkItem::MapListBookmarkItem(const MapListBookmarkData &data,
  ...
  sstream << std::to_string((int)data.Position.x) << " , "
          << std::to_string((int)data.Position.y) << " , "
          << std::to_string((int)data.Position.z);
...
QSize MapListBookmarkItem::minimumSizeHint() const { return QSize(300, 32); }
