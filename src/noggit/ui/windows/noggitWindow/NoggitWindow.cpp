...
NoggitWindow::NoggitWindow(
    ...
    : QMainWindow(nullptr), _null_widget(new QWidget(this)),
      _applicationConfiguration(application), _project(project) {
  ...
  Log << "Project version : "
      << Noggit::Project::ClientVersionFactory::MapToStringVersion(
  ...
  if (project->projectVersion == Project::ProjectVersion::WOTLK) {
    ...
    assert(false); // TODO
    LogError << "NoggitWindow() : Unsupported project version, skipping "
                "loading DBCs."
             << std::endl;
  }
  ...
  bool use_sql_client_database =
      settings.value("project/mysql/client_db_enabled", false).toBool();

  ClientDatabase::setDatabaseMode(use_sql_client_database
                                      ? DatabaseMode::Sql
                                      : DatabaseMode::ClientStorage);
  ...
  if (use_mysql_uid || use_sql_client_database) {
    ...
    db_manager.initializeDbConnection(Sql::SQLDbType::Noggit, host, port,
                                      db_name, user, pass);
  }
  ...
  if (!settings.value("systemWindowFrame", true).toBool()) {
    ...
    ::Ui::TitleBar *titleBarWidget =
        setupFramelessWindow(widget, this, minimumSize(), maximumSize(), true);
  ...
  QObject::connect(about_action, &QAction::triggered, [&] { _about->show(); });
  ...
  QObject::connect(proj_selec_action, &QAction::triggered, [this] {
    ...
    // auto project_selection = new
    // Noggit::Ui::Windows::NoggitProjectSelectionWindow(noggit);
  ...
  QObject::connect(mapmenu_action, &QAction::triggered, [this] { close(); });
  ...
  _map_creation_wizard =
      new Noggit::Ui::Tools::MapCreationWizard::Ui::MapCreationWizard(_project,
                                                                      this);

  buildMenu();
}

void NoggitWindow::check_uid_then_enter_map(glm::vec3 pos,
  ...
  if ((Noggit::Sql::SqlUIDStorage::hasMaxUIDStoredDB(world_map_id)) ||
    ...
    enterMapAt(pos, camera_pitch, camera_yaw, uid_fix_mode::none,
               from_bookmark);
  }
  ...
  from_bookmark);
    ...
    connect(uid_fix_window, &Noggit::Ui::UidFixWindow::fix_uid,
            [this, from_bookmark](glm::vec3 pos, math::degrees camera_pitch,
                                  math::degrees camera_yaw,
...
void NoggitWindow::enterMapAt(glm::vec3 pos, math::degrees camera_pitch,
  ...
  if (world->mapIndex.hasAGlobalWMO()) {
    ...
    // pos = glm::vec3(_world->mWmoEntry[0], _world->mWmoEntry.pos[1],
    // _world->mWmoEntry.pos[2]);

    // better, enter at model's max extent, facing toward min extent
    auto min_extent = glm::vec3(world->mWmoEntry.extents[0][0],
                                world->mWmoEntry.extents[0][1],
                                world->mWmoEntry.extents[0][2]);
    auto max_extent = glm::vec3(world->mWmoEntry.extents[1][0],
                                world->mWmoEntry.extents[1][1] * 2,
                                world->mWmoEntry.extents[1][2]);
    float dx = min_extent.x - max_extent.x;
    float dy = min_extent.z -
               max_extent.z; // flipping z and y works better for some reason
    float dz = min_extent.y - max_extent.y;

    pos = {world->mWmoEntry.pos[0], world->mWmoEntry.pos[1],
           world->mWmoEntry.pos[2]};
  ...
  _map_view = (new MapView(camera_yaw, camera_pitch, pos, this, _project,
                           std::move(_map_creation_wizard->_world), uid_fix,
                           from_bookmark));
  connect(_map_view, &MapView::uid_fix_failed,
          [this]() { promptUidFixFailure(); });
...
void NoggitWindow::applyFilterSearch(const QString &name, int type,
                                     int expansion, bool wmo_maps) {
  for (int i = 0; i < _continents_table->count(); ++i) {
    auto item_widget = _continents_table->item(i);
    auto widget = qobject_cast<Noggit::Ui::Widget::MapListItem *>(
        _continents_table->itemWidget(item_widget));
...
void NoggitWindow::loadMap(int map_id) {
  ...
  _world = std::make_unique<World>(record.Columns["Directory"].Value, map_id,
...
void NoggitWindow::buildMenu() {
  ...
  _continents_table->setSelectionMode(
      QAbstractItemView::SelectionMode::SingleSelection);
  _continents_table->setSelectionBehavior(QAbstractItemView::SelectItems);

  // in some situations like when returning to menu an item is selected and
  // itemSelectionChanged won't fire when reclicking it so might need to also
  // connect itemClicked but then they both trigger at the same time.
  QObject::connect(
      _continents_table, &QListWidget::itemSelectionChanged, [this]() {
  ...
  /* set-up widget for seaching etc... through _continents_table */
  {
    ...
    _combo_search->addItems(QStringList()
                            << tr("All") << tr("Unknown") << tr("Continent")
                            << tr("Dungeon") << tr("Raid") << tr("Battleground")
                            << tr("Arena") << tr("Scenario"));
    QSettings settings;
    ...
    _combo_exp_search->addItem(QIcon(":/icon-wrath"),
                               tr("Wrath of the Lich King"));
    _combo_exp_search->addItem(QIcon(":/icon-cata"), tr("Cataclism"));
    _combo_exp_search->addItem(QIcon(":/icon-panda"), tr("Mist of Pandaria"));
    _combo_exp_search->addItem(QIcon(":/icon-warlords"),
                               tr("Warlords of Draenor"));
    _combo_exp_search->addItem(QIcon(":/icon-legion"), tr("Legion"));
    _combo_exp_search->addItem(QIcon(":/icon-battle"),
                               tr("Battle for Azeroth"));
    _combo_exp_search->addItem(QIcon(":/icon-shadow"), tr("Shadowlands"));
    _combo_exp_search->setCurrentIndex(0);

    QCheckBox *_wmo_maps_search =
        new QCheckBox("Display WMO maps (No terrain)", this);

    QObject::connect(_line_edit_search,
                     QOverload<const QString &>::of(&QLineEdit::textChanged),
                     [this, _combo_search, _combo_exp_search,
                      _wmo_maps_search](const QString &name) {
                       applyFilterSearch(name, _combo_search->currentIndex(),
                                         _combo_exp_search->currentIndex(),
                                         _wmo_maps_search->isChecked());
                     });

    QObject::connect(_combo_search,
                     QOverload<int>::of(&QComboBox::currentIndexChanged),
                     [this, _line_edit_search, _combo_exp_search,
                      _wmo_maps_search](int index) {
                       applyFilterSearch(_line_edit_search->text(), index,
                                         _combo_exp_search->currentIndex(),
                                         _wmo_maps_search->isChecked());
                       QSettings settings;
    ...
    QObject::connect(
        _combo_exp_search, QOverload<int>::of(&QComboBox::currentIndexChanged),
        [this, _line_edit_search, _combo_search, _wmo_maps_search](int index) {
          applyFilterSearch(_line_edit_search->text(),
                            _combo_search->currentIndex(), index,
                            _wmo_maps_search->isChecked());
        });

    QObject::connect(
        _wmo_maps_search, &QCheckBox::stateChanged,
        [this, _line_edit_search, _combo_search, _combo_exp_search](bool b) {
          applyFilterSearch(_line_edit_search->text(),
                            _combo_search->currentIndex(),
                            _combo_exp_search->currentIndex(), b);
        });
    ...
    applyFilterSearch(_line_edit_search->text(), _combo_search->currentIndex(),
                      _combo_exp_search->currentIndex(),
                      _wmo_maps_search->isChecked());
  }
  ...
  for (auto entry : _project->Bookmarks) {
    ...
    auto map_bookmark_item =
        new Widget::MapListBookmarkItem(bookmark_data, bookmarks_table);
        ...
        for (DBCFile::Iterator it = gMapDB.begin(); it != gMapDB.end(); ++it) {
          if (it->getInt(MapDB::MapID) == entry.map_id) {
            ...
            _map_creation_wizard->_world = std::make_unique<World>(
                it->getString(MapDB::InternalName), entry.map_id,
                Noggit::NoggitRenderContext::MAP_VIEW);
                   ...
                   [this](::glm::vec3 const &pos) {
                     if (getWorld()->mapIndex.hasAGlobalWMO()) // skip uid check
                       enterMapAt(pos, math::degrees(30.f), math::degrees(90.f),
                                  uid_fix_mode::none, false);
                     else
                       check_uid_then_enter_map(pos, math::degrees(30.f),
                                                math::degrees(90.f));
                   });
  ...
  _map_wizard_connection = connect(
      _map_creation_wizard,
        ...
        if (map_id >= 0) {
          for (int i = 0; i < _continents_table->count(); ++i) {
            ...
            if (item && item->data(Qt::UserRole).toInt() == map_id) {
              _continents_table->setCurrentItem(
                  item); // calls itemSelectionChanged -> loadmap
...
void NoggitWindow::handleEventMapListContextMenuPinMap(int mapId,
                                                       std::string MapName) {
...
World *NoggitWindow::getWorld() { return _map_creation_wizard->getWorld(); }
...
void NoggitWindow::promptUidFixFailure() {
  ...
  QMessageBox::critical(nullptr, "UID fix failed",
                        "The UID fix couldn't be done because some models were "
                        "missing or fucked up.\n"
...
void NoggitWindow::startWowClient() {
  ...
  // std::filesystem::path WoW_path =
  // std::filesystem::path(Noggit::Project::CurrentProject::get()->ClientPath) /
  // "Wow.exe";
  std::filesystem::path WoW_path =
      std::filesystem::path(_project->ClientPath) / "Wow.exe";
  ...
  arguments << "-windowed";
  // auto login using https://github.com/FrostAtom/awesome_wotlk, requires
  // patched Wow.exe arguments << "-login \"LOGIN\" -password \"PASSWORD\"
  // -realmlist \"REALMLIST\" -realmname \"REALMNAME\"";
  ...
  if (checkFile.exists() && checkFile.isFile()) {
    ...
    if (!process->waitForStarted()) {
      ...
      QMessageBox::information(this, "Error",
                               "Failed to start process Wow.exe");
    }
  } else {
    // qWarning("File does not exist");
    QMessageBox::information(
        this, "Error",
        std::format("File {} does not exist", WoW_path.string()).c_str());
  }
}
...
void NoggitWindow::patchWowClient() {
  ...
  mpq_patch_params_layout->addWidget(
      new QLabel("<font color=orange><h4> Warning :\
          \nErrors can cause MPQ Corruption, make sure to have backup.</h4></font>\
          \nWhile Noggit is writting the archive, files are not accessible by the client, please wait.",
                 mpq_patch_params));

  mpq_patch_params_layout->addWidget(new QLabel("MPQ Name:", mpq_patch_params));

  QLineEdit *mpq_patch_params_ledit =
      new QLineEdit("patch-A.MPQ", mpq_patch_params);
  QSettings settings;
  // saved last set patch name
  mpq_patch_params_ledit->setText(
      settings.value("noggit_window/mpq_name", "patch-A.MPQ").toString());
  mpq_patch_params_layout->addWidget(mpq_patch_params_ledit);

  QCheckBox *mpq_patch_params_locale_chk =
      new QCheckBox("Save DBC to Locale:", mpq_patch_params);
  mpq_patch_params_locale_chk->setToolTip(
      "This is recommended for non English clients");
  // auto set locale mode if client isn't english.
  bool non_english = false;
  BlizzardArchive::Locale locale_mode =
      Noggit::Application::NoggitApplication::instance()
          ->clientData()
          ->locale_mode();
  if (!(locale_mode != BlizzardArchive::Locale::AUTO) &&
      !(locale_mode != BlizzardArchive::Locale::enGB) &&
  ...
  QCheckBox *mpq_overwrite_chk =
      new QCheckBox("Overwrite Archive", mpq_patch_params);
  mpq_overwrite_chk->setToolTip(
      "If there is already an archive with this name, it will be deleted and "
  ...
  QCheckBox *mpq_compact_chk =
      new QCheckBox("Compact Archive", mpq_patch_params);
  mpq_compact_chk->setToolTip(
      "Adding and removing files creates empty space in the MPQ, making it grow in size.\
  ...
  QCheckBox *mpq_compress_files_chk =
      new QCheckBox("Compress Files (slow)", mpq_patch_params);
  mpq_compress_files_chk->setToolTip(
      "Files will be compressed in the MPQ, but writting becomes much slower(about 3x slower).\
          \nRecommended for distribution.");
  mpq_compress_files_chk->setChecked(true);
  mpq_patch_params_layout->addWidget(mpq_compress_files_chk);

  QPushButton *mpq_patch_params_okay =
  ...
  connect(mpq_patch_params_okay, &QPushButton::clicked, [=, this]() {
    ...
    if (!Noggit::Application::NoggitApplication::instance()
             ->clientData()
             ->isMPQNameValid(
                 mpq_patch_params_ledit->text().toLower().toStdString(),
                 true)) {
      QMessageBox::warning(
          this, "Name Error",
      ...
      QSettings settings;
      settings.setValue("noggit_window/mpq_name",
                        mpq_patch_params_ledit->text());
      settings.sync();
  ...
  // execute dialog, and run code when Mpq_patch_params_okay calls
  // Mpq_patch_params->accept();
  if (mpq_patch_params->exec() == QDialog::Accepted) {
    ...
    int totalItems = std::count_if(
        fs::recursive_directory_iterator(clientData->projectPath()), {},
        regular_file);

    if (!totalItems) {
      QMessageBox::warning(
          this, "Error",
          std::format("No files found in project {}", clientData->projectPath())
              .c_str());
      return;
    }
    ...
    if (!clientData->mpqArchiveExistsOnDisk(archive_name)) {
      try {
        ...
        QMessageBox::critical(
            nullptr, "Error",
            "Failed to create MPQ Archive. Unhandled exception.");
      }
    }
    {
      auto archive = clientData->getMPQArchive(archive_name);
      if (archive.has_value()) {
        auto progress_box = new QMessageBox(
            QMessageBox::Information, "Working...",
            std::format("Saving {} files to patch {}...\
                      \nClosing the program now can corrupt the MPQ.",
                        std::to_string(totalItems), archive_name)
                .c_str(),
            QMessageBox::StandardButton::NoButton, this);
        progress_box->setStandardButtons(QMessageBox::NoButton);
        progress_box->setWindowFlags(progress_box->windowFlags() &
        ...
        try {
          ...
          std::array<int, 2> result = clientData->saveLocalFilesToArchive(
              archive.value(), mpq_compress_files_chk->isChecked(),
              mpq_compact_chk->isChecked());
          ...
          // if no file was processed, archive was most likely opened and not
          // accessible
          // TODO : we can throw an error message in saveLocalFilesToArchive if
          // (!archive->openForWritting()) instead
          if (!processed_files) {
            QMessageBox::warning(
                this, "Error",
                "Project Folder is not a valid directory or client MPQ is not accessible.\
                              \nMake sure it isn't opened by Wow or MPQ editor");
          } else
            QMessageBox::information(
                this, "Archive Updated",
                std::format(
                    "Added {} files to existing Archive {} in {} seconds.\
                        \n{} Files failed.",
        ...
        QMessageBox::warning(
            this, "Error",
            std::format("Error accessing archive {}", archive_name).c_str());
    }
  }
}
} // namespace Noggit::Ui::Windows
