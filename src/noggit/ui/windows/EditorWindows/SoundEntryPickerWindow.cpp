...
namespace Noggit {
namespace Ui {
SoundEntryPickerWindow::SoundEntryPickerWindow(QPushButton *button,
                                               int sound_type_filter,
  ...
  connect(_tree_filter_cbbox, qOverload<int>(&QComboBox::currentIndexChanged),
          [this](int index) {
            if (index == 0) // load all
  ...
  connect(add_file_button, &QPushButton::clicked, ([=, this]() {
            int new_row_id = _files_listview->count();
            if (new_row_id > 9)
              return; // can't have more than 10

            _filenames_ledits[new_row_id] =
                new QLineEdit(); // set parent in the widget class
            _filenames_ledits[new_row_id]->setText("your_sound_file.mp3");
            auto file_widget = new SoundFileWListWidgetItem(
                _filenames_ledits[new_row_id],
                _directory_ledit->text().toStdString());

            auto item = new QListWidgetItem(_files_listview);
            _files_listview->setItemWidget(item, file_widget);
            item->setSizeHint(QSize(_files_listview->width(),
                                    _files_listview->height() / 10));
...
void SoundEntryPickerWindow::filter_sound_type(int sound_type, bool load_all) {
  ...
  for (DBCFile::Iterator i = gSoundEntriesDB.begin();
       i != gSoundEntriesDB.end(); ++i) {
    if (!(i->getInt(SoundEntriesDB::SoundType) == sound_type) &&
        !load_all) // it freezes for 2sec if we try to add directly all 13k
                   // items
      continue;
    ...
    std::stringstream ss;
    ss << i->getInt(SoundEntriesDB::ID) << "-"
       << i->getString(SoundEntriesDB::Name);
...
void SoundEntryPickerWindow::select_entry(int id) {
  ...
  _max_distance_spinbox->setValue(
      record.getFloat(SoundEntriesDB::distanceCutoff));
  ...
  for (int i = 0; i < 10; i++) {
    ...
    auto file_widget = new SoundFileWListWidgetItem(
        _filenames_ledits[i], _directory_ledit->text().toStdString());

    auto item = new QListWidgetItem(_files_listview);
    _files_listview->setItemWidget(item, file_widget);
    item->setSizeHint(
        QSize(_files_listview->width(), _files_listview->height() / 10));
...
void SoundEntryPickerWindow::save_entry(int entry_id) {
  ...
  for (; i < _files_listview->count(); i++) {
    record.writeString(SoundEntriesDB::Filenames + i,
                       _filenames_ledits[i]->text().toStdString());
  ...
  record.writeString(SoundEntriesDB::FilePath,
                     _directory_ledit->text().toStdString());
  record.write(SoundEntriesDB::Volume,
               _volume_slider->value() / 100.0f); // should be a float
  ...
  record.write(SoundEntriesDB::minDistance,
               static_cast<float>(_min_distance_spinbox->value()));
  record.write(SoundEntriesDB::distanceCutoff,
               static_cast<float>(_max_distance_spinbox->value()));
...
SoundFileWListWidgetItem::SoundFileWListWidgetItem(
    QLineEdit *filename_ledit, std::string dirpath,
    QWidget *parent) // std::string filename
    : QWidget(parent) {
  auto layout = new QHBoxLayout(this);
  ...
  connect(play_sound_button, &QPushButton::clicked, ([=, this]() {
            if (!filename_ledit->text().toStdString().empty() &&
                !dirpath.empty()) {
              auto sound_player = new SoundEntryPlayer(this);
              sound_player->PlaySingleSoundFile(
                  filename_ledit->text().toStdString(), dirpath);
              sound_player->show();
            }
          }));
}
