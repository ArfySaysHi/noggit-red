...
namespace Noggit {
namespace Ui {
ZoneIntroMusicPickerWindow::ZoneIntroMusicPickerWindow(QPushButton *button,
                                                       QWidget *parent)
    : QWidget(parent) {
  ...
  for (DBCFile::Iterator i = gZoneIntroMusicTableDB.begin();
       i != gZoneIntroMusicTableDB.end(); ++i) {
    auto item = new QListWidgetItem();
    item->setData(1, i->getInt(ZoneIntroMusicTableDB::ID));

    std::stringstream ss;
    ss << i->getInt(ZoneIntroMusicTableDB::ID) << "-"
       << i->getString(ZoneIntroMusicTableDB::Name);
  ...
  connect(_sound_button, &QPushButton::clicked, ([=, this]() {
            auto window = new SoundEntryPickerWindow(
                _sound_button, SoundEntryTypes::ZONE_MUSIC_FILES, false, this);
            window->show();
          }));
  ...
  connect(play_music_button, &QPushButton::clicked, ([=, this]() {
  ...
  connect(duplicate_entry_btn, &QPushButton::clicked, ([=, this]() {
            auto new_id = gZoneIntroMusicTableDB.getEmptyRecordID(
                ZoneIntroMusicTableDB::ID);
...
void ZoneIntroMusicPickerWindow::select_entry(int id) {
  ...
  _priority = record.getInt(
      ZoneIntroMusicTableDB::Priority); // always 1 except for 1 test entry

  _min_delay_spinbox->setValue(
      record.getInt(ZoneIntroMusicTableDB::MinDelayMinutes));
}

void ZoneIntroMusicPickerWindow::save_entry(int entry_id) {
  DBCFile::Record record = gZoneIntroMusicTableDB.getByID(
      entry_id); // is_new_record ? gLightDB.addRecord(Id) :
                 // gLightDB.getByID(Id);

  record.write(ZoneIntroMusicTableDB::ID, entry_id);
  record.writeString(ZoneIntroMusicTableDB::Name,
                     _name_ledit->text().toStdString());
  record.write(ZoneIntroMusicTableDB::SoundId,
               _sound_button->property("id").toInt());
  record.write(ZoneIntroMusicTableDB::Priority, _priority);
  record.write(ZoneIntroMusicTableDB::MinDelayMinutes,
               _min_delay_spinbox->value());
