...
namespace Noggit {
namespace Ui {
ZoneMusicPickerWindow::ZoneMusicPickerWindow(QPushButton *button,
                                             QWidget *parent)
    : QWidget(parent) {
  ...
  for (DBCFile::Iterator i = gZoneMusicDB.begin(); i != gZoneMusicDB.end();
       ++i) {
  ...
  select_entry_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::check));
  auto select_entry_none_btn = new QPushButton("Select -NONE-", this);
  select_entry_none_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::check));
  auto duplicate_entry_btn =
      new QPushButton("Duplicate selected Entry (create new)", this);
  duplicate_entry_btn->setIcon(
  ...
  connect(_day_music_button, &QPushButton::clicked, ([=, this]() {
            auto window = new SoundEntryPickerWindow(
                _day_music_button, SoundEntryTypes::ZONE_MUSIC_FILES, true,
                this);
            window->show();
          }));
  ...
  play_day_music_button->setIcon(
      Noggit::Ui::FontAwesomeIcon(FontAwesome::play));
  ...
  connect(_night_music_button, &QPushButton::clicked, ([=, this]() {
            auto window = new SoundEntryPickerWindow(
                _night_music_button, SoundEntryTypes::ZONE_MUSIC_FILES, true,
                this);
            window->show();
          }));
  ...
  play_night_music_button->setIcon(
      Noggit::Ui::FontAwesomeIcon(FontAwesome::play));
  ...
  connect(_tree_searchbar, &QLineEdit::textChanged, ([=, this](QString obj) {
            if (obj.isEmpty()) {
              // unhide all
            }
            ...
            auto matching_items =
                _picker_listview->findItems(obj, Qt::MatchContains);
  ...
  connect(_picker_listview, &QListWidget::itemClicked, this,
          ([=, this](QListWidgetItem *item) {
            select_entry(item->data(1).toInt());
          }));

  connect(select_entry_btn, &QPushButton::clicked, ([=, this]() {
  ...
  connect(select_entry_none_btn, &QPushButton::clicked, ([=, this]() {
            button->setText("-NONE-");
            button->setProperty("id", 0);
            this->close();
          }));

  connect(play_day_music_button, &QPushButton::clicked, ([=, this]() {
  ...
  connect(play_night_music_button, &QPushButton::clicked, ([=, this]() {
  ...
  connect(save_music_entry_btn, &QPushButton::clicked,
          ([=, this]() { save_entry(_entry_id); }));

  connect(duplicate_entry_btn, &QPushButton::clicked, ([=, this]() {
...
void ZoneMusicPickerWindow::select_entry(int id) {
  ...
  if (day_sound_entry != 0 &&
      gSoundEntriesDB.CheckIfIdExists(
          day_sound_entry)) // some entries reference sound entries that don't
                            // exist
  {
    DBCFile::Record day_sound_record = gSoundEntriesDB.getByID(day_sound_entry);
    std::stringstream ss_day;
    ss_day << day_sound_entry << "-"
           << day_sound_record.getString(SoundEntriesDB::Name);
  ...
  if (night_sound_entry != 0 &&
      gSoundEntriesDB.CheckIfIdExists(night_sound_entry)) {
    DBCFile::Record night_sound_record =
        gSoundEntriesDB.getByID(night_sound_entry);
    std::stringstream ss_night;
    ss_night << night_sound_entry << "-"
             << night_sound_record.getString(SoundEntriesDB::Name);
  ...
  _day_min_interval_spinbox->setValue(
      record.getInt(ZoneMusicDB::SilenceIntervalMinDay));
  _day_max_interval_spinbox->setValue(
      record.getInt(ZoneMusicDB::SilenceIntervalMaxDay));
  _night_min_interval_spinbox->setValue(
      record.getInt(ZoneMusicDB::SilenceIntervalMinNight));
  _night_max_interval_spinbox->setValue(
      record.getInt(ZoneMusicDB::SilenceIntervalMaxNight));
}

void ZoneMusicPickerWindow::save_entry(int entry_id) {
  DBCFile::Record record =
      gZoneMusicDB.getByID(entry_id); // is_new_record ? gLightDB.addRecord(Id)
                                      // : gLightDB.getByID(Id);

  record.write(ZoneMusicDB::ID, entry_id);
  record.writeString(ZoneMusicDB::Name, _name_ledit->text().toStdString());
  record.write(ZoneMusicDB::SilenceIntervalMinDay,
               _day_min_interval_spinbox->value());
  record.write(ZoneMusicDB::SilenceIntervalMaxDay,
               _day_max_interval_spinbox->value());
  record.write(ZoneMusicDB::SilenceIntervalMinNight,
               _night_min_interval_spinbox->value());
  record.write(ZoneMusicDB::SilenceIntervalMaxNight,
               _night_max_interval_spinbox->value());
  record.write(ZoneMusicDB::DayMusic,
               _day_music_button->property("id").toInt());
  record.write(ZoneMusicDB::NightMusic,
               _night_music_button->property("id").toInt());
