...
namespace Noggit::Ui {
DownloadFileDialog::DownloadFileDialog(QString url, QString fileDownloadPath,
                                       QWidget *parent)
  ...
  QObject::connect(_response, SIGNAL(downloadProgress(qint64, qint64)), this,
                   SLOT(updateProgress(qint64, qint64)));
  QObject::connect(_response, SIGNAL(readyRead()), this,
...
void DownloadFileDialog::updateProgress(qint64 recieved, qint64 total) {
  ...
  QVariant contentDispositionHeader =
      _response->header(QNetworkRequest::ContentDispositionHeader);
  if (contentDispositionHeader.isValid()) {
    ...
    for (auto const &part : values) {
      if (part.contains("=")) {
        ...
        headerMappedvalues.insert({actualSize[0].toLower().toStdString(),
                                   actualSize[1].toStdString()});
      } else {
  ...
  QVariant contentLengthHeader =
      _response->header(QNetworkRequest::ContentLengthHeader);
  if (contentLengthHeader.isValid()) {
    auto contentLengthHeaderValue = contentLengthHeader.toInt();
    headerMappedvalues.insert(
        {"size", std::to_string(contentLengthHeaderValue)});
  }
