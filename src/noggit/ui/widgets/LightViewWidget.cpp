...
LightViewWidget::LightViewWidget(QWidget *parent) : QWidget(parent) {}

QColor LightViewWidget::InterpolateColor(QColor &a, QColor &b, float t) {
  QColor Color;

  Color.setRgb(ClampColor(a.red() * (1 - t) + b.red() * t),
...
QColor LightViewWidget::GetColorFromStyleSheet() {
  const QString SearchPattern =
      ".*qmainwindow{background-color:(#[A-Fa-f0-9]{3-6});}.*";
...
LightViewPreview::LightViewPreview(QString LightName, QSize Size,
                                   QWidget *parent)
    : DynamicMouseWidget(parent), MainLayout(new QVBoxLayout(this)),
      Name(new QLabel(this)), Preview(new LightViewPixmap(Size, this)) {
...
void LightViewPreview::ShowLines(bool Show) { Preview->ShowLines(Show); }
...
void LightViewPreview::UpdatePixmap(const QPixmap Updated) {
  Preview->setPixmap(
      Updated.scaled(Preview->pixmap(Qt::ReturnByValueConstant{}).size(),
  ...
  for (int i = 0; i < Data.size(); ++i) {
    ...
    if (i == 0) {
      ...
      for (int time = 0; time <= Data[i].time; ++time) {
        ...
        if (DistanceWithFirstTime != 0)
          t = (float(time) + float(abs(LastTimeOffset))) /
              float(DistanceWithFirstTime);

        QColor Calculated =
            LightViewWidget::InterpolateColor(PreColor, CurrentColor, t);
      ...
      for (int time = Data[i].time; time <= MAX_TIME_VALUE; ++time) {
        float t = 1.f;
        if (DistanceWithFirstTime != 0)
          t = (float(time) - float(Data[i].time)) /
              float(DistanceWithFirstTime);

        QColor Calculated =
            LightViewWidget::InterpolateColor(CurrentColor, NextColor, t);
    ...
    for (int time = Data[i].time; time <= Data[i + 1].time; ++time) {
      ...
      if (Data[i + 1].time != Data[i].time)
        t = (float(time) - float(Data[i].time)) /
            (float(Data[i + 1].time) - float(Data[i].time));
...
bool LightViewPixmap::DrawLines() {
  ...
  for (int i = 0; i < Data.size(); ++i) {
    for (int y = 0; y < LIGHT_VIEW_PREVIEW_LINE_SIZE; ++y) {
      int X = Data[i].time * Final.width() / MAX_TIME_VALUE -
              LIGHT_VIEW_PREVIEW_LINE_SIZE / 2 + y;
      Final = LightViewWidget::FillImagePart(
          Final, X, QColor(LIGHT_VIEW_PREVIEW_LINE_COLOR));
    }
  }
...
LightViewEditor::LightViewEditor(MapView *Map, SkyParam *Sky,
                                 SkyColorNames eSkyColor, QWidget *parent)
    : QWidget(parent), _World(Map->getWorld()), _Map(Map), _Sky(Sky),
      _eSkyColorIndex(eSkyColor) {
  ...
  setWindowTitle(
      QString(tr("Light Color Editor - %1 Color")).arg(_ColorName.c_str()));

  Indicator = new LightViewDayIndicator(
      QSize(LIGHT_VIEW_PREVIEW_WIDTH * 2 + 30, LIGHT_VIEW_PREVIEW_HEIGHT),
      this);

  QHBoxLayout *PreviewLayout = new QHBoxLayout(this);

  Preview = new LightViewPixmap(
      QSize(LIGHT_VIEW_PREVIEW_WIDTH * 2, LIGHT_VIEW_PREVIEW_HEIGHT * 2), this);
  Preview->ShowLines(true);
  Preview->EnableEvent();
  Preview->SetPreview(_Sky->colorRows[_eSkyColorIndex]);

  Arrow = new LightViewArrow(
      QSize(LIGHT_VIEW_PREVIEW_WIDTH * 2 + 30, LIGHT_VIEW_PREVIEW_HEIGHT * 2),
      this);
  Arrow->UpdateData(_Sky->colorRows[_eSkyColorIndex]);

  connect(Preview, &LightViewPixmap::UpdatePixmap, this,
          ([=, this](const QPixmap Updated) { emit UpdatePixmap(Updated); }));

  connect(Preview, SIGNAL(CreateValue(int)), this, SLOT(CreateValue(int)));

  connect(
      Arrow, &LightViewArrow::ChangeTime, this,
      ([=, this](int Index, int Time) {
  ...
  connect(Arrow, &LightViewArrow::ReleaseArrow, this, ([=, this]() {
            LightViewWidget::SortSkyColorVector(
                _Sky->colorRows[_eSkyColorIndex]);
          }));

  connect(Arrow, SIGNAL(SelectValue(int)), this,
          SLOT(UpdateWidgetEdition(int)));
  connect(Arrow, SIGNAL(DeleteValue(int)), this, SLOT(DeleteValue(int)));

  PreviewLayout->addSpacerItem(
  ...
  QHBoxLayout *SecLayout = new QHBoxLayout(this);
  {
    ...
    connect(ColorPicker, &color_widgets::ColorSelector::colorChanged, this,
            ([=, this](QColor Changed) {
              if (!ValidCurrentIndex())
                return;

              _Sky->colorRows[_eSkyColorIndex][CurrentIndex].color =
                  glm::vec3(Changed.redF(), Changed.greenF(), Changed.blueF());
              UpdateSkyColorRowsValue();
            }));
    ...
    connect(TimeSelectorHour, &QSpinBox::textChanged, ([=, this](QString) {
              ...
              int Time = ((TimeSelectorHour->value() * 60) +
                          TimeSelectorMin->value()) *
                         MAX_TIME_VALUE / (23 * 60 + 59);
  ...
  QHBoxLayout *ThirdLayout = new QHBoxLayout(this);
  {
    ...
    connect(Remove, &QPushButton::clicked, this,
            ([=, this]() { DeleteValue(CurrentIndex); }));

    ThirdLayout->addSpacerItem(
        new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Expanding));
  ...
  QHBoxLayout *PreLayout = new QHBoxLayout(this);
  {
    ...
    connect(Add, &QPushButton::clicked, this, ([=, this]() {
              int Time = ((AddHour->value() * 60) + AddMin->value()) *
                         MAX_TIME_VALUE / (24 * 60);
              CreateValue(Time);
            }));

    PreLayout->addSpacerItem(
        new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Expanding));
  ...
  MainLayout->addSpacerItem(
      new QSpacerItem(0, 8, QSizePolicy::Expanding, QSizePolicy::Fixed));
  ...
  MainLayout->addSpacerItem(
      new QSpacerItem(0, 16, QSizePolicy::Expanding, QSizePolicy::Fixed));
...
void LightViewEditor::UpdateSkyColorRowsValue() {
  ...
  _World->renderer()->skies()->update_sky_colors(
      _Map->getCamera()->position,
      static_cast<int>(_World->time) % MAX_TIME_VALUE);
...
bool LightViewEditor::ValidCurrentIndex() {
  return (CurrentIndex >= 0 &&
          CurrentIndex < _Sky->colorRows[_eSkyColorIndex].size());
}
...
void LightViewEditor::CreateValue(int Time) {
  ...
  for (int i = 0; i < _Sky->colorRows[_eSkyColorIndex].size(); ++i) {
    if (_Sky->colorRows[_eSkyColorIndex][i].time == Time) {
      ...
      _Sky->colorRows[_eSkyColorIndex][i].color =
...
void LightViewEditor::DeleteValue(int Index) {
  ...
  if (Index >= 0 && Index < _Sky->colorRows[_eSkyColorIndex].size()) {
    _Sky->colorRows[_eSkyColorIndex].erase(
        _Sky->colorRows[_eSkyColorIndex].begin() + Index);
...
void LightViewDayIndicator::UpdateWorldTime(int Time) {
  ...
  const int PixelPointing =
      Time * (PixmapSize.width() - 30) / MAX_TIME_VALUE + 15;
...
void LightViewArrow::UpdateData(std::vector<SkyColor> &data,
                                bool UpdateDataColor) {
  Data = data;
  if (Data.size() == 0)
    return;
  ...
  for (int i = 0; i < Data.size(); ++i) {
    ...
    const int PixelPointing =
        Data[i].time * (PixmapSize.width() - 30) / MAX_TIME_VALUE + 15;
    ...
    // draw rectangle preview with correct color
    {
      ...
      Painter.setPen(Qt::NoPen);
      Painter.fillPath(
          Color, QColor(Data[i].color.r, Data[i].color.g, Data[i].color.b));
    }
  }
...
void LightViewArrow::mouseMoveEvent(QMouseEvent *event) {
  if (!bDragging) {
    ...
    if (event->pos().y() >= 10 && event->pos().y() <= 30) {
      for (int i = 0; i < Data.size(); ++i) {
        const int PixelPointing =
            Data[i].time * (PixmapSize.width() - 30) / MAX_TIME_VALUE + 20;

        if (event->pos().x() >= PixelPointing - 10 &&
            event->pos().x() <= PixelPointing + 10) {
    ...
    int Time =
        (event->pos().x() - 15) * MAX_TIME_VALUE / (PixmapSize.width() - 30);
