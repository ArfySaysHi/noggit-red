...
model_import::model_import(Noggit::Ui::object_editor *object_editor)
  ...
  connect(_textBox, &QLineEdit::textChanged, [this] { buildModelList(); });
...
void model_import::buildModelList() {
  ...
  std::ifstream fileReader(settings.value("project/import_file", "import.txt")
                               .toString()
                               .toStdString());
  std::string const filter(
      BlizzardArchive::ClientData::normalizeFilenameInternal(
          _textBox->text().toStdString()));
  ...
  while (std::getline(fileReader, line)) {
    ...
    std::string path(
        BlizzardArchive::ClientData::normalizeFilenameInternal(line));
