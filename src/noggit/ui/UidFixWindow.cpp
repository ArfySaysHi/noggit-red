...
namespace Noggit {
namespace Ui {
UidFixWindow::UidFixWindow(glm::vec3 pos, math::degrees camera_pitch,
  ...
  auto label1 =
      new QLabel("<big><p>In order to avoid issues with duplicating/missing "
  ...
  auto label3 = new QLabel(
      ...
      "Any <b>missing or fuckported</b> model will lead to collision and "
      "culling issues. "
  ...
  auto fix_all_fuckporting_edition(new QPushButton(
      "Fix all UIDs but potentially fuck up collisions and culling for models",
      this));
  layout->addWidget(fix_all_fuckporting_edition);

  connect(get_max, &QPushButton::clicked, ([=, this] {
            hide();
            emit fix_uid(pos, camera_pitch, camera_yaw, uid_fix_mode::max_uid);
            deleteLater();
          }));

  connect(fix_all, &QPushButton::clicked, ([=, this] {
            hide();
  ...
  connect(fix_all_fuckporting_edition, &QPushButton::clicked, ([=, this] {
            hide();
            emit fix_uid(pos, camera_pitch, camera_yaw,
