...
// On Windows this sets up a custom titlebar using the FramelessWindowsManager.
// On Linux/Mac, the native window manager handles titlebars — return nullptr
// and callers will use the system frame.
...
template <typename T>
::Ui::TitleBar *setupFramelessWindow(QWidget *titlebar_target, T *window,
                                     QSize minimum_size, QSize maximum_size,
  ...
  titleBarWidget->minimizeButton->setIcon(
      FontAwesomeIcon(FontAwesome::windowminimize));
  titleBarWidget->minimizeButton->setIconSize(QSize(16, 16));
  titleBarWidget->minimizeButton->setAccessibleName("titlebar_minimize");
  titleBarWidget->maximizeButton->setIcon(
      FontAwesomeIcon(FontAwesome::windowmaximize));
  ...
  QObject::connect(titleBarWidget->closeButton, &QPushButton::clicked, window,
                   &T::close);

  QObject::connect(titleBarWidget->minimizeButton, &QPushButton::clicked,
                   window, &T::showMinimized);

  QObject::connect(
      titleBarWidget->maximizeButton, &QPushButton::clicked,
  ...
  FramelessWindowsManager::addIgnoreObject(window,
...
template <typename T>
::Ui::TitleBar *setupFramelessWindow(QWidget * /*titlebar_target*/,
                                     T * /*window*/, QSize /*minimum_size*/,
                                     QSize /*maximum_size*/,
                                     bool /*is_resizeable*/ = true) {
  // On Linux/Mac the OS window manager draws the titlebar.
  // Return nullptr — callers already handle this case.
  return nullptr;
}
