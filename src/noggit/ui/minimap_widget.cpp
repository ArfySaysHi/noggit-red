...
minimap_widget::minimap_widget(QWidget *parent)
    : QWidget(parent), _world(nullptr), _camera(nullptr), _draw_skies(false),
      _selected_tiles(nullptr), _resizeable(false) {
  setSizePolicy(QSizePolicy::Preferred, QSizePolicy::Preferred);
  setMouseTracking(true);
  setMaximumSize(QSize(1024, 1024));
  setMinimumSize(QSize(128, 128));

  connect(this, &minimap_widget::tile_clicked, [this](QPoint tile) {
    if (!_selected_tiles)
      return;
  ...
  connect(this, &minimap_widget::reset_selection, [this]() {
    if (!_selected_tiles)
      return;
...
QSize minimap_widget::sizeHint() const { return QSize(512, 512); }

//! \todo Only redraw stuff as told in event.
// called by _minimap->update()
// \todo : massive performance drop after clicking the minimap once until moving
// cursor out of frame, paintEvent gets called repeatidly
  ...
  if (world()) {
        ...
        for (int j(0); j < 64; ++j) {
          ...
          if (world()->mapIndex.hasTile(tile)) {
            if (world()->mapIndex.tileLoaded(tile)) {
              if (world()->mapIndex.hasUnsavedChanges(tile)) {
                changed = true;
              }
          ...
          painter.drawRect(
              QRect(tile_size * i, tile_size * j, tile_size, tile_size));
    ...
    if (draw_skies() && _world->renderer()->skies()) {
      for (const Sky &sky : _world->renderer()->skies()->skies) {
        ...
        painter.drawEllipse(
            QPointF(sky.pos.x * scale_factor, sky.pos.z * scale_factor),
            10.0 // radius
            ,
            10.0);
      }
    ...
    if (_world->mapIndex.hasAGlobalWMO()) {
      ...
      painter.drawRect(QRectF(min_point_x, min_point_y, width // width
                              ,
                              height // height
                              ));
    }
    ...
    painter.drawText(drawing_rect, Qt::AlignCenter, tr("Select a map"));
  }
}
