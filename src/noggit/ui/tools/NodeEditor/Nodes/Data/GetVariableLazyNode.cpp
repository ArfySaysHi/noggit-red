...
GetVariableLazyNodeBase::GetVariableLazyNodeBase() : BaseNode() {
...
void GetVariableLazyNodeBase::compute() {
  ...
  auto variable_name =
      _in_ports[0].connected
          ? static_cast<StringData *>(_in_ports[0].in_value.lock().get())
                ->value()
          : static_cast<QLineEdit *>(_in_ports[0].default_widget)
                ->text()
                .toStdString();
  ...
  if (_out_ports[0].data_type->type().id != it->second.first.c_str()) {
    auto sstream = std::stringstream();
    sstream << "Error: variable \"" << variable_name << "\" of type \""
            << it->second.first << "\" does not match required type \""
            << _out_ports[0].data_type->type().id.toStdString() << "\".";
...
QJsonObject GetVariableLazyNodeBase::save() const {
  ...
  json_obj["variable_name"] =
      static_cast<QLineEdit *>(_in_ports[0].default_widget)->text();
...
void GetVariableLazyNodeBase::restore(const QJsonObject &json_obj) {
  ...
  static_cast<QLineEdit *>(_in_ports[0].default_widget)
      ->setText(json_obj["variable_name"].toString());
}

void GetVariableLazyNodeBase::outputConnectionCreated(
...
void GetVariableLazyNodeBase::outputConnectionDeleted(
    const Connection &connection) {
...
GetVariableLazyNode::GetVariableLazyNode() : GetVariableLazyNodeBase() {
  setName("Data :: GetVariableLazy");
  setCaption("Data :: GetVariableLazy");
}

VariableMap *GetVariableLazyNode::getVariableMap() {
  return static_cast<NodeScene *>(_node->nodeGraphicsObject().scene())
      ->getVariableMap();
}
