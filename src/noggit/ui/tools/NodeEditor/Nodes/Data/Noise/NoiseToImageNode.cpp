...
NoiseToImageNode::NoiseToImageNode() : LogicNodeBase() {
...
void NoiseToImageNode::compute() {
  noise::module::Module *noise_module =
  ...
  map_builder.SetBounds(bounds_lower.x, bounds_upper.x, bounds_lower.y,
                        bounds_upper.y);

  map_builder.EnableSeamless(
      defaultPortData<BooleanData>(PortType::In, 5)->value());

  map_builder.Build();

  QImage image =
      QImage(QSize(resolution.x, resolution.y), QImage::Format_RGBA64);
