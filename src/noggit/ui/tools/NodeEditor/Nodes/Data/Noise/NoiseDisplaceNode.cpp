...
NoiseDisplaceNode::NoiseDisplaceNode() : BaseNode() {
...
void NoiseDisplaceNode::compute() {
  ...
  _module.SetXDisplaceModule(
      *static_cast<NoiseData *>(_in_ports[1].in_value.lock().get())->value());
  _module.SetYDisplaceModule(
      *static_cast<NoiseData *>(_in_ports[2].in_value.lock().get())->value());
  _module.SetZDisplaceModule(
      *static_cast<NoiseData *>(_in_ports[3].in_value.lock().get())->value());
