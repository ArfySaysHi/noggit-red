...
NoiseBillowNode::NoiseBillowNode() : NoiseGeneratorBase() {
...
void NoiseBillowNode::compute() {
  ...
  if (!checkBounds(frequency, 0.0, std::numeric_limits<double>::max(),
                   "Frequency"))
    return;
  _module.SetFrequency(frequency);

  double lacunarity = defaultPortData<DecimalData>(PortType::In, 1)->value();
  if (!checkBounds(lacunarity, 0.0, std::numeric_limits<double>::max(),
                   "Lacunarity"))
    return;
  _module.SetLacunarity(lacunarity);

  _module.SetNoiseQuality(
      static_cast<noise::NoiseQuality>(_quality->currentIndex()));
  ...
  if (!checkBounds(persistence, 0.0, std::numeric_limits<double>::max(),
                   "Persistence"))
    return;
