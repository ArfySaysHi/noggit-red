...
NoiseTransformPointNode::NoiseTransformPointNode() : BaseNode() {
...
void NoiseTransformPointNode::compute() {
  ...
  if (_last_module < 0 || _last_module != _operation->currentIndex()) {
    ...
    case 0: // Translate
    {
      ...
      _last_module = 0;
      translate->SetSourceModule(
          0, *static_cast<NoiseData *>(_in_ports[0].in_value.lock().get())
                  ->value());
    ...
    case 1: // Rotate
    {
      ...
      _last_module = 1;
      rotate->SetSourceModule(
          0, *static_cast<NoiseData *>(_in_ports[0].in_value.lock().get())
                  ->value());
      glm::vec3 angles =
          defaultPortData<Vector3DData>(PortType::In, 1)->value();
