...
DataConstantNode::DataConstantNode() : BaseNode() {
  ...
  _type->addItems({"Integer", "UInteger", "Decimal", "String", "Boolean",
                   "Vector2D", "Vector3D", "Vector4D", "Color"});
  ...
  QComboBox::connect(
      ...
      [this](int index) {
        ...
        addDefaultWidget(
            _in_ports[0].data_type->default_widget(&_embedded_widget),
            PortType::In, 0);
...
void DataConstantNode::compute() {
  ...
  if (value_shared)
    ...
    _out_ports[0].out_value = _in_ports[0].data_type->default_widget_data(
