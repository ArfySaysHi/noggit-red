...
ImageSetRegionNode::ImageSetRegionNode() : LogicNodeBase() {
...
void ImageSetRegionNode::compute() {
  ...
  if (image_to_set->width() + pos.x - 1 >= image->width() ||
      image_to_set->height() + pos.y - 1 >= image->height()) {
    setValidationState(NodeValidationState::Error);
  ...
  for (int i = 0; i < image_to_set->width(); ++i) {
    for (int j = 0; j < image_to_set->height(); ++j) {
      result.setPixelColor(pos.x + i, pos.y + j,
                           image_to_set->pixelColor(i, j));
    }
  }
