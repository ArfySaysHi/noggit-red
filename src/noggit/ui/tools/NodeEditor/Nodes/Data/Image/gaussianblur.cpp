...
double GaussianBlur::GetNumberOnNormalDistribution(int i, int j,
                                                   const int center,
                                                   double sigma) const {
  return (
      1.0 / (2 * M_PI * pow(sigma, 2)) *
      exp(-(pow(i - center, 2) + pow(j - center, 2)) / (2 * pow(sigma, 2))));
}

void GaussianBlur::GetPixelMatrix(const QPoint &center, const QImage &image,
                                  QRgb **matrix) {
  QSize image_size = image.size();
  for (int i = 0; i < size_; i++)
    for (int j = 0; j < size_; j++)
      matrix[i][j] =
          image.pixel(GetCoordinate(QPoint(i, j) + center, image_size));
}
...
QPoint GaussianBlur::GetCoordinate(const QPoint &point,
...
GaussianBlur::GaussianBlur(int radius, double sigma)
    : radius_(radius), diviation_(sigma), values_is_set_(false) {
  if (true == (radius_ % 2)) {
    size_ = 2 * radius_ + 1;
    ...
    for (int i = 0; i < size_; i++)
      for (int j = 0; j < size_; j++)
        matrix_[i][j] =
            GetNumberOnNormalDistribution(i, j, radius_, diviation_);
...
GaussianBlur::GaussianBlur(const GaussianBlur &original)
    : radius_(original.radius_), diviation_(original.diviation_) {
  size_ = 2 * radius_ + 1;
  matrix_ = new double *[size_];
