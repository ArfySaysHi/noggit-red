...
ImageCreateNode::ImageCreateNode() : LogicNodeBase() {
  setName("Image :: Create");
  setCaption("Image :: Create");
  _validation_state = NodeValidationState::Valid;

  _image_format = new QComboBox(&_embedded_widget);
  _image_format->addItems(
      {"32 bit (8 per channel)", "64 bit (16 per channel)"});
...
void ImageCreateNode::compute() {
  ...
  QImage image = QImage(QSize(width, height), _image_format->currentIndex()
