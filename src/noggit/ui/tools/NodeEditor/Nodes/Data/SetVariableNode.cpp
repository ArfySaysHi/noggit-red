...
SetVariableNodeBase::SetVariableNodeBase() : LogicNodeBase() {
...
void SetVariableNodeBase::compute() {
  ...
  if (it != variables->end() &&
      _in_ports[2].data_type->type().id != it->second.first.c_str()) {
  ...
  (*variables)[variable_name] =
      std::make_pair<std::string, std::shared_ptr<NodeData>>(
          _in_ports[2].data_type->type().id.toStdString(),
...
SetVariableNode::SetVariableNode() : SetVariableNodeBase() {
...
SetContextVariableNode::SetContextVariableNode() : SetVariableNodeBase() {
