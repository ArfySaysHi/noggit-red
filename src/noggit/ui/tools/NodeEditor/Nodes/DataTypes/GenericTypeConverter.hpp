...
template <typename T_from, typename T_to> struct UnderlyingTypeConvertGeneric {
  static T_to convert(T_from const &value) { return static_cast<T_to>(value); }
};

template <typename T_from> struct StringConverter {
...
template <typename T_from> struct IntegerVector4DConverter {
  static glm::vec4 convert(T_from value) {
...
struct DecimalVector4DConverter {
  static glm::vec4 convert(double value) {
    return glm::vec4(value, value, value, value);
  }
};

template <typename T_from> struct IntegerVector3DConverter {
  static glm::vec3 convert(T_from value) {
    double val = static_cast<double>(value);
...
struct DecimalVector3DConverter {
  static glm::vec3 convert(double value) {
    return glm::vec3(value, value, value);
  }
};

template <typename T_from> struct IntegerVector2DConverter {
  static glm::vec2 convert(T_from value) {
    double val = static_cast<double>(value);
...
template <typename T_from> struct ColorIntegerConverter {
  static glm::vec4 convert(T_from const &value) {
    return glm::vec4(value / 255.0, value / 255.0, value / 255.0,
                     value / 255.0);
  }
};

struct ColorDecimalConverter {
  static glm::vec4 convert(double const &value) {
    return glm::vec4(value, value, value, value);
  }
};
...
struct ColorStringConverter {
  static std::string convert(glm::vec4 const &value) {
    ...
    sstream << "Color<" << value[0] << "," << value[1] << "," << value[2] << ","
            << value[3] << ">";
    return sstream.str();
  }
};
...
template <typename T_from> struct GenericJsonConverter {
  static QJsonValue convert(T_from const &value) { return QJsonValue(value); }
};

struct StringJsonConverter {
  static QJsonValue convert(std::string const &value) {
    return QJsonValue(value.c_str());
  }
};
...
struct Matrix4x4JsonConverter {
  static QJsonValue convert(glm::mat4 const &value) {
    QJsonArray array{value[0].x, value[0].y, value[0].z, value[0].w, value[1].x,
                     value[1].y, value[1].z, value[1].w, value[2].x, value[2].y,
                     value[2].z, value[2].w, value[3].x, value[3].y, value[3].z,
                     value[3].w}; // TODO: verify if this is correct
    return QJsonValue(array);
  }
};
...
#define DECLARE_TYPE_CONVERTER(DATA_FROM, DATA_TO, UTYPE_FROM, UTYPE_TO)       \
  using DATA_FROM##To##DATA_TO##TypeConverter = GenericTypeConverter<          \
      DATA_FROM##Data, DATA_TO##Data,                                          \
...
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, String,
                           StringConverter<unsigned int>)

DECLARE_TYPE_CONVERTER_EXT(Integer, Vector3D, IntegerVector3DConverter<int>)
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, Vector3D,
                           IntegerVector3DConverter<unsigned int>)
DECLARE_TYPE_CONVERTER_EXT(Integer, Vector4D, IntegerVector4DConverter<int>)
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, Vector4D,
                           IntegerVector4DConverter<unsigned int>)
DECLARE_TYPE_CONVERTER_EXT(Integer, Vector2D, IntegerVector2DConverter<int>)
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, Vector2D,
                           IntegerVector2DConverter<unsigned int>)
...
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, Color,
                           ColorIntegerConverter<unsigned int>)
...
DECLARE_TYPE_CONVERTER_EXT(UnsignedInteger, JSONValue,
                           GenericJsonConverter<qint64>)
