...
class GenericData : public NodeData {
  ...
  /* bypassing indirection overhead: passing by value if Ty fits into a machine
   * word, passing by reference otherwise */
  typedef std::conditional_t<sizeof(Ty) <= sizeof(std::size_t), Ty, Ty const &>
      Type;

public:
  GenericData() : _value{} {}

  GenericData(Type value) : _value{value} {}
  ...
  void to_json(QWidget *widget, QJsonObject &json_obj,
               const std::string &name) override {
    D::toJson(widget, json_obj, name);
  };

  void from_json(QWidget *widget, const QJsonObject &json_obj,
                 const std::string &name) override {
...
template <typename T> struct toQStringGeneric {
  static QString to_string(T const &value) {
    return QString(std::to_string(value).c_str());
  }
};

template <typename T> struct toQStringString {
  static QString to_string(T const &value) { return QString(value.c_str()); }
};

template <typename T> struct toQStringGlm {
  static QString to_string(T const &value) {
    return QString(glm::to_string(value).c_str());
  }
};

template <typename T> struct toQStringNA {
...
struct DefaultIntWidget {
  static QWidget *generate(QWidget *parent) {
    ...
    widget->setRange(std::numeric_limits<std::int32_t>::min(),
  ...
  static void toJson(QWidget *widget, QJsonObject &json_obj,
                     const std::string &name) {
    json_obj[name.c_str()] = static_cast<QSpinBox *>(widget)->value();
  }

  static void fromJson(QWidget *widget, const QJsonObject &json_obj,
                       const std::string &name) {
...
struct DefaultUnsignedIntWidget {
  ...
  static void setValue(QWidget *widget, QJsonValue &value) {
    static_cast<QUnsignedSpinBox *>(widget)->setValue(
  ...
  static void toJson(QWidget *widget, QJsonObject &json_obj,
                     const std::string &name) {
    json_obj[name.c_str()] =
        QString::number(static_cast<QUnsignedSpinBox *>(widget)->value());
  }
...
struct DefaultDecimalWidget {
  static QWidget *generate(QWidget *parent) {
    ...
    widget->setRange(std::numeric_limits<double>::lowest(),
...
struct DefaultProcedureWidget {
  ...
  static std::string value(QWidget *widget) {
    return static_cast<QPushButton *>(widget->layout()->itemAt(0)->widget())
        ->text()
        .toStdString();
  }

  static void setValue(QWidget *widget, QJsonValue &value) {
    static_cast<QPushButton *>(widget->layout()->itemAt(0)->widget())
...
template <typename T, int SIZE> struct DefaultVectorWidget {
  static QWidget *generate(QWidget *parent) {
    ...
    for (int i = 0; i < SIZE; ++i) {
      ...
      axis->setRange(std::numeric_limits<double>::lowest(),
                     std::numeric_limits<double>::max());
      axis->setDecimals(7);
  ...
  static T value(QWidget *widget) {
    ...
    for (int i = 0; i < SIZE; ++i) {
      vector[i] =
          static_cast<QDoubleSpinBox *>(widget->layout()->itemAt(i)->widget())
              ->value();
    }
...
struct DefaultColorWidget {
  ...
  static glm::vec4 value(QWidget *widget) {
    ...
    return glm::vec4(color.redF(), color.greenF(), color.blueF(),
                     color.alphaF());
  }

  static void setValue(QWidget *widget, QJsonValue &value) {
    auto array = value.toArray();
    QColor color = QColor::fromRgbF(array[0].toDouble(), array[1].toDouble(),
...
struct NoDefaultWidget {
  ...
  static std::nullptr_t value(QWidget *widget) { return nullptr; }
...
#define DECLARE_NODE_DATA_TYPE(TYPE_ID, TYPE_NAME, UNDERLYING_TYPE,            \
  ...
  using TYPE_NAME##Data =                                                      \
      GenericData<UNDERLYING_TYPE, TYPE_ID##_typeid, TYPE_ID##_typename,       \
                  toQStringGeneric<UNDERLYING_TYPE>, DEFAULT_WIDGET_GEN>;

#define DECLARE_NODE_DATA_TYPE_EXT(TYPE_ID, TYPE_NAME, UNDERLYING_TYPE,        \
                                   DEFAULT_WIDGET_GEN, STRING_C)               \
  inline constexpr char TYPE_ID##_typeid[] = #TYPE_ID;                         \
  inline constexpr char TYPE_ID##_typename[] = #TYPE_NAME;                     \
  using TYPE_NAME##Data =                                                      \
      GenericData<UNDERLYING_TYPE, TYPE_ID##_typeid, TYPE_ID##_typename,       \
                  STRING_C<UNDERLYING_TYPE>, DEFAULT_WIDGET_GEN>;

DECLARE_NODE_DATA_TYPE(logic, Logic, bool, NoDefaultWidget)

DECLARE_NODE_DATA_TYPE(int, Integer, int, DefaultIntWidget)
DECLARE_NODE_DATA_TYPE(uint, UnsignedInteger, unsigned int,
                       DefaultUnsignedIntWidget)
...
DECLARE_NODE_DATA_TYPE_EXT(string, String, std::string, DefaultStringWidget,
                           toQStringString)

// GLM types
DECLARE_NODE_DATA_TYPE_EXT(vec2, Vector2D, glm::vec2, DefaultVector2DWidget,
                           toQStringGlm)
DECLARE_NODE_DATA_TYPE_EXT(vec3, Vector3D, glm::vec3, DefaultVector3DWidget,
                           toQStringGlm)
DECLARE_NODE_DATA_TYPE_EXT(vec4, Vector4D, glm::vec4, DefaultVector4DWidget,
                           toQStringGlm)
DECLARE_NODE_DATA_TYPE_EXT(mat4, Matrix4x4, glm::mat4, NoDefaultWidget,
                           toQStringGlm)
DECLARE_NODE_DATA_TYPE_EXT(quat, Quaternion, glm::quat, NoDefaultWidget,
                           toQStringGlm)

// Polymorph types
DECLARE_NODE_DATA_TYPE_EXT(any, Any, std::nullptr_t, NoDefaultWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(basic, Basic, std::nullptr_t, NoDefaultWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(undefined, Undefined, std::nullptr_t,
                           NoDefaultWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(procedure, Procedure, std::string,
                           DefaultProcedureWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(list, List, std::vector<std::shared_ptr<NodeData>> *,
                           NoDefaultWidget, toQStringNA);

// Custom types
DECLARE_NODE_DATA_TYPE_EXT(color, Color, glm::vec4, DefaultColorWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(image, Image, QImage, NoDefaultWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(noise, Noise, noise::module::Module *,
                           NoDefaultWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(chunk, Chunk, MapChunk *, NoDefaultWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(tile, Tile, MapTile *, NoDefaultWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(object, ObjectInstance, SceneObject *,
                           NoDefaultWidget, toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(json, JSON, QJsonObject, NoDefaultWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(json_array, JSONArray, QJsonArray, NoDefaultWidget,
                           toQStringNA);
DECLARE_NODE_DATA_TYPE_EXT(json_value, JSONValue, QJsonValue, NoDefaultWidget,
                           toQStringNA);
...
struct TypeFactory {
  ...
  static NodeData *create(const std::string &id) {
    ...
    auto lambda =
        [&id](
            std::pair<std::string_view, NodeData *(*)()> const &pair) -> bool {
      return pair.first == id;
    };

    assert(std::find_if(_creators_map.begin(), _creators_map.end(), lambda) !=
           _creators_map.end());

    return std::find_if(_creators_map.begin(), _creators_map.end(), lambda)
        ->second();
  }

private:
  template <typename Ty> static NodeData *_create() { return new Ty(); }

  static constexpr std::array<std::pair<std::string_view, NodeData *(*)()>, 25>
      _creators_map = {CREATE_DATA_PAIR(logic, Logic),
