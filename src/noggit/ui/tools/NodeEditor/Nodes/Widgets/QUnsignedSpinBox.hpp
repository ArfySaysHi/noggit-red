...
class QUnsignedSpinBox : public QAbstractSpinBox {
  ...
  Q_PROPERTY(
      quint32 value READ value WRITE setValue NOTIFY valueChanged USER true)
...
public:
  ...
  quint32 minimum() const { return m_minimum; };

  void setMinimum(quint32 min) { m_minimum = min; }

  quint32 maximum() const { return m_maximum; };

  void setMaximum(quint32 max) { m_maximum = max; }
