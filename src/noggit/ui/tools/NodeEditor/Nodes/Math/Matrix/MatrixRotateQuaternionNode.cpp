...
MatrixRotateQuaternionNode::MatrixRotateQuaternionNode() : BaseNode() {
...
void MatrixRotateQuaternionNode::compute() {
  glm::mat4 matrix =
      static_cast<Matrix4x4Data *>(_in_ports[0].in_value.lock().get())->value();
  glm::quat quat =
      static_cast<QuaternionData *>(_in_ports[1].in_value.lock().get())
          ->value();

  glm::mat4 rot_matrix = glm::toMat4(quat);

  _out_ports[0].out_value =
      std::make_shared<Matrix4x4Data>(matrix * rot_matrix);
