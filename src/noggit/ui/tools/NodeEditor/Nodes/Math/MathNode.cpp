...
MathNode::MathNode() : BaseNode() {
  ...
  _operation->addItems(
      {"Add", "Subtract", "Multiply", "Divide", "Modulo", "Min", "Max", "Pow"});
...
void MathNode::restore(const QJsonObject &json_obj) {
  ...
  if (_first_type >= 0) {
    _in_ports[0].data_type.reset(
        TypeFactory::create(_types[_first_type].first.data()));
    _in_ports[0].caption = _in_ports[0].data_type->type().name;
  }

  if (_second_type >= 0) {
    _in_ports[1].data_type.reset(
        TypeFactory::create(_types[_second_type].first.data()));
    _in_ports[1].caption = _in_ports[1].data_type->type().name;
  }

  if (_result_type >= 0) {
    _out_ports[0].data_type.reset(
        TypeFactory::create(_types[_result_type].first.data()));
...
void MathNode::calculateResultType(PortIndex port_index,
                                   PortIndex other_port_index) {
  int first_temp =
      std::find_if(
          _types.cbegin(), _types.cend(),
          [this,
           port_index](std::pair<std::string_view, int> const &pair) -> bool {
            return pair.first ==
                   _in_ports[port_index].data_type->type().id.toStdString();
          })
          ->second;

  int second_temp =
      std::find_if(_types.cbegin(), _types.cend(),
                   [this, other_port_index](
                       std::pair<std::string_view, int> const &pair) -> bool {
                     return pair.first == _in_ports[other_port_index]
                                              .data_type->type()
                                              .id.toStdString();
                   })
          ->second;
...
void MathNode::inputConnectionCreated(const Connection &connection) {
  ...
  if (_in_ports[port_index].data_type->type().id != data_type.id) {
    _in_ports[port_index].data_type.reset(
        TypeFactory::create(data_type.id.toStdString()));
  ...
  if (_in_ports[other_port_index].connected) {
    ...
    std::string_view result_type =
        std::find_if(
            _types.cbegin(), _types.cend(),
            [this](std::pair<std::string_view, int> const &pair) -> bool {
              return pair.second == _result_type;
            })
            ->first;

    if (_out_ports[0].connected &&
        _out_ports[0].data_type->type().id != result_type.data()) {
      for (auto &con : _node->nodeState().connections(PortType::Out, 0)) {
        static_cast<NodeScene *>(_node->nodeGraphicsObject().scene())
            ->deleteConnection(*con.second);
      }
    }
...
template <typename T> T MathNode::commonCast(NodeData *data, int type_id) {
  ...
  if constexpr (std::is_same<T, std::string>::value) {
    ...
    case 3:
      throw std::logic_error(
          "Casting String type to numeric types is not supported in MathNode.");
      break;
    }
  }
...
template <typename T> void MathNode::handleOperation(T first, T second) {
