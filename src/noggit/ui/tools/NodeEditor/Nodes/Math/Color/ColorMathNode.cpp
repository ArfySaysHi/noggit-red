...
ColorMathNode::ColorMathNode() : BaseNode() {
  setName("Color :: Math");
  ...
  addDefaultWidget(_in_ports[0].data_type->default_widget(&_embedded_widget),
                   PortType::In, 0);
  static_cast<QDoubleSpinBox *>(_in_ports[0].default_widget)
      ->setRange(0.0, 1.0);
...
void ColorMathNode::compute() {
  QColor q_color_1 =
      static_cast<color_widgets::ColorSelector *>(_in_ports[2].default_widget)
          ->color();
  glm::vec4 color_1 =
      _in_ports[2].in_value.lock()
          ? static_cast<ColorData *>(_in_ports[2].in_value.lock().get())
                ->value()
          : glm::vec4(q_color_1.redF(), q_color_1.greenF(), q_color_1.blueF(),
                      q_color_1.alphaF());

  QColor q_color_2 =
      static_cast<color_widgets::ColorSelector *>(_in_ports[3].default_widget)
          ->color();
  glm::vec4 color_2 =
      _in_ports[3].in_value.lock()
          ? static_cast<ColorData *>(_in_ports[3].in_value.lock().get())
                ->value()
          : glm::vec4(q_color_2.redF(), q_color_2.greenF(), q_color_2.blueF(),
                      q_color_2.alphaF());

  bool clamp =
      _in_ports[1].in_value.lock()
          ? static_cast<BooleanData *>(_in_ports[1].in_value.lock().get())
                ->value()
          : static_cast<QCheckBox *>(_in_ports[3].default_widget)->isChecked();

  double factor =
      _in_ports[0].in_value.lock()
          ? static_cast<DecimalData *>(_in_ports[0].in_value.lock().get())
                ->value()
...
QJsonObject ColorMathNode::save() const {
  ...
  for (int i = 0; i < 4; ++i) {
    _in_ports[i].data_type->to_json(_in_ports[i].default_widget, json_obj,
...
void ColorMathNode::restore(const QJsonObject &json_obj) {
  ...
  for (int i = 0; i < 4; ++i) {
    _in_ports[i].data_type->from_json(_in_ports[i].default_widget, json_obj,
                                      "port_" + std::to_string(i));
  }
}
