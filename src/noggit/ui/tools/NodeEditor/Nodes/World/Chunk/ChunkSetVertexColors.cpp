...
ChunkSetVertexColorsNode::ChunkSetVertexColorsNode() : ContextLogicNodeBase() {
...
void ChunkSetVertexColorsNode::compute() {
  ...
  for (int i = 0; i < mapbufsize; ++i) {
    glm::vec4 const &color =
        static_cast<ColorData *>(list->at(i).get())->value();
...
NodeValidationState ChunkSetVertexColorsNode::validate() {
  ...
  if (list->value()->size() != 145) {
    ...
    setValidationMessage(
        "Error: list of heights should have 145 decimal values.");
    return _validation_state;
  }
