...
ChunkInfoNode::ChunkInfoNode() : ContextNodeBase() {
  setName("Chunk :: Info");
...
void ChunkInfoNode::compute() {
  ...
  if (_out_ports[1].connected) {
    _out_ports[1].out_value =
        std::make_shared<Vector2DData>(glm::vec2(chunk->px, chunk->py));
    _node->onDataUpdated(1);
  }

  if (_out_ports[2].connected) {
    _out_ports[2].out_value = std::make_shared<Vector3DData>(
        glm::vec3(chunk->xbase, chunk->ybase, chunk->zbase));
    _node->onDataUpdated(2);
  }

  if (_out_ports[3].connected) {
    _out_ports[3].out_value =
        std::make_shared<UnsignedIntegerData>(chunk->getAreaID());
    _node->onDataUpdated(3);
  }

  if (_out_ports[4].connected) {
    _out_ports[4].out_value =
        std::make_shared<DecimalData>(chunk->getMinHeight());
    _node->onDataUpdated(4);
  }

  if (_out_ports[5].connected) {
    _out_ports[5].out_value =
        std::make_shared<DecimalData>(chunk->getMaxHeight());
    _node->onDataUpdated(5);
  }

  if (_out_ports[6].connected) {
    auto center = chunk->getCenter();
    _out_ports[6].out_value =
        std::make_shared<Vector3DData>(glm::vec3(center.x, center.y, center.z));
    _node->onDataUpdated(6);
  }

  if (_out_ports[7].connected) {
    auto center = chunk->getCenter();
    _out_ports[7].out_value = std::make_shared<UnsignedIntegerData>(
        static_cast<unsigned>(chunk->texture_set->num()));
