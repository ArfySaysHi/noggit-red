...
ChunkSwapTextureNode::ChunkSwapTextureNode() : ContextLogicNodeBase() {
...
void ChunkSwapTextureNode::compute() {
  ...
  scoped_blp_texture_reference b_tex_from(
      tex_from, gCurrentContext->getViewport()->getRenderContext());
  scoped_blp_texture_reference b_tex_to(
      tex_to, gCurrentContext->getViewport()->getRenderContext());
