...
ChunkAddDetailDoodads::ChunkAddDetailDoodads() : ContextLogicNodeBase() {
  ...
  addPortDefault<UnsignedIntegerData>(PortType::In, "Global Density", true);
  addPort<LogicData>(PortType::Out, "Logic", true);
}

class BlizzardRandomizer {
public:
  constexpr BlizzardRandomizer(void);
  constexpr auto init(unsigned source) -> unsigned;
  auto shuffle(void) -> unsigned;

private:
  static constexpr std::size_t _nTerms{4};
  static constexpr std::array _sumPairs{
      std::pair{-28, 216}, std::pair{-24, 212}, std::pair{-12, 200},
      std::pair{-4, 184}};
  static constexpr char _noise[]{
      'a'
...
constexpr BlizzardRandomizer::BlizzardRandomizer(void) : _source{}, _seed{} {
  init(0);
}

constexpr auto BlizzardRandomizer::init(unsigned source) -> unsigned {
  _source = source;
  _seed = (source % 0x2F << 26) | (source % 0x35 << 18) |
          (source % 0x3B << 10) | 4 * (source % 0x3D);
...
auto BlizzardRandomizer::shuffle(void) -> unsigned {
  ...
  _source +=
      rol(*(uint32_t *)(&_noise[a]), 0) ^ rol(*(uint32_t *)(&_noise[d]), 1) ^
...
struct DetailDoodadMgr {
  ...
  static constexpr float unit{4.16667f};
  static constexpr float half{2.08333f};
  static constexpr std::size_t nElems{8 /* x */ * 8 /* y */ * 4 /* layer */};
  static constexpr std::array subchunkCoords{.0f, .0f,   .0f,   .0f,   -unit,
                                             .0f, -unit, -unit, .0f,   -unit,
                                             .0f, .0f,   -half, -half, .0f};
  static constexpr std::array fanIndices{11u, 0u, 0u, 1u, 12u, 11u, 1u, 12u};
  static constexpr std::array subchunkIndices{3u, 0u, 0u, 1u, 2u, 3u, 1u, 2u};
  static constexpr std::array bitSplatMask0x1{1u,  2u,  4u,  8u,
                                              16u, 32u, 64u, 128u};
  static constexpr std::array bitSplatShft0x1{0u, 1u, 2u, 3u, 4u, 5u, 6u, 7u};
  static constexpr std::array bitSplatMask0x2{3u, 12u, 48u, 194u,
                                              3u, 12u, 48u, 194u};
  static constexpr std::array bitSplatShft0x2{0u, 2u,  4u,  6u,
                                              8u, 10u, 12u, 14u};
  static constexpr std::array holeMask{1u, 2u, 4u, 8u, 16u, 32u, 64u, 128u,
                                       1u, 2u, 4u, 8u, 16u, 32u, 64u, 128u};
  static auto genCoord(BlizzardRandomizer &randomizer, float &value) -> void;
};

DetailDoodadMgr::Plane::Plane(void) : a{}, b{}, c{1.f}, d{} {}

auto DetailDoodadMgr::genCoord(BlizzardRandomizer &randomizer, float &value)
    -> void {
...
void ChunkAddDetailDoodads::compute() {
  ...
  MapChunk *const chunk{defaultPortData<ChunkData>(PortType::In, 1)->value()};

  if (!chunk->texture_set)
    return;

  unsigned const globalDensity{
      defaultPortData<UnsignedIntegerData>(PortType::In, 2)->value() & 0xFFu};
  ...
  randomizer.init(static_cast<unsigned>(chunk->mt->index.z * 16 + chunk->py)
                      << 0x10u |
  ...
  for (unsigned curDensity{}; curDensity < globalDensity; ++curDensity) {
    ...
    for (std::size_t i{}; i < 4; ++i) {
      ...
      for (std::size_t j{}; j < 2; ++j) {
        data[j].diff =
            curOrigin[DetailDoodadMgr::fanIndices[i * 2 + j]].y - ref;

        for (std::size_t axis{}; axis < 2; ++axis)
          data[j].coords[axis] =
              rels[axis] +
              DetailDoodadMgr::subchunkCoords
                  [DetailDoodadMgr::subchunkIndices[2 * i + j] * 3 + !axis] -
              relHs[axis];
      }

      float const intermedA{data[0].diff * data[1].coords[0] -
                            data[1].diff * data[0].coords[0]};
      float const intermedB{data[1].diff * data[0].coords[1] -
                            data[0].diff * data[1].coords[1]};
      float const intermedC{data[0].coords[0] * data[1].coords[1] -
                            data[1].coords[0] * data[0].coords[1]};
      float const dist{std::sqrt(intermedA * intermedA + intermedB * intermedB +
                                 intermedC * intermedC)};
      DetailDoodadMgr::Plane &curPlane{planes[curIdx]};
      curPlane.a = intermedA * dist;
      curPlane.b = intermedB * dist;
      curPlane.c = intermedC * dist;
      curPlane.d = std::fabs(relHs[1] * curPlane.a + relHs[0] * curPlane.b +
                             dist * curPlane.c * ref);
    }
  ...
  for (unsigned curDensity{}; curDensity < globalDensity; ++curDensity) {
    ...
    if ((chunk->texture_set->getDoodadStencilBase()[curSplat[1]] &
         DetailDoodadMgr::bitSplatMask0x1[curSplat[0]]) >>
            DetailDoodadMgr::bitSplatShft0x1[curSplat[0]] ||
        DetailDoodadMgr::holeMask[curSplat[1] / 2 * 4 + curSplat[0]] &
            chunk->getHoleMask())
      continue;

    unsigned const curRecId{chunk->texture_set->getEffectForLayer(
        (chunk->texture_set->getDoodadMappingBase()[curSplat[1]] &
         DetailDoodadMgr::bitSplatMask0x2[curSplat[0]]) >>
    ...
    for (std::size_t i{}; i < 4; ++i) {
      unsigned const curDoodadId{
          curRecord.getUInt(GroundEffectTextureDB::Doodads + i)};
      unsigned const curWeight{
          curRecord.getUInt(GroundEffectTextureDB::Weights + i)};
    ...
    for (std::size_t i{accumWeight}; i < 16; ++i) {
      doodadIds[val & 15u] =
          curRecord.getUInt(GroundEffectTextureDB::Doodads + accumWeight & 3u);
      val += 13;
    }
    ...
    for (std::size_t i{}; i < curDbDensity; ++i) {
      ...
      std::for_each(inMinichunkCoords.begin(), inMinichunkCoords.end(),
                    std::bind(DetailDoodadMgr::genCoord, std::ref(randomizer),
                              std::placeholders::_1));
      ...
      unsigned const curOfs{curIdx * 4 +
                            (inMinichunkCoords[0] < inMinichunkCoords[1])};
      DetailDoodadMgr::Plane &curPlane{planes[curOfs]};

      if (.4f > (inMinichunkCoords[0] < inMinichunkCoords[1] ? curPlane.c
                                                             : curPlane.d))
        continue;
      ...
      float const curHeight{curPlane.a * inMinichunkCoords[1] +
                            curPlane.b * inMinichunkCoords[0] +
                            std::fabs(curPlane.d) / curPlane.c};
      ...
      world->addM2(filename.toStdString(),
                   {chunk->xbase - inMinichunkCoords[0], 0,
                    chunk->zbase - inMinichunkCoords[1]},
                   1.0,
                   {math::degrees(0)._, math::degrees(0)._, math::degrees(0)._},
                   nullptr);
    }
  }
