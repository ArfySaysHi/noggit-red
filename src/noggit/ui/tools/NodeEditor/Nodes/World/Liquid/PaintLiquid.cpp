...
PaintLiquidNode::PaintLiquidNode() : ContextLogicNodeBase() {
  ...
  for (DBCFile::Iterator i = gLiquidTypeDB.begin(); i != gLiquidTypeDB.end();
    ...
    _liquid_type->addItem(QString::fromUtf8(ss.str().c_str()),
                          QVariant(liquid_id));
  }
  ...
  addPortDefault<BooleanData>(PortType::In, "OverrideLiquidType<Boolean>",
                              true);
...
void PaintLiquidNode::compute() {
  ...
  double radius =
      glm::radians(defaultPortData<DecimalData>(PortType::In, 2)->value());

  bool add = defaultPortData<BooleanData>(PortType::In, 3)->value();
  double angle =
      glm::radians(defaultPortData<DecimalData>(PortType::In, 4)->value());
  double orientation =
      glm::radians(defaultPortData<DecimalData>(PortType::In, 5)->value());
  ...
  bool override_liquid_id =
      defaultPortData<BooleanData>(PortType::In, 9)->value();
  double opacity_factor =
      defaultPortData<DecimalData>(PortType::In, 10)->value();

  world->paintLiquid({pos.x, pos.y, pos.z}, radius,
                     _liquid_type->currentData().toInt(), add,
                     math::radians(angle), math::radians(orientation), lock,
                     {origin.x, origin.y, origin.z}, override_height,
                     override_liquid_id, opacity_factor);
