...
SelectionInfoNode::SelectionInfoNode() : ContextNodeBase() {
...
void SelectionInfoNode::compute() {
  ...
  if (_out_ports[0].connected) {
    _out_ports[0].out_value =
        std::make_shared<BooleanData>(world->has_selection());
    _node->onDataUpdated(0);
  }

  if (_out_ports[1].connected) {
    _out_ports[1].out_value =
        std::make_shared<BooleanData>(world->has_multiple_model_selected());
    _node->onDataUpdated(1);
  }

  if (_out_ports[2].connected) {
    _out_ports[2].out_value = std::make_shared<UnsignedIntegerData>(
        world->get_selected_model_count());
  ...
  if (_out_ports[3].connected) {
    ...
    _out_ports[3].out_value = std::make_shared<Vector3DData>(
        glm::vec3(pivot_val.x, pivot_val.y, pivot_val.z));
