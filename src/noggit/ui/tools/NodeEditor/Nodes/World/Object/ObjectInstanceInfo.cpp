...
ObjectInstanceInfoNode::ObjectInstanceInfoNode() : ContextNodeBase() {
...
void ObjectInstanceInfoNode::compute() {
  ...
  SceneObject *obj =
      defaultPortData<ObjectInstanceData>(PortType::In, 0)->value();
  ...
  if (_out_ports[2].connected) {
    _out_ports[2].out_value = std::make_shared<DecimalData>(
        obj->which() == eMODEL ? obj->scale : 1.0);
  ...
  if (_out_ports[5].connected) {
    _out_ports[5].out_value = std::make_shared<UnsignedIntegerData>(
        obj->which() == eMODEL ? static_cast<ModelInstance *>(obj)->uid
  ...
  if (_out_ports[6].connected) {
    _out_ports[6].out_value = std::make_shared<StringData>(
