...
WorldConstantsNode::WorldConstantsNode() : ContextNodeBase() {
  ...
  _out_ports[7].out_value = std::make_shared<DecimalData>(
      std::sqrt(std::pow(533.33333, 2) + std::pow(533.33333, 2)));
  addPort<UnsignedIntegerData>(PortType::Out, "nChunkVertices<UInteger>", true);
  _out_ports[8].out_value =
      std::make_shared<UnsignedIntegerData>(9 * 9 + 8 * 8);
