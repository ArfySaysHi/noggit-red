...
GetChunksInRangeNode::GetChunksInRangeNode() : ContextLogicNodeBase() {
...
void GetChunksInRangeNode::compute() {
  ...
  for (MapTile *tile :
       world->mapIndex.tiles_in_range({pos.x, pos.y, pos.z}, radius)) {
    if (!tile->finishedLoading()) {
      tile->wait_until_loaded();
    }

    for (MapChunk *chunk :
         tile->chunks_in_range({pos.x, pos.y, pos.z}, radius)) {
