...
void TexturingSwapTextureAtPosNode::compute() {
  ...
  scoped_blp_texture_reference tex_from(
      defaultPortData<StringData>(PortType::In, 2)->value(),
      gCurrentContext->getViewport()->getRenderContext());

  scoped_blp_texture_reference tex_to(
      defaultPortData<StringData>(PortType::In, 3)->value(),
      gCurrentContext->getViewport()->getRenderContext());

  world->overwriteTextureAtCurrentChunk({pos.x, pos.y, pos.z}, tex_from,
                                        tex_to);
