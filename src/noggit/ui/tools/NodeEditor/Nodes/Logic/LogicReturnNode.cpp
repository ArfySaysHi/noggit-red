...
LogicReturnNode::LogicReturnNode() : LogicNodeBase() {
...
QJsonObject LogicReturnNode::save() const {
  ...
  json_obj["n_dynamic_ports"] = static_cast<int>(
      _in_ports.size() - 2); // 1st 2 ports are presumed to be static

  for (int i = 1; i < _in_ports.size(); ++i) {
    json_obj[("port_caption_" + std::to_string(i)).c_str()] =
        _in_ports[i].caption;
  }
...
NodeValidationState LogicReturnNode::validate() {
  ...
  for (int i = 1; i < _in_ports.size(); ++i) {
    ...
    if (!data_ptr) {
      ...
      sstream << "Error: Failed to evaluate input of type <"
              << _in_ports[i].data_type->type().name.toStdString()
              << "> at port " << i << ".";
...
void LogicReturnNode::inputConnectionCreated(const Connection &connection) {
  ...
  auto data_type = connected_model->dataType(
      PortType::In, connection.getPortIndex(PortType::Out));

  auto connected_index = connection.getPortIndex(PortType::Out);
  auto &connected_data =
      connected_model->dataModel(PortType::Out, connected_index);

  _in_ports[port_index].data_type = connected_data->instantiate();
  _in_ports[port_index].caption =
      connected_model->portCaption(PortType::Out, connected_index);
...
void LogicReturnNode::portDoubleClicked(PortType port_type,
                                        PortIndex port_index) {
  if (port_type == PortType::Out || !port_index ||
      _in_ports[port_index].data_type->type().id == "any")
    return;

  bool ok;
  QString text = QInputDialog::getText(
      &_embedded_widget, "Rename port", "Port name", QLineEdit::Normal,
...
void LogicReturnNode::restorePostConnection(const QJsonObject &json_obj) {
  for (int i = 1; i < _in_ports.size(); ++i) {
    _in_ports[i].caption =
        json_obj[("port_caption_" + std::to_string(i)).c_str()].toString();
  }
}
