...
LogicBeginNode::LogicBeginNode() : LogicNodeBase() {
  setName("Logic :: Begin");
...
void LogicBeginNode::compute() {
  ...
  for (int i = 1; i < _out_ports.size(); ++i) {
    ...
    auto default_value = _out_ports[i].data_type->default_widget_data(
        _out_ports[i].default_widget);
    ...
    if (!_out_ports[i].out_value) {
      if (default_value) {
        ...
        sstream << "Error: Argument of type <"
                << _out_ports[i].data_type->type().name.toStdString()
                << "> at port " << i
                << " does not have a default valueand was not passed.";
...
NodeValidationState LogicBeginNode::validate() { return BaseNode::validate(); }

QJsonObject LogicBeginNode::save() const {
  auto json_obj = BaseNode::save();
  json_obj["n_dynamic_ports"] = static_cast<int>(
      _out_ports.size() - 2); // 1st 2 ports are presumed to be static
  ...
  for (int i = 1; i < _out_ports.size(); ++i) {
    ...
    _out_ports[i].data_type->to_json(_out_ports[i].default_widget, json_obj,
                                     port_name);
...
void LogicBeginNode::outputConnectionCreated(const Connection &connection) {
  ...
  if (!port_index) // no need to execute the following code for the first logic
                   // input
    return;
  ...
  auto data_type = connected_model->dataType(
      PortType::In, connection.getPortIndex(PortType::In));

  auto connected_index = connection.getPortIndex(PortType::In);
  auto &connected_data =
      connected_model->dataModel(PortType::In, connected_index);

  _out_ports[port_index].data_type = connected_data->instantiate();
  _out_ports[port_index].data_type->set_parameter_type(
      data_type.parameter_type_id);

  deleteDefaultWidget(PortType::Out, port_index);
  addDefaultWidget(
      _out_ports[port_index].data_type->default_widget(&_embedded_widget),
      PortType::Out, port_index);

  _out_ports[port_index].caption =
      connected_model->portCaption(PortType::In, connected_index);

  if (_out_ports[_out_ports.size() - 1].connected) {
    addPort<AnyData>(PortType::Out, "Any", true, ConnectionPolicy::One);
    addDefaultWidget(new QLabel(&_embedded_widget), PortType::Out,
                     static_cast<QtNodes::PortIndex>(_out_ports.size() - 1));
    emit portAdded(PortType::Out,
                   static_cast<QtNodes::PortIndex>(_out_ports.size() - 1));
  }
...
void LogicBeginNode::restorePostConnection(const QJsonObject &json_obj) {
  ...
  for (int i = 1; i < _out_ports.size(); ++i) {
    ...
    auto port_name = "default_out_port_" + std::to_string(i);
    _out_ports[i].data_type->from_json(_out_ports[i].default_widget, json_obj,
                                       port_name);
    _out_ports[i].caption =
        json_obj[(port_name + "_caption").c_str()].toString();
  }
...
void LogicBeginNode::portDoubleClicked(PortType port_type,
                                       PortIndex port_index) {
  if (port_type == PortType::In || !port_index ||
      _out_ports[port_index].data_type->type().id == "any")
    return;

  bool ok;
  QString text =
      QInputDialog::getText(&_embedded_widget, "Rename port", "Port name",
                            QLineEdit::Normal, _out_ports[port_index].caption,
