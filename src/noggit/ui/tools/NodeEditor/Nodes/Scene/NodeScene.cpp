...
bool NodeScene::execute() {
  ...
  NOGGIT_ACTION_MGR->beginAction(
      reinterpret_cast<MapView *>(gCurrentContext->getViewport()));
  ...
  for (auto &pair : _nodes) {
    ...
    if (model->isLogicNode()) {
      if (model->getInterpreterToken() == NodeInterpreterTokens::BEGIN) {
        if (_begin_node) {
          LogError << "Error: Found more than one begin node in the script."
                   << std::endl;
          return false;
        }
      ...
      if (model->getInterpreterToken() == NodeInterpreterTokens::RETURN) {
        if (_return_node) {
          LogError
              << "Error: Found more than one data return node in the script."
              << std::endl;
          return false;
        }
