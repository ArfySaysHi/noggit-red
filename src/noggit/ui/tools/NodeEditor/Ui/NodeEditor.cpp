...
NodeEditorWidget::NodeEditorWidget(QWidget *parent)
  ...
  _model->setFilter(QDir::Files | QDir::AllDirs | QDir::AllEntries |
                    QDir::NoDotAndDotDot);
  ...
  gCurrentContext->setViewport(
      reinterpret_cast<ViewportManager::Viewport *>(parent));
  gCurrentContext->setWorld(reinterpret_cast<MapView *>(parent)->getWorld());

  connect(ui->scriptsTree, &QTreeView::clicked, [=](const QModelIndex &index) {
    auto path = _model->filePath(index);
    loadScene(path);
  });

  connect(
      _model, &QFileSystemModel::fileRenamed,
      [this](const QString &path, const QString &old_name,
             const QString &new_name) {
        auto relative_path_old =
            QDir("./scripts/").relativeFilePath(QDir(path).filePath(old_name));
        auto relative_path_new =
            QDir("./scripts/").relativeFilePath(QDir(path).filePath(new_name));

        for (int i = 0; i < ui->nodeArea->count(); ++i) {
          auto tab = ui->nodeArea->widget(i);

          auto scene =
              static_cast<FlowView *>(tab->layout()->itemAt(0)->widget())
                  ->getScene();

          if (scene->getRelativePath() == relative_path_old) {
            scene->setRelativePath(relative_path_new);
            scene->setSceneName(new_name);
            ui->nodeArea->setTabText(i, scene->getChanged() ? new_name + " *"
                                                            : new_name);
          }
        }
      });

  connect(ui->nodeArea, &QTabWidget::tabCloseRequested, [this](int index) {
    QWidget *tab_item = ui->nodeArea->widget(index);

    auto scene =
        static_cast<FlowView *>(tab_item->layout()->itemAt(0)->widget())
            ->getScene();
    ...
    if (scene->getChanged()) {
      ...
      prompt.setInformativeText(
          "This page has unsaved changes. Exit without saving?");
      prompt.setDefaultButton(
          prompt.addButton("Save", QMessageBox::AcceptRole));
  ...
  ui->newButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::file));
  ...
  ui->clearButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::eraser));
  connect(ui->clearButton, &QPushButton::clicked, [this]() {
    ...
    auto scene =
        static_cast<FlowView *>(tab->layout()->itemAt(0)->widget())->getScene();
    static_cast<NodeScene *>(scene)->clearScene();
  });

  ui->loadButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::folderopen));
  connect(ui->loadButton, &QPushButton::clicked, [this]() {
    auto path = QFileDialog::getOpenFileName(
        nullptr, tr("Open Noggit Script file"), "./scripts/",
        tr("Noggit Script files (*.ns)"));

    loadScene(path);
  });

  ui->saveButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::save));
  connect(ui->saveButton, &QPushButton::clicked, [this]() {
    ...
    auto scene =
        static_cast<FlowView *>(tab->layout()->itemAt(0)->widget())->getScene();
    scene->save();
    ui->nodeArea->setTabText(ui->nodeArea->currentIndex(),
                             scene->getSceneName());
  ...
  ui->saveAllButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::save));
  connect(ui->saveAllButton, &QPushButton::clicked, [this]() {
    for (int i = 0; i < ui->nodeArea->count(); ++i) {
      auto tab = ui->nodeArea->widget(i);

      auto scene = static_cast<FlowView *>(tab->layout()->itemAt(0)->widget())
                       ->getScene();
      scene->save();
  ...
  ui->executeButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::Icons::play));
  connect(ui->executeButton, &QPushButton::clicked, [this]() {
    auto tab = ui->nodeArea->currentWidget();
    if (!tab)
      return;
    ...
    LogDebug << "Time elapsed: "
             << QTime::fromMSecsSinceStartOfDay(time.elapsed())
...
void NodeEditorWidget::loadScene(const QString &filepath) {
  for (int i = 0; i < ui->nodeArea->count(); ++i) {
    ...
    if (QDir("./scripts/").relativeFilePath(filepath) ==
        scene->getRelativePath()) {
  ...
  int tab_index = ui->nodeArea->addTab(
      tab,
  ...
  connect(node_view, &FlowView::changed, ([=, this] {
            int tab_idx = ui->nodeArea->currentIndex();
            auto text = ui->nodeArea->tabText(tab_idx);
