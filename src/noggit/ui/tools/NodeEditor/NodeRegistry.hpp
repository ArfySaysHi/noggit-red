...
#define REGISTER_TYPE_CONVERTER(T_FROM, T_TO)                                  \
  registerTypeConverter(                                                       \
      std::make_pair(T_FROM##Data().type(), T_TO##Data().type()),              \
...
static std::shared_ptr<DataModelRegistry> registerDataModels() {
  ...
  ret->registerModel<TexturingRemoveTexDuplisAdtAtPosNode>(
      "World//Texturing//");
  ...
  ret->registerModel<AddObjectInstancesToSelectionRangeNode>(
      "World//Selection//");
  ...
  ret->registerModel<SetSelectedObjectInstancesPositionNode>(
      "World//Selection//");
  ret->registerModel<SetSelectedObjectInstancesRotationNode>(
      "World//Selection//");
  ret->registerModel<SnapSelectedObjectInstancesToGroundNode>(
      "World//Selection//");
...
static void setStyle() {
  ...
  if (theme != "System" && theme_dir.exists() &&
      QDir(theme_dir.path() + "/" + theme).exists("nodes_theme.json")) {
    QFile json_file = QFile(
        QDir(theme_dir.path() + "/" + theme).filePath("nodes_theme.json"));
