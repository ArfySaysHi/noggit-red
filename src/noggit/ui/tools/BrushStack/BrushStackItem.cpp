...
BrushStackItem::BrushStackItem(QWidget *parent)
  ...
  connect(_ui.expanderButton, &QPushButton::clicked, ([=, this](bool state) {
            _ui.contentWidget->setVisible(state);
            _ui.expanderButton->setIcon(state ? _expanded_icon
                                              : _collapsed_icon);
          }));
  ...
  _disabled_icon =
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::eyeslash);
  _ui.enabledButton->setIcon(_enabled_icon);
  connect(_ui.enabledButton, &QPushButton::clicked, ([=, this](bool state) {
  ...
  connect(_is_radius_affecting, &QCheckBox::clicked,
          ([=, this](bool checked) { emit settingsChanged(this); }));

  _is_inner_radius_affecting =
      new QCheckBox("Inherit inner radius", _settings_popup);
  _is_inner_radius_affecting->setChecked(true);
  _settings_popup_layout->addWidget(_is_inner_radius_affecting);
  connect(_is_inner_radius_affecting, &QCheckBox::clicked,
          ([=, this](bool checked) { emit settingsChanged(this); }));

  _is_mask_rotation_affecting =
      new QCheckBox("Inherit rotation", _settings_popup);
  _is_mask_rotation_affecting->setChecked(true);
  _settings_popup_layout->addWidget(_is_mask_rotation_affecting);
  connect(_is_mask_rotation_affecting, &QCheckBox::clicked,
          ([=, this](bool checked) { emit settingsChanged(this); }));

  _is_speed_affecting = new QCheckBox("Inherit speed", _settings_popup);
  _is_mask_rotation_affecting->setChecked(true);
  _settings_popup_layout->addWidget(_is_speed_affecting);
  connect(_is_speed_affecting, &QCheckBox::clicked,
          ([=, this](bool checked) { emit settingsChanged(this); }));
  ...
  _ui.settingsButton->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::wrench));
  connect(_ui.settingsButton, &QPushButton::clicked, [this]() {
    QPoint new_pos = mapToGlobal(
        QPoint(_ui.settingsButton->pos().x() - _settings_popup->width() - 12,
               _ui.settingsButton->pos().y()));

    _settings_popup->setGeometry(new_pos.x(), new_pos.y(),
  ...
  connect(_ui.brushNameLabel, &QToolButton::clicked, ([=, this](bool checked) {
            if (checked)
              emit activated(this);
          }));
...
void BrushStackItem::setTool(operation_type tool) {
  ...
  case eRaiseLower:
    _ui.contentWidget->layout()->addWidget(
        std::get<Noggit::Ui::TerrainTool *>(_tool_widget));
    _ui.brushNameLabel->setIcon(
        Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::TOOL_RAISE_LOWER));
    _ui.brushNameLabel->setToolTip("Raise | Lower");
    break;
  case eFlattenBlur:
    _ui.contentWidget->layout()->addWidget(
        std::get<Noggit::Ui::flatten_blur_tool *>(_tool_widget));
    _ui.brushNameLabel->setIcon(
        Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::TOOL_FLATTEN_BLUR));
    _ui.brushNameLabel->setToolTip("Flatten | Blur");
    break;
  case eTexturing:
    _ui.contentWidget->layout()->addWidget(
        std::get<Noggit::Ui::texturing_tool *>(_tool_widget));
    _ui.brushNameLabel->setIcon(
        Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::TOOL_TEXTURE_PAINT));
    _ui.brushNameLabel->setToolTip("Texture");

    _texture_palette = new Noggit::Ui::tileset_chooser(this);

    connect(
        std::get<Noggit::Ui::texturing_tool *>(_tool_widget)->_current_texture,
    ...
    connect(_texture_palette, &Noggit::Ui::tileset_chooser::selected,
            ([=, this](std::string const &filename) {
              std::get<Noggit::Ui::texturing_tool *>(_tool_widget)
                  ->_current_texture->set_texture(filename);
  ...
  case eShader:
    _ui.contentWidget->layout()->addWidget(
        std::get<Noggit::Ui::ShaderTool *>(_tool_widget));
    _ui.brushNameLabel->setIcon(
        Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::TOOL_VERTEX_PAINT));
...
void BrushStackItem::syncSliders(double radius, double inner_radius,
                                 double speed, int rot, int brushMode) {
  switch (_tool_widget.index()) {
  case eRaiseLower: {
    auto rad_slider =
        std::get<Noggit::Ui::TerrainTool *>(_tool_widget)->getRadiusSlider();
    rad_slider->setEnabled(!_is_radius_affecting->isChecked());

    if (_is_radius_affecting->isChecked())
      rad_slider->setValue(radius);

    auto inner_rad_slider = std::get<Noggit::Ui::TerrainTool *>(_tool_widget)
                                ->getInnerRadiusSlider();
    ...
    auto speed_slider =
        std::get<Noggit::Ui::TerrainTool *>(_tool_widget)->getSpeedSlider();
    ...
    std::get<Noggit::Ui::TerrainTool *>(_tool_widget)
        ->getImageMaskSelector()
        ->enableControls(_is_mask_rotation_affecting->isChecked());
  ...
  case eFlattenBlur: {
    auto rad_slider = std::get<Noggit::Ui::flatten_blur_tool *>(_tool_widget)
                          ->getRadiusSlider();
    rad_slider->setEnabled(!_is_radius_affecting->isChecked());
    ...
    auto speed_slider = std::get<Noggit::Ui::flatten_blur_tool *>(_tool_widget)
                            ->getSpeedSlider();
    speed_slider->setEnabled(!_is_speed_affecting->isChecked());
  ...
  case eTexturing: {
    auto rad_slider =
        std::get<Noggit::Ui::texturing_tool *>(_tool_widget)->getRadiusSlider();
    rad_slider->setEnabled(!_is_radius_affecting->isChecked());

    if (_is_radius_affecting->isChecked())
      rad_slider->setValue(radius);

    auto inner_rad_slider = std::get<Noggit::Ui::texturing_tool *>(_tool_widget)
                                ->getInnerRadiusSlider();
    inner_rad_slider->setEnabled(!_is_inner_radius_affecting->isChecked());
    ...
    auto speed_slider =
        std::get<Noggit::Ui::texturing_tool *>(_tool_widget)->getSpeedSlider();
    speed_slider->setEnabled(!_is_speed_affecting->isChecked());
  ...
  case eShader: {
    auto rad_slider =
        std::get<Noggit::Ui::ShaderTool *>(_tool_widget)->getRadiusSlider();
    rad_slider->setEnabled(!_is_radius_affecting->isChecked());
    ...
    auto speed_slider =
        std::get<Noggit::Ui::ShaderTool *>(_tool_widget)->getSpeedSlider();
    speed_slider->setEnabled(!_is_speed_affecting->isChecked());
...
void BrushStackItem::execute(glm::vec3 const &cursor_pos, World *world,
                             float dt, bool mod_shift_down, bool mod_alt_down,
                             bool mod_ctrl_down, bool is_under_map) {
  ...
  case eTexturing:
    ...
    if (_is_texture_dirty) {
      _selected_texture = scoped_blp_texture_reference(
          std::get<Noggit::Ui::texturing_tool *>(_tool_widget)
    ...
    if (mod_shift_down && mod_ctrl_down && mod_alt_down)
      ...
      if (tool->getImageMaskSelector()->isEnabled() &&
          !tool->getImageMaskSelector()->getBrushMode()) {
        ...
        std::get<Noggit::Ui::texturing_tool *>(_tool_widget)
            ->paint(world, cursor_pos, dt, _selected_texture.value());
      }
    }
    break;
  case eShader:
    ...
    if (tool->getImageMaskSelector()->isEnabled() &&
      ...
      if (mod_shift_down)
        std::get<Noggit::Ui::ShaderTool *>(_tool_widget)
            ->changeShader(world, cursor_pos, 1000.f, true);
      else if (mod_ctrl_down)
        std::get<Noggit::Ui::ShaderTool *>(_tool_widget)
            ->changeShader(world, cursor_pos, 1000.f, false);
