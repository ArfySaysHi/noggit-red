...
BrushStack::BrushStack(MapView *map_view, QWidget *parent)
  ...
  _add_operation_combo->addItems(
      {"Raise | Lower", "Flatten | Blur", "Texture", "Shader"});
  ...
  connect(okay_button, &QPushButton::clicked, ([=, this]() {
            ...
            case eTools::eRaiseLower:
              brush_stack_item->setTool(
                  new Noggit::Ui::TerrainTool(_map_view, this, true));
              break;
            case eTools::eFlattenBlur:
              brush_stack_item->setTool(
                  new Noggit::Ui::flatten_blur_tool(this));
              break;
            case eTools::eTexturing:
              brush_stack_item->setTool(new Noggit::Ui::texturing_tool(
                  &_map_view->getCamera()->position, _map_view, nullptr, this));
              break;
            case eTools::eShader:
              brush_stack_item->setTool(
                  new Noggit::Ui::ShaderTool(_map_view, this));
              break;
            }
  ...
  connect(_ui.addBrushButton, &QPushButton::clicked, ([=, this]() {
            QPoint new_pos = mapToGlobal(
                QPoint(_ui.addBrushButton->pos().x() - _add_popup->width() - 12,
                       _ui.addBrushButton->pos().y()));

            _add_popup->setGeometry(new_pos.x(), new_pos.y(),
                                    _add_popup->width(), _add_popup->height());
  ...
  connect(
      _ui.clearBrushesButton, &QPushButton::clicked, ([=, this]() {
        QLayoutItem *item;
        while ((item = _ui.brushList->layout()->takeAt(0)) != nullptr) {
          _active_item_button_group->removeButton(
              static_cast<BrushStackItem *>(item->widget())->getActiveButton());
  ...
  connect(_ui.radiusSlider,
          &Noggit::Ui::Tools::UiCommon::ExtendedSlider::valueChanged,
          [this](double value) {
            for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
              BrushStackItem *item{reinterpret_cast<BrushStackItem *>(
                  _ui.brushList->layout()->itemAt(i)->widget())};

              if (!item->isRadiusAffecting())
                continue;

              reinterpret_cast<BrushStackItem *>(
                  _ui.brushList->layout()->itemAt(i)->widget())
                  ->setRadius(value);
            }
          });

  connect(_ui.innerRadiusSlider,
          &Noggit::Ui::Tools::UiCommon::ExtendedSlider::valueChanged,
          [this](double value) {
            for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
              BrushStackItem *item{reinterpret_cast<BrushStackItem *>(
  ...
  connect(_ui.brushRotation, &QDial::valueChanged, [this](int value) {
    for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
      BrushStackItem *item{reinterpret_cast<BrushStackItem *>(
          _ui.brushList->layout()->itemAt(i)->widget())};
      ...
      reinterpret_cast<BrushStackItem *>(
          _ui.brushList->layout()->itemAt(i)->widget())
          ->setMaskRotation(value);
    }
  });

  connect(_ui.sculptRadio, &QRadioButton::clicked, [this](bool checked) {
    for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
      reinterpret_cast<BrushStackItem *>(
          _ui.brushList->layout()->itemAt(i)->widget())
          ->setBrushMode(checked);
    }
  });

  connect(_ui.stampRadio, &QRadioButton::clicked, [this](bool checked) {
    for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
      reinterpret_cast<BrushStackItem *>(
          _ui.brushList->layout()->itemAt(i)->widget())
          ->setBrushMode(!checked);
...
void BrushStack::addAction(BrushStackItem *brush_stack_item) {
  ...
  brush_stack_item->syncSliders(
      _ui.radiusSlider->value(), _ui.innerRadiusSlider->value(),
      _ui.speedSlider->value(), _ui.brushRotation->value(),
      _ui.sculptRadio->isChecked());
  ...
  connect(brush_stack_item, &BrushStackItem::requestDelete,
          [this](BrushStackItem *item) {
            ...
            for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
              ...
              _active_item_button_group->removeButton(
                  static_cast<BrushStackItem *>(l_item->widget())
                      ->getActiveButton());
...
void BrushStack::execute(glm::vec3 const &cursor_pos, World *world, float dt,
                         bool mod_shift_down, bool mod_alt_down,
                         bool mod_ctrl_down, bool is_under_map) {
  for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
    auto item = reinterpret_cast<BrushStackItem *>(
        _ui.brushList->layout()->itemAt(i)->widget());

    if (!item->isAffecting())
      continue;

    item->execute(cursor_pos, world, dt, mod_shift_down, mod_alt_down,
                  mod_ctrl_down, is_under_map);
  }
}
...
float BrushStack::getRadius() { return _ui.radiusSlider->value(); }

float BrushStack::getInnerRadius() { return _ui.innerRadiusSlider->value(); }

float BrushStack::getSpeed() { return _ui.speedSlider->value(); }
...
QJsonObject BrushStack::toJSON() {
  ...
  for (int i = 0; i < _ui.brushList->layout()->count(); ++i) {
    array.append(reinterpret_cast<BrushStackItem *>(
                     _ui.brushList->layout()->itemAt(i)->widget())
                     ->toJSON());
  }
