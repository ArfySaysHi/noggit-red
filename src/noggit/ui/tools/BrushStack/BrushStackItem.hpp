...
using operation_type =
    std::variant<Noggit::Ui::TerrainTool *, Noggit::Ui::flatten_blur_tool *,
                 Noggit::Ui::texturing_tool *, Noggit::Ui::ShaderTool *>;

//! \note Keep in same order as variant!
enum eTools { eRaiseLower, eFlattenBlur, eTexturing, eShader };
...
public:
  ...
  void execute(glm::vec3 const &cursor_pos, World *world, float dt,
               ...
               bool is_under_map);
  void syncSliders(double radius, double inner_radius, double speed, int rot,
                   int brushMode);

  bool isRadiusAffecting() { return _is_radius_affecting->isChecked(); };
  bool isInnerRadiusAffecting() {
    return _is_inner_radius_affecting->isChecked();
  };
  bool isMaskRotationAffecting() {
