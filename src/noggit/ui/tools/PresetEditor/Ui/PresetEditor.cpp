...
PresetEditorWidget::PresetEditorWidget(
  ...
  viewport_overlay_ui->toggleParticlesButton->setIcon(
      FontNoggitIcon(FontNoggit::Icons::VISIBILITY_UNUSED));
  ...
  _preview_renderer = new PreviewRenderer(
      90, 90, Noggit::NoggitRenderContext::PRESET_EDITOR_PREVIEW, this);
  ...
  while (iterator.HasRecords()) {
    ...
    ui->worldSelector->addItem(QString::number(map_id) + " - " +
    ...
    ui->worldSelector->setItemData(
        count, QVariant(QString::fromStdString(map_internal_name)),
        Qt::UserRole + 1);
...
void PresetEditorWidget::setupConnectsCommon() {
  ...
  connect(
      ui->worldUnloadButton, &QPushButton::clicked,
      [this]() {
        int map_id =
            ui->worldSelector
                ->itemData(ui->worldSelector->currentIndex(), Qt::UserRole)
                .toInt();

        ui->viewport->loadWorldUnderlay(
            ui->worldSelector
                ->itemData(ui->worldSelector->currentIndex(), Qt::UserRole + 1)
                .toString()
                .toStdString(),
            map_id);
