...
MapCreationWizard::MapCreationWizard(
    std::shared_ptr<Project::NoggitProject> project, QWidget *parent)
    : Noggit::Ui::widget(parent), _project(project) {
  ...
  auto info_label = new QLabel("Left Click on the grid to add Tiles, "
                               "Ctrl+click to erase, Shift+Click to add 3x3.");
  info_label->setWindowIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::info));
  ...
  auto mapTable =
      _project->ClientDatabase->LoadTable(table, readFileAsIMemStream);
  ...
  while (iterator.HasRecords()) {
    ...
    _corpse_map_id->addItem(QString::number(map_id) + " - " +
  ...
  remove_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::times));
  ...
  _sort_by_size_cat->setToolTip("Sorts models based on their size on save. May "
                                "help increase loading speed of the map.");
  ...
  _time_of_day_override->setMaximum(
      2880); // Time Values from 0 to 2880 where each number represents a half
             // minute from midnight to midnight
  ...
  _difficulty_req_message->setDisabled(
      true); // disable them until they're actually saveable, only "display" it
             // for now
  difficulty_settings_layout->addRow("Requirement Message",
                                     _difficulty_req_message);
  ...
  difficulty_settings_layout->addRow("Instance Duration(days)",
                                     _difficulty_raid_duration);
  ...
  difficulty_settings_layout->addItem(
  ...
  connect(save_btn, &QPushButton::clicked, [&]() { saveCurrentEntry(); });

  connect(discard_btn, &QPushButton::clicked, [&]() { discardChanges(); });

  connect(add_btn, &QPushButton::clicked, [&]() { addNewMap(); });

  connect(remove_btn, &QPushButton::clicked, [&]() { removeMap(); });

  _connection = connect(
      reinterpret_cast<Noggit::Ui::Windows::NoggitWindow *>(parent),
      QOverload<int>::of(&Noggit::Ui::Windows::NoggitWindow::mapSelected),
      [&](int index) { selectMap(index); });

  connect(_difficulty_type, qOverload<int>(&QComboBox::currentIndexChanged),
          [this](int index) { selectMapDifficulty(); });
...
std::string MapCreationWizard::getDifficultyString() {
  if (_instance_type->itemData(_instance_type->currentIndex()).toInt() == 1 &&
      _difficulty_max_players->value() == 5) // dungeon
  {
    if (_difficulty_type->currentIndex() == 0)
      return "DUNGEON_DIFFICULTY_5PLAYER";
    else
      return "DUNGEON_DIFFICULTY_5PLAYER_HEROIC";
  } else if (_instance_type->itemData(_instance_type->currentIndex()).toInt() ==
             2) {
    ...
    case 25:
      // in BC 25men was difficulty 0, after the 10men mode in wrath it is
      // difficulty 1
      if (_difficulty_type->currentIndex() ==
          (0 || 1)) // maybe instead check if a difficulty 25 already exists
...
void MapCreationWizard::selectMap(int map_id) {
  ...
  _world =
      new World(directoryName, map_id, Noggit::NoggitRenderContext::MAP_VIEW);

  // check if map has a wdl and prompt to create a new one
  std::stringstream filename;
  filename << "World/Maps/" << _world->basename << "/" << _world->basename
           << ".wdl";
  if (!Application::NoggitApplication::instance()->clientData()->exists(
          filename.str())) {
    QMessageBox prompt;
    prompt.setText(
        std::string("This map has no existing horizon data (.wdl file).")
            .c_str());
    prompt.setInformativeText(
        std::string("Do you want to generate a new .wdl file ?").c_str());
    ...
    if (answer) {
      ...
      // _world = new World(directoryName, map_id,
      // Noggit::NoggitRenderContext::MAP_VIEW); // refresh minimap
    }
  }
  ...
  _sort_by_size_cat->setChecked(_world->mapIndex.sortModelsBySizeClass());
  ...
  _raid_offset->setValue(std::atoi(
      raidOffset.c_str())); // only ever used in 2 places? not sure what for

  _max_players->setValue(std::atoi(maxPlayers.c_str()));

  _project->ClientDatabase->UnloadTable("Map");

  auto difficulty_table = _project->ClientDatabase->LoadTable(
      "MapDifficulty", readFileAsIMemStream);
  ...
  while (iterator.HasRecords()) {
    ...
    auto difficulty_type =
        std::atoi(record.Columns["Difficulty"].Value.c_str());
    if (diff_mapId == map_id) {
      std::string diff_text =
          "Difficulty " + record.Columns["Difficulty"].Value;
      _difficulty_type->insertItem(difficulty_type, diff_text.c_str(),
                                   QVariant(record_id));
    }
  }
...
void MapCreationWizard::selectMapDifficulty() {
  ...
  auto selected_difficulty_id =
      _difficulty_type->itemData(_difficulty_type->currentIndex()).toInt();
  ...
  _difficulty_raid_duration->setValue(raid_duration / 60 / 60 /
                                      24); // convert from seconds to days

  _difficulty_max_players->setValue(
      std::atoi(record.Columns["MaxPlayers"].Value.c_str()));
...
void MapCreationWizard::saveCurrentEntry() {
  ...
  if (_is_new_record) {
    ...
    if (id >= 0) {
      ...
      prompt.setInformativeText("Saving failed. The map with this directory "
                                "name already exists. Rename it to save.");
    ...
    if (!_world->mapIndex
             .getNumExistingTiles()) // _world->mapIndex.hasAGlobalWMO()
    {
      QMessageBox prompt;
      prompt.setIcon(QMessageBox::Warning);
      ...
      prompt.setInformativeText(
          "You didn't set any tile. To add tiles to your map click on the "
    ...
    std::stringstream filename;
    filename << "World/Maps/" << map_internal_name << "/" << map_internal_name
             << ".wdt";

    auto project_path = std::filesystem::path(
        Noggit::Project::CurrentProject::get()->ProjectPath.c_str());

    QDir dir(
        (project_path / "/world/maps/" / map_internal_name).string().c_str());
  ...
  _world->mapIndex.setSortModelsBySizeClass(_sort_by_size_cat->isChecked());
  _world->mapIndex.saveChanged(_world, true);
  _world->mapIndex.save(); // save wdt file
  // create default wdl
  if (_is_new_record)
    _world->mapIndex.createEmptyWdl();

  // Save Map.dbc record
  DBCFile::Record record = _is_new_record ? gMapDB.addRecord(_cur_map_id)
                                          : gMapDB.getByID(_cur_map_id);

  record.writeString(1, _directory->text().toStdString());

  record.write(
      2, _instance_type->itemData(_instance_type->currentIndex()).toInt());
  ...
  record.write(
      59, _corpse_map_id->itemData(_corpse_map_id->currentIndex()).toInt());
  ...
  record.write(63,
               _expansion_id->itemData(_expansion_id->currentIndex()).toInt());
...
void MapCreationWizard::addNewMap() {
  ...
  _world = new World("New_Map", _cur_map_id,
                     Noggit::NoggitRenderContext::MAP_VIEW, true);
  _minimap_widget->world(_world);
...
void MapCreationWizard::removeMap() {
  ...
  prompt.setText(
      std::string("Are you sure you want to remove this map ?").c_str());
  prompt.setStandardButtons(QMessageBox::StandardButton::Yes |
                            QMessageBox::StandardButton::No);
  prompt.setDefaultButton(QMessageBox::No);
  prompt.setWindowFlags(Qt::CustomizeWindowHint | Qt::WindowTitleHint |
...
LocaleDBCEntry::LocaleDBCEntry(QWidget *parent) : QWidget(parent) {
  ...
  _widget_map = {
      ...
      {"enUS", _en},        {"koKR", _kr},        {"frFR", _fr},
      {"deDE", _de},        {"zhCN", _cn},        {"zhTW", _tw},
      {"esES", _es},        {"esMX", _mx},        {"ruRU", _ru},
      {"jaJP", _jp},        {"ptPT", _pt},        {"itIT", _it},
      {"Unknown 1", _unk1}, {"Unknown 2", _unk2}, {"Unknown 3", _unk3},
      {"Unknown 4", _unk4}};

  layout->addWidget(_current_locale);
  // Connect

  connect(_current_locale, &QComboBox::currentTextChanged, [&](QString s) {
