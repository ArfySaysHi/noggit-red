...
private:
  ...
  void add_tool_icon(
      MapView *mapView, Noggit::BoolToggleProperty *view_state,
      const QString &name, const Noggit::Ui::FontNoggit::Icons &icon,
...
public:
  template <typename T>
  SliderAction(const QString &title, int min, int max, int value,
               const QString &prec, std::function<T(T v)> func)
      : QWidgetAction(NULL) {
    static_assert(std::is_arithmetic<T>::value,
                  "<T>SliderAction - T must be numeric");
    ...
    QLabel *_display =
        new QLabel(QString::number(func(static_cast<T>(value)), 'f', 2) +
                   tr(" %1").arg(prec));
    ...
    connect(_slider, &QSlider::valueChanged, [_display, prec, func](int value) {
      _display->setText(QString::number(func(static_cast<T>(value)), 'f', 2) +
                        tr(" %1").arg(prec));
    });
...
public:
  PushButtonAction(const QString &text) : QWidgetAction(NULL) {
    QWidget *_widget = new QWidget(NULL);
    QHBoxLayout *_layout = new QHBoxLayout();
    _layout->setMargin(0);
...
public:
  IconAction(const QIcon &icon) : QWidgetAction(NULL) {
    QWidget *_widget = new QWidget(NULL);
    QHBoxLayout *_layout = new QHBoxLayout();
    _layout->setMargin(0);
...
public:
  SpacerAction(Qt::Orientation orientation) : QWidgetAction(NULL) {
    QWidget *_widget = new QWidget(NULL);
    QHBoxLayout *_layout = new QHBoxLayout();
    _layout->setMargin(0);

    if (orientation == Qt::Vertical)
      _layout->addSpacerItem(
...
public:
  SubToolBarAction() : QWidgetAction(NULL) {
    QWidget *_widget = new QWidget(NULL);
    QHBoxLayout *_layout = new QHBoxLayout();
