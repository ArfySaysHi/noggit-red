...
ActionHistoryNavigator::ActionHistoryNavigator(QWidget *parent)
  ...
  connect(action_mgr, &Noggit::ActionManager::popBack, this,
          &ActionHistoryNavigator::popBack);
  connect(action_mgr, &Noggit::ActionManager::popFront, this,
          &ActionHistoryNavigator::popFront);
  connect(action_mgr, &Noggit::ActionManager::addedAction, this,
          &ActionHistoryNavigator::pushAction);
  connect(action_mgr, &Noggit::ActionManager::purged, this,
          &ActionHistoryNavigator::purge);
  connect(action_mgr, &Noggit::ActionManager::currentActionChanged, this,
          &ActionHistoryNavigator::changeCurrentAction);

  connect(_active_action_button_group, &QButtonGroup::idClicked,
          ([=, this](int index) {
            QSignalBlocker const blocker(action_mgr);
            emit currentActionChanged(
                (_action_stack->count() - (_action_stack->count() - index)) -
                1);
...
void ActionHistoryNavigator::popFront() {
  ...
  auto radio = static_cast<QRadioButton *>(
      static_cast<QHBoxLayout *>(
          _action_stack->itemWidget(_action_stack->item(0))->layout())
          ->itemAt(0)
          ->widget());
...
void ActionHistoryNavigator::popBack() {
  auto radio = static_cast<QRadioButton *>(
      static_cast<QHBoxLayout *>(
          _action_stack
              ->itemWidget(_action_stack->item(_action_stack->count() - 1))
              ->layout())
          ->itemAt(0)
          ->widget());
  _active_action_button_group->removeButton(radio);
  ...
  _action_stack->removeItemWidget(
      _action_stack->item(_action_stack->count() - 1));
...
void ActionHistoryNavigator::purge() {
  for (int i = 0; i < _action_stack->count(); ++i) {
    auto radio = static_cast<QRadioButton *>(
        static_cast<QHBoxLayout *>(
            _action_stack->itemWidget(_action_stack->item(i))->layout())
            ->itemAt(0)
            ->widget());
    _active_action_button_group->removeButton(radio);
    delete radio;
...
void ActionHistoryNavigator::changeCurrentAction(unsigned index) {
  ...
  if (idx >= 0) {
    static_cast<QRadioButton *>(
        static_cast<QHBoxLayout *>(
            _action_stack->itemWidget(_action_stack->item(idx))->layout())
            ->itemAt(0)
            ->widget())
        ->setChecked(true);
  ...
  //_action_stack->setCurrentItem(_action_stack->item(_action_stack->count() - 1
  //- index));
}
