...
ChunkClipboard::ChunkClipboard(World *world, QObject *parent)
    : QObject(parent), _world(world), _copy_flags() {}

void ChunkClipboard::selectRange(glm::vec3 const &cursor_pos, float radius,
                                 ChunkSelectionMode mode) {

  switch (mode) {
  case ChunkSelectionMode::SELECT: {
    _world->for_all_chunks_in_range(
        cursor_pos, radius, [this](MapChunk *chunk) -> bool {
          _selected_chunks.emplace(SelectedChunkIndex{
              TileIndex{glm::vec3{chunk->xbase, 0.f, chunk->zbase}},
  ...
  case ChunkSelectionMode::DESELECT: {
    _world->for_all_chunks_in_range(
        cursor_pos, radius, [this](MapChunk *chunk) -> bool {
          _selected_chunks.erase(SelectedChunkIndex{
              TileIndex{glm::vec3{chunk->xbase, 0.f, chunk->zbase}},
              static_cast<unsigned>(chunk->px),
              static_cast<unsigned>(chunk->py)});
          return true;
...
void ChunkClipboard::selectChunk(glm::vec3 const &pos,
                                 ChunkSelectionMode mode) {
  switch (mode) {
  case ChunkSelectionMode::SELECT: {
...
void ChunkClipboard::selectChunk(TileIndex const &tile_index, unsigned x,
                                 unsigned z, ChunkSelectionMode mode) {
  SelectedChunkIndex index{tile_index, static_cast<unsigned>(x),
...
void ChunkClipboard::copySelected(glm::vec3 const &pos, ChunkCopyFlags flags) {
  ...
  for (auto const &index : _selected_chunks) {
    ...
    // SelectedChunkIndexRelative selected_index{index.tile_index, index.x,
    // index.z, };
    ...
    if (static_cast<unsigned>(flags) &
      ...
      std::memcpy(chunk_cache.terrain_height->data(), &chunk->mVertices,
                  145 * 3 * sizeof(float));
    }
    ...
    if (static_cast<unsigned>(flags) &
      ...
      std::memcpy(chunk_cache.vertex_colors->data(), &chunk->mccv,
                  145 * 3 * sizeof(float));
    }
    ...
    if (static_cast<unsigned>(flags) &
      ...
      std::memcpy(chunk_cache.shadows->data(), &chunk->_shadow_map,
...
void ChunkClipboard::clearSelection() { _selected_chunks.clear(); }

void ChunkClipboard::pasteSelection(glm::vec3 const &pos,
                                    ChunkPasteFlags flags) {}
