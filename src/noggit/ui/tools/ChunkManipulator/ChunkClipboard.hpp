...
enum class ChunkPasteFlags { REPLACE = 0x1 };

enum class ChunkSelectionMode { SELECT, DESELECT };
...
enum class ChunkManipulatorObjectTypes { WMO, M2 };
...
struct SelectedChunkIndex {
  ...
  friend bool operator<(SelectedChunkIndex const &lhs,
                        SelectedChunkIndex const &rhs) {
...
struct SelectedChunkIndexRelative : public SelectedChunkIndex {
  ...
  friend bool operator<(SelectedChunkIndexRelative const &lhs,
                        SelectedChunkIndexRelative const &rhs) {
...
struct ChunkCache {
  ...
  std::optional<std::vector<LiquidLayer>> liquid_layers;
...
public:
  ...
  void selectRange(glm::vec3 const &cursor_pos, float radius,
                   ChunkSelectionMode mode);
  void selectChunk(glm::vec3 const &pos, ChunkSelectionMode mode);
  void selectChunk(TileIndex const &tile_index, unsigned x, unsigned z,
                   ChunkSelectionMode mode);
