...
ImageBrowserFilesystemModel::ImageBrowserFilesystemModel(QObject *parent)
  ...
  connect(this, &QFileSystemModel::directoryLoaded, this,
          &ImageBrowserFilesystemModel::onDirectoryLoaded);
}

QVariant ImageBrowserFilesystemModel::data(const QModelIndex &index,
                                           int role) const {
...
ImageBrowser::ImageBrowser(QWidget *parent) : QWidget(parent) {
  setWindowFlag(Qt::WindowStaysOnTopHint);
  _ui.setupUi(this);
  _model = new ImageBrowserFilesystemModel(this);
  auto samples_path = QDir(QDir::cleanPath(
      QCoreApplication::applicationDirPath() + QDir::separator() + "samples"));
  ...
  _ui.treeView->setRootIndex(_dir_proxy_model->mapFromSource(
      _model->index(samples_path.absolutePath())));
  _ui.treeView->hideColumn(1);
  _ui.treeView->hideColumn(2);
  _ui.treeView->hideColumn(3);
  _ui.treeView->setIconSize(QSize(64, 64));
  _ui.treeView->expandRecursively(_dir_proxy_model->mapFromSource(
      _model->index(samples_path.absolutePath())));

  connect(_ui.treeView, &QTreeView::clicked,
          ([=, this](const QModelIndex &index) {
            auto path = _model->fileInfo(_dir_proxy_model->mapToSource(index))
                            .absoluteFilePath();
            _ui.treeView->setRootIndex(_dir_proxy_model->mapFromSource(
                _model->index(samples_path.absolutePath())));
          }));
  ...
  connect(
      _ui.treeView->selectionModel(), &QItemSelectionModel::selectionChanged,
...
void ImageBrowser::keyPressEvent(QKeyEvent *event) {
  if (event->key() == Qt::Key_Enter || event->key() == Qt::Key_Return) {
    _dir_proxy_model->setFilterFixedString(_ui.searchField->text());
    auto samples_path = QDir::cleanPath(QCoreApplication::applicationDirPath() +
                                        QDir::separator() + "samples");
    _ui.treeView->setRootIndex(
        _dir_proxy_model->mapFromSource(_model->index(samples_path)));
  }
}
