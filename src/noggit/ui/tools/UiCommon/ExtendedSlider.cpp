...
ExtendedSlider::ExtendedSlider(QWidget *parent)
    : QWidget(parent), _tablet_manager(TabletManager::instance()) {
  _ui.setupUi(this);
  ...
  _ui.tabletControlMenuButton->setIcon(
      FontAwesomeIcon(FontAwesome::Icons::pen));

  connect(tablet_enabled, &QCheckBox::stateChanged, ([=, this](int state) {
  ...
  connect(sens_slider, &QSlider::valueChanged, ([=, this](int value) {
  ...
  // ui
  connect(_ui.tabletControlMenuButton, &QPushButton::clicked, ([=, this]() {
            QPoint new_pos =
                mapToGlobal(QPoint(_ui.tabletControlMenuButton->pos().x() -
                                       _tablet_popup->width() - 12,
                                   _ui.tabletControlMenuButton->pos().y()));

            _tablet_popup->setGeometry(new_pos.x(), new_pos.y(),
                                       _tablet_popup->width(),
  ...
  connect(_ui.slider, &QSlider::valueChanged, ([=, this](int v) {
            ...
            double spin_value =
                (_ui.doubleSpinBox->maximum() - _ui.doubleSpinBox->minimum()) *
                (v / static_cast<float>(
                         (_ui.slider->maximum() - _ui.slider->minimum())));
  ...
  connect(_tablet_manager, &Noggit::TabletManager::pressureChanged,
          ([=, this](double pressure) {
            int p_value = (_ui.slider->maximum() - _ui.slider->minimum()) *
                          (value() / (_ui.doubleSpinBox->maximum() -
                                      _ui.doubleSpinBox->minimum()));
...
double ExtendedSlider::value() {
  ...
  if (_is_tablet_supported && _is_tablet_affecting &&
      _tablet_manager->isActive()) {
    return std::min(
        _ui.doubleSpinBox->maximum(),
        _ui.doubleSpinBox->value() +
            (_ui.doubleSpinBox->maximum() - _ui.doubleSpinBox->minimum()) *
                _tablet_manager->pressure() * (_tablet_sens_factor / 1000.0));
  }

  return _ui.doubleSpinBox->value();
}

double ExtendedSlider::rawValue() { return _ui.doubleSpinBox->value(); }
