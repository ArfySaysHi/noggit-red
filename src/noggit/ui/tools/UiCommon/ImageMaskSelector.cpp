...
ImageMaskSelector::ImageMaskSelector(MapView *map_view, QWidget *parent)
  ...
  connect(_ui.curImageLabel, &Noggit::Ui::ClickableLabel::clicked, [this]() {
    ...
    _image_browser->setGeometry(new_pos.x(), y, _image_browser->width(),
  ...
  connect(_image_browser, &ImageBrowser::imageSelected,
          [this, parent](QString name) { setImageMask(name); });

  connect(_ui.dial, &QDial::valueChanged,
          ([=, this](int value) { emit rotationUpdated(value); }));
