...
LightEditor::LightEditor(MapView *map_view, QWidget *parent)
  ...
  auto light_selection_layout =
      new QFormLayout(light_selection_widget); // QFormLayout
  ...
  time_dial->setRange(
      0, 2880); // Time Values from 0 to 2880 where each number represents a
                // half minute from midnight to midnight
  ...
  QPushButton *GetCurrentSkyButton =
      new QPushButton("Edit current light", this);
  ...
  for (DBCFile::Iterator i = gLightDB.begin(); i != gLightDB.end(); ++i) {
    if (i->getInt(LightDB::Map) == _world->getMapID()) {
      ...
      else if (i->getFloat(LightDB::PositionX) == 0.0f &&
               i->getFloat(LightDB::PositionY) == 0.0f &&
               i->getFloat(LightDB::PositionZ) == 0.0f)
        light_name = "Global Light";

      ss << i->getUInt(LightDB::ID) << "-"
         << light_name; // gAreaDB.getAreaName(area_id);
  ...
  QPushButton *GetSelectedSkyButton =
      new QPushButton("Edit selected light", this);
  ...
  QPushButton *SaveCurrentSkyButton =
      new QPushButton("Save Light(Write DBCs)", this);
  ...
  // alpha_values_group->setSizePolicy(QSizePolicy::Preferred,
  // QSizePolicy::Maximum);
  ...
  pos_x_spin->setRange(-17066.66656 * 2,
                       17066.66656 * 2); // size = ±17066.66656
  ...
  pos_y_spin->setRange(-17066.66656 * 2,
                       17066.66656 * 2); // size = ±17066.66656
  ...
  inner_radius_spin->setRange(0,
                              100000); // max seen in dbc is 3871 (139363 E36 )
  ...
  param_combobox->addItem(
      "Clear Weather Underwater"); // Used in clear weather while being
                                   // underwater.
  param_combobox->addItem(
      "Storm Weather"); // Used in rainy/snowy/sandstormy weather.
  param_combobox->addItem(
      "Storm Weather Underwater"); // Used in rainy/snowy/sandstormy weather
                                   // while being underwater.
  param_combobox->addItem(
      "Death Effect"); // ParamsDeath. Only 4 and in newer ones 3 are used as
                       // value here (with some exceptions). Changing this seems
                       // to have no effect in 3.3.5a (is death light setting
                       // hardcoded?)
  ...
  glow_slider->setRange(
      0, 100); // between 0 and 1, increases by 0.05. Multiplying everything by
               // 100 cuz Qslider doesn't seem to support floats
  ...
  QGroupBox *alpha_values_group =
      new QGroupBox("Alpha Values", light_param_group);
  ...
  alpha_values_layout->addWidget(
      new QLabel("Shallow Water", alpha_values_group), 0, 0);
  shallow_water_alpha_slider = new QSlider(Qt::Horizontal, alpha_values_group);
  shallow_water_alpha_slider->setRange(
      0, 100); // between 0 and 1, increases by 0.05. Multiplying everything by
               // 100 cuz Qslider doesn't seem to support floats
  ...
  alpha_values_layout->addWidget(new QLabel("Deep Water", alpha_values_group),
                                 1, 0);
  deep_water_alpha_slider = new QSlider(Qt::Horizontal, alpha_values_group);
  deep_water_alpha_slider->setRange(
      0, 100); // between 0 and 1, increases by 0.05. Multiplying everything by
               // 100 cuz Qslider doesn't seem to support floats
  ...
  alpha_values_layout->addWidget(
      new QLabel("Shallow Ocean", alpha_values_group), 2, 0);
  shallow_ocean_alpha_slider = new QSlider(Qt::Horizontal, alpha_values_group);
  shallow_ocean_alpha_slider->setRange(
      0, 100); // between 0 and 1, increases by 0.05. Multiplying everything by
               // 100 cuz Qslider doesn't seem to support floats
  ...
  alpha_values_layout->addWidget(new QLabel("Deep Ocean", alpha_values_group),
                                 3, 0);
  deep_ocean_alpha_slider = new QSlider(Qt::Horizontal, alpha_values_group);
  deep_ocean_alpha_slider->setRange(
      0, 100); // between 0 and 1, increases by 0.05. Multiplying everything by
               // 100 cuz Qslider doesn't seem to support floats
  ...
  for (int i = 0; i < NUM_SkyColorNames; ++i) {
    ...
    LightViewPreview *LightPrev =
        new LightViewPreview(QString("%1 Color").arg(color_name.c_str()),
    ...
    connect(LightPrev, &LightViewPreview::LeftClicked, [this, i, LightPrev]() {
      ...
      connect(Editor, &LightViewEditor::UpdatePixmap,
              [this, LightPrev](const QPixmap Updated) {
  ...
  connect(GetCurrentSkyButton, &QPushButton::clicked, ([=, this]() {
            ...
            if (_curr_sky->skybox.has_value())
              skybox_model_lineedit->setText(
                  QString::fromStdString(default_param->skybox.value()
                                             .model.get()
                                             ->file_key()
                                             .filepath()));
            // alpha values
            ...
            for (int i = 0; i < NUM_SkyColorNames; ++i) {
              ...
              /// 16 values", default_param->colorRows[i].size())));
  ...
  connect(GetSelectedSkyButton, &QPushButton::clicked, ([=, this]() {
            ...
            if (selected_items.size()) {
              ...
              for (int i = 0;
                   ...
                   i++) {
                auto &sky =
                    _map_view->getWorld()->renderer()->skies()->skies[i];
                if (sky.Id == selected_light_id) {
                  ...
                  // auto chunk = _world->getChunkAt(glm::vec3(_curr_sky->pos.x,
                  // _curr_sky->pos.y, _curr_sky->pos.z)); // need to load tile
                  // first ?? if (chunk != nullptr)
                  // 	_map_view->_camera.position.z = chunk->getMaxHeight()
                  // + 20.0f;
                  // TODO : initialise
                }
              }
      ...
      [this](int index) {
        ...
        for (DBCFile::Iterator i = gLightDB.begin(); i != gLightDB.end(); ++i) {
          for (int l = 0; l < NUM_SkyParamsNames; l++) {
            if (i->getInt(LightDB::DataIDs + l) ==
                data.getInt(LightDB::DataIDs + index))
              nb_user++;
          }
        }
        ...
        if (_curr_sky->skybox.has_value())
          skybox_model_lineedit->setText(QString::fromStdString(
  ...
  connect(pos_x_spin, qOverload<double>(&QDoubleSpinBox::valueChanged),
          [&](double v) {
            ...
            _world->renderer()->skies()->update_sky_colors(
                _map_view->getCamera()->position,
                static_cast<int>(_world->time) %
                    2880); // find how to update sky
          });
