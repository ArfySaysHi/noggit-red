...
AreaTriggerEditor::areatrigger_description::areatrigger_description(
    uint32_t id, std::string zone_name, std::string sub_category,
    std::string trigger_name, bool is_builtin)
    : id{id}, zone_name{std::move(zone_name)},
      sub_category{std::move(sub_category)},
      trigger_name{std::move(trigger_name)}, is_builtin{is_builtin} {}

AreaTriggerEditor::AreaTriggerEditor(MapView *map_view, QWidget *parent)
    : QWidget{parent}, _map_view{map_view} {
  parseDescriptions();
...
void AreaTriggerEditor::set_selected(area_trigger &trigger) {
  ...
  if (auto description = _trigger_descriptions.At(_selected_trigger_id);
      description) {
    _selection_widgets.zone_name_widget->setText(
        description->zone_name.c_str());
    _selection_widgets.category_name_widget->setText(
        description->sub_category.c_str());
    _selection_widgets.trigger_name_widget->setText(
        description->trigger_name.c_str());
  }
  ...
  std::visit(
      [&](auto &&t) {
        if constexpr (std::is_same_v<std::remove_cvref_t<decltype(t)>,
                                     sphere_trigger>) {
          ...
          _selection_widgets.size_widget->clear();
          _selection_widgets.rotation_widget->clear();
        } else if constexpr (std::is_same_v<std::remove_cvref_t<decltype(t)>,
                                            box_trigger>) {
...
void AreaTriggerEditor::save() {
  auto const file_path = Noggit::Application::NoggitApplication::instance()
                             ->getConfiguration()
  ...
  for (auto &&[_, desc] : _trigger_descriptions) {
    file << std::format("{0},{1},{2},{3},{4},\n", desc.id, desc.zone_name,
                        desc.sub_category, desc.trigger_name,
                        desc.is_builtin ? '1' : '0');
  }
}
...
void AreaTriggerEditor::parseDescriptions() {
  ...
  char const *expeceted_header =
      "ID,Zone Name,Sub Category,Trigger Name,IsBuiltIn,";
  ...
  if (auto header = stream.readLine(); header != expeceted_header) {
    ...
    throw std::runtime_error(std::format("File {} uses invalid header `{}`!",
                                         file_path, header.toStdString()));
  }
  ...
  while (!stream.atEnd()) {
    ...
    if (tokens.size() != expected_num_tokens) {
      LogError << std::format("Encountered wrong number of tokens (expected: "
                              "{}, encountered: {}) in file {}!",
                              expected_num_tokens, tokens.size(), file_path);
      continue;
    }

    descriptions.emplace_back(std::make_pair(
        tokens[0].toUInt(),
        areatrigger_description{
            tokens[0].toUInt(), tokens[1].trimmed().toStdString(),
            tokens[2].trimmed().toStdString(),
            tokens[3].trimmed().toStdString(), tokens[4].toUInt() == 1}));
  }

  _trigger_descriptions =
      util::FlatMap<uint32_t, areatrigger_description>{std::move(descriptions)};
}
...
std::string AreaTriggerEditor::formatTriggerName(uint32_t trigger_id) {
  ...
  return std::format("{}: {} - {} - {}", trigger_id, info->zone_name,
                     info->sub_category, info->trigger_name);
}
...
void AreaTriggerEditor::createListView(QVBoxLayout *layout) {
  ...
  connect(search_bar, &QLineEdit::textChanged,
          [&, search_btn](QString const &new_text) {
            ...
            if (new_text.isEmpty()) {
              ...
              for (auto &&[_, item] : _list_items) {
                bool hide = rapidfuzz::fuzz::token_set_ratio(
                                item->text().toStdString(),
                                new_text.toStdString()) <= score_cutoff;
                item->setHidden(hide);
              }
            }
          });
  ...
  for (auto &&record : gAreaTriggerDB) {
    ...
    if (trigger.trigger.index() == 0) {
      item->setIcon(Noggit::Ui::FontNoggitIcon(
          Noggit::Ui::FontNoggit::AREA_TRIGGER_SPHERE));
    } else {
      item->setIcon(
          Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::AREA_TRIGGER));
    }
  ...
  connect(_list_widget, &QListWidget::currentItemChanged,
  ...
  connect(add_btn, &QPushButton::pressed, [=, this] {
    ...
    radio1->setIcon(Noggit::Ui::FontNoggitIcon(
        Noggit::Ui::FontNoggit::AREA_TRIGGER_SPHERE));
    ...
    connect(add_btn, &QPushButton::pressed, [=, this] {
      addNewTrigger(radio1->isChecked() ? TriggerKind::Sphere
                                        : TriggerKind::Box);
      dialog->close();
    });
...
void AreaTriggerEditor::createSelectionWidgets(QVBoxLayout *layout) {
  ...
  auto update_text = [&]() {
    ...
    _trigger_descriptions.Transform(
        _selected_trigger_id,
        [&](uint32_t key, areatrigger_description &description) {
          description.zone_name =
              _selection_widgets.zone_name_widget->text().toStdString();
          description.sub_category =
              _selection_widgets.category_name_widget->text().toStdString();
          description.trigger_name =
              _selection_widgets.trigger_name_widget->text().toStdString();
        });

    _list_items[_selected_trigger_id]->setText(
        QString("%1: %2 - %3 - %4")
            .arg(_selected_trigger_id)
  ...
  connect(_selection_widgets.zone_name_widget, &QLineEdit::textEdited,
          update_text);
  ...
  connect(_selection_widgets.category_name_widget, &QLineEdit::textEdited,
          update_text);
  ...
  connect(_selection_widgets.trigger_name_widget, &QLineEdit::textEdited,
          update_text);
  ...
  connect(_selection_widgets.position_widget, &Vector3fWidget::valueChanged,
          [=, this](glm::vec3 const &value) {
            ...
            NOGGIT_ACTION_MGR->beginAction(
                _map_view, Noggit::ActionFlags::eAREA_TRIGGER_TRANSFORMED,
  ...
  connect(_selection_widgets.size_widget, &Vector3fWidget::valueChanged,
          [=, this](glm::vec3 const &value) {
            if (_selected_trigger_id == std::numeric_limits<uint32_t>::max()) {
              return;
            }
            ...
            box.extents_min =
                glm::vec3{-value.x / 2, -value.y / 2, -value.z / 2};
  ...
  connect(_selection_widgets.rotation_widget,
          ...
          [=, this](double value) {
            ...
            std::get<1>(trigger.trigger).orientation =
  ...
  connect(_selection_widgets.radius_widget,
          static_cast<void (QDoubleSpinBox::*)(double)>(
              &QDoubleSpinBox::valueChanged),
          [=, this](double value) {
            if (_selected_trigger_id == std::numeric_limits<uint32_t>::max()) {
...
void generate_hotkey_row(std::initializer_list<FontNoggit::Icons> &&hotkeys,
                         const char *description, QWidget *parent,
                         QFormLayout *layout) {
...
void AreaTriggerEditor::createInfoWidgets(QVBoxLayout *layout) {
  ...
  generate_hotkey_row({FontNoggit::mmb}, "\aJump to selected Trigger",
                      group_box, radius_layout);
}
...
void AreaTriggerEditor::addNewTrigger(TriggerKind kind) {
  ...
  if (!selection.empty()) {
    std::visit(
        [&](auto &&selection) {
          if constexpr (std::is_same_v<std::remove_cvref_t<decltype(selection)>,
                                       selected_object_type>) {
            // TODO: get the hit vertex and create the area trigger there
          } else if constexpr (std::is_same_v<
                                   std::remove_cvref_t<decltype(selection)>,
                                   selected_chunk_type>) {
  ...
  QIcon icon =
      Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::AREA_TRIGGER_SPHERE);
  ...
  areatrigger_description desc{trigger.id, "ZoneName", "SubCategory",
                               "TriggerName", false};
...
void AreaTriggerEditor::deleteSelectedTrigger() {
  ...
  connect(no_btn, &QPushButton::pressed, [=, this] { dialog->close(); });
