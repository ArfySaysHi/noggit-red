...
PreviewRenderer::PreviewRenderer(int width, int height,
    ...
    : Noggit::Ui::Tools::ViewportManager::Viewport(parent),
      _camera(glm::vec3(0.0f, 0.0f, 0.0f), math::degrees(0.0f),
              math::degrees(0.0f)),
      _settings(new QSettings()), _width(width), _height(height),
...
void PreviewRenderer::setModel(std::string const &filename) {
  ...
  if (q_filename.endsWith(".wmo")) {
    ...
    throw std::logic_error(
        "Preview renderer only supports viewing M2 and WMO for now.");
  }
...
void PreviewRenderer::resetCamera(float x, float y, float z, float roll,
  ...
  radius = std::max(glm::distance(_camera.position, extents[0]),
                    glm::distance(_camera.position, extents[1]));
...
void PreviewRenderer::draw() {
  ...
  gl.clearColor(_background_color.r, _background_color.g, _background_color.b,
                1.0f);
  ...
  if (_draw_wmo.get() && !_wmo_instances.empty()) {
    ...
    {_liquid_render->shader_program()}; water_shader.uniform("animtime",
    _animtime / 2880.f);
...

    {
      ...
      wmo_program.uniform("camera",
                          glm::vec3(_camera.position.x, _camera.position.y,
                                    _camera.position.z));
      ...
      for (auto &wmo_instance : _wmo_instances) {
        ...
        wmo_instance.draw(
            ...
            _camera.position, _draw_boxes.get(), _draw_models.get(), false,
            std::vector<selection_type>(), 0, false, display_mode::in_3D, true);
        ...
        if (doodads) {
          for (auto &pair : *doodads) {
            for (auto &doodad : pair.second)
              _wmo_doodads[doodad.model->file_key().filepath()].push_back(
                  &doodad);
          }
        }
      }
  ...
  if (_draw_models.get() &&
      !(_model_instances.empty() && _wmo_doodads.empty())) {
    ...
    for (auto &model_instance : _model_instances) {
      ...
      model_instance.model->renderer()->draw(
          mv, instance_mtx, m2_shader, model_render_state, frustum,
          culldistance, _camera.position, _animtime, _draw_boxes.get(),
          model_boxes_to_draw, display_mode::in_3D);
    }
  ...
  if (_draw_animated.get() && !model_with_particles.empty())
  {
    ...
    {
      it.first->draw_particles(model_view().transposed(), particles_shader,
  it.second);
    }
  }
  ...
  */
  ...
  if (_draw_grid.get()) {
    _grid.draw(mvp, glm::vec3(0.f, 0.f, 0.f), glm::vec4(0.7f, 0.7f, 0.7f, 1.0f),
               30.f);
  }
}
...
void Noggit::Ui::Tools::PreviewRenderer::updateLightingUniformBlock() {
  ...
  _lighting_ubo_data.DiffuseColor_FogStart = {
      _diffuse_light.x, _diffuse_light.y, _diffuse_light.z, 0};
  _lighting_ubo_data.AmbientColor_FogEnd = {_ambient_light.x, _ambient_light.y,
                                            _ambient_light.z, 0};
  _lighting_ubo_data.FogColor_FogOn = {0, 0, 0, 0};
  _lighting_ubo_data.LightDir_FogRate = {_light_dir.x, _light_dir.y,
                                         _light_dir.z, 1.0f};
...
void Noggit::Ui::Tools::PreviewRenderer::updateMVPUniformBlock(
    const glm::mat4x4 &model_view, const glm::mat4x4 &projection) {
