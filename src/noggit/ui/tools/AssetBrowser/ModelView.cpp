...
ModelViewer::ModelViewer(QWidget *parent, Noggit::NoggitRenderContext context)
    : PreviewRenderer(0, 0, context, parent), look(false), mousedir(-1.0f) {
...
void ModelViewer::wheelEvent(QWheelEvent *event) {
  if (event->angleDelta().y() > 0) {
    ...
    _move_sensitivity =
        std::max(_move_sensitivity - 0.5f / 30.0f, 1.0f / 30.0f);
  }
...
void ModelViewer::setActiveDoodadSet(const std::string &filename,
                                     const std::string &doodadset_name) {
  for (auto &wmo_instance : _wmo_instances) {
    if (wmo_instance.wmo->file_key().filepath() != filename) {
      continue;
    }
...
ModelViewer::~ModelViewer() { disconnect(_gl_guard_connection); }
