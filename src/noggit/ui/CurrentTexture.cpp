...
current_texture::current_texture(bool accept_drop, QWidget *parent)
    : ClickableLabel(parent), _filename("tileset/generic/black.blp"),
      _need_update(true), _is_selected(false), _is_swap_selected(false) {
...
void current_texture::update_texture_if_needed() {
  ...
  show();
  _texture_save =
      *BLPRenderer::getInstance().render_blp_to_pixmap(_filename, 128, 128);
