...
ShaderTool::ShaderTool(MapView *map_view, QWidget *parent)
    : QWidget(parent), _map_view(map_view), _color(1.f, 1.f, 1.f, 1.f) {
  ...
  color_picker->setColor(
      QColor::fromRgbF(_color.x, _color.y, _color.z, _color.w));
  ...
  _image_mask_group->setSizePolicy(
  ...
  QObject::connect(_slide_saturation,
                   ...
                   &ShaderTool::set_hsv);
  QObject::connect(_slide_value, &color_widgets::GradientSlider::valueChanged,
                   this, &ShaderTool::set_hsv);
  QObject::connect(_slide_hue, &color_widgets::HueSlider::valueChanged, this,
                   &ShaderTool::set_hsv);

  QObject::connect(_slide_saturation, SIGNAL(valueChanged(int)),
                   _spin_saturation, SLOT(setValue(int)));
  QObject::connect(_slide_value, SIGNAL(valueChanged(int)), _spin_value,
                   SLOT(setValue(int)));
  ...
  QObject::connect(_spin_hue, SIGNAL(valueChanged(int)), _slide_hue,
                   SLOT(setValue(int)));
  QObject::connect(_spin_value, SIGNAL(valueChanged(int)), _slide_value,
                   SLOT(setValue(int)));

  QObject::connect(color_wheel, &color_widgets::ColorWheel::colorSelected, this,
                   &ShaderTool::update_color_widgets);
  QObject::connect(color_picker, &color_widgets::ColorSelector::colorChanged,
                   this, &ShaderTool::update_color_widgets);

  connect(_color_palette, &color_widgets::ColorListWidget::color_added, [&]() {
    _color_palette->setColorAt(_color_palette->colors().length() - 1,
                               color_wheel->color());
  });
  ...
  connect(color_wheel, &color_widgets::ColorWheel::colorChanged,
          [this](QColor color) { color_picker->setColor(color); });

  connect(_image_mask_group,
          &Noggit::Ui::Tools::ImageMaskSelector::rotationUpdated, this,
          &ShaderTool::updateMaskImage);
  connect(_radius_slider,
          &Noggit::Ui::Tools::UiCommon::ExtendedSlider::valueChanged, this,
          &ShaderTool::updateMaskImage);
  connect(_image_mask_group,
          &Noggit::Ui::Tools::ImageMaskSelector::pixmapUpdated, this,
          &ShaderTool::updateMaskImage);
...
void ShaderTool::changeShader(World *world, glm::vec3 const &pos, float dt,
                              bool add) {
  if (!_image_mask_group->isEnabled()) {
    world->changeShader(pos, _color, 2.0f * dt * _speed_slider->value(),
                        _radius_slider->value(), add);
  } else {
    world->stampShader(pos, _color, 2.0f * dt * _speed_slider->value(),
                       _radius_slider->value(), add, &_mask_image,
                       _image_mask_group->getBrushMode(),
                       _use_image_colors->isChecked());
  }
}
...
void ShaderTool::setRadius(float radius) { _radius_slider->setValue(radius); }

void ShaderTool::changeSpeed(float change) {
  _speed_slider->setValue(_speed_slider->value() + change);
}

void ShaderTool::setSpeed(float speed) { _speed_slider->setValue(speed); }
...
void ShaderTool::addColorToPalette() { _color_palette->append(); }

void ShaderTool::set_hsv() {
  if (!signalsBlocked()) {
    color_wheel->setColor(QColor::fromHsv(_slide_hue->value(),
...
void ShaderTool::update_color_widgets() {
  ...
  _slide_value->setFirstColor(
      QColor::fromHsvF(color_wheel->hue(), color_wheel->saturation(), 0));
  _slide_value->setLastColor(
      QColor::fromHsvF(color_wheel->hue(), color_wheel->saturation(), 1));
...
QSize ShaderTool::sizeHint() const { return QSize(215, height()); }
...
void ShaderTool::updateMaskImage() {
  ...
  if (_map_view->get_editing_mode() != editing_mode::stamp ||
      (_map_view->getActiveStampModeItem() &&
       _map_view->getActiveStampModeItem() == this))
...
void ShaderTool::fromJSON(QJsonObject const &json) {
  ...
  color_picker->setColor(QColor(color_picker->color().redF(),
                                color_picker->color().greenF(),
                                color_picker->color().blueF()));
