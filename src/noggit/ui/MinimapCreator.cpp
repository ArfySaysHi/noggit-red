...
namespace Noggit {
namespace Ui {
MinimapCreator::MinimapCreator(MapView *mapView, World *world, QWidget *parent)
    : QWidget(parent) {
  setMinimumWidth(250);
  setMaximumWidth(250);
  ...
  auto m2_instance_include_box_layout =
      new QFormLayout(m2_instance_include_widget);
  ...
  auto m2_instance_include_box_layout_btns =
      new QHBoxLayout(m2_instance_include_widget);
  ...
  auto wmo_instance_exclude_box_layout =
      new QFormLayout(wmo_instance_exclude_widget);
  ...
  auto wmo_instance_exclude_box_layout_btns =
      new QHBoxLayout(wmo_instance_exclude_widget);
  ...
  auto clear_btn_wmo_i =
      new QPushButton("Clear all", wmo_instance_exclude_widget);
  ...
  ocean_color_light->setColor(
      QColor::fromRgbF(_render_settings.ocean_color_light.x,
  ...
  ocean_color_dark->setColor(QColor::fromRgbF(
      _render_settings.ocean_color_dark.x, _render_settings.ocean_color_dark.y,
  ...
  river_color_light->setColor(
      QColor::fromRgbF(_render_settings.river_color_light.x,
  ...
  river_color_dark->setColor(QColor::fromRgbF(
      _render_settings.river_color_dark.x, _render_settings.river_color_dark.y,
  ...
  connect(diffuse_color, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.diffuse_color = {
                static_cast<float>(new_color.redF()),
  ...
  connect(ambient_color, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.ambient_color = {
                static_cast<float>(new_color.redF()),
                static_cast<float>(new_color.greenF()),
                static_cast<float>(new_color.blueF())};
          });
  ...
  connect(ocean_color_light, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.ocean_color_light = {
                static_cast<float>(new_color.redF()),
                static_cast<float>(new_color.greenF()),
                static_cast<float>(new_color.blueF()),
  ...
  connect(ocean_color_dark, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.ocean_color_dark = {
                static_cast<float>(new_color.redF()),
                static_cast<float>(new_color.greenF()),
                static_cast<float>(new_color.blueF()),
                static_cast<float>(new_color.alphaF())};
  ...
  connect(river_color_light, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.river_color_light = {
                static_cast<float>(new_color.redF()),
                static_cast<float>(new_color.greenF()),
                static_cast<float>(new_color.blueF()),
                static_cast<float>(new_color.alphaF())};
  ...
  connect(river_color_dark, &color_widgets::ColorSelector::colorChanged,
          [this](QColor new_color) {
            _render_settings.river_color_dark = {
                static_cast<float>(new_color.redF()),
                static_cast<float>(new_color.greenF()),
                static_cast<float>(new_color.blueF()),
                static_cast<float>(new_color.alphaF())};
  ...
  connect(add_btn, &QPushButton::clicked, ([=, this]() {
            ...
            for (auto &selection : world->current_selection()) {
              ...
              if (obj->which() == eMODEL) {
                includeM2Model(static_cast<ModelInstance *>(obj)
                                   ->model->file_key()
                                   .filepath());
              }
            }
  ...
  connect(remove_btn, &QPushButton::clicked, ([=, this]() {
            if (!world->get_selected_model_count()) {
              for (auto item : _m2_model_filter_include->selectedItems()) {
                auto item_ = _m2_model_filter_include->takeItem(
                    _m2_model_filter_include->row(item));
                delete item_;
              }
            ...
            for (auto &selection : world->current_selection()) {
              ...
              if (obj->which() == eMODEL) {
                unincludeM2Model(static_cast<ModelInstance *>(obj)
                                     ->model->file_key()
                                     .filepath());
              }
            }
  ...
  connect(clear_btn, &QPushButton::clicked,
          ([=, this]() { _m2_model_filter_include->clear(); }));

  // M2 instance filter
  connect(add_btn_m2i, &QPushButton::clicked, ([=, this]() {
            if (!world->has_selection()) {
              return;
            }
  ...
  connect(remove_btn_m2i, &QPushButton::clicked, ([=, this]() {
            if (!world->get_selected_model_count()) {
              for (auto item : _m2_instance_filter_include->selectedItems()) {
                auto item_ = _m2_instance_filter_include->takeItem(
                    _m2_instance_filter_include->row(item));
                delete item_;
              }
  ...
  connect(clear_btn_m2i, &QPushButton::clicked,
          ([=, this]() { _m2_instance_filter_include->clear(); }));

  // WMO model filter
  connect(
      add_btn_wmo, &QPushButton::clicked, ([=, this]() {
        if (!world->has_selection()) {
          return;
        }
        ...
        for (auto &selection : world->current_selection()) {
          ...
          if (obj->which() == eWMO) {
            excludeWMOModel(
                static_cast<WMOInstance *>(obj)->wmo->file_key().filepath());
          }
        }
      }));

  connect(
      remove_btn_wmo, &QPushButton::clicked, ([=, this]() {
        if (!world->get_selected_model_count()) {
          for (auto item : _wmo_model_filter_exclude->selectedItems()) {
            auto item_ = _wmo_model_filter_exclude->takeItem(
                _wmo_model_filter_exclude->row(item));
            delete item_;
          }
  ...
  connect(clear_btn_wmo, &QPushButton::clicked,
          ([=, this]() { _wmo_model_filter_exclude->clear(); }));

  // WMO instance filter
  connect(add_btn_wmo_i, &QPushButton::clicked, ([=, this]() {
            if (!world->has_selection()) {
              return;
            }
  ...
  connect(remove_btn_wmo_i, &QPushButton::clicked, ([=, this]() {
            if (!world->get_selected_model_count()) {
              for (auto item : _wmo_instance_filter_exclude->selectedItems()) {
                auto item_ = _wmo_instance_filter_exclude->takeItem(
                    _wmo_instance_filter_exclude->row(item));
                delete item_;
              }
  ...
  connect(clear_btn_wmo_i, &QPushButton::clicked,
          ([=, this]() { _wmo_instance_filter_exclude->clear(); }));
  ...
  connect(search_filter_m2, qOverload<const QString &>(&QLineEdit::textChanged),
          [&](const QString &s) {
            for (int i = 0; i < _m2_model_filter_include->count(); ++i) {
              MinimapM2ModelFilterEntry *item_wgt =
                  reinterpret_cast<MinimapM2ModelFilterEntry *>(
                      _m2_model_filter_include->itemWidget(
                          _m2_model_filter_include->item(i)));

              std::string filename = item_wgt->getFileName().toStdString();
              std::string filter = s.toStdString();
              bool isHidden =
                  !filter.empty() && filename.find(filter) == std::string::npos;
              _m2_model_filter_include->item(i)->setHidden(isHidden);
            }
          });

  connect(search_filter_m2i,
          qOverload<const QString &>(&QLineEdit::textChanged),
          [&](const QString &s) {
            for (int i = 0; i < _m2_instance_filter_include->count(); ++i) {
              MinimapInstanceFilterEntry *item_wgt =
                  reinterpret_cast<MinimapInstanceFilterEntry *>(
                      _m2_instance_filter_include->itemWidget(
                          _m2_instance_filter_include->item(i)));

              std::string uid = std::to_string(item_wgt->getUid());
              std::string filter = s.toStdString();
              bool isHidden =
                  !filter.empty() && uid.find(filter) == std::string::npos;
              _m2_instance_filter_include->item(i)->setHidden(isHidden);
            }
          });

  connect(search_filter_wmo,
          qOverload<const QString &>(&QLineEdit::textChanged),
          [&](const QString &s) {
            for (int i = 0; i < _wmo_model_filter_exclude->count(); ++i) {
              MinimapWMOModelFilterEntry *item_wgt =
                  reinterpret_cast<MinimapWMOModelFilterEntry *>(
                      _wmo_model_filter_exclude->itemWidget(
  ...
  connect(search_filter_wmo_i,
          qOverload<const QString &>(&QLineEdit::textChanged),
          [&](const QString &s) {
            for (int i = 0; i < _wmo_instance_filter_exclude->count(); ++i) {
              MinimapInstanceFilterEntry *item_wgt =
                  reinterpret_cast<MinimapInstanceFilterEntry *>(
                      _wmo_instance_filter_exclude->itemWidget(
  ...
  connect(resolution, &QComboBox::currentTextChanged,
          [&](QString s) { _render_settings.resolution = s.toInt(); });

  connect(file_format, &QComboBox::currentTextChanged,
          [&](QString s) { _render_settings.file_format = s.toStdString(); });

  connect(draw_models, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_m2 = s; });

  connect(draw_wmos, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_wmo = s; });

  connect(draw_water, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_water = s; });

  connect(draw_adt, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_adt_grid = s; });

  connect(draw_elevation, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_elevation = s; });

  connect(use_filters, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.use_filters = s; });

  connect(draw_shadows, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.draw_shadows = s; });

  connect(combined_minimap, &QCheckBox::stateChanged,
          [this](int s) { _render_settings.combined_minimap = s; });
...
QSize MinimapCreator::sizeHint() const { return QSize(width(), height()); }

void MinimapCreator::includeM2Model(std::string filename, float size_cat) {
  bool already_added = false;

  for (int i = 0; i < _m2_model_filter_include->count(); ++i) {
    if (!reinterpret_cast<MinimapM2ModelFilterEntry *>(
             _m2_model_filter_include->itemWidget(
                 _m2_model_filter_include->item(i)))
             ->getFileName()
             .toStdString()
...
void MinimapCreator::includeM2Instance(uint32_t uid) {
  ...
  for (int i = 0; i < _m2_instance_filter_include->count(); ++i) {
    if (reinterpret_cast<MinimapInstanceFilterEntry *>(
            _m2_instance_filter_include->itemWidget(
                _m2_instance_filter_include->item(i)))
            ->getUid() == uid) {
      already_added = true;
...
void MinimapCreator::excludeWMOModel(std::string filename) {
  ...
  for (int i = 0; i < _wmo_model_filter_exclude->count(); ++i) {
    if (!reinterpret_cast<MinimapWMOModelFilterEntry *>(
             _wmo_model_filter_exclude->itemWidget(
                 _wmo_model_filter_exclude->item(i)))
             ->getFileName()
             .toStdString()
...
void MinimapCreator::saveFiltersToJSON() {
  ...
  if (!json_file.open(QIODevice::WriteOnly | QIODevice::Text |
                      QFile::Truncate)) {
    LogError << "Unable to save minimap creator model filters to JSON document."
             << std::endl;
    return;
  }
  ...
  // m2 models
  for (int i = 0; i < _m2_model_filter_include->count(); ++i) {
    auto item_wgt = reinterpret_cast<MinimapM2ModelFilterEntry *>(
        _m2_model_filter_include->itemWidget(
            _m2_model_filter_include->item(i)));

    QJsonObject item = QJsonObject();
    item.insert("filename", QJsonValue(item_wgt->getFileName()));
    item.insert("size_category",
                QJsonValue(static_cast<double>(item_wgt->getSizeCategory())));
    m2_models.append(item);
  }

  // m2 instances
  for (int i = 0; i < _m2_instance_filter_include->count(); ++i) {
    auto item_wgt = reinterpret_cast<MinimapInstanceFilterEntry *>(
        _m2_instance_filter_include->itemWidget(
            _m2_instance_filter_include->item(i)));
  ...
  // wmo models
  for (int i = 0; i < _wmo_model_filter_exclude->count(); ++i) {
    auto item_wgt = reinterpret_cast<MinimapWMOModelFilterEntry *>(
        _wmo_model_filter_exclude->itemWidget(
            _wmo_model_filter_exclude->item(i)));
...
MinimapM2ModelFilterEntry::MinimapM2ModelFilterEntry(QWidget *parent)
    : QWidget(parent) {
...
MinimapWMOModelFilterEntry::MinimapWMOModelFilterEntry(QWidget *parent)
    : QWidget(parent) {
  setAttribute(Qt::WA_TranslucentBackground);
  auto layout = new QHBoxLayout(this);
  layout->addWidget(_filename = new QLineEdit(this));
...
MinimapInstanceFilterEntry::MinimapInstanceFilterEntry(QWidget *parent)
    : QWidget(parent) {
  setAttribute(Qt::WA_TranslucentBackground);
  auto layout = new QHBoxLayout(this);
