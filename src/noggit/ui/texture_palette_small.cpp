...
texture_palette_small::texture_palette_small(
    std::shared_ptr<Noggit::Project::NoggitProject> Project, int mapId,
    QWidget *parent)
    : widget(parent), layout(new ::QGridLayout(this)), _project(Project),
      _map_id(mapId) {
  ...
  connect(_texture_list, &QListWidget::itemClicked, this,
...
void texture_palette_small::addTexture() {
  ...
  if (Noggit::Ui::selected_texture::get())
    filename =
        Noggit::Ui::selected_texture::get().value()->file_key().filepath();
  else
...
void texture_palette_small::addTextureByFilename(const std::string &filename,
  ...
  list_item->setIcon(*BLPRenderer::getInstance().render_blp_to_pixmap(
...
void texture_palette_small::dragEnterEvent(QDragEnterEvent *event) {
  if (event->mimeData()->hasText() &&
      (_texture_paths.find(
           event->mimeData()->text().remove("tileset/").toStdString()) ==
       _texture_paths.end()))
    event->accept();
}
