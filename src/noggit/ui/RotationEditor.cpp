...
rotation_editor::rotation_editor(QWidget *parent, World *world)
  ...
  connect(_rotation_x, qOverload<double>(&QDoubleSpinBox::valueChanged),
          [&, world, parent] {
            NOGGIT_ACTION_MGR->beginAction(
                reinterpret_cast<MapView *>(parent),
  ...
  connect(_rotation_x, &QDoubleSpinBox::editingFinished, [&, world, parent] {
  ...
  connect(_rotation_z, &QDoubleSpinBox::editingFinished, [&, world, parent] {
    if (world->has_multiple_model_selected()) {
      // avoid rotation changes when losing focus
  ...
  connect(_rotation_y, &QDoubleSpinBox::editingFinished, [&, world, parent] {
    if (world->has_multiple_model_selected()) {
      // avoid rotation changes when losing focus
  ...
  connect(_scale, &QDoubleSpinBox::editingFinished, [&, world, parent] {
    if (world->has_multiple_model_selected()) {
      ...
      if (_scale->hasFocus()) {
        NOGGIT_ACTION_MGR->beginAction(
            reinterpret_cast<MapView *>(parent),
            Noggit::ActionFlags::eOBJECTS_TRANSFORMED);
        world->scale_selected_models(_scale->value(),
...
void rotation_editor::updateValues(World *world) {
  ...
  if (world->has_multiple_model_selected()) {
    ...
    // default value for rotation and scaling, affect the models only when
    // pressing enter
    _rotation_x->setValue(0.f);
    _rotation_y->setValue(0.f);
    _rotation_z->setValue(0.f);
...
void rotation_editor::set_model_rotation(World *world) {
  ...
  if (!world->has_multiple_model_selected()) {
    world->set_selected_models_rotation(math::degrees(_rotation_x->value()),
...
void rotation_editor::change_models_rotation(World *world) {
  ...
  if (world->has_multiple_model_selected()) {
    world->rotate_selected_models(math::degrees(_rotation_x->value()),
                                  math::degrees(_rotation_y->value()),
                                  math::degrees(_rotation_z->value()),
