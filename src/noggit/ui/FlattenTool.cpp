...
flatten_blur_tool::flatten_blur_tool(QWidget *parent)
    : QWidget(parent), _angle(45.0f), _orientation(0.0f),
      _flatten_type(eFlattenType_Linear), _flatten_mode(true, true) {
  setMinimumWidth(250);
  setMaximumWidth(250);
  ...
  flatten_type_group->setSizePolicy(QSizePolicy::Preferred,
                                    QSizePolicy::Maximum);
  ...
  flatten_blur_group->setSizePolicy(QSizePolicy::Preferred,
                                    QSizePolicy::Maximum);
  ...
  flatten_blur_layout->addWidget(_lock_down_checkbox = new QCheckBox(this), 0,
                                 1);
  ...
  _orientation_dial->setSliderPosition(_orientation -
  ...
  connect(_angle_slider, &QSlider::valueChanged, [&](int v) {
    _angle = v;
...
void flatten_blur_tool::flatten(World *world, glm::vec3 const &cursor_pos,
                                float dt) {
  // TODO : objects snapping may be optimized by reusing the flatten code for
  // objects instead of ray intersection for ground distance store the ground
  // height diff at center of all objects hit before editing it
  std::vector<std::pair<SceneObject *, float>> objects_ground_distance =
      world->getObjectsGroundDistance(cursor_pos, _radius_slider->value(),
  ...
  world->flattenTerrain(cursor_pos,
                        ...
                        _radius_slider->value(), _flatten_type, _flatten_mode,
...
void flatten_blur_tool::blur(World *world, glm::vec3 const &cursor_pos,
                             float dt) {
  ...
  world->blurTerrain(cursor_pos, 1.f - pow(0.5f, dt * _speed_slider->value()),
                     _radius_slider->value(), _flatten_type, _flatten_mode);
...
QSize flatten_blur_tool::sizeHint() const { return QSize(250, height()); }
