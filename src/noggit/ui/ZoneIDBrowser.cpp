...
namespace Noggit {
namespace Ui {
zone_id_browser::zone_id_browser(QWidget *parent) : QWidget(parent), mapID(-1) {
  ...
  edit_area_button->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::cog));
  QPushButton *add_zone_button =
      new QPushButton("Add a new Zone(parent Area)", this);
  add_zone_button->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::plus));
  QPushButton *add_subzone_button =
      new QPushButton("Add a new Subzone(selected as Parent)", this);
  add_subzone_button->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::plus));
...
int zone_id_browser::GetSelectedAreaId() {
  ...
  if (selected_items.size()) {
    int selected_area_id =
        selected_items.back()->data(0, Qt::UserRole + 1).toInt();
...
void zone_id_browser::open_area_editor() {
  // opens and loads the area editor with the currently selected item in the
  // tree.
...
void zone_id_browser::add_new_zone() {
  ...
  zone_create_params_layout->addWidget(
      new QLabel("AreaTable Id : ", zone_create_params));
  ...
  zone_create_params_layout->addWidget(
      new QLabel("Zone Name : ", zone_create_params));
  ...
  connect(params_okay, &QPushButton::clicked, [=]() {
    ...
    if (!gAreaDB.CheckIfIdExists(params_id_spinbox->value())) {
      ...
      QMessageBox::warning(nullptr, "ID already in use",
  ...
  if (zone_create_params->exec() == QDialog::Accepted) {
    auto new_record = gAreaDB.addRecord(
        params_id_spinbox->value()); // could add a try catch but we already
                                     // check if id is used before
    // init some defaults
    ...
    new_record.write(AreaDB::UnderwaterSoundProviderPreferences,
                     11); // underwater sound pref, usually set

    // locale stuff
    // new_record.writeString(AreaDB::Name,
    // params_name_ledit->text().toStdString()); // only write default name for
    // enUS and enGB ? maybe get the client's locale somehow
    int locale_id = Noggit::Application::NoggitApplication::instance()
                        ->clientData()
                        ->getLocaleId();
    new_record.writeLocalizedString(
        AreaDB::Name, params_name_ledit->text().toStdString(), locale_id);
    new_record.write(AreaDB::Name + 16,
                     16712190); // loc mask, only verified for enUS
...
void zone_id_browser::add_new_subzone() {
  ...
  std::uint32_t selected_parent_area_id =
      gAreaDB.get_area_parent(selected_areaid); // the selected area's parentid
  if (selected_parent_area_id) {
    QMessageBox::information(nullptr, "Wrong Parent type",
  ...
  zone_create_params_layout->addWidget(
      new QLabel("Subzone Name : ", zone_create_params));
  QLineEdit *params_name_ledit = new QLineEdit(zone_create_params);
  ...
  if (zone_create_params->exec() == QDialog::Accepted) {
    ...
    new_record.write(AreaDB::Region,
                     selected_areaid); // set selecetd area as parent.
    // get new areabit
    new_record.write(AreaDB::AreaBit, gAreaDB.get_new_areabit());
    unsigned int flags = 0;
    flags |= (1 << 6);  // Allow Dueling
    ...
    new_record.write(AreaDB::Flags,
                     flags); // allow dueling + force area on dynamic transport
                             // + enable flight bounds+ subzone flags
    new_record.write(AreaDB::UnderwaterSoundProviderPreferences,
                     11); // underwater sound pref, usually set
    // lcoale stuff
    int locale_id = Noggit::Application::NoggitApplication::instance()
    ...
    new_record.write(AreaDB::MinElevation,
                     -500.0f); // loc mask, only verified for enUS
    // save dbc instantly ?
    gAreaDB.save();
    // add to tree
    auto areawidgetitem = add_area(params_id_spinbox->value());
...
AreaEditor::AreaEditor(QWidget *parent) : QWidget(parent) {
  ...
  _set_parent_button =
      new QPushButton("Set Selected Zone as Parent", area_settings_group);
  ...
  _area_name =
      new Tools::MapCreationWizard::Ui::LocaleDBCEntry(area_settings_group);
  ...
  _ambiant_multiplier->setRange(
      0, 100); // 0.0 - 1.0, decimal not supported so *100
  ...
  // for (DBCFile::Iterator i = gLiquidTypeDB.begin(); i != gLiquidTypeDB.end();
  // ++i)
  // {
  //     std::stringstream ss;
  //     int liquid_type = i->getInt(LiquidTypeDB::Type);
  //     ss << i->getInt(LiquidTypeDB::ID) << "-" <<
  //     i->getString(LiquidTypeDB::Name); switch (liquid_type)
  //     {
  //     case 0:
  ...
  for (DBCFile::Iterator i = gSoundProviderPreferencesDB.begin();
       i != gSoundProviderPreferencesDB.end(); ++i) {
    std::stringstream ss;
    ss << i->getInt(SoundProviderPreferencesDB::ID) << "-"
       << i->getString(SoundProviderPreferencesDB::Description);
  ...
  // selectionOptionsBox->setExpanded(_settings->value("object_editor/movement_options",
  // false).toBool());
  ...
  _min_elevation_spinbox->setRange(
      -5000, 5000); // only 3 known values : -5000, -500, 1000
  ...
  // advancedOptions_layout->addRow("Water Liquid Type override:",
  // _liquid_type_water_combobox); advancedOptions_layout->addRow("Ocean Liquid
  // Type override:", _liquid_type_ocean_combobox);
  // advancedOptions_layout->addRow("Magma Liquid Type override:",
  // _liquid_type_magma_combobox); advancedOptions_layout->addRow("Slime Liquid
  // Type override:", _liquid_type_slime_combobox);
  advancedOptions_layout->addRow("Sound Provider Preference:",
                                 _sound_provider_preferences_cbbox);
  advancedOptions_layout->addRow("Underwater Sound Provider Preference:",
                                 _underwater_sound_provider_preferences_cbbox);

  QPushButton *save_area_button =
      new QPushButton("Save changes (write DBC)", this);
  ...
  _flags_value_spinbox->setRange(
      0, INT_MAX); // uint not necessary for wrath because we only have 31
                   // flags. for cata+ we'll need all 32 bits instead, might
                   // need a line edit
  flags_layout->addWidget(_flags_value_spinbox);

  // TODO : update checkboxes when value is changed, temporarly disable it from
  // edit
  _flags_value_spinbox->setEnabled(false);

  for (int i = 0; i < 31; ++i) {
    QCheckBox *flag_checkbox =
        new QCheckBox(QString::fromStdString(area_flags_names.at(i)), this);
  ...
  flags_checkboxes[20]->setHidden(
      true); // tournement realm thingy, useless for pservers
  flags_checkboxes[17]->setHidden(
      true); // "Area not in use", prob some blizz dev stuff
  //************************************//
  ...
  layout->addRow(new QLabel("If only day or night is set but not the other, "
                            "the other will be saved as the same sound."));
  ...
  connect(_set_parent_button, &QPushButton::clicked, [=]() {
    ...
    // a zone must be a top level item, and not have a parent or the subzone
    // flag.
    ...
    std::uint32_t selected_parent_area_id = gAreaDB.get_area_parent(
        tree_selected_id); // the selected area's parentid
    if (selected_parent_area_id) {
    ...
    try {
      ...
      // select the item ?
      auto item = parent->create_or_get_tree_widget_item(
          _area_id_label->text().toInt());
  ...
  connect(_flags_value_spinbox, qOverload<int>(&QSpinBox::valueChanged),
          [&](int v) {
...
void AreaEditor::load_area(int area_id) {
  try {
    ...
    _exploration_level_spinbox->setValue(
        record.getInt(AreaDB::ExplorationLevel));

    int faction_group_mask = record.getInt(AreaDB::FactionGroup);
    switch (
        faction_group_mask) // hardcoded but can be read from factiongroup.dbc
    {
    case 2:
    ...
    _min_elevation_spinbox->setValue(record.getFloat(
        AreaDB::MinElevation)); // only 3 known values : -5000, -500, 1000

    _ambiant_multiplier->setValue(record.getFloat(AreaDB::AmbientMultiplier) *
                                  100);

    int sound_provider_id = record.getInt(AreaDB::SoundProviderPreferences);
    if (sound_provider_id != 0 &&
        gSoundProviderPreferencesDB.CheckIfIdExists(sound_provider_id)) {
      int row_id =
          gSoundProviderPreferencesDB.getRecordRowId(sound_provider_id);
      _sound_provider_preferences_cbbox->setCurrentIndex(row_id +
                                                         1); // index 0 = "None"
    } else
      _sound_provider_preferences_cbbox->setCurrentIndex(0);

    int underwater_sound_provider_id =
        record.getInt(AreaDB::UnderwaterSoundProviderPreferences);
    if (underwater_sound_provider_id != 0 &&
        gSoundProviderPreferencesDB.CheckIfIdExists(
            underwater_sound_provider_id)) {
      int row_id = gSoundProviderPreferencesDB.getRecordRowId(
          underwater_sound_provider_id);
      _underwater_sound_provider_preferences_cbbox->setCurrentIndex(
          row_id + 1); // index 0 = "None"
    } else
    ...
    if (zone_music_id != 0 && gZoneMusicDB.CheckIfIdExists(zone_music_id)) {
      ...
      std::stringstream ss;
      ss << zone_music_id << "-"
         << zone_music_record.getString(ZoneMusicDB::Name);
    ...
    if (zone_intro_music_id != 0 &&
        gZoneIntroMusicTableDB.CheckIfIdExists(zone_intro_music_id)) {
      _zone_intro_music_button->setProperty("id", zone_intro_music_id);
      auto zone_intro_music_record =
          gZoneIntroMusicTableDB.getByID(zone_intro_music_id);
      std::stringstream ss;
      ss << zone_intro_music_id << "-"
         << zone_intro_music_record.getString(ZoneIntroMusicTableDB::Name);
    ...
    if (sound_ambiance_id != 0 &&
        gSoundAmbienceDB.CheckIfIdExists(sound_ambiance_id)) {
      auto sound_ambiance_record = gSoundAmbienceDB.getByID(sound_ambiance_id);

      int day_sound_id =
          sound_ambiance_record.getInt(SoundAmbienceDB::SoundEntry_day);
      if (day_sound_id != 0 && gSoundEntriesDB.CheckIfIdExists(day_sound_id)) {
        ...
        ss_day << day_sound_id << "-"
               << sound_entry_day_record.getString(SoundEntriesDB::Name);
      ...
      int night_sound_id =
          sound_ambiance_record.getInt(SoundAmbienceDB::SoundEntry_night);
      if (night_sound_id != 0 &&
          gSoundEntriesDB.CheckIfIdExists(night_sound_id)) {
        auto sound_entry_night_record = gSoundEntriesDB.getByID(night_sound_id);
        std::stringstream ss_night;
        ss_night << night_sound_id << "-"
                 << sound_entry_night_record.getString(SoundEntriesDB::Name);
...
void AreaEditor::save_area() {
  try {
    DBCFile::Record record =
        gAreaDB.getByID(_area_id_label->text()
                            .toInt()); // is_new_record ? gLightDB.addRecord(Id)
                                       // : gLightDB.getByID(Id);
    ...
    if (_sound_provider_preferences_cbbox->currentIndex() != 0) {
      auto rec = gSoundProviderPreferencesDB.getRecord(
          _sound_provider_preferences_cbbox->currentIndex() - 1);
    ...
    if (_underwater_sound_provider_preferences_cbbox->currentIndex() != 0) {
      auto rec = gSoundProviderPreferencesDB.getRecord(
          _underwater_sound_provider_preferences_cbbox->currentIndex() - 1);
      underwaterSoundProviderPreferences_id =
          rec.getInt(SoundProviderPreferencesDB::ID);
    }
    record.write(AreaDB::UnderwaterSoundProviderPreferences,
                 underwaterSoundProviderPreferences_id);

    // Ambiance ID. Blizzard stores those as unamed pair
    // Just iterate the DBC to see if an entry with our settings already exists,
    // if not create it. The reasoning for not having a selector/picker with the
    // existing pairs is that it has less freedom, is harder to use and it's
    // ugly if they don't have a name. This doesn't have the option to edit that
    // entry for all its users though.

    int soundambience_day = _sound_ambiance_day_button->property("id").toInt();
    int soundambience_night =
        _sound_ambiance_night_button->property("id").toInt();

    if (soundambience_day &&
        !soundambience_night) // if day is set but not night, set night to day
      soundambience_night = soundambience_day;
    else if (!soundambience_day && soundambience_night) // night to day
      soundambience_night = soundambience_day;

    if (soundambience_day &&
        soundambience_night) // check if both day and night are set
    {
      bool sound_ambiance_exists{false};
      for (DBCFile::Iterator i = gSoundAmbienceDB.begin();
           i != gSoundAmbienceDB.end(); ++i) {
      ...
      if (!sound_ambiance_exists) {
        ...
        new_record.write(SoundAmbienceDB::SoundEntry_night,
                         soundambience_night);
        gSoundAmbienceDB.save();
        record.write(AreaDB::SoundAmbience, new_id);
      }
    } else
      record.write(AreaDB::SoundAmbience,
                   0); // if night or day isn't set, set to 0

    record.write(AreaDB::ZoneMusic, _zone_music_button->property("id").toInt());
    record.write(AreaDB::ZoneIntroMusicTable,
                 _zone_intro_music_button->property("id").toInt());
    ...
    auto parent = static_cast<zone_id_browser *>(this->parentWidget());
    auto item =
        parent->create_or_get_tree_widget_item(_area_id_label->text().toInt());
    std::stringstream ss;
    std::string areaName = record.getLocalizedString(AreaDB::Name);
    ss << _area_id_label->text().toInt() << "-" << areaName;
    item->setText(0, QString(ss.str().c_str()));

    record.write(AreaDB::FactionGroup,
                 _faction_group_combobox->currentIndex() * 2);
    record.write(AreaDB::MinElevation,
                 static_cast<float>(_min_elevation_spinbox->value()));
    record.write(AreaDB::AmbientMultiplier,
                 _ambiant_multiplier->value() / 100.0f);
    record.write(AreaDB::LightId, 0); // never used

    gAreaDB.save();

    load_area(_area_id_label->text()
                  .toInt()); // reload ui, especially for night/day ambience
  } catch (AreaDB::NotFound) {
  }
}
