...
void zone_id_browser::add_new_zone() {
  ...
  new_record.write(
      AreaDB::Flags,
      64); // allow dueling, seems to the be the common default for regions
  new_record.write(AreaDB::UnderwaterSoundProviderPreferences,
                   11); // underwater sound pref, usually set

  // locale stuff
  new_record.writeString(
      AreaDB::Name,
      "Unnamed Noggit Zone"); // only write default name for enUS and enGB ?
                              // maybe get the client's locale somehow
...
void zone_id_browser::add_new_subzone() {
  ...
  new_record.write(
      AreaDB::Flags,
      1073759296); // allow dueling + force area on dynamic transport + enable
                   // flight bounds+ subzone flags
  new_record.write(AreaDB::UnderwaterSoundProviderPreferences,
                   11); // underwater sound pref, usually set
  // lcoale stuff
  new_record.writeString(
      AreaDB::Name,
      "Unnamed Noggit Subzone"); // only write default name for enUS and enGB ?
                                 // maybe get the client's locale somehow
...
AreaEditor::AreaEditor(QWidget *parent) : QWidget(parent) {
  ...
  connect(_sound_ambiance_day_button, &QPushButton::clicked, ([=, this]() {
            auto window = new SoundEntryPickerWindow(
                _sound_ambiance_day_button, SoundEntryTypes::ZONE_AMBIENCE,
                false, this);
            window->show();
          }));
  ...
  connect(_sound_ambiance_night_button, &QPushButton::clicked, ([=, this]() {
            auto window = new SoundEntryPickerWindow(
                _sound_ambiance_night_button, SoundEntryTypes::ZONE_AMBIENCE,
                false, this);
            window->show();
          }));
  ...
  connect(_set_parent_button, &QPushButton::clicked, ([=, this]() {
            // Current design choice : Cannot change a zone to be a subzone or
            // the opposite, only current subzones can have a parent set.
            ...
            // a zone must be a top level item, and not have a parent or the
            // subzone flag.
            ...
            std::stringstream ss;
            ss << _parent_area_id << "-"
               << gAreaDB.getAreaName(_parent_area_id);
            _parent_area_label->setText(ss.str().c_str());

            // _parent_area_label->setText(std::to_string(
            // tree_selected_id).c_str());
...
void AreaEditor::save_area() {
  DBCFile::Record record = gAreaDB.getByID(
      _area_id_label->text().toInt()); // is_new_record ? gLightDB.addRecord(Id)
                                       // : gLightDB.getByID(Id);
  ...
  // existing pairs is that it has less freedom, is harder to use and it's ugly
  // if they don't have a name. This doesn't have the option to edit that entry
  // for all its users though.
