...
#define DESTRUCTIVE_ACTION(ACTION_CODE)                                        \
  ...
  reply = QMessageBox::question(this, "Destructive action",                    \
                                "This action cannot be undone. Current "       \
                                "change history will be lost. Continue?",      \
...
#define ADD_TOGGLE(menu_, name_, shortcut_, property_)                         \
  do {                                                                         \
    ...
    connect(action, &QAction::toggled, &property_,                             \
            &Noggit::BoolToggleProperty::set);                                 \
    connect(&property_, &Noggit::BoolToggleProperty::changed, action,          \
            &QAction::setChecked);                                             \
  } while (false)
...
#define ADD_TOGGLE_NS_POST(menu_, name_, property_, code_)                     \
  do {                                                                         \
    ...
    connect(&property_, &Noggit::bool_toggle_property::changed, action,        \
            &QAction::setChecked);                                             \
    connect(action, &QAction::toggled, code_);                                 \
    connect(&property_, &Noggit::bool_toggle_property::changed, code_);        \
  } while (false)
...
void MapView::set_editing_mode(editing_mode mode) {
  ...
  if (context() && context()->isValid()) {
    _world->renderer()->getTerrainParamsUniformBlock()->draw_areaid_overlay =
        false;
    _world->renderer()->getTerrainParamsUniformBlock()->draw_impass_overlay =
        false;
    _world->renderer()
        ->getTerrainParamsUniformBlock()
        ->draw_paintability_overlay = false;
    _world->renderer()->getTerrainParamsUniformBlock()->draw_selection_overlay =
        false;
    _world->renderer()
        ->getTerrainParamsUniformBlock()
        ->draw_groundeffectid_overlay = false;
    _world->renderer()
        ->getTerrainParamsUniformBlock()
        ->draw_groundeffect_layerid_overlay = false;
    _world->renderer()
        ->getTerrainParamsUniformBlock()
        ->draw_noeffectdoodad_overlay = false;
    _world->renderer()->getTerrainParamsUniformBlock()->draw_only_normals =
        false;
    _world->renderer()->getTerrainParamsUniformBlock()->point_normals_up =
        false;
...
editing_mode MapView::get_editing_mode() const { return terrainMode; }
...
void MapView::setToolPropertyWidgetVisibility(editing_mode mode) {
  ...
  case editing_mode::object:
    _asset_browser_dock->setVisible(
        !ui_hidden &&
...
bool MapView::isRotatingCamera() const { return look; }
...
void MapView::setupViewportOverlay() {
  ...
  _viewport_overlay_ui->gizmoVisibleButton->setIcon(Noggit::Ui::FontNoggitIcon(
      Noggit::Ui::FontNoggit::Icons::GIZMO_VISIBILITY));
  _viewport_overlay_ui->gizmoModeButton->setIcon(
      Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::Icons::GIZMO_LOCAL));
  _viewport_overlay_ui->gizmoRotateButton->setIcon(
      Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::Icons::GIZMO_ROTATE));
  _viewport_overlay_ui->gizmoScaleButton->setIcon(
      Noggit::Ui::FontNoggitIcon(Noggit::Ui::FontNoggit::Icons::GIZMO_SCALE));
  _viewport_overlay_ui->gizmoTranslateButton->setIcon(
      Noggit::Ui::FontNoggitIcon(
          Noggit::Ui::FontNoggit::Icons::GIZMO_TRANSLATE));

  connect(this, &MapView::resized,
          [this]() { _overlay_widget->setGeometry(0, 0, width(), height()); });

  connect(_viewport_overlay_ui->gizmoVisibleButton, &QPushButton::clicked,
          [this]() {
            _gizmo_on.set(
                _viewport_overlay_ui->gizmoVisibleButton->isChecked());
          });

  connect(&_gizmo_on, &Noggit::BoolToggleProperty::changed, [this](bool state) {
  ...
  connect(_viewport_overlay_ui->gizmoTranslateButton, &QPushButton::clicked,
          [this]() { updateGizmoOverlay(ImGuizmo::OPERATION::TRANSLATE); });

  connect(_viewport_overlay_ui->gizmoRotateButton, &QPushButton::clicked,
          [this]() { updateGizmoOverlay(ImGuizmo::OPERATION::ROTATE); });

  connect(_viewport_overlay_ui->gizmoScaleButton, &QPushButton::clicked,
          [this]() { updateGizmoOverlay(ImGuizmo::OPERATION::SCALE); });
}
...
void MapView::setupNodeEditor() {
  auto _node_editor =
      new Noggit::Ui::Tools::NodeEditor::Ui::NodeEditorWidget(this);
  _node_editor_dock = new QDockWidget("Node editor", this);
  _node_editor_dock->setWidget(_node_editor);
  _node_editor_dock->setAllowedAreas(Qt::BottomDockWidgetArea |
  ...
  _node_editor_dock->setVisible(
      _settings->value("map_view/node_editor", false).toBool());
...
void MapView::setupAssetBrowser() {
  ...
  _asset_browser =
      new Noggit::Ui::Tools::AssetBrowser::Ui::AssetBrowserWidget(this, this);
  ...
  connect(this, &QObject::destroyed, _asset_browser_dock,
          &QObject::deleteLater);
}
...
void MapView::setupDetailInfos() {
  ...
  _detail_infos_dock->setAllowedAreas(Qt::BottomDockWidgetArea |
                                      Qt::TopDockWidgetArea |
                                      Qt::LeftDockWidgetArea);
  ...
  connect(NOGGIT_ACTION_MGR, &Noggit::ActionManager::onActionBegin,
          [this](Noggit::Action *) { updateDetailInfos(); });
  ...
  connect(NOGGIT_ACTION_MGR, &Noggit::ActionManager::currentActionChanged,
          [this](unsigned) { updateDetailInfos(); });
}
...
void MapView::updateDetailInfos() {
  ...
  if (guidetailInfos->isVisible()) {
    if (!current_selection.empty()) {
      selection_type &selection_last =
          const_cast<selection_type &>(current_selection.back());
      ...
      case eEntry_MapChunk: {
        selected_chunk_type &chunk_sel(
            std::get<selected_chunk_type>(selection_last));
...
void MapView::setupToolbars() {
  _toolbar = new Noggit::Ui::toolbar(
      _tools, [this](editing_mode mode) { set_editing_mode(mode); });
  _toolbar->setOrientation(Qt::Vertical);
  auto left_toolbar_layout =
      new QVBoxLayout(_viewport_overlay_ui->leftToolbarHolder);
  ...
  auto left_sec_toolbar_layout =
      new QVBoxLayout(_viewport_overlay_ui->leftSecondaryToolbarHolder);
  left_sec_toolbar_layout->setContentsMargins(5, 0, 5, 0);

  _left_sec_toolbar =
      new Noggit::Ui::Tools::ViewToolbar::Ui::ViewToolbar(this, terrainMode);
  connect(this, &QObject::destroyed, _left_sec_toolbar, &QObject::deleteLater);
  left_sec_toolbar_layout->addWidget(_left_sec_toolbar);

  auto top_toolbar_layout =
      new QVBoxLayout(_viewport_overlay_ui->upperToolbarHolder);
  top_toolbar_layout->setContentsMargins(5, 0, 5, 0);
  auto sec_toolbar_layout =
      new QVBoxLayout(_viewport_overlay_ui->secondaryToolbarHolder);
  sec_toolbar_layout->setContentsMargins(5, 0, 5, 0);

  _viewport_overlay_ui->secondaryToolbarHolder->hide();
  _secondary_toolbar =
      new Noggit::Ui::Tools::ViewToolbar::Ui::ViewToolbar(this);
  connect(this, &QObject::destroyed, _secondary_toolbar, &QObject::deleteLater);

  _view_toolbar = new Noggit::Ui::Tools::ViewToolbar::Ui::ViewToolbar(
      this, _secondary_toolbar);
...
void MapView::setupMainToolbar() {
  _main_window->_app_toolbar =
      new QToolBar("Menu Toolbar", this); // this or mainwindow as parent?
  connect(this, &QObject::destroyed, _main_window->_app_toolbar,
          &QObject::deleteLater);

  _main_window->_app_toolbar->setOrientation(Qt::Horizontal);
  _main_window->addToolBar(_main_window->_app_toolbar);
  _main_window->_app_toolbar->setVisible(
      _settings->value("map_view/app_toolbar", false)
          .toBool()); // hide by default.
  ...
  QAction* start_server_action = _main_window->_app_toolbar->addAction("Start
  Server"); start_server_action->setToolTip("Start World and Auth servers.");
  start_server_action->setIcon(Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::server));

  QAction* extract_server_map_action =
  _main_window->_app_toolbar->addAction("Extract Server Map");
  extract_server_map_action->setToolTip("Start server extractors for this
  map.");
...
*/
  ...
  build_data_btn->setToolTip(
      "Save content of project folder as MPQ patch in the client.");
  build_data_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::filearchive));
  connect(build_data_btn, &QPushButton::clicked, ([=, this]() {
            _main_window->patchWowClient(); // code to open dialog
          }));

  auto start_wow_btn = new QPushButton(this);
  start_wow_btn->setIcon(
      Noggit::Ui::FontAwesomeIcon(Noggit::Ui::FontAwesome::play));
  ...
  connect(start_wow_btn, &QPushButton::clicked,
          ([=, this]() { _main_window->startWowClient(); }));

  // TODO : restart button while WoW is running?

  // IDEAs : various client utils like synchronize client view with noggit,
  // reload, patch WoW.exe with community patches like unlock md5 check, set WoW
  // client version
}
...
void MapView::activeTool(editing_mode newTool) {
  ...
  throw std::runtime_error{
      std::format(
          "Tried to call MapView::activeTool with invalid editing_mode `{}`!",
          static_cast<int>(newTool))
          .c_str()};
}

Noggit::Ui::Tools::ViewToolbar::Ui::ViewToolbar *
MapView::getLeftSecondaryViewToolbar() {
  return _left_sec_toolbar;
}

QSettings *MapView::settings() { return _settings; }

Noggit::Ui::Windows::NoggitWindow *MapView::mainWindow() {
  return _main_window;
}

bool MapView::isUiHidden() const { return ui_hidden; }

bool MapView::drawAdtGrid() const { return _draw_lines.get(); }

bool MapView::drawHoleGrid() const { return _draw_hole_lines.get(); }

void MapView::invalidate() { _needs_redraw = true; }

void MapView::selectObjects(std::array<glm::vec2, 2> selection_box,
                            float depth) {
  _world->select_objects_in_area(selection_box, !_mod_shift_down, _model_view,
                                 _projection, width(), height(), depth,
                                 _camera.position);
}
...
float MapView::timeSpeed() const { return mTimespeed; }
...
void MapView::setupAssistMenu() {
  ...
  ADD_ACTION_NS(cleanup_menu, "Clear duplicate models", [this] {
    DESTRUCTIVE_ACTION(makeCurrent();
                       OpenGL::context::scoped_setter const _(::gl, context());
  ...
  // vertices can support up to 32bit but other things break at 16bit like WDL
  // and MFBO
  ...
  heightmap_export_params_layout->addWidget(
      new QLabel("Import with the same values \nto keep the same coordinates.",
                 heightmap_export_params));

  heightmap_export_params_layout->addWidget(
      new QLabel("Min Height:", heightmap_export_params));
  ...
  heightmap_export_params_layout->addWidget(
      new QLabel("Max Height:", heightmap_export_params));
  ...
  std::string const autoheights_tooltip_str =
      "Sets fields to this tile's min and max heights\nDefaults : Min: " +
      std::to_string(DEFAULT_MIN_HEIGHT) +
      ", Max: " + std::to_string(DEFAULT_MAX_HEIGHT);
  QPushButton *heightmap_export_params_auto_height =
      new QPushButton("Auto Heights", heightmap_export_params);
  heightmap_export_params_auto_height->setToolTip(
      autoheights_tooltip_str.c_str());
  heightmap_export_params_layout->addWidget(
      heightmap_export_params_auto_height);
  ...
  // alphamaps import
  auto const alphamap_image_format =
  ...
  QCheckBox *adt_import_params_cleanup_layers =
      new QCheckBox("Cleanup unused chunk layers", adt_import_params);
  adt_import_params_cleanup_layers->setToolTip(
      "Remove textures that have empty layers from chunks.");
  ...
  auto const alphamap_file_info_tooltip =
      ...
      " folder with the following naming : MAPNAME_XX_YY_layer1.png (or "
      "layer2...)."
      "\nFor example "
      "\"C:/noggitproject/world/maps/MAPNAME/MAPNAME_29_53_layer2.png\"";
  ...
  ADD_ACTION_NS(
      cur_adt_import_menu, "Import alphamap (file)", ([=, this] {
        ...
        if (adt_import_params->exec() == QDialog::Accepted) {
          ...
          _world->importADTAlphamap(
              _camera.position, img, adt_import_params_layer->value(),
              adt_import_params_cleanup_layers->isChecked());
          NOGGIT_ACTION_MGR->endAction();
        }
      }));

  ADD_ACTION_NS(cur_adt_import_menu, "Import alphamaps", ([=, this] {
                  makeCurrent();
                  OpenGL::context::scoped_setter const _(::gl, context());
  ...
  auto const heightmap_image_format =
      "Required Image format :\n257x257 or 256x256(tiled edges)\nand 16bit per "
      "color channel.";

  auto const heightmap_file_info_tooltip =
      "Requires a .png image of 257x257, or 256x256 in Tiled Edges "
      "mode.(Otherwise it will be stretched)"
      "\nThe image file must be placed in the map's directory in the project "
      "folder with the following naming : MAPNAME_XX_YY_height.png."
      "\nFor example "
      "\"C:/noggitproject/world/maps/MAPNAME/MAPNAME_29_53_height.png\"";

  auto const tiled_edges_tooltip_str =
      "Tiled edge uses a 256x256 image instead 257."
      "\nTiled image imports encroach on edge vertices on neighboring tiles to "
      "avoid duplicate edges. ";

  /*auto const multiplier_tooltip_str = "Multiplies pixel values by this to
     obtain the final position."
      "\n For example a pixel grayscale of 40%(0.4%) with a multiplier of 100
     means this vertex's height will be 0.4*100 = 40.";
*/
  ...
  adt_import_height_params_layout->addWidget(
      new QLabel(heightmap_image_format, adt_import_height_params));

  adt_import_height_params_layout->addWidget(
      new QLabel("Min Height:", adt_import_height_params));
  QDoubleSpinBox *heightmap_import_min =
  ...
  adt_import_height_params_layout->addWidget(
      new QLabel("Max Height:", adt_import_height_params));
  ...
  QPushButton *adt_import_height_params_auto_height =
      new QPushButton("Auto Heights", adt_import_height_params);
  adt_import_height_params_auto_height->setToolTip(
      autoheights_tooltip_str.c_str());
  adt_import_height_params_layout->addWidget(
      adt_import_height_params_auto_height);
  ...
  ADD_ACTION_NS(cur_adt_import_menu, "Import heightmap (file)", ([=, this] {
                  if (adt_import_height_params->exec() == QDialog::Accepted) {
                    ...
                    _world->importADTHeightmap(
                        _camera.position, img, heightmap_import_min->value(),
                        heightmap_import_max->value(),
  ...
  ADD_ACTION_NS(cur_adt_import_menu, "Import heightmap", ([=, this] {
                  if (adt_import_height_params->exec() == QDialog::Accepted) {
                    ...
                    NOGGIT_ACTION_MGR->beginAction(
                        this, Noggit::ActionFlags::eCHUNKS_TERRAIN);
                    _world->importADTHeightmap(
                        _camera.position, heightmap_import_min->value(),
                        heightmap_import_max->value(),
                        adt_import_height_params_mode->currentIndex(),
                        adt_import_height_tiled_edges->isChecked());
                    NOGGIT_ACTION_MGR->endAction();
  ...
  QVBoxLayout *adt_import_water_params_layout =
      new QVBoxLayout(adt_import_water_params);

  // MIN MAX
  adt_import_water_params_layout->addWidget(
      new QLabel("Min Height:", adt_import_water_params));
  QDoubleSpinBox *watermap_import_min =
      new QDoubleSpinBox(adt_import_water_params);
  ...
  adt_import_water_params_layout->addWidget(
      new QLabel("Max Height:", adt_import_water_params));
  QDoubleSpinBox *watermap_import_max =
      new QDoubleSpinBox(adt_import_water_params);
  ...
  adt_import_water_params_layout->addWidget(
      new QLabel("Mode:", adt_import_water_params));
  QComboBox *adt_import_water_params_mode =
      new QComboBox(adt_import_water_params);
  adt_import_water_params_layout->addWidget(adt_import_water_params_mode);
  adt_import_water_params_mode->addItems(
      {"Set", "Add", "Subtract", "Multiply"});

  QCheckBox *adt_import_water_tiled_edges =
      new QCheckBox("Tiled Edges", adt_import_water_params);
  adt_import_water_params_layout->addWidget(adt_import_water_tiled_edges);

  QPushButton *adt_import_water_params_okay =
      new QPushButton("Okay", adt_import_water_params);
  ...
  connect(adt_import_water_params_okay, &QPushButton::clicked,
          ([=, this]() { adt_import_water_params->accept(); }));

  ADD_ACTION_NS(cur_adt_import_menu, "Import watermap (file)", ([=, this] {
                  if (adt_import_water_params->exec() == QDialog::Accepted) {
                    makeCurrent();
                    OpenGL::context::scoped_setter const _(::gl, context());

                    QString filepath = QFileDialog::getOpenFileName(
                        this, tr("Open watermap (257x257)"), "",
                        "PNG file (*.png);;");
                    ...
                    NOGGIT_ACTION_MGR->beginAction(
                        this, Noggit::ActionFlags::eCHUNKS_WATER);
                    _world->importADTWatermap(
                        _camera.position, img, watermap_import_min->value(),
                        watermap_import_max->value(),
                        adt_import_water_params_mode->currentIndex(),
                        adt_import_water_tiled_edges->isChecked());
                    NOGGIT_ACTION_MGR->endAction();
                  }
                }));
  ...
  assist_menu->addSeparator();
  ADD_ACTION_NS(assist_menu, "Fix terrain gaps between chunks", [this] {
    makeCurrent();
    OpenGL::context::scoped_setter const _(::gl, context());
    NOGGIT_ACTION_MGR->beginAction(this, Noggit::ActionFlags::eCHUNKS_TERRAIN);
    _world->fixAllGaps();
  ...
  ADD_ACTION_NS(assist_menu, "Cleanup empty texture chunks", [this] {
    makeCurrent();
    OpenGL::context::scoped_setter const _(::gl, context());
    NOGGIT_ACTION_MGR->beginAction(this, Noggit::ActionFlags::eCHUNKS_TEXTURE);
  ...
  assist_menu->addSeparator();
  ADD_ACTION_NS(assist_menu, "Convert Map to 8bits alphamaps", [this] {
    DESTRUCTIVE_ACTION(
        makeCurrent(); OpenGL::context::scoped_setter const _(::gl, context());
        if (_world->mapIndex.hasBigAlpha()) {
          QMessageBox::information(this, "Noggit", "Map is already Big Alpha.",
                                   QMessageBox::Ok);
        } else {
          QProgressDialog progress_dialog(
              "Converting Alpha format...", "", 0,
              _world->mapIndex.getNumExistingTiles(), this);
  ...
  ADD_ACTION_NS(
      assist_menu, "Convert Map to 4bits alphamaps (old format)", [this] {
        DESTRUCTIVE_ACTION(
            makeCurrent();
            OpenGL::context::scoped_setter const _(::gl, context());
            if (!_world->mapIndex.hasBigAlpha()) {
              QMessageBox::information(
                  this, "Noggit", "Map is already Old Alpha.", QMessageBox::Ok);
            } else {
              QProgressDialog progress_dialog(
                  "Converting Alpha format...", "", 0,
                  _world->mapIndex.getNumExistingTiles(), this);
  ...
  ADD_ACTION_NS(all_adts_import_menu, "Import alphamaps", [this] {
    DESTRUCTIVE_ACTION(
        ...
        QProgressDialog progress_dialog("Importing Alphamaps...", "Cancel", 0,
                                        num_tiles, this);
        progress_dialog.setWindowModality(Qt::WindowModal);
        // if (num_tiles > 30)
        NOGGIT_ACTION_MGR->beginAction(this,
        ...
        setUpdatesEnabled(false); QSignalBlocker blocker(this); {
          Log << "Benchmark : Importing alphamaps for " << num_tiles
              << " tiles." << std::endl;
          QElapsedTimer timer;
          timer.start();
          ...
          Log << "Alphamaps import finished in " << (elapsedMs * 1000)
              << "seconds." << std::endl;
        }
  ...
  ADD_ACTION_NS(
      all_adts_import_menu, "Import heightmaps", ([=, this] {
        if (adt_import_height_params->exec() == QDialog::Accepted) {
          DESTRUCTIVE_ACTION(
              makeCurrent();
              OpenGL::context::scoped_setter const _(::gl, context());
              unsigned int num_tiles = _world->mapIndex.getNumExistingTiles();
              QProgressDialog progress_dialog("Importing Heightmaps...",
                                              "Cancel", 0, num_tiles, this);
              progress_dialog.setWindowModality(Qt::WindowModal);

              if (num_tiles > 30) NOGGIT_ACTION_MGR->beginAction(
                  this, Noggit::ActionFlags::eCHUNKS_TERRAIN);

              // block paint event triggered by progress bar
              setUpdatesEnabled(false); QSignalBlocker blocker(this);
              _world->importAllADTsHeightmaps(
                  &progress_dialog, heightmap_import_min->value(),
                  heightmap_import_max->value(),
                  adt_import_height_params_mode->currentIndex(),
                  adt_import_height_tiled_edges->isChecked());
              setUpdatesEnabled(true);

              if (num_tiles > 30) NOGGIT_ACTION_MGR->endAction();)
        }
      }));
      ...
      all_adts_import_menu, "Import vertex color maps", ([=, this] {
        if (adt_import_vcol_params->exec() == QDialog::Accepted) {
          DESTRUCTIVE_ACTION(
              ...
              NOGGIT_ACTION_MGR->beginAction(
                  this, Noggit::ActionFlags::eCHUNKS_VERTEX_COLOR);
              QSignalBlocker blocker(this); _world->importAllADTVertexColorMaps(
                  adt_import_vcol_params_mode->currentIndex(),
                  adt_import_vcol_params_mode_tiled_edges->isChecked());
              NOGGIT_ACTION_MGR->endAction();)
        }
...
void MapView::setupViewMenu() {
  ...
  auto hide_widgets = ([=, this] {
    QWidget *widget_list[] = {
        _detail_infos_dock, _keybindings,    _minimap_dock, _asset_browser_dock,
        _overlay_widget,    _tool_panel_dock
  ...
  addHotkey(
      ...
      [this] {
        return activeTool()->hotkeyCondition("toggleTexturePalette"_hash);
      });
  ...
  connect(&_debug_cam_mode, &Noggit::BoolToggleProperty::changed
      , [this]
      {
          _debug_cam = Noggit::Camera(_camera.position, _camera.yaw(),
  _camera.pitch());
      }
  );
  ...
  ADD_ACTION_NS(view_menu
      ...
      , [this]
      {
          _camera = Noggit::Camera(_debug_cam.position, _debug_cam.yaw(),
  _debug_cam.pitch());
      }
  );*/

  ADD_TOGGLE_NS(view_menu, "FPS camera", _fps_mode);
  connect(&_fps_mode, &Noggit::BoolToggleProperty::changed, [this] {
    setCameraDirty();
...
void MapView::setupHelpMenu() {
  ...
  ADD_ACTION_NS(help_menu, "Noggit Red Repository", [] {
    ShellExecute(nullptr, "open", "https://gitlab.com/prophecy-rp/noggit-red/",
...
void MapView::setupClientMenu() {
  ...
  connect(this, &QObject::destroyed, client_menu,
          &QObject::deleteLater); // to remove from main menu

  // ADD_ACTION_NS(client_menu, "Start Client",  [this] {
  // _main_window->startWowClient(); });
  auto start_client_action(client_menu->addAction("Start Client"));
  connect(start_client_action, &QAction::triggered,
          [this] { _main_window->startWowClient(); });

  // ADD_ACTION_NS(client_menu, "Patch Client", [this] {
  // _main_window->patchWowClient(); });
  auto pack_client_action(client_menu->addAction("Patch Client"));
  pack_client_action->setToolTip(
      "Save content of project folder as MPQ patch in the client.");
  connect(pack_client_action, &QAction::triggered,
          [this] { _main_window->patchWowClient(); });
}
...
void MapView::setupHotkeys() {
  ...
  addHotkey(
      Qt::Key_H, MOD_space, [&] { _draw_hidden_models.toggle(); },
      [&] {
        return terrainMode == editing_mode::object && !NOGGIT_CUR_ACTION;
      });
  ...
  addHotkey(
      ...
      [&] {
        NOGGIT_ACTION_MGR->beginAction(
            this, Noggit::ActionFlags::eOBJECTS_TRANSFORMED);
  ...
  addHotkey(
      Qt::Key_1, MOD_none, [this] { set_editing_mode(editing_mode::ground); },
  ...
  addHotkey(
      ...
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_3, MOD_none, [this] { set_editing_mode(editing_mode::paint); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_4, MOD_none, [this] { set_editing_mode(editing_mode::holes); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_5, MOD_none, [this] { set_editing_mode(editing_mode::areaid); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_6, MOD_none, [this] { set_editing_mode(editing_mode::impass); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_7, MOD_none, [this] { set_editing_mode(editing_mode::water); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_8, MOD_none, [this] { set_editing_mode(editing_mode::mccv); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
  addHotkey(
      Qt::Key_9, MOD_none, [this] { set_editing_mode(editing_mode::object); },
      [this] { return !_mod_num_down && !NOGGIT_CUR_ACTION; });
...
void MapView::setupMinimap() {
  ...
  _minimap_dock->setAllowedAreas(Qt::LeftDockWidgetArea |
  ...
  connect(_minimap, &Noggit::Ui::minimap_widget::map_clicked,
          [this](glm::vec3 const &pos) { move_camera_with_auto_height(pos); });
  ...
  connect(_minimap_dock, &QDockWidget::visibilityChanged, &_show_minimap_window,
          &Noggit::BoolToggleProperty::set);
...
void MapView::createGUI() {
  ...
  _tools.emplace_back(std::make_unique<Noggit::RaiseLowerTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::FlattenBlurTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::TexturingTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::HoleTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::AreaTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::ImpassTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::WaterTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::VertexPainterTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::ObjectTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::MinimapTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::StampTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::LightTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::ScriptingTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::ChunkTool>(this))
      ->setupUi(_tool_panel_dock);
  _tools.emplace_back(std::make_unique<Noggit::AreaTriggerTool>(this))
      ->setupUi(_tool_panel_dock);
  ...
  connect(_main_window, &Noggit::Ui::Windows::NoggitWindow::exitPromptOpened,
          this, &MapView::on_exit_prompt);
...
MapView::MapView(math::degrees camera_yaw0, math::degrees camera_pitch0,
                 ...
                 bool from_bookmark)
    : _camera(camera_pos, camera_yaw0, camera_pitch0), mTimespeed(0.0f),
      _uid_fix(uid_fix), _from_bookmark(from_bookmark),
      _settings(new QSettings(this)), cursor_color(1.f, 1.f, 1.f, 1.f),
      _cursorType{CursorType::CIRCLE}, _main_window(NoggitWindow),
      _debug_cam(camera_pos, camera_yaw0, camera_pitch0),
      _world(std::move(world)), _status_position(new QLabel(this)),
      _status_selection(new QLabel(this)), _status_area(new QLabel(this)),
      _status_time(new QLabel(this)), _status_fps(new QLabel(this)),
      _status_culling(new QLabel(this)), _status_database(new QLabel(this)),
      _texBrush{new OpenGL::texture{}},
      _transform_gizmo(
          Noggit::Ui::Tools::ViewportGizmo::GizmoContext::MAP_VIEW),
      _tablet_manager(Noggit::TabletManager::instance()), _project(Project) {
  ...
  _world->LoadSavedSelectionGroups(); // not doing this in world constructor
                                      // because noggit loads world twice
  ...
  connect(this, &QObject::destroyed, _main_window, ([=, this] {
  ...
  connect(this, &MapView::selectionUpdated,
          [this](std::vector<selection_type> &) {
            // updateDetailInfos();
          });
  ...
  int _frametime =
      static_cast<int>((1.f / static_cast<float>(_fps_limit)) * 1000.f);
  std::cout << "FPS limit is set to : " << _fps_limit << " (" << _frametime
            << ")" << std::endl;
  ...
  connect(&_update_every_event_loop, &QTimer::timeout, [=, this] {
    ...
    if (_main_window->isMinimized() &&
        _settings->value("background_fps_limit", true).toBool()) {
      _needs_redraw = false;
      // return;
    }
...
Noggit::Camera *MapView::getCamera() { return &_camera; }
...
void MapView::initializeGL() {
  ...
  if (uid_warning) {
    QMessageBox::warning(
        ...
        "It is recommended to fix those models (listed in the log file) and "
        "run the uid fix all again.",
        QMessageBox::Ok);
  }
...
void MapView::paintGL() {
  ...
  if (_gizmo_on.get() && _world->has_selection()) {
    ...
    _transform_gizmo.setUseMultiselectionPivot(
        activeTool()->useMultiselectionPivot());

    auto pivot = _world->multi_select_pivot().has_value()
                     ? _world->multi_select_pivot().value()
                     : glm::vec3(0.f, 0.f, 0.f);

    _transform_gizmo.setMultiselectionPivot(pivot);

    _transform_gizmo.handleTransformGizmo(this, _world->current_selection(),
...
MapView::~MapView() {
  ...
  bool const has_current_context =
  ...
  if (has_current_context) {
    ...
    if (!_uid_fix_failed) {
      ...
      // context related crash delete objectEditor; since the ground effect tool
      // preview renderer got added, this causes crashing on exit to menu. Now
      // it crashes in application exit. delete texturingTool;
    ...
    LogError << "Map view cleanup could not release OpenGL resources because "
                "its context is no longer current."
             << std::endl;
...
void MapView::tick(float dt) {
  ...
  // note : selection update most commonly happens in mouseReleaseEvent, which
  // sets leftMouse to false
  ...
  if (_display_mode == display_mode::in_3D) {
    ...
    if (_camera_moved_since_last_draw) {
      if (_fps_mode.get()) {
        // there is a also hack to update camera when entering mode in void
        // ViewToolbar::add_tool_icon()
    ...
    //! \todo this is total bullshit. there should be a seperate view and camera
    //! class for tilemode
    if (moving) {
  ...
  // _minimap->update(); // causes massive performance issues, should only be
  // done when moving
  ...
  if (currentSelection.size() >
      0) // currently disabled, change to == to enable status bar selection
  {
      ...
      if (obj->which() == eMODEL) {
        ...
        _status_selection->setText(
            QString("%1: %2")
                ...
                .arg(QString::fromStdString(
                    instance->model->file_key().stringRepr())));
      } else if (obj->which() == eWMO) {
        ...
        _status_selection->setText(
            QString("%1: %2")
                .arg(instance->uid)
                .arg(QString::fromStdString(
                    instance->wmo->file_key().stringRepr())));
      }
    ...
    case eEntry_MapChunk: {
      auto chunk(
          std::get<selected_chunk_type>(*currentSelection.begin()).chunk);
      _status_selection->setText(
    ...
    _status_selection->setText(QString::number(currentSelection.size()) +
                               " objects selected");
  }
  ...
  _status_culling->setText(
      "Loaded tiles: " + QString::number(_world->getNumLoadedTiles()) +
      ", Rendered tiles: " + QString::number(_world->getNumRenderedTiles()) +
      "\t Loaded objects: " +
      QString::number(_world->getModelInstanceStorage().getTotalModelsCount()) +
      ", Rendered objects: " +
      QString::number(_world->getNumRenderedObjects()));
}
...
math::ray MapView::intersect_ray() const {
  ...
  if (_display_mode == display_mode::in_3D) {
    ...
    glm::mat4x4 const invertedViewMatrix =
        glm::inverse(_projection * _model_view);
    auto normalisedView = invertedViewMatrix * normalized_device_coords(mx, mz);

    auto pos = glm::vec3(normalisedView.x / normalisedView.w,
                         normalisedView.y / normalisedView.w,
                         normalisedView.z / normalisedView.w);

    return {_camera.position, pos - _camera.position};
  } else {
    glm::vec3 const pos(_camera.position.x - (width() * 0.5f - mx) * _2d_zoom,
                        _camera.position.y,
...
selection_result MapView::intersect_result(bool terrain_only) {
  selection_result results(_world->intersect(
      glm::transpose(_model_view), intersect_ray(), terrain_only,
      terrainMode == editing_mode::object ||
          terrainMode == editing_mode::minimap,
      _draw_terrain.get(), _draw_wmo.get(), _draw_models.get(),
      _draw_hidden_models.get(), _draw_wmo_exterior.get(),
      _draw_model_animations.get(), false, false, 0.0f,
      true // !_draw_wmo_exterior.get() // invert so that we only cast interiors
...
void MapView::doSelection(bool selectTerrainOnly, bool mouseMove) {
  if (_world->get_selected_model_count() && _gizmo_on.get() &&
      (_transform_gizmo.isUsing() || _transform_gizmo.isOver()))
    return;
  ...
  if (results.empty()) {
    ...
    if (!action || (!action->getBlockCursor()) || !_locked_cursor_mode.get()) {
      _cursor_pos = hit.index() == eEntry_Object
                        ? std::get<selected_object_type>(hit)->pos
                    : hit.index() == eEntry_MapChunk
                        ? std::get<selected_chunk_type>(hit).position
...
void MapView::update_cursor_pos() {
  ...
  if (false &&
      terrainMode != editing_mode::holes) // figure out why this does not work
                                          // on every hardware.
  {
    float mx = _last_mouse_pos.x(), mz = _last_mouse_pos.y();

    // gl.readBuffer(GL_FRONT);
    gl.bindBuffer(GL_PIXEL_PACK_BUFFER,
                  _buffers[static_cast<unsigned>(buffer_switch)]);

    gl.readPixels(mx, height() - mz - 1, 1, 1, GL_DEPTH_COMPONENT,
                  GL_UNSIGNED_SHORT, 0);

    gl.bindBuffer(GL_PIXEL_PACK_BUFFER,
                  _buffers[static_cast<unsigned>(!buffer_switch)]);
    GLushort *ptr = static_cast<GLushort *>(
        gl.mapBuffer(GL_PIXEL_PACK_BUFFER, GL_READ_ONLY));
    ...
    if (ptr) {
      ...
      glm::vec3 objcoord =
          glm::unProject(wincoord, _model_view, _projection, viewport);
...
glm::mat4x4 MapView::projection() const {
  ...
  // don't access qsettings in mainloop, it's slow
  ...
  if (_display_mode == display_mode::in_2D) {
    ...
    return glm::ortho(-half_width, half_width, -half_height, half_height, -1.f,
                      far_z);
  } else {
    return glm::perspective(_camera.fov()._, aspect_ratio(),
                            _fps_mode.get() ? 0.1f : 1.f, far_z);
  }
}
...
void MapView::draw_map() {
  ...
  bool show_unpaintable = _classic_ui
  ...
  renderParams.draw_only_inside_light_sphere =
      _left_sec_toolbar->drawOnlyInsideSphereLight();
  renderParams.draw_wireframe_light_sphere =
  ...
  renderParams.display = _display_mode;
  ...
  _world->renderer()->draw(_model_view, _projection, _cursor_pos, cursorColor,
                           ref_pos, _camera.position, &minimapRenderSettings,
                           renderParams);
...
void MapView::keyPressEvent(QKeyEvent *event) {
  ...
  if (event->key() == Qt::Key_Home) {
    _camera.position =
        glm::vec3(_cursor_pos.x, _cursor_pos.y + 50, _cursor_pos.z);
    _camera_moved_since_last_draw = true;
  }
  ...
  if (_display_mode == display_mode::in_2D) {
    ...
    if (event->key() == Qt::Key_Up) {
      ...
      _camera.position =
          glm::vec3((cur_tile.x * TILESIZE) + (TILESIZE / 2),
                    _camera.position.y, (next_z * TILESIZE) + (TILESIZE / 2));
      _camera_moved_since_last_draw = true;
      ...
      _camera.position =
          glm::vec3((next_x * TILESIZE) + (TILESIZE / 2), _camera.position.y,
                    (cur_tile.z * TILESIZE) + (TILESIZE / 2));
      _camera_moved_since_last_draw = true;
...
void MapView::checkInputsSettings() {
  ...
  // default is QWERTY
  _inputs = std::array<Qt::Key, 6>{Qt::Key_W, Qt::Key_S, Qt::Key_D,
                                   Qt::Key_A, Qt::Key_Q, Qt::Key_E};

  if (_locale == "AZERTY") {
    _inputs = std::array<Qt::Key, 6>{Qt::Key_Z, Qt::Key_S, Qt::Key_D,
                                     Qt::Key_Q, Qt::Key_A, Qt::Key_E};
  }
}
...
void MapView::mouseMoveEvent(QMouseEvent *event) {
  ...
  Noggit::MouseMoveParameters params{.displayMode = _display_mode,
                                     .left_mouse = leftMouse,
                                     .right_mouse = rightMouse,
                                     .mod_shift_down = _mod_shift_down,
                                     .mod_ctrl_down = _mod_ctrl_down,
      ...
      if (!_mod_ctrl_down) {
        ...
        if (_drag_start_pos != drag_end_pos && !ImGuizmo::IsUsing()) {
          const std::array<glm::vec2, 2> selection_box{
              glm::vec2(std::min(_drag_start_pos.x(), drag_end_pos.x()),
                        std::min(_drag_start_pos.y(), drag_end_pos.y())),
              glm::vec2(std::max(_drag_start_pos.x(), drag_end_pos.x()),
                        std::max(_drag_start_pos.y(), drag_end_pos.y()))};
          // _world->select_objects_in_area(selection_box, !_mod_shift_down,
          // model_view(), projection(), width(), height(),
          // objectEditor->drag_selection_depth(), _camera.position);
          _world->select_objects_in_area(selection_box, !_mod_shift_down,
                                         _model_view, _projection, width(),
                                         height(), 50000.0f, _camera.position);
...
void MapView::save(save_mode mode) {
  ...
  if (AsyncLoader::instance->important_object_failed_loading()) {
    ...
    if (first_warning.clickedButton() == yes) {
      ...
      second_warning.setText(
          "If you save you will have to save again all the adt containing the "
          "defective/missing models once you've fixed said models to correct "
          "all the issues.\n"
          "By clicking yes you accept to bear all the consequences of your "
          "action and forfeit the right to complain to the developers about "
          "any culling and collision issues.\n\n"
          "So... do you REALLY want to save ?");
      no = second_warning.addButton("No", QMessageBox::ButtonRole::YesRole);
      yes =
          second_warning.addButton("Yes", QMessageBox::ButtonRole::AcceptRole);
  ...
  if (save) {
    ...
    case save_mode::current:
      _world->mapIndex.saveTile(TileIndex(_camera.position), _world.get());
      break;
    ...
    // write wdl, we update wdl data prior in the mapIndex saving fucntions
    // above
...
void MapView::addHotkey(Qt::Key key, size_t modifiers, StringHash hotkeyName) {
  hotkeys.emplace_front(
      key, modifiers, [=, this] { activeTool()->onHotkeyPress(hotkeyName); },
      [=, this] { return activeTool()->hotkeyCondition(hotkeyName); },
      [=, this] { activeTool()->onHotkeyRelease(hotkeyName); });
}
...
void MapView::unloadOpenglData() {
  ...
  if (!context() || !context()->isValid() ||
      QOpenGLContext::currentContext() != context()) {
    LogError << "Map view cleanup was skipped because its OpenGL context could "
                "not be made current."
             << std::endl;
    _gl_initialized = false;
    return;
...
[[nodiscard]]
Noggit::Ui::Tools::AssetBrowser::Ui::AssetBrowserWidget *
MapView::getAssetBrowserWidget() {
  return _asset_browser;
}

glm::vec3 MapView::cursorPosition() const { return _cursor_pos; }

void MapView::cursorPosition(glm::vec3 position) { _cursor_pos = position; }

void MapView::enableGizmoBar() { _viewport_overlay_ui->gizmoBar->show(); }

void MapView::disableGizmoBar() { _viewport_overlay_ui->gizmoBar->hide(); }
...
void MapView::onSettingsSave() {
  ...
  OpenGL::TerrainParamsUniformBlock *params =
      _world->renderer()->getTerrainParamsUniformBlock();
  params->wireframe_type = _settings->value("wireframe/type", false).toBool();
  params->wireframe_radius =
  ...
  _world->renderer()->directional_lightning =
      _settings->value("directional_lightning", true).toBool();
  _world->renderer()->local_lightning =
      _settings->value("local_lightning", true).toBool();

  // refresh rendering
  _world->renderer()->markTerrainParamsUniformBlockDirty();
  _world->renderer()->skies()->force_update();

  _world->renderer()->_view_distance =
      _settings->value("view_distance", 2000.f).toFloat() + TILE_RADIUS;
  _world.get()->mapIndex.setLoadingRadius(
      _settings->value("loading_radius", 2).toInt());
  _world.get()->mapIndex.setUnloadDistance(
      _settings->value("unload_dist", 5).toInt());
  _world.get()->mapIndex.setUnloadInterval(
      _settings->value("unload_interval", 30).toInt());
  ...
  format().setSwapInterval(
      vsync ? 1
            : Noggit::Application::NoggitApplication::instance()
                  ->getConfiguration()
                  ->GraphicsConfiguration.SwapChainInternal);

  bool doAntiAliasing = _settings->value("anti_aliasing", false).toBool();
  format().setSamples(doAntiAliasing
                          ? 4
                          : Noggit::Application::NoggitApplication::instance()
                                ->getConfiguration()
                                ->GraphicsConfiguration.SamplesCount);

  _render_m2_aabb = _settings->value("render/m2_aabb", false).toBool();
  _render_m2_collission_bbox =
      _settings->value("render/m2_coll_bb", false).toBool();
  _render_wmo_aabb = _settings->value("render/wmo_aabb", false).toBool();
  _render_wmo_groups_bounds =
      _settings->value("render/wmo_groups_bounds", false).toBool();

  // force updating rendering
  _camera_moved_since_last_draw = true;

  auto app_config =
      Noggit::Application::NoggitApplication::instance()->getConfiguration();
  app_config->modern_features =
      _settings->value("modern_features", false).toBool();
}

void MapView::setCameraDirty() { _camera_moved_since_last_draw = true; }
...
void MapView::ShowContextMenu(QPoint pos) {
  ...
  bool mouse_moved = (QApplication::startDragDistance() / 5) <
                     (_right_click_pos - pos).manhattanLength();
...
void MapView::onApplicationStateChanged(Qt::ApplicationState state) {
  ...
  case Qt::ApplicationState::ApplicationHidden: {
    ...
    // this isn't minimized, it's when the window is entirely hidden, should
    // never happen on noggit
  ...
  case Qt::ApplicationState::ApplicationInactive: {
    ...
    _update_every_event_loop.setInterval(fps_calcul *
                                         2); // half fps if inactive
    break;
  }
