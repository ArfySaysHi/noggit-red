...
void DBCFile::open(std::shared_ptr<BlizzardArchive::ClientData> clientData) {
  ...
  if (f.isEof()) {
    LogError << "The DBC file \"" << filename
             << "\" could not be opened. This application may crash soon as "
                "the file is most likely needed."
             << std::endl;
    return;
  }
...
void DBCFile::save() {
  ...
  std::string filename_proj =
      BlizzardArchive::ClientData::normalizeFilenameInternal(str.toStdString() +
                                                             filename);
  QDir dir(QString::fromStdString(
      BlizzardArchive::ClientData::normalizeFilenameInternal(
          str.toStdString() + "DBFilesClient/")));
  if (!dir.exists())
    dir.mkpath(".");

  std::ofstream stream(filename_proj, std::ios_base::out |
                                          std::ios_base::trunc |
                                          std::ios_base::binary);
...
DBCFile::Record DBCFile::addRecord(size_t id, size_t id_field) {
  ...
  *reinterpret_cast<unsigned int *>(data.data() + old_size +
                                    id_field * sizeof(std::uint32_t)) =
      static_cast<unsigned int>(id);

  return Record(*this, data.data() + old_size);
}

DBCFile::Record DBCFile::addRecordCopy(size_t id, size_t id_from,
                                       size_t id_field) {
  recordCount++;
  ...
  std::copy(data.data() + from_idx * recordSize,
            data.data() + from_idx * recordSize + recordSize,
            data.data() + old_size);
  *reinterpret_cast<unsigned int *>(data.data() + old_size +
                                    id_field * sizeof(std::uint32_t)) =
      static_cast<unsigned int>(id);
...
void DBCFile::removeRecord(size_t id, size_t id_field) {
  ...
  for (Iterator i = begin(); i != end(); ++i) {
    if (i->getUInt(id_field) == id) {
      ...
      std::memmove(record, record + recordSize,
                   recordSize * (recordCount - counter + 1));
