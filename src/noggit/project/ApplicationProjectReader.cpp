...
std::optional<NoggitProject> ApplicationProjectReader::readProject(
    ...
    if (entry.path().extension() == ".noggitproj") {
      ...
      if (root.contains("Project") && root["Project"].isObject()) {
        ...
        if (project_configuration.contains("ProjectName"))
          project.ProjectName =
              project_configuration["ProjectName"].toString().toStdString();

        if (project_configuration.contains("Bookmarks") &&
            project_configuration["Bookmarks"].isArray()) {
          ...
          for (auto const &json_bookmark : project_bookmarks) {
            ...
            bookmark.name = json_bookmark.toObject()
                                .value("BookmarkName")
                                .toString()
                                .toStdString();
            bookmark.camera_pitch =
                json_bookmark.toObject().value("CameraPitch").toDouble();
            bookmark.camera_yaw =
                json_bookmark.toObject().value("CameraYaw").toDouble();

            auto bookmark_position = json_bookmark.toObject().value("Position");
            auto bookmark_position_x =
                bookmark_position.toObject().value("X").toDouble();
            auto bookmark_position_y =
                bookmark_position.toObject().value("Y").toDouble();
            auto bookmark_position_z =
                bookmark_position.toObject().value("Z").toDouble();
            bookmark.position = glm::vec3(
                bookmark_position_x, bookmark_position_y, bookmark_position_z);

            project.Bookmarks.push_back(bookmark);
          }
        }

        if (project_configuration.contains("PinnedMaps") &&
            project_configuration["PinnedMaps"].isArray()) {
          auto project_pinned_maps =
              project_configuration["PinnedMaps"].toArray();

          for (auto const &json_pinned_map : project_pinned_maps) {
            auto pinned_map = NoggitProjectPinnedMap();
            pinned_map.MapId =
                json_pinned_map.toObject().value("MapId").toInt();
            pinned_map.MapName = json_pinned_map.toObject()
                                     .value("MapName")
                                     .toString()
                                     .toStdString();
        ...
        if (project_configuration.contains("Client") &&
            project_configuration["Client"].isObject()) {
          auto project_client_configuration =
              project_configuration["Client"].toObject();

          if (project_client_configuration.contains("ClientPath")) {
            project.ClientPath = project_client_configuration["ClientPath"]
                                     .toString()
                                     .toStdString();
          }

          if (project_client_configuration.contains("ClientVersion")) {
            auto client_version = project_client_configuration["ClientVersion"]
                                      .toString()
                                      .toStdString();
  ...
  if (json_doc.isObject()) {
    ...
    if (root.contains("TexturePalettes") && root["TexturePalettes"].isArray()) {
      ...
      for (auto const &json_texture_palette : project_texture_palettes) {
        ...
        texture_palette.MapId =
            json_texture_palette.toObject().value("MapId").toInt();
        auto json_filepaths =
            json_texture_palette.toObject().value("Filepaths").toArray();
    ...
    if (root.contains("ObjectPalettes") && root["ObjectPalettes"].isArray()) {
      ...
      for (auto const &json_object_palette : project_object_palettes) {
        ...
        object_palette.MapId =
            json_object_palette.toObject().value("MapId").toInt();
        auto json_filepaths =
            json_object_palette.toObject().value("Filepaths").toArray();
...
void ApplicationProjectReader::readObjectSelectionGroups(
    NoggitProject *project) {
  QString str = QString(project->ProjectPath.c_str());
  if (!(str.endsWith('\\') || str.endsWith('/'))) {
    str += "/";
  }
  ...
  if (json_doc.isObject()) {
    ...
    if (root.contains("ObjectSelectionGroups") &&
        root["ObjectSelectionGroups"].isArray()) {
      auto project_selection_groups = root["ObjectSelectionGroups"].toArray();

      for (auto const &json_object_map_selec_groups :
           project_selection_groups) {
        auto object_selection_groups = NoggitProjectSelectionGroups();
        object_selection_groups.MapId =
            json_object_map_selec_groups.toObject().value("MapId").toInt();
        auto json_selection_groups = json_object_map_selec_groups.toObject()
                                         .value("ObjectGroups")
                                         .toArray();
        ...
        for (auto const &json_selection_group : json_selection_groups) {
          ...
          object_selection_groups.SelectionGroups.push_back(
              proj_object_uid_array);
        }
