...
public:
  ...
  void
  saveTexturePalette(const NoggitProjectTexturePalette &new_texture_palette) {
  ...
  void saveObjectSelectionGroups(
      const NoggitProjectSelectionGroups &new_selection_groups) {
    ObjectSelectionGroups.erase(
        std::remove_if(
            ObjectSelectionGroups.begin(), ObjectSelectionGroups.end(),
    ...
    _projectWriter->saveObjectSelectionGroups(
        this, std::filesystem::path(ProjectPath));
  }
};
...
public:
  ...
  void createProject(std::filesystem::path const &project_path,
                     std::filesystem::path const &client_path,
  ...
  loadProject(std::filesystem::path const &project_path) {
    ...
    try {
      ...
      QMessageBox::critical(nullptr, "Error",
                            "The client does not appear to be valid.");
      return {};
    }
