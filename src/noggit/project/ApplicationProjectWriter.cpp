...
void ApplicationProjectWriter::saveProject(
    NoggitProject *project, std::filesystem::path const &project_path) {
  auto project_configuration_file_path =
      (project_path / (project->ProjectName + std::string(".noggitproj")));
  auto project_configuration_file = QFile(
      QString::fromStdString(project_configuration_file_path.generic_string()));
  ...
  client_configuration.insert(
      "ClientVersion",
      ClientVersionFactory::MapToStringVersion(project->projectVersion)
          .c_str());
...
void ApplicationProjectWriter::savePalettes(
    NoggitProject *project, std::filesystem::path const &project_path) {
  ...
  if (!project_configuration_file.open(QIODevice::WriteOnly |
                                       QFile::Truncate)) {
...
void ApplicationProjectWriter::saveObjectSelectionGroups(
    NoggitProject *project, std::filesystem::path const &project_path) {
  QString str = QString(project->ProjectPath.c_str());
  if (!(str.endsWith('\\') || str.endsWith('/'))) {
    str += "/";
  ...
  QFile project_configuration_file =
      QFile(str + "/noggit_object_selection_groups.json");
  if (!project_configuration_file.open(QIODevice::WriteOnly |
                                       QFile::Truncate)) {
    LogError << "Unable to save palettes to JSON document." << std::endl;
    return;
  ...
  for (auto const &objectSelectionGroupsMap : project->ObjectSelectionGroups) {
    auto json_object_map_selec_groups =
        QJsonObject(); // contain map id + arrays
    ...
    for (auto &selection_group : objectSelectionGroupsMap.SelectionGroups) {
      auto json_group_objects_uids =
          QJsonArray(); // array of objects(Uids) in each group
    ...
    json_object_map_selec_groups.insert("MapId",
                                        objectSelectionGroupsMap.MapId);

    object_selection_groups.push_back(json_object_map_selec_groups);
  }

  // project_configuration.insert("ObjectSelectionGroups",
  // object_selection_groups);
