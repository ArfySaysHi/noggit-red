...
void ApplicationProject::createProject(
  ...
  project.projectVersion =
      ClientVersionFactory::mapToEnumVersion(client_version);
...
std::shared_ptr<NoggitProject>
ApplicationProject::loadProject(std::filesystem::path const &project_path) {
  ...
  std::string dbd_file_directory =
      _configuration->ApplicationDatabaseDefinitionsPath;
  ...
  project->ClientDatabase =
      std::make_shared<BlizzardDatabaseLib::BlizzardDatabase>(
          dbd_file_directory, client_build);

  Log << "Loading Client Path : " << project->ClientPath << std::endl;

  try {
    project->ClientData = std::make_shared<BlizzardArchive::ClientData>(
        project->ClientPath, client_archive_version, client_archive_locale,
...
void ApplicationProject::loadExtraData(NoggitProject &project) {
  ...
  if (std::filesystem::exists(extraDataFolder) &&
      std::filesystem::is_directory(extraDataFolder)) {
    for (const auto &entry :
         std::filesystem::directory_iterator(extraDataFolder)) {
      if (entry.path().extension() == ".cfg") {
        ...
        if (entry.path().stem() == "global") {
          for (auto const &entry : keys) {
            ...
            newData.heightOffset =
                root[entry].toObject()["HeightOffset"].toDouble();
            newData.heightScale =
                root[entry].toObject()["HeightScale"].toDouble();
            project.ExtraMapData.SetTextureHeightData_Global(
...
void NoggitExtraMapData::SetTextureHeightData_Global(
    const std::string &texture, texture_heightmapping_data data,
    World *worldToUpdate) {
...
void NoggitExtraMapData::SetTextureHeightDataForADT(
    int mapID, const TileIndex &ti, const std::string &texture,
    texture_heightmapping_data data, World *worldToUpdate) {
...
const texture_heightmapping_data NoggitExtraMapData::GetTextureHeightDataForADT(
    int mapID, const TileIndex &tileIndex, const std::string &texture) const {
...
ProjectVersion
ClientVersionFactory::mapToEnumVersion(std::string const &projectVersion) {
...
std::string
ClientVersionFactory::MapToStringVersion(ProjectVersion const &projectVersion) {
...
void NoggitProject::pinMap(int map_id, const std::string &map_name) {
  ...
  auto pinnedMapFound =
      std::find_if(std::begin(PinnedMaps), std::end(PinnedMaps),
...
void NoggitProject::saveTexturePalette(
    const NoggitProjectTexturePalette &new_texture_palette) {
  TexturePalettes.erase(
      std::remove_if(TexturePalettes.begin(), TexturePalettes.end(),
                     [=](NoggitProjectTexturePalette texture_palette) {
                       return texture_palette.MapId ==
                              new_texture_palette.MapId;
                     }),
      TexturePalettes.end());
...
void NoggitProject::saveObjectPalette(
    const NoggitProjectObjectPalette &new_object_palette) {
  ObjectPalettes.erase(
      std::remove_if(ObjectPalettes.begin(), ObjectPalettes.end(),
...
void NoggitProject::saveObjectSelectionGroups(
    const NoggitProjectSelectionGroups &new_selection_groups) {
  ObjectSelectionGroups.erase(
      std::remove_if(ObjectSelectionGroups.begin(), ObjectSelectionGroups.end(),
                     [=](NoggitProjectSelectionGroups proj_selection_group) {
                       return proj_selection_group.MapId ==
                              new_selection_groups.MapId;
                     }),
      ObjectSelectionGroups.end());

  ObjectSelectionGroups.push_back(new_selection_groups);

  _projectWriter->saveObjectSelectionGroups(this,
