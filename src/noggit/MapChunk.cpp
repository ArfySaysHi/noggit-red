...
MapChunk::MapChunk(MapTile *maintile, BlizzardArchive::ClientFile *f,
    ...
    : _mode(mode), mt(maintile), use_big_alphamap(bigAlpha), _context(context),
      ...
      _chunk_update_flags(ChunkUpdateFlags::VERTEX |
                          ChunkUpdateFlags::ALPHAMAP |
  ...
  if (header.ofsMCCV) {
    ...
    for (int i = 0; i < mapbufsize; ++i) {
      f->read(t, 4);
      mccv[i] = glm::vec3((float)t[2] / 127.0f, (float)t[1] / 127.0f,
                          (float)t[0] / 127.0f);
    }
  } else {
...
bool MapChunk::GetVertex(float x, float z, glm::vec3 *V) {
  ...
  const int column =
      static_cast<int>((xdiff - UNITSIZE * 0.5f * (row % 2)) / UNITSIZE + 0.5f);
...
void MapChunk::draw(math::frustum const &frustum,
                    ...
                    const float &cull_distance, const glm::vec3 &camera,
                    bool need_visibility_update, bool show_unpaintable_chunks,
...

    {
      ...
      if (texture_set->num())
      {
        ...
        for (int i = 0; i < texture_set->num(); ++i)
        {
          ...
          //mcnk_shader.uniform("tex_temp_" + std::to_string(i),
    (*texture_set->getTextures())[i]->array_index());
...
bool MapChunk::intersect(math::ray const &ray, selection_result *results) {
  ...
  static constexpr std::array<std::uint16_t, 768> indices = {
      9,   0,   17,  9,   17,  18,  9,   18,  1,   9,   1,   0,   26,  17,  34,
      26,  34,  35,  26,  35,  18,  26,  18,  17,  43,  34,  51,  43,  51,  52,
      43,  52,  35,  43,  35,  34,  60,  51,  68,  60,  68,  69,  60,  69,  52,
      60,  52,  51,  77,  68,  85,  77,  85,  86,  77,  86,  69,  77,  69,  68,
      94,  85,  102, 94,  102, 103, 94,  103, 86,  94,  86,  85,  111, 102, 119,
      111, 119, 120, 111, 120, 103, 111, 103, 102, 128, 119, 136, 128, 136, 137,
      128, 137, 120, 128, 120, 119, 10,  1,   18,  10,  18,  19,  10,  19,  2,
      10,  2,   1,   27,  18,  35,  27,  35,  36,  27,  36,  19,  27,  19,  18,
      44,  35,  52,  44,  52,  53,  44,  53,  36,  44,  36,  35,  61,  52,  69,
      61,  69,  70,  61,  70,  53,  61,  53,  52,  78,  69,  86,  78,  86,  87,
      78,  87,  70,  78,  70,  69,  95,  86,  103, 95,  103, 104, 95,  104, 87,
      95,  87,  86,  112, 103, 120, 112, 120, 121, 112, 121, 104, 112, 104, 103,
      129, 120, 137, 129, 137, 138, 129, 138, 121, 129, 121, 120, 11,  2,   19,
      11,  19,  20,  11,  20,  3,   11,  3,   2,   28,  19,  36,  28,  36,  37,
      28,  37,  20,  28,  20,  19,  45,  36,  53,  45,  53,  54,  45,  54,  37,
      45,  37,  36,  62,  53,  70,  62,  70,  71,  62,  71,  54,  62,  54,  53,
      79,  70,  87,  79,  87,  88,  79,  88,  71,  79,  71,  70,  96,  87,  104,
      96,  104, 105, 96,  105, 88,  96,  88,  87,  113, 104, 121, 113, 121, 122,
      113, 122, 105, 113, 105, 104, 130, 121, 138, 130, 138, 139, 130, 139, 122,
      130, 122, 121, 12,  3,   20,  12,  20,  21,  12,  21,  4,   12,  4,   3,
      29,  20,  37,  29,  37,  38,  29,  38,  21,  29,  21,  20,  46,  37,  54,
      46,  54,  55,  46,  55,  38,  46,  38,  37,  63,  54,  71,  63,  71,  72,
      63,  72,  55,  63,  55,  54,  80,  71,  88,  80,  88,  89,  80,  89,  72,
      80,  72,  71,  97,  88,  105, 97,  105, 106, 97,  106, 89,  97,  89,  88,
      114, 105, 122, 114, 122, 123, 114, 123, 106, 114, 106, 105, 131, 122, 139,
      131, 139, 140, 131, 140, 123, 131, 123, 122, 13,  4,   21,  13,  21,  22,
      13,  22,  5,   13,  5,   4,   30,  21,  38,  30,  38,  39,  30,  39,  22,
      30,  22,  21,  47,  38,  55,  47,  55,  56,  47,  56,  39,  47,  39,  38,
      64,  55,  72,  64,  72,  73,  64,  73,  56,  64,  56,  55,  81,  72,  89,
      81,  89,  90,  81,  90,  73,  81,  73,  72,  98,  89,  106, 98,  106, 107,
      98,  107, 90,  98,  90,  89,  115, 106, 123, 115, 123, 124, 115, 124, 107,
      115, 107, 106, 132, 123, 140, 132, 140, 141, 132, 141, 124, 132, 124, 123,
      14,  5,   22,  14,  22,  23,  14,  23,  6,   14,  6,   5,   31,  22,  39,
      31,  39,  40,  31,  40,  23,  31,  23,  22,  48,  39,  56,  48,  56,  57,
      48,  57,  40,  48,  40,  39,  65,  56,  73,  65,  73,  74,  65,  74,  57,
      65,  57,  56,  82,  73,  90,  82,  90,  91,  82,  91,  74,  82,  74,  73,
      99,  90,  107, 99,  107, 108, 99,  108, 91,  99,  91,  90,  116, 107, 124,
      116, 124, 125, 116, 125, 108, 116, 108, 107, 133, 124, 141, 133, 141, 142,
      133, 142, 125, 133, 125, 124, 15,  6,   23,  15,  23,  24,  15,  24,  7,
      15,  7,   6,   32,  23,  40,  32,  40,  41,  32,  41,  24,  32,  24,  23,
      49,  40,  57,  49,  57,  58,  49,  58,  41,  49,  41,  40,  66,  57,  74,
      66,  74,  75,  66,  75,  58,  66,  58,  57,  83,  74,  91,  83,  91,  92,
      83,  92,  75,  83,  75,  74,  100, 91,  108, 100, 108, 109, 100, 109, 92,
      100, 92,  91,  117, 108, 125, 117, 125, 126, 117, 126, 109, 117, 109, 108,
      134, 125, 142, 134, 142, 143, 134, 143, 126, 134, 126, 125, 16,  7,   24,
      16,  24,  25,  16,  25,  8,   16,  8,   7,   33,  24,  41,  33,  41,  42,
      33,  42,  25,  33,  25,  24,  50,  41,  58,  50,  58,  59,  50,  59,  42,
      50,  42,  41,  67,  58,  75,  67,  75,  76,  67,  76,  59,  67,  59,  58,
      84,  75,  92,  84,  92,  93,  84,  93,  76,  84,  76,  75,  101, 92,  109,
      101, 109, 110, 101, 110, 93,  101, 93,  92,  118, 109, 126, 118, 126, 127,
      118, 127, 110, 118, 110, 109, 135, 126, 143, 135, 143, 144, 135, 144, 127,
      135, 127, 126};
  ...
  for (int i(0); i < indices.size(); i += 3) {
    if (auto distance = ray.intersect_triangle(mVertices[indices[i + 0]],
      ...
      results->emplace_back(
          *distance,
          selected_chunk_type(
              this, std::make_tuple(indices[i], indices[i + 1], indices[i + 2]),
...
void MapChunk::updateVerticesData() {
  // This method assumes tile's heightmap texture is currently bound to the
  // active tex unit
...
glm::vec3 MapChunk::getNeighborVertex(int i, unsigned dir) {
  ...
  // returns a virtual vertex with the height that equals to the height of
  // MapChunks's vertex i.

  constexpr float half_unit = UNITSIZE / 2.f;
  static constexpr std::array xdiff{-half_unit, half_unit, half_unit,
                                    -half_unit};
  static constexpr std::array zdiff{-half_unit, -half_unit, half_unit,
                                    half_unit};
  ...
  if (tile.x == mt->index.x && tile.z == mt->index.z) {
    ...
    mt->_world->mapIndex.getTile(tile)->getVertexInternal(vertex_x, vertex_z,
                                                          &result);
  } else {
  ...
  if ((i >= 1 && i <= 7 && dir < 2 && !py)
      ...
      || (i == 136 && ((dir == 3 || dir == 2) && py == 15) || (!dir || dir == 3)
  && !px)
      || (i == 144 && ((dir == 3 || dir == 2) && py == 15) || ((dir == 1 || dir
  == 2) && px == 15))
      || (!(i % 17) && (!dir || dir == 3) && !px)
      || (!(i % 25) && (dir == 1 || dir == 2) && px == 15))
  {
    ...
    {
      return glm::vec3( mVertices[i].x + xdiff[dir], mVertices[i].y,
  mVertices[i].z + zdiff[dir]);
    }

    glm::vec3 vertex{};
    mt->_world->mapIndex.getTile(tile)->getVertexInternal(mVertices[i].x +
  xdiff[dir], mVertices[i].z + zdiff[dir], &vertex);

    return glm::vec3( mVertices[i].x + xdiff[dir], vertex.y, mVertices[i].z +
  zdiff[dir]);
...
void MapChunk::recalcNorms() {
  ...
  for (int i = 0; i < mapbufsize; ++i) {
    ...
    //! \todo: find out why recalculating normals without changing the terrain
    //! result in slightly different normals
...
void MapChunk::updateNormalsData() {
  // gl.texSubImage2D(GL_TEXTURE_2D, 0, 0, px * 16 + py, mapbufsize, 1, GL_RGB,
  // GL_FLOAT, mNormals);
}

bool MapChunk::changeTerrain(glm::vec3 const &pos, float change, float radius,
  ...
  for (int i = 0; i < mapbufsize; ++i) {
    float dt = change;
    if (changeTerrainProcessVertex(pos, mVertices[i], dt, radius, inner_radius,
                                   BrushType)) {
      changed = true;
      mVertices[i].y += dt;
    }
...
bool MapChunk::ChangeMCCV(glm::vec3 const &pos, glm::vec4 const &color,
...
bool MapChunk::stampMCCV(glm::vec3 const &pos, glm::vec4 const &color,
  ...
  for (int i = 0; i < mapbufsize; ++i) {
    ...
    if (std::abs(pos.x - mVertices[i].x) > radius ||
        std::abs(pos.z - mVertices[i].z) > radius)
      continue;

    glm::vec3 const diff{mVertices[i] - pos};

    int pixel_x =
        std::round(((diff.x + radius) / (2.f * radius)) * img->width());
    int pixel_y =
        std::round(((diff.z + radius) / (2.f * radius)) * img->height());
    ...
    if (use_image_colors) {
      ...
      if (pixel_x >= 0 && pixel_x < img->width() && pixel_y >= 0 &&
        ...
        image_factor =
            (mask_color.redF() + mask_color.greenF() + mask_color.blueF()) /
            3.0f;
      } else {
        image_factor = 0.f;
      }

      float edit = image_factor * (paint ? ((change * (1.0f - dist / radius)))
                                         : (change * 20.f));
      if (editMode) {
        mccv[i].x += (color.x / 0.5f - mccv[i].x) * edit;
        mccv[i].y += (color.y / 0.5f - mccv[i].y) * edit;
...
bool MapChunk::flattenTerrain(glm::vec3 const &pos, float remain, float radius,
                              int BrushType, flatten_mode const &mode,
                              glm::vec3 const &origin, math::degrees angle,
  ...
  for (int i(0); i < mapbufsize; ++i) {
    ...
    mVertices[i].y = glm::mix(mVertices[i].y, ah,
...
bool MapChunk::blurTerrain(
    glm::vec3 const &pos, float remain, float radius, int BrushType,
...
bool MapChunk::changeTerrainProcessVertex(glm::vec3 const &pos,
                                          glm::vec3 const &vertex, float &dt,
  ...
  if (brushType == eTerrainType_Quadra) {
    if ((std::abs(xdiff) < std::abs(radiusOuter / 2)) &&
        (std::abs(zdiff) < std::abs(radiusOuter / 2))) {
    ...
    if (dist < radiusOuter) {
      ...
      case eTerrainType_Polynom:
        dt = dt * ((dist / radiusOuter) * (dist / radiusOuter) +
                   dist / radiusOuter + 1.0f);
        break;
      case eTerrainType_Trigo:
        dt = dt * cos(dist / radiusOuter);
        break;
      case eTerrainType_Gaussian:
        dt = dist < radiusOuter * radiusInner
                 ? dt * std::exp(
                            -(std::pow(radiusOuter * radiusInner / radiusOuter,
                                       2) /
                              (2 * std::pow(0.39f, 2))))
                 : dt * std::exp(-(std::pow(dist / radiusOuter, 2) /
                                   (2 * std::pow(0.39f, 2))));
...
auto MapChunk::stamp(glm::vec3 const &pos, float dt, QImage const *img,
                     float radiusOuter, float radiusInner, int brushType,
                     bool sculpt) -> void {
  if (sculpt) {
    for (int i{}; i < mapbufsize; ++i) {
      if (std::abs(pos.x - mVertices[i].x) > radiusOuter ||
          std::abs(pos.z - mVertices[i].z) > radiusOuter)
        continue;

      float delta = dt;
      changeTerrainProcessVertex(pos, mVertices[i], delta, radiusOuter,
                                 radiusInner, brushType);

      glm::vec3 const diff{mVertices[i] - pos};

      int pixel_x = std::round(((diff.x + radiusOuter) / (2.f * radiusOuter)) *
                               img->width());
      int pixel_y = std::round(((diff.z + radiusOuter) / (2.f * radiusOuter)) *
                               img->height());
    ...
    for (int i{}; i < mapbufsize; ++i) {
      ...
      glm::vec3 const diff{glm::vec3{original_heightmap[i * 3],
                                     original_heightmap[i * 3 + 1],
                                     original_heightmap[i * 3 + 2]} -
                           pos};
...
void MapChunk::set_texture_effect(scoped_blp_texture_reference const &tex,
                                  int effectID) {
  if (!texture_set)
    return;
  int idx = texture_set->texture_id(tex);
  if (idx >= 0)
    texture_set->setEffect(idx, effectID);
}
...
bool MapChunk::switchTexture(scoped_blp_texture_reference const &oldTexture,
...
bool MapChunk::paintTexture(glm::vec3 const &pos, Brush *brush, float strength,
                            float pressure,
                            scoped_blp_texture_reference texture) {
  return texture_set->paintTexture(xbase, zbase, pos.x, pos.z, brush, strength,
                                   pressure, std::move(texture));
}

bool MapChunk::stampTexture(glm::vec3 const &pos, Brush *brush, float strength,
                            float pressure,
                            scoped_blp_texture_reference texture, QImage *img,
                            bool paint) {
  return texture_set->stampTexture(xbase, zbase, pos.x, pos.z, brush, strength,
                                   pressure, std::move(texture), img, paint);
}

bool MapChunk::replaceTexture(glm::vec3 const &pos, float radius,
...
void MapChunk::update_shadows() {
  if (_chunk_update_flags & ChunkUpdateFlags::SHADOW)
    gl.texSubImage3D(GL_TEXTURE_2D_ARRAY, 0, 0, 0, px * 16 + py, 64, 64, 1,
                     GL_RED, GL_UNSIGNED_BYTE, _shadow_map);
}
    ...
    for (int x = 0; x < 4; ++x) {
      for (int y = 0; y < 4; ++y) {
        if (misc::getShortestDist(pos.x, pos.z, xbase + (MINICHUNKSIZE * x),
                                  zbase + (MINICHUNKSIZE * y),
                                  MINICHUNKSIZE) <= radius) {
          int v = 1 << (y * 4 + x);
...
void MapChunk::save(sExtendableArray &lADTFile, int &lCurrentPosition,
  ...
  lADTFile.Extend(8 + 0x80); // This is only the size of the header. More chunks
                             // will increase the size.
  SetChunkHeader(lADTFile, lCurrentPosition, 'MCNK', lMCNK_Size);
  lADTFile.GetPointer<MCIN>(lMCIN_Position + 8)->mEntries[py * 16 + px].offset =
      lCurrentPosition; // check this
  ...
  if (texture_set) {
    std::copy(texture_set->getDoodadMappingBase(),
              texture_set->getDoodadMappingBase() + 8,
  ...
  SetChunkHeader(lADTFile, lCurrentPosition, 'MCLY',
                 static_cast<int>(lMCLY_Size));

  lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->ofsLayer =
      lCurrentPosition - lMCNK_Position;
  lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nLayers =
      texture_set ? static_cast<std::uint32_t>(texture_set->num()) : 0;
  ...
  if (texture_set) {
    ...
    for (size_t j = 0; j < texture_set->num(); ++j) {
      ENTRY_MCLY *lLayer = lADTFile.GetPointer<ENTRY_MCLY>(
          lCurrentPosition + 8 + 0x10 * static_cast<unsigned long>(j));
  ...
  int lMCRF_Size =
      static_cast<int>(4 * (lDoodadIDs.size() + lObjectIDs.size()));
  ...
  lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nDoodadRefs =
      static_cast<std::uint32_t>(lDoodadIDs.size());
  lADTFile.GetPointer<MapChunkHeader>(lMCNK_Position + 8)->nMapObjRefs =
      static_cast<std::uint32_t>(lObjectIDs.size());

  // MCRF data
  int *lReferences = lADTFile.GetPointer<int>(lCurrentPosition + 8);

  lID = 0;
  for (std::list<int>::iterator it = lDoodadIDs.begin(); it != lDoodadIDs.end();
       ++it) {
    lReferences[lID] = *it;
    lID++;
  }

  for (std::list<int>::iterator it = lObjectIDs.begin(); it != lObjectIDs.end();
       ++it) {
    lReferences[lID] = *it;
    lID++;
  }
...
void MapChunk::selectVertex(glm::vec3 const &pos, float radius,
...
QImage MapChunk::getHeightmapImage(float min_height, float max_height) {
  ...
  for (unsigned y = 0; y < SUM; ++y)
    for (unsigned x = 0; x < SUM; ++x) {
      ...
      float value =
          is_virtual
              ? (heightmap[idx].y + heightmap[idx + (erp ? SUM : 1)].y) / 2.f
              : heightmap[idx].y;
      value = std::min(1.0f, std::max(0.0f, ((value - min_height) /
...
void MapChunk::setHeightmapImage(QImage const &image, float multiplier,
                                 int mode) {
  glm::vec3 *heightmap = getHeightmap();
...
void MapChunk::setAlphamapImage(const QImage &image, unsigned int layer) {
  ...
  for (int i = 0; i < 64; ++i) {
    for (int j = 0; j < 64; ++j) {
      temp_alphamaps[layer][64 * j + i] =
          static_cast<float>(qGray(image.pixel(i, j))) / 255.0f;
    }
  }
...
QImage MapChunk::getVertexColorImage() {
  ...
  for (unsigned y = 0; y < SUM; ++y)
    for (unsigned x = 0; x < SUM; ++x) {
      ...
      unsigned const idx{(plain - (is_virtual ? (erp ? SUM : 1) : 0)) / 2};
      float r = is_virtual
                    ? (colors[idx].x + colors[idx + (erp ? SUM : 1)].x) / 4.f
                    : colors[idx].x / 2.f;
      float g = is_virtual
                    ? (colors[idx].y + colors[idx + (erp ? SUM : 1)].y) / 4.f
                    : colors[idx].y / 2.f;
      float b = is_virtual
                    ? (colors[idx].z + colors[idx + (erp ? SUM : 1)].z) / 4.f
                    : colors[idx].z / 2.f;
