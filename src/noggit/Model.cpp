...
Model::Model(const std::string &filename, Noggit::NoggitRenderContext context)
    : AsyncObject(filename), _context(context), _renderer(this) {
...
void Model::finishLoading() {
  ...
  if (f.isEof() || f.getSize() < sizeof(ModelHeader)) {
    // LogError << "Error loading file \"" << _file_key.stringRepr() << "\".
    // Aborting to load model." << std::endl; finished = true;
    throw std::runtime_error("Error loading file \"" + _file_key.stringRepr() +
                             "\". Aborting to load model.");
  }
  ...
  if (!valid_version) [[unlikely]] {
    LogError << "Error loading file \"" << _file_key.stringRepr()
             << "\". Wrong M2 version " << std::to_string(packed_version)
             << std::endl;
    throw std::runtime_error("Error loading file \"" + _file_key.stringRepr() +
                             "\". Wrong M2 version " +
                             std::to_string(packed_version));
  }

  // blend mode override
  if (header.Flags & m2_flag_use_texture_combiner_combos) {
    // go to the end of the header (where the blend override data is)
    uint32_t const *blend_override_info =
        reinterpret_cast<uint32_t const *>(f.getBuffer() + sizeof(ModelHeader));
  ...
  animated = isAnimated(
      f, header); // isAnimated will set animGeometry and animTextures
...
void Model::toggle_visibility() { _hidden = !_hidden; }

void Model::show() { _hidden = false; }

void Model::hide() { _hidden = true; }
...
[[nodiscard]]
bool Model::particles_only()
    const { // some particle emitters like wisps in ashenvale have a few
            // vertices but no collision, using that to detect
  return !_particles.empty() && (_renderer.renderPasses().empty() ||
                                 _vertices.empty() || !nBoundingTriangles);
}
...
bool Model::isAnimated(const BlizzardArchive::ClientFile &f,
  ...
  ModelVertex const *verts =
      reinterpret_cast<ModelVertex const *>(f.getBuffer() + header.ofsVertices);
    ...
    for (size_t b = 0; b < 4; b++) {
      if (verts[i].weights[b] > 0) {
        ...
        if ((bb.flags & 0x200) || billboard) {
          if (billboard) {
            // if we have billboarding, the model will need per-instance
            // animation
...
void Model::initCommon(const BlizzardArchive::ClientFile &f,
  ...
  for (auto &v : _vertices) {
    ...
    if ((animGeometry || animBones)) {
      // for animated models, calculate base vertex box of initial vertice
      // positions to know the mesh size
  ...
  // collision_indices = M2Array<glm::uint16_t>(f, header.ofsBoundingTriangles,
  // header.nBoundingTriangles); collision_vertices = M2Array<glm::vec3>(f,
  // header.ofsBoundingVertices, header.nBoundingVertices); collision_normals =
  // M2Array<glm::vec3>(f, header.ofsBoundingNormals, header.nBoundingNormals);
  ...
  for (size_t i = 0; i < header.nTextures; ++i) {
    if (texdef[i].type == 0) {
      ...
      _textureFilenames[i] =
          std::string(blp_ptr, texdef[i].nameLen - (invalid_size ? 0 : 1));
    } else {
  ...
  if (header.nViews > 0) {
    ...
    std::string lodname =
        _file_key.filepath().substr(0, _file_key.filepath().length() - 3);
    lodname.append("00.skin");
    BlizzardArchive::ClientFile g(
        lodname,
        Noggit::Application::NoggitApplication::instance()->clientData());
    if (g.isEof()) {
    ...
    auto triangles =
        reinterpret_cast<uint16_t const *>(g.getBuffer() + view->ofs_triangle);
    ...
    // render ops
    auto model_geosets = reinterpret_cast<ModelGeoset const *>(
        g.getBuffer() + view->ofs_submesh);
    auto texture_unit = reinterpret_cast<ModelTexUnit const *>(
        g.getBuffer() + view->ofs_texture_unit);
  ...
  if (header.nAnimations > 0) {
    ...
    for (auto &anim : animations) {
      ...
      if (Noggit::Application::NoggitApplication::instance()
              ->clientData()
              ->exists(tempname.str())) {
        animation_files.push_back(std::make_unique<BlizzardArchive::ClientFile>(
            tempname.str(),
            Noggit::Application::NoggitApplication::instance()->clientData()));
      }
    }
  ...
  if (header.nParticleEmitters) {
    ...
    for (size_t i = 0; i < header.nParticleEmitters; ++i) {
      try {
        ...
        LogError << "Loading particles for '" << _file_key.stringRepr() << "' "
                 << error.what() << std::endl;
      }
    }
  }
...
void Model::calcBones(glm::mat4x4 const &model_view, int _anim, int time,
                      int animation_time) {
...
void Model::animate(glm::mat4x4 const &model_view, int anim_id, int anim_time) {
  ...
  for (size_t i = 0; i < _lights.size(); ++i) {
    if (_lights[i].parent >= 0) {
      _lights[i].tpos =
          bones[_lights[i].parent].mat * glm::vec4(_lights[i].pos, 0);
      _lights[i].tdir =
          bones[_lights[i].parent].mrot * glm::vec4(_lights[i].dir, 0);
    }
  }
...
std::vector<ModelVertex> Model::getTransformVertices() const {
  ...
  // Don't need to store them anymore, we only need them for operations like
  // intersections _current_vertices = _vertices;
  ...
  for (auto &vertex : _current_vertices) {
    ...
    for (size_t b(0); b < 4; ++b) {
      ...
      ::glm::vec3 tv =
          bones[vertex.bones[b]].mat * glm::vec4(vertex.position, 1.0f);
      ::glm::vec3 tn =
          bones[vertex.bones[b]].mrot * glm::vec4(vertex.normal, 1.0f);
  ...
  (_vertices_buffer); gl.bufferData (GL_ARRAY_BUFFER, _current_vertices.size() *
  sizeof (ModelVertex), _current_vertices.data(), GL_STREAM_DRAW);
...
float Model::calcMeshBoundsRatio() const {
  ...
  float global_bounds_volume =
      math::aabb(bounding_box_min, bounding_box_max).volume();

  if (global_bounds_volume == 0.0f)
    return 1.0f;

  float vertex_box_volume =
      math::aabb(vertices_bounds[0], vertices_bounds[1]).volume();
...
ModelColor::ModelColor(const BlizzardArchive::ClientFile &f,
                       const ModelColorDef &mcd, int *global)
    : color(mcd.color, f, global), opacity(mcd.opacity, f, global) {}

ModelTransparency::ModelTransparency(const BlizzardArchive::ClientFile &f,
                                     const ModelTransDef &mcd, int *global)
    : trans(mcd.trans, f, global) {}

ModelLight::ModelLight(const BlizzardArchive::ClientFile &f,
                       const ModelLightDef &mld, int *global)
    : type(mld.type), parent(mld.bone), pos(fixCoordSystem(mld.pos)),
      tpos(fixCoordSystem(mld.pos)), dir(::glm::vec3(0, 1, 0)),
      tdir(::glm::vec3(0, 1, 0)) // obviously wrong
      ,
      diffColor(mld.color, f, global), ambColor(mld.ambColor, f, global),
...
void ModelLight::setup(int time, OpenGL::light, int animtime) {
  auto ambient = ambColor.getValue(0, time, animtime) *
                 ambIntensity.getValue(0, time, animtime);
  auto diffuse = diffColor.getValue(0, time, animtime) *
...
TextureAnim::TextureAnim(const BlizzardArchive::ClientFile &f,
                         const ModelTexAnimDef &mta, int *global)
    : trans(mta.trans, f, global), rot(mta.rot, f, global),
      scale(mta.scale, f, global), mat(glm::mat4x4()) {}

Bone::Bone(const BlizzardArchive::ClientFile &f, const ModelBoneDef &b,
           int *global,
    ...
    : trans(b.translation, f, global, animation_files),
      ...
      pivot(fixCoordSystem(b.pivot)), parent(b.parent) {
...
void Bone::calcMatrix(glm::mat4x4 const &model_view, Bone *allbones, int anim,
  ...
  if (flags.transformed || flags.billboard ||
    ...
    if (rot.uses(anim)) {
      ...
      glm::vec3 test_rot_vec =
          glm::vec3(rot_euler[2], -(rot_euler[1] + glm::radians(180.f)),
...
Model::intersect(glm::mat4x4 const &model_view, math::ray const &ray,
                 int animtime, bool calc_anims, bool first_occurence,
    ...
    for (size_t i = 0; i < fake_geom.indices.size(); i += 3) {
      if (auto distance = ray.intersect_triangle(
        ...
        results.emplace_back(*distance,
                             std::make_tuple(static_cast<int>(i),
                                             static_cast<int>(i + 1), 1 + 2));
        return results;
      }
    }
  ...
  for (auto const &pass : _renderer.renderPasses()) {
    ...
    for (int i(pass.index_start); i < pass.index_start + pass.index_count;
         i += 3) {
      std::optional<float> distance;
      if (!has_transformed_verts) {
        distance = ray.intersect_triangle(
            _vertices[_indices[static_cast<std::size_t>(i + 0)]].position,
            _vertices[_indices[static_cast<std::size_t>(i + 1)]].position,
            _vertices[_indices[static_cast<std::size_t>(i + 2)]].position);
      } else {
        distance = ray.intersect_triangle(
            transformed_verts[_indices[static_cast<std::size_t>(i + 0)]]
                .position,
            transformed_verts[_indices[static_cast<std::size_t>(i + 1)]]
                .position,
            transformed_verts[_indices[static_cast<std::size_t>(i + 2)]]
                .position);
      }
...
void Model::lightsOn(OpenGL::light lbase) {
  // setup lights
  for (unsigned int i = 0, l = lbase; i < _lights.size(); ++i)
