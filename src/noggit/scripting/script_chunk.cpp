...
chunk::chunk(script_context *ctx, MapChunk *chunk)
    : script_object(ctx), _chunk(chunk) {}
...
int chunk::add_texture(std::string const &texture, int effectID) {
  ...
  int tex_index = _chunk->texture_set->addTexture(
      scoped_blp_texture_reference(tex, Noggit::NoggitRenderContext::MAP_VIEW));
  if (effectID >= -1) {
...
int chunk::get_effect(int index) { return _chunk->texture_set->effect(index); }
...
void chunk::clear_textures() { _chunk->texture_set->eraseTextures(); }

void chunk::remove_texture(int index) {
  if (index < 0 || index > 3) {
    throw script_exception("chunk_remove_texture",
...
std::string chunk::get_texture(int index) {
  if (index < 0 || index > 3) {
    throw script_exception("chunk_get_texture",
                           "invalid texture index, must be between 0-3");
  }

  if (_chunk->texture_set->num() <= index) {
    throw script_exception("chunk_get_texture", "texture index out of range");
  }
...
void chunk::apply_heightmap() {
  _chunk->registerChunkUpdate(ChunkUpdateFlags::VERTEX |
                              ChunkUpdateFlags::NORMALS);
...
int chunk::get_area_id() { return _chunk->getAreaID(); }

void chunk::set_area_id(int value) { return _chunk->setAreaID(value); }

void chunk::set_impassable(bool add) { _chunk->setFlag(add, 0x2); }

void chunk::clear_colors() {
  std::fill(_chunk->mccv, _chunk->mccv + mapbufsize, glm::vec3(1.f, 1.f, 1.f));
}

tex chunk::get_tex(int index) { return tex(state(), _chunk, index); }

vert chunk::get_vert(int index) { return vert(state(), _chunk, index); }
...
void chunk::set_deep_flag(std::uint32_t low, std::uint32_t high) {
  getOrCreateRender().fatigue =
      std::uint64_t(low) | (std::uint64_t(high) << 32);
}

void chunk::set_deep_flag_1(std::uint32_t low) { set_deep_flag(low, 0); }
...
std::shared_ptr<selection> chunk::to_selection() {
  return std::make_shared<selection>(state(), "chunk#to_selection",
                                     _chunk->vmin, _chunk->vmax);
}
...
void register_chunk(script_context *state) {
  ...
  state->new_usertype<chunk>(
      "chunk", "remove_texture", &chunk::remove_texture, "get_texture_count",
      &chunk::get_texture_count, "get_texture", &chunk::get_texture,
      "get_effect", &chunk::get_effect, "set_effect", &chunk::set_effect,
      "add_texture", sol::overload(&chunk::add_texture, &chunk::add_texture_1),
      "clear_textures", &chunk::clear_textures, "set_hole", &chunk::set_hole,
      "clear_colors", &chunk::clear_colors, "apply_textures",
      &chunk::apply_textures, "apply_heightmap", &chunk::apply_heightmap,
      "apply_vertex_color", &chunk::apply_vertex_color, "apply_all",
      &chunk::apply_all, "apply", &chunk::apply_all, "set_impassable",
      &chunk::set_impassable, "get_area_id", &chunk::get_area_id, "set_area_id",
      &chunk::set_area_id, "to_selection", &chunk::to_selection, "get_tex",
      &chunk::get_tex, "get_vert", &chunk::get_vert, "set_deep_flag",
      sol::overload(&chunk::set_deep_flag, &chunk::set_deep_flag_1),
      "get_deep_flag", &chunk::get_deep_flag, "get_deep_flag_high",
      &chunk::get_deep_flag_high, "set_fishable_flag",
      sol::overload(&chunk::set_fishable_flag, &chunk::set_fishable_flag_1),
      "get_fishable_flag", &chunk::get_fishable_flag, "get_fishable_flag_high",
      &chunk::get_fishable_flag_high);
}
} // namespace Scripting
} // namespace Noggit
