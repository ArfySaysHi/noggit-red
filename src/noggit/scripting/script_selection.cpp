...
namespace Noggit {
namespace Scripting {
selection::selection(script_context *ctx, std::string const &,
                     glm::vec3 const &point1, glm::vec3 const &point2)
    : script_object(ctx), _world(ctx->world()) {
  _min = glm::vec3(std::min(point1.x, point2.x), std::min(point1.y, point2.y),
                   std::min(point1.z, point2.z));

  _max = glm::vec3(std::max(point1.x, point2.x), std::max(point1.y, point2.y),
...
glm::vec3 selection::center() { return _center; }
glm::vec3 selection::min() { return _min; }
glm::vec3 selection::max() { return _max; }
glm::vec3 selection::size() { return _size; }

std::shared_ptr<noisemap> selection::make_noise(float frequency,
  ...
  return Noggit::Scripting::make_noise(
      ...
      ,
      std::floor(_min.x) - 2, std::floor(_min.z) - 2, std::ceil(_size.x) + 4,
      std::ceil(_size.z) + 4, frequency, algorithm, seed);
}
...
std::vector<chunk> selection::chunks_raw() {
  ...
  for (auto &chnk : mapChunks)
    chunks.emplace_back(state(), chnk);
  return chunks;
}
...
std::vector<vert> selection::verts_raw() {
  ...
  for (auto chunk : chunks_raw()) {
    for (int i = 0; i < mapbufsize; ++i) {
      ...
      if (v.x >= _min.x && v.x <= _max.x && v.z >= _min.z && v.z <= _max.z) {
...
void register_selection(script_context *state) {
  state->new_usertype<selection>(
      "selection", "center", &selection::center, "min", &selection::min, "max",
      &selection::max, "size", &selection::size, "apply", &selection::apply,
      "verts", &selection::verts, "tex", &selection::textures, "models",
      &selection::models, "chunks", &selection::chunks, "make_noise",
      &selection::make_noise);

  state->set_function(
      "select_origin",
      [state](glm::vec3 const &origin, float xRadius, float zRadius = -1) {
        if (zRadius <= 0) {
          zRadius = xRadius;
        }
  ...
  state->set_function("select_between", [state](glm::vec3 const &point1,
                                                glm::vec3 const &point2) {
    return std::make_shared<selection>(state, "select_between", point1, point2);
  });
}
} // namespace Scripting
} // namespace Noggit
