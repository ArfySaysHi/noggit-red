...
public:
  ...
  void change_terrain(glm::vec3 const &, float change, float radius,
                      float inner_radius, int brush_type);

  void set_area_id(glm::vec3 const &, int id, bool adt);

  void change_vertex_color(glm::vec3 const &pos, glm::vec3 const &color,
                           float alpha, float change, float radius,
                           bool editMode);

  glm::vec3 get_vertex_color(glm::vec3 const &pos);

  void flatten_terrain(glm::vec3 const &pos, float remain, float radius,
                       int brush_type, bool lower, bool raise,
                       glm::vec3 const &origin, double angle,
                       double orientation);

  void blur_terrain(glm::vec3 const &pos, float remain, float radius,
                    int brush_type, bool lower, bool raise);
  ...
  void paint_texture(glm::vec3 const &pos, float strength, float pressure,
                     float hardness, float radius, std::string const &texture);
};
