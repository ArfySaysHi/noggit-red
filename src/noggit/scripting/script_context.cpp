...
script_context::script_context(scripting_tool *tool) : _tool(tool) {
  ...
  script_scoped_function<std::shared_ptr<script_brush>(std::string const &)>
      add_brush(this, "brush", [this](std::string const &name) {
        auto brush = std::make_shared<script_brush>(this, name);
        this->get_scripts().push_back(brush);
        return brush;
      });

  set_function("require",
               [this](std::string const &name) { return this->require(name); });
  ...
  for (std::filesystem::recursive_directory_iterator dir("scripts"); dir != end;
       ++dir) {
    std::string file = dir->path().string();

    if (!(dir->path().extension().string() == ".lua") ||
        (dir->path().extension().string() == ".spec.lua")) {
      continue;
    }
    ...
    try {
      ...
      if (what.size() == 0) {
        what = std::string("Generic error in file during initialization (check "
  ...
  if (file_count == 0) {
    _tool->addLog(
        "[error]: no script files found (check your 'scripts' directory)");
    set_tool_error(_tool);
    ++error_count;
  }
...
scripting_tool *script_context::tool() { return _tool; }
...
int script_context::get_selection() { return _selected; }
...
World *script_context::world() { return _tool->get_view()->_world.get(); }

std::string script_context::file_to_module(std::string const &file) {
  auto rel = std::filesystem::relative(std::filesystem::path(file),
...
std::string script_context::module_to_file(std::string const &mod) {
  return (std::filesystem::path("scripts") /
          std::filesystem::path(mod + ".lua"))
      .string();
}
...
void script_context::execute_file(std::string const &file) {
  ...
  _modules[mod] = res.get_type() == sol::type::table ? res.get<sol::table>()
                                                     : create_table();
