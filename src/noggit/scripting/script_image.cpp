...
void image::resize(int width, int height) {
  if (width <= 0 || height <= 0) {
    throw script_exception(
        "img_resize", std::string("tried to resize to invalid image size: x=") +
                          std::to_string(width) + std::string(" y=") +
...
image::image(script_context *ctx, int width, int height) : script_object(ctx) {
  resize(width, height);
}
...
int image::get_index(int x, int y) const {
  ...
  if (index < 0 || index >= static_cast<int>(_size)) {
    throw script_exception(
        "img_get_index",
        "image coordinates out of bounds: x=" + std::to_string(x) +
            " y=" + std::to_string(y) + " width=" + std::to_string(_width) +
...
unsigned image::get_pixel(int x, int y) const {
  ...
  return get_image()[index] << 24 | get_image()[index + 1] << 16 |
         get_image()[index + 2] << 8 | get_image()[index + 3];
}
...
void image::save(std::string const &filename) {
  ...
  unsigned error =
      lodepng::encode(writable_path.string(), get_image(), _width, _height);
  if (error) {
    throw script_exception("img_save", "failed to save image with error " +
                                           std::to_string(error));
  }
}
...
float image::gradient_scale(float rel) const {
  if (rel < 0 || rel >= 1) {
    throw script_exception(
        "img_gradient_scale",
        "relative image coordinate out of bounds: " + std::to_string(rel) +
            " (should be >= 0 and < 1)");
  }
...
void register_image(script_context *state) {
  state->new_usertype<image>(
      "image", "get_index", &image::get_index, "get_blue", &image::get_blue,
      "get_red", &image::get_red, "get_alpha", &image::get_alpha, "get_green",
      &image::get_green, "get_pixel", &image::get_pixel, "gradient_scale",
      &image::gradient_scale, "set_pixel", &image::set_pixel,
      "set_pixel_floats",
      sol::overload(&image::set_pixel_floats, &image::set_pixel_floats_1),
      "save", &image::save, "width", &image::width, "height", &image::height);
