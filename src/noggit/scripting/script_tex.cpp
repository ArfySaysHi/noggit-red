...
void tex::set_alpha(int index, float value) {
  if (index < 0 || index > 3) {
    throw script_exception("tex_set_alpha",
...
glm::vec3 tex::get_pos_2d() { return tex_location(_chunk, _index); }

void collect_textures(script_context *ctx, MapChunk *chnk,
                      std::vector<tex> &vec, glm::vec3 const &min,
                      glm::vec3 const &max) {
...
void register_tex(script_context *state) {
  state->new_usertype<tex>("tex", "set_alpha", &tex::set_alpha, "get_alpha",
                           &tex::get_alpha, "get_pos_2d", &tex::get_pos_2d);
}
} // namespace Scripting
} // namespace Noggit