...
template <typename T>
T script_settings::get_json_safe(
    std::string const &key, T def,
    std::function<bool(nlohmann::basic_json<>)> check) {
...
#define ADD_SLIDER(path, T, min, max, def, decimals, has_slider)               \
  ...
  connect(spinner, qOverload<double>(&QDoubleSpinBox::valueChanged),           \
          ([=, this](double v) {                                               \
            set_json<T>(path, (T)v);                                           \
  ...
  set_json<T>(                                                                 \
      path,                                                                    \
      std::min(max, std::max(min, get_json_safe<T>(path, def, [](auto json) {  \
                               return json.is_number();                        \
                             }))));                                            \
  auto v =                                                                     \
      get_json_safe<T>(path, def, [](auto json) { return json.is_number(); }); \
  slider->setSliderPosition((int)std::round(v *dp1));                          \
  spinner->setValue(v);
...
void script_settings::add_double(std::string const &name, double min,
                                 double max, double def, int zeros,
                                 bool has_slider) {
  ADD_SLIDER(name, double, min, max, def, zeros, has_slider);
}

void script_settings::add_int(std::string const &name, int min, int max,
                              int def, bool has_slider) {
  ADD_SLIDER(name, int, min, max, def, 0, has_slider);
}

void script_settings::add_string(std::string const &name,
  ...
  tline->setText(get_json_safe<std::string>(name, def, [](auto json) {
...
void script_settings::add_bool(std::string const &name, bool def) {
  ...
  checkbox->setCheckState(
      get_json_safe<bool>(name, def,
                          [](auto json) { return json.is_boolean(); })
...
void script_settings::add_string_list(std::string const &name,
                                      std::string const &value) {
  if (_string_arrays.find(name) == _string_arrays.end()) {
    auto box = new QComboBox(this);
    auto label = new QLabel(this);

    connect(box, QOverload<int>::of(&QComboBox::activated), this,
            ([=, this](auto index) {
              set_json<std::string>(name,
                                    box->itemText(index).toUtf8().constData());
            }));
    ...
    get_json_safe<std::string>(name, value,
                               [](auto json) { return json.is_string(); });
  } else {
    auto box = _string_arrays[name];
    box->addItem(value.c_str());

    // we found the last selection, so change the index for that.
    if (get_json_safe<std::string>(
            name, "", [](auto json) { return json.is_string(); }) == value) {
...
void script_settings::load_json() {
  ...
  try {
    ...
    // back up broken script settings, since they won't be read and will be
    // overwritten.
...
void script_settings::initialize() {
  double inner_radius = get_json_safe<double>(
      INNER_RADIUS_PATH, 0.5, [](auto json) { return json.is_number(); });
  double outer_radius = get_json_safe<double>(
      OUTER_RADIUS_PATH, 40, [](auto json) { return json.is_number(); });
...
nlohmann::json *script_settings::get_raw_json() { return &_json; }

tag::tag(script_context *ctx, std::string const &script,
         std::string const &item)
    : script_object(ctx), _tool(ctx->tool()), _script(script), _item(item) {}

int_tag::int_tag(script_context *ctx, std::string const &script,
                 std::string const &item, int min, int max, int def,
                 bool has_slider)
    : tag(ctx, script, item), _min(min), _max(max), _def(def),
...
null_tag::null_tag(script_context *ctx, std::string const &script,
                   std::string const &item, std::string const &text)
    : tag(ctx, script, item), _text(text) {}

void null_tag::add_to_settings() { _tool->get_settings()->add_null(_text); }

real_tag::real_tag(script_context *ctx, std::string const &script,
                   std::string const &item, double min, double max, double def,
                   int zeros, bool has_slider)
    : tag(ctx, script, item), _min(min), _max(max), _def(def), _zeros(zeros),
      _has_slider(has_slider) {}
...
void real_tag::add_to_settings() {
  _tool->get_settings()->add_double(_item, _min, _max, _def, _zeros,
                                    _has_slider);
}

string_tag::string_tag(script_context *ctx, std::string const &script,
                       std::string const &item, std::string const &def)
    : tag(ctx, script, item), _def(def) {}
...
string_list_tag::string_list_tag(script_context *ctx, std::string const &script,
                                 std::string const &item,
                                 std::vector<std::string> const &values)
    : tag(ctx, script, item), _values(values) {}
...
bool_tag::bool_tag(script_context *ctx, std::string const &script,
                   std::string const &item, bool def)
    : tag(ctx, script, item), _def(def) {}
...
void register_settings(script_context *state) {
  ...
  state->new_usertype<string_list_tag>("string_list_tag", "get",
                                       &string_list_tag::get);
}
} // namespace Scripting
} // namespace Noggit
