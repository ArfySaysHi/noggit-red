...
random::random(script_context *ctx)
    : script_object(ctx), _state(std::chrono::high_resolution_clock::now()
                                     .time_since_epoch()
                                     .count()) {}
...
long random::integer(long low, long high) {
  ...
  if (low >= high) {
    throw script_exception("rand_int64",
                           "random lower bound " + std::to_string(low) +
                               " >= higher bound " + std::to_string(high));
  }
...
double random::real(double low, double high) {
  ...
  if (low >= high) {
    throw script_exception("rand_double",
                           "random lower bound " + std::to_string(low) +
                               " >= higher bound " + std::to_string(high));
  }
  double val;
...
std::shared_ptr<random> random_from_seed(script_context *ctx,
...
void register_random(script_context *state) {
  state->new_usertype<random>("random", "integer", &random::integer, "real",
                              &random::real);
  state->set_function("random_from_seed", [state](std::string const &seed) {
    return random_from_seed(state, seed);
  });

  state->set_function("random_from_time",
                      [state]() { return random_from_time(state); });
}
} // namespace Scripting
} // namespace Noggit
