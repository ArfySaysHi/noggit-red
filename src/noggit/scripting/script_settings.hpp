...
public:
  ...
  void add_double(std::string const &name, double min, double max,
                  double def = 0, int zeros = 2, bool has_slider = false);
  ...
  template <typename T>
  T get_setting(std::string const &script, std::string const &profile,
                std::string const &item, T def) {
...
private:
  ...
  T get_json_safe(std::string const &key, T def,
                  std::function<bool(nlohmann::basic_json<>)>);
  template <typename T> T get_json_unsafe(std::string const &key);
  template <typename T> void set_json(std::string const &key, T def);
...
public:
  tag(script_context *ctx, std::string const &script, std::string const &item);
...
public:
  int_tag(script_context *ctx, std::string const &script,
          std::string const &item, int min, int max, int def, bool has_slider);
  int get();
  void set(int value);
...
public:
  real_tag(script_context *ctx, std::string const &script,
...
public:
  null_tag(script_context *ctx, std::string const &script,
...
public:
  string_tag(script_context *ctx, std::string const &script,
...
public:
  string_list_tag(script_context *ctx, std::string const &script,
...
public:
  bool_tag(script_context *ctx, std::string const &script,
