...
namespace Noggit {
namespace Scripting {
void procedures::paint_texture(selection &sel, image const &img, int layer,
                               float pressure, float angle) {
  auto center = glm::vec3(sel.min().x + (sel.max().x - sel.min().x) / 2, 0,
                          sel.min().z + (sel.max().z - sel.min().z) / 2);
  ...
  for (auto &tex : sel.textures_raw()) {
    ...
    if (!(rel_x < -1 || rel_x > 1 || rel_z < -1 || rel_z > 1)) {
      ...
      if (img_x >= 0 && img_x < width && img_z >= 0 && img_z < height) {
        ...
        tex.set_alpha(
            layer, old + (img.get_red(img_x, img_z) * pressure * (1 - dist)));
      }
    }
  }
}
...
void register_procedures(script_context *state) {
  state->new_usertype<procedures>("procedures_class", "paint_texture",
                                  &procedures::paint_texture);
