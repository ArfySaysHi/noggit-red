...
vert::vert(script_context *ctx, MapChunk *chunk, int index)
    : script_object(ctx), _chunk(chunk), _index(index) {}

void vert::set_height(float value) { _chunk->mVertices[_index].y = value; }

void vert::add_height(float value) { _chunk->mVertices[_index].y += value; }

void vert::sub_height(float value) { _chunk->mVertices[_index].y -= value; }
...
void vert::set_water(int type, float height) {
  ...
  _chunk->liquid_chunk()->paintLiquid(
      get_pos(), 1, type, true, math::radians(0), math::radians(0), true,
      glm::vec3(0, height, 0), true, true, _chunk, 1);
}

void vert::set_hole(bool add) { _chunk->setHole(get_pos(), 1.f, false, add); }

glm::vec3 vert::get_pos() { return _chunk->mVertices[_index]; }

void vert::set_alpha(int index, float alpha) {
  if (index < 0 || index > 3) {
    throw script_exception("vert::set_alpha",
  ...
  for (auto iter = std::begin(tex_indices.indices);
       iter != std::end(tex_indices.indices); ++iter) {
    if (*iter == -1)
      break;
...
float vert::get_alpha(int index) {
  if (index < 0 || index > 3) {
    throw script_exception("vert::get_alpha",
                           std::string("invalid texture layer: ") +
                               std::to_string(index));
  }
  if (index == 0) {
...
sol::as_table_t<std::vector<tex>> vert::textures() {
  ...
  for (int i = 0; i < MAX_TEXUNITS_PER_VERT; ++i) {
    if (texture_index[_index].indices[i] == -1)
      break;
...
void register_vert(script_context *state) {
  state->new_usertype<vert>(
      "vert", "get_pos", &vert::get_pos, "set_height", &vert::set_height,
      "add_height", &vert::add_height, "sub_height", &vert::sub_height,
      "set_color", &vert::set_color, "get_color", &vert::get_color, "set_water",
      &vert::set_water, "set_hole", &vert::set_hole, "set_alpha",
      &vert::set_alpha, "get_alpha", &vert::get_alpha, "tex", &vert::textures,
