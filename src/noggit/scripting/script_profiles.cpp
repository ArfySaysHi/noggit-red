...
script_profiles::script_profiles(Noggit::Scripting::scripting_tool *tool)
  ...
  connect(_selection, QOverload<int>::of(&QComboBox::activated), this,
          [this](auto index) { select_profile_gui(index); });
  ...
  connect(_create_button, &QPushButton::released, this, [this]() {
    ...
    if (!(*json)[script_name].contains(newText.toStdString())) {
      if ((*json)[script_name].contains(_cur_profile)) {
        (*json)[script_name][newText.toStdString()] =
            (*json)[script_name][_cur_profile];
      }
    }
...
std::string script_profiles::get_cur_profile() { return _cur_profile; }

void script_profiles::clear() { _selection->clear(); }

void script_profiles::add_profile(std::string const &profile) {
  _selection->addItem(QString::fromStdString(profile));
}

int script_profiles::profile_count() { return _selection->count(); }
