...
std::filesystem::path get_writable_path(std::string const &caller,
  ...
  if (state->tool()
          ->get_noggit_settings()
          ->value("allow_scripts_write_any_file", false)
          .toBool()) {
    return canonical;
  }
  ...
  QMessageBox prompt;
  prompt.setText(
      std::string("A script wants to write to the file " + canonical.string())
          .c_str());
  prompt.setInformativeText(
      std::string("Do you want to allow the script to write to " +
                  canonical.string() + "?")
          .c_str());
  ...
  if (!answer) {
    throw script_exception(caller,
                           "No permission to write file " + canonical.string());
  }
  return *allowed_files.emplace(canonical).first;
}

void write_file(script_context *ctx, std::string const &path,
...
void append_file(script_context *ctx, std::string const &path,
                 std::string const &input) {
  auto writable_path = get_writable_path("append_file", ctx, path);
  mkdirs(writable_path.string());
  std::ofstream outfile;
  outfile.open(writable_path.string(),
               std::ios_base::app); // append instead of overwrite
  outfile << input;
}

bool path_exists(std::string const &path) { return fs::exists(path); }

void register_filesystem(script_context *state) {
  state->set_function(
      "write_file", [state](std::string const &path, std::string const &input) {
        write_file(state, path, input);
      });
  state->set_function("append_file", [state](std::string const &path,
                                             std::string const &input) {
