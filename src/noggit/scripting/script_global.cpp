...
void register_global(script_context *state) {
  ...
  state->set_function("add_m2", [global](std::string const &filename,
                                         glm::vec3 const &pos, float scale,
                                         glm::vec3 const &rotation) {
    // note: we set both min/max random scale and the normal scale parameter,
    // because noggit picks one based on random scale settings in the object
    // tool
    object_paste_params p;
    p.minScale = scale;
    p.maxScale = scale;
    global->get_view()->_world.get()->addM2(filename, pos, scale,
                                            math::degrees::vec3(rotation), &p);
  });

  state->set_function(
      "vec", [](float x, float y, float z) { return glm::vec3(x, y, z); });

  state->set_function("add_wmo", [global](std::string const &filename,
                                          glm::vec3 const &pos,
                                          glm::vec3 const &rotation) {
    global->get_view()->_world.get()->addWMO(filename, pos,
  ...
  state->set_function("get_area_id", [global](glm::vec3 const &pos) {
  ...
  state->set_function(
      "cam_yaw", [global]() { return global->get_view()->_camera.yaw()._; });

  state->set_function("holding_alt",
                      [global]() { return global->get_view()->_mod_alt_down; });
  ...
  state->set_function("holding_left_mouse",
                      [global]() { return global->get_view()->leftMouse; });

  state->set_function("holding_right_mouse",
                      [global]() { return global->get_view()->rightMouse; });
