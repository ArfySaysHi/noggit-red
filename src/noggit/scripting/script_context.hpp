...
template <typename T> class script_scoped_function {
public:
  script_scoped_function(script_context *lua, std::string const &name,
                         std::function<T> fun)
      : _lua(lua), _name(name) {
    lua->set_function(name, fun);
  }

  script_scoped_function() { _lua->set_function(_name, nullptr); }
