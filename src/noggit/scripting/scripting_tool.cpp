...
void scripting_tool::doReload() {
  ...
  try {
    std::string old_name =
        get_context() == nullptr ? "" : get_context()->get_selected_name();
...
scripting_tool::scripting_tool(QWidget *parent, MapView *view,
    ...
    : QWidget(parent), _cur_profile("Default"), _view(view),
  ...
  connect(_reload_button, &QPushButton::released, this,
          [this]() { doReload(); });
  ...
  connect(_selection, QOverload<int>::of(&QComboBox::activated), this,
          [this](auto index) {
            clearLog();
            change_script(index);
          });
...
scripting_tool::~scripting_tool() { get_settings()->save_json(); }
...
void scripting_tool::sendBrushEvent(glm::vec3 const &pos, float dt) {
  ...
  auto evt = std::make_shared<script_brush_event>(get_settings(), pos, dt);
  ...
  try {
    ...
    if (sel >= 0) {
      ...
      if (new_left) {
        if (!_last_left)
          brush->on_left_click.call_if_exists("brush_event", evt);
        else
          brush->on_left_hold.call_if_exists("brush_event", evt);
      } else {
        if (_last_left)
      ...
      if (new_right) {
        if (!_last_right)
          brush->on_right_click.call_if_exists("brush_event", evt);
        else
          brush->on_right_hold.call_if_exists("brush_event", evt);
      } else {
        if (_last_right)
    ...
    doReload();
    // TEMP: remove when exceptions are working
    if (std::string(e.what()).find("C++ exception") == 0 &&
        cur_exception.size() > 0) {
...
script_context *scripting_tool::get_context() { return _script_context.get(); }

MapView *scripting_tool::get_view() { return _view; }

void scripting_tool::resetLogScroll() {
  _log->verticalScrollBar()->setValue(0);
}

void scripting_tool::clearLog() { _log->clear(); }

void scripting_tool::clearDescription() { _description->clear(); }

script_settings *scripting_tool::get_settings() { return _settings; }

script_profiles *scripting_tool::get_profiles() { return _profiles; }

QSettings *scripting_tool::get_noggit_settings() { return _noggit_settings; }
} // namespace Scripting
} // namespace Noggit
