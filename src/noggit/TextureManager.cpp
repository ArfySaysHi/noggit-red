...
void TextureManager::report() {
  ...
  _.apply(
      [&](BlizzardArchive::Listfile::FileKey const &key, blp_texture const &) {
...
void TextureManager::unload_all(Noggit::NoggitRenderContext context) {
  _.context_aware_apply([&](BlizzardArchive::Listfile::FileKey const &,
                            blp_texture &blp_texture) { blp_texture.unload(); },
                        context);
  ...
  for (auto &pair : arrays_for_context) {
    gl.deleteTextures(static_cast<GLuint>(pair.second.arrays.size()),
                      pair.second.arrays.data());
  }
}

TexArrayParams &
TextureManager::get_tex_array(int width, int height, int mip_level,
                              Noggit::NoggitRenderContext context) {
  TexArrayParams &array_params =
      _tex_arrays[context][std::make_tuple(-1, width, height, mip_level)];
...
TexArrayParams &
TextureManager::get_tex_array(GLint compression, int width, int height,
                              int mip_level,
                              std::map<int, std::vector<uint8_t>> &comp_data,
                              Noggit::NoggitRenderContext context) {

  TexArrayParams &array_params =
      _tex_arrays[context]
                 [std::make_tuple(compression, width, height, mip_level)];
  ...
  if (!_compression_format) {
    ...
    for (int i = 0; i < _compressed_data.size(); ++i) {
      gl.compressedTexSubImage3D(
          ...
          static_cast<GLsizei>(_compressed_data[i].size()),
          _compressed_data[i].data());
...
void blp_texture::upload() {
  ...
  if (!_compression_format) {
    auto &params = TextureManager::get_tex_array(
        _width, _height, static_cast<int>(_data.size()), _context);
    ...
    auto &params = TextureManager::get_tex_array(
        _compression_format.value(), _width, _height,
        static_cast<int>(_compressed_data.size()), _compressed_data, _context);
    ...
    // LogDebug << "Mip level (compressed): " <<
    // std::to_string(_compressed_data.size()) << std::endl;
    _compressed_data.clear();
  }
...
void blp_texture::loadFromUncompressedData(BLPHeader const *lHeader,
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    if (lHeader->offsets[i] > 0 && lHeader->sizes[i] > 0) {
      buf =
          reinterpret_cast<unsigned char const *>(&lData[lHeader->offsets[i]]);
...
void blp_texture::loadFromCompressedData(BLPHeader const *lHeader,
  ...
  _compression_format =
      format == GL_COMPRESSED_RGB_S3TC_DXT1_EXT
          ? (lHeader->attr_1_alphadepth == 1 ? GL_COMPRESSED_RGBA_S3TC_DXT1_EXT
  ...
  for (int i = 0; i < 16; ++i) {
    ...
    // make sure the vector is of the right size, blizzard seems to fuck those
    // up for some small mipmaps
    int size = std::floor((width + 3) / 4) * std::floor((height + 3) / 4) *
               blocksizes[alpha_type];

    if (size < lHeader->sizes[i]) {
      LogDebug << "mipmap size mismatch in '" << _file_key.stringRepr() << "'"
               << std::endl;
      return;
    }
...
blp_texture::blp_texture(BlizzardArchive::Listfile::FileKey const &file_key,
                         Noggit::NoggitRenderContext context)
    : AsyncObject(file_key), _context(context) {}

void blp_texture::finishLoading() {
  bool exists =
      Noggit::Application::NoggitApplication::instance()->clientData()->exists(
          _file_key.filepath());
  if (!exists) {
    LogError << "file not found: '" << _file_key.stringRepr() << "'"
             << std::endl;
  }
  ...
  if (_file_key.filepath().starts_with("tileset/")) {
    ...
    spec_filename =
        _file_key.filepath().substr(0, _file_key.filepath().find_last_of(".")) +
        "_s.blp";
    has_specular = Noggit::Application::NoggitApplication::instance()
                       ->clientData()
                       ->exists(spec_filename);
  ...
  BlizzardArchive::ClientFile f(
      exists ? (has_specular ? spec_filename : _file_key.filepath())
             : "textures/shanecube.blp",
      Noggit::Application::NoggitApplication::instance()->clientData());
  if (f.isEof()) {
    finished = true;
    throw std::runtime_error("File " + _file_key.stringRepr() +
                             " does not exist");
  }
...
QPixmap *BLPRenderer::render_blp_to_pixmap(std::string const &blp_filename,
  ...
  if (result.isNull()) {
    throw std::runtime_error("failed rendering " + blp_filename + " to pixmap");
  }
...
void BLPRenderer::upload() {
  ...
  std::vector<glm::vec2> texcoords = {
      {0.f, 0.f}, {0.f, 1.0f}, {1.0f, 1.0f}, {1.0f, 0.f}};
  std::vector<std::uint16_t> indices = {0, 1, 2, 2, 3, 0};

  gl.bufferData<GL_ARRAY_BUFFER, glm::vec2>(vertices_vbo, vertices,
                                            GL_STATIC_DRAW);
  gl.bufferData<GL_ARRAY_BUFFER, glm::vec2>(texcoords_vbo, texcoords,
                                            GL_STATIC_DRAW);
  gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>(indices_vbo, indices,
                                                        GL_STATIC_DRAW);
...
scoped_blp_texture_reference::scoped_blp_texture_reference(
    ...
    : _blp_texture(
          other._blp_texture
              ? TextureManager::_.emplace(
                    other._blp_texture->file_key().filepath(), other._context)
              : nullptr),
      _context(other._context) {}

void scoped_blp_texture_reference::Deleter::operator()(
    blp_texture *texture) const {
  TextureManager::_.erase(texture->file_key().filepath(),
                          texture->getContext());
}
