...
template <>
inline glm::quat Conversion<packed_quaternion, glm::quat>::operator()(
    const packed_quaternion &value) {
  //! \todo Check if this is really correct.
  return glm::quat(
      static_cast<float>((value.w > 0 ? value.w - 32767 : value.w + 32767) /
                         32767.0f),
      static_cast<float>((value.x > 0 ? value.x - 32767 : value.x + 32767) /
                         32767.0f),
      static_cast<float>((value.y > 0 ? value.y - 32767 : value.y + 32767) /
                         32767.0f),
      static_cast<float>((value.z > 0 ? value.z - 32767 : value.z + 32767) /
                         32767.0f));
}
...
template <class AnimatedType, class DataType = AnimatedType> class M2Value {
private:
...
public:
    ...
    if (!timestampVector.empty()) {
      ...
      if (pos == timestampVector.size() - 1 ||
        ...
        case Animation::Interpolation::Type::LINEAR: {
          // result = math::interpolation::linear (percentage, dataVector[pos],
          // dataVector[pos + 1]);
          if constexpr (std::is_same_v<AnimatedType, glm::quat>) {
            result =
                glm::slerp(dataVector[pos], dataVector[pos + 1], percentage);
          } else {
        ...
        case Animation::Interpolation::Type::HERMITE: {
          result = math::interpolation::hermite(percentage, dataVector[pos],
                                                dataVector[pos + 1],
                                                inVector[pos], outVector[pos]);
        } break;
        }
      }
    }
  ...
  //! \todo Use a vector of BlizzardArchive::ClientFile& for the anim files
  //! instead for safety.
  M2Value(const AnimationBlock &animationBlock,
          const BlizzardArchive::ClientFile &file, int32_t *globalSequences,
    ...
    if (_globalSequenceID != NO_GLOBAL_SEQUENCE) {
      assert(_globalSequences && "Animation said to have global sequence, but "
                                 "pointer to global sequence data is nullptr");
    }
