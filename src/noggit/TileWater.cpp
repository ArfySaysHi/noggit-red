...
TileWater::TileWater(MapTile *pTile, float pXbase, float pZbase,
    ...
    : tile(pTile), _renderer(pTile), xbase(pXbase), zbase(pZbase),
      _extents{glm::vec3{pXbase, std::numeric_limits<float>::max(), pZbase},
               glm::vec3{pXbase + TILESIZE,
                         std::numeric_limits<float>::lowest(),
                         pZbase + TILESIZE}} {
  for (int z = 0; z < 16; ++z) {
    for (int x = 0; x < 16; ++x) {
      chunks[z][x] = std::make_unique<ChunkWater>(
          tile->getChunk(z, x), this, xbase + CHUNKSIZE * x,
          zbase + CHUNKSIZE * z, use_mclq_green_lava);
    }
  }
}

void TileWater::readFromFile(BlizzardArchive::ClientFile &theFile,
                             size_t basePos) {
  for (int z = 0; z < 16; ++z) {
    for (int x = 0; x < 16; ++x) {
...
void TileWater::recalcExtents() {
  _extents = {glm::vec3{xbase, std::numeric_limits<float>::max(), zbase},
              glm::vec3{xbase + TILESIZE, std::numeric_limits<float>::lowest(),
                        zbase + TILESIZE}};
