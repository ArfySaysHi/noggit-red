...
namespace Noggit {
class WorldModelInstancesStorage {
public:
  WorldModelInstancesStorage(World *world);

  WorldModelInstancesStorage() = delete;
  WorldModelInstancesStorage(WorldModelInstancesStorage const &) = delete;
  WorldModelInstancesStorage(WorldModelInstancesStorage &&) = delete;
  WorldModelInstancesStorage &
  operator=(WorldModelInstancesStorage const &) = delete;
  WorldModelInstancesStorage &operator=(WorldModelInstancesStorage &&) = delete;
  ...
  bool uid_duplicates_found() const { return _uid_duplicates_found.load(); }
...
public:
  template <typename Fun> void for_each_wmo_instance(Fun &&function) {
  ...
  template <typename Fun> void for_each_m2_instance(Fun &&function) {
    std::unique_lock<std::mutex> const lock(_mutex);
