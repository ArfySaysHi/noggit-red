...
TileRender::TileRender(MapTile *map_tile) : _map_tile(map_tile) {}
...
void TileRender::upload() {
  ...
  gl.bindBufferRange(GL_UNIFORM_BUFFER,
                     OpenGL::ubo_targets::CHUNK_INSTANCE_DATA,
                     _chunk_instance_data_ubo, 0,
                     sizeof(OpenGL::ChunkInstanceDataUniformBlock) * 256);
  gl.bufferData(GL_UNIFORM_BUFFER,
                sizeof(OpenGL::ChunkInstanceDataUniformBlock) * 256, NULL,
                GL_DYNAMIC_DRAW);
...
void TileRender::unload() {
  ...
  _map_tile->_chunk_update_flags =
      ChunkUpdateFlags::VERTEX | ChunkUpdateFlags::ALPHAMAP |
...
void TileRender::draw(OpenGL::Scoped::use_program &mcnk_shader,
                      const glm::vec3 &camera, bool show_unpaintable_chunks,
                      bool draw_paintability_overlay, bool is_selected) {
  ZoneScopedN(NOGGIT_CURRENT_FUNCTION);

  static constexpr unsigned NUM_SAMPLERS = 11;

  if (!_map_tile->finished.load()) [[unlikely]] {
    return;
  }
  ...
  if (_requires_paintability_recalc && draw_paintability_overlay &&
      show_unpaintable_chunks) [[unlikely]] {
    ...
    for (int j = 0; j < 16; ++j) {
      for (int i = 0; i < 16; ++i) {
        ...
        if (chunk->currently_paintable != !cant_paint) {
          ...
          _chunk_instance_data[i * 16 + j]
              .ChunkHoles_DrawImpass_TexLayerCount_CantPaint[3] = cant_paint;
  ...
  // run chunk updates. running this when splitdraw call detected unused sampler
  // configuration as well.
  if (_map_tile->_chunk_update_flags || is_selected != _selected ||
      need_paintability_update || _requires_sampler_reset ||
      _texture_not_loaded) {

    gl.bindBuffer(GL_UNIFORM_BUFFER, _chunk_instance_data_ubo);

    if (_requires_sampler_reset) [[unlikely]] {
      _draw_calls.clear();
      MapTileDrawCall &draw_call = _draw_calls.emplace_back();
      std::fill(draw_call.samplers.begin(), draw_call.samplers.end(), -1);
      draw_call.start_chunk = 0;
    ...
    for (int i = 0; i < 256; ++i) {
      ...
      _chunk_instance_data[i].ChunkXZ_TileXZ[2] =
          static_cast<int>(_map_tile->index.x);
      _chunk_instance_data[i].ChunkXZ_TileXZ[3] =
          static_cast<int>(_map_tile->index.z);
      ...
      if (flags & ChunkUpdateFlags::ALPHAMAP || _requires_sampler_reset ||
        ...
        if (!_split_drawcall &&
            !fillSamplers(chunk.get(), i,
                          static_cast<unsigned int>(_draw_calls.size() - 1))) {
      ...
      if (flags & ChunkUpdateFlags::VERTEX ||
          flags & ChunkUpdateFlags::NORMALS) {
        ...
        gl.texSubImage2D(
            GL_TEXTURE_2D, 0, 0, i, mapbufsize, 1, GL_RGBA, GL_FLOAT,
            _map_tile->_chunk_heightmap_buffer.data() + i * mapbufsize * 4);
      }
      ...
      if (flags & ChunkUpdateFlags::HOLES) {
        _chunk_instance_data[i]
            .ChunkHoles_DrawImpass_TexLayerCount_CantPaint[0] = chunk->holes;
      }

      if (flags & ChunkUpdateFlags::FLAGS) {
        _chunk_instance_data[i]
            .ChunkHoles_DrawImpass_TexLayerCount_CantPaint[1] =
            chunk->header_flags.flags.impass;
        ...
        for (int k = 0; k < chunk->texture_set->num(); ++k) {
          ...
          _chunk_instance_data[i].ChunkTexDoAnim[k] =
              flag_view->animation_enabled;
          _chunk_instance_data[i].ChunkTexAnimSpeed[k] =
              flag_view->animation_speed;
          _chunk_instance_data[i].ChunkTexAnimDir[k] =
              flag_view->animation_rotation;
        }

        _chunk_instance_data[i].ChunkTexDoAnim[1] =
            chunk->header_flags.flags.impass;
      }

      if (flags & ChunkUpdateFlags::AREA_ID) {
        _chunk_instance_data[i].AreaIDColor_Pad2_DrawSelection[0] =
            chunk->areaID;
      }
    ...
    if (_split_drawcall) {
      ...
      for (int i = 0; i < 256; ++i) {
        ...
        if (!fillSamplers(chunk.get(), i,
                          static_cast<unsigned int>(_draw_calls.size() - 1))) {
          MapTileDrawCall &previous_draw_call =
              _draw_calls[_draw_calls.size() - 1];
          unsigned new_start =
              previous_draw_call.start_chunk + previous_draw_call.n_chunks;

          MapTileDrawCall &new_draw_call = _draw_calls.emplace_back();
          std::fill(new_draw_call.samplers.begin(),
                    new_draw_call.samplers.end(), -1);
  ...
  if (_texture_not_loaded) [[unlikely]] {
    gl.bindBufferRange(GL_UNIFORM_BUFFER,
                       OpenGL::ubo_targets::CHUNK_INSTANCE_DATA,
                       _chunk_instance_data_ubo, 0,
                       sizeof(OpenGL::ChunkInstanceDataUniformBlock) * 256);
  ...
  for (auto &draw_call : _draw_calls) {
    ...
    bool is_lod = misc::dist(tile_center_x, tile_center_z, camera.x, camera.z) >
                  TILESIZE * 3;
    mcnk_shader.uniform("lod_level", int(is_lod));

    assert(draw_call.n_chunks <= 256);
    mcnk_shader.uniform("base_instance",
                        static_cast<int>(draw_call.start_chunk));
    ...
    if (is_lod) {
      gl.drawElementsInstanced(
          GL_TRIANGLES, 192, GL_UNSIGNED_SHORT,
          reinterpret_cast<void *>(768 * sizeof(std::uint16_t)),
          draw_call.n_chunks);
    } else {
...
void TileRender::uploadTextures() {
  ...
  gl.activeTexture(GL_TEXTURE0 + 0);
  gl.bindTexture(GL_TEXTURE_2D, _height_tex);
  gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, mapbufsize, 256, 0, GL_RGBA,
                GL_FLOAT, nullptr);
  ...
  gl.activeTexture(GL_TEXTURE0 + 2);
  gl.bindTexture(GL_TEXTURE_2D, _mccv_tex);
  gl.texImage2D(GL_TEXTURE_2D, 0, GL_RGB32F, mapbufsize, 256, 0, GL_RGB,
                GL_FLOAT, nullptr);
  ...
  gl.bindTexture(GL_TEXTURE_2D_ARRAY, _alphamap_tex);
  gl.texImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RGB, 64, 64, 256, 0, GL_RGB,
                GL_FLOAT, nullptr);
  ...
  gl.activeTexture(GL_TEXTURE0 + 3);
  gl.bindTexture(GL_TEXTURE_2D_ARRAY, _shadowmap_tex);
  gl.texImage3D(GL_TEXTURE_2D_ARRAY, 0, GL_RED, 64, 64, 256, 0, GL_RED,
...
void TileRender::doTileOcclusionQuery(
    OpenGL::Scoped::use_program &occlusion_shader) {
  ...
  occlusion_shader.uniform("aabb", _map_tile->_combined_extents.data(),
                           _map_tile->_combined_extents.size());
...
bool TileRender::getTileOcclusionQueryResult(glm::vec3 const &camera) {
  ...
  if (!_tile_occlusion_query_in_use) [[unlikely]] {
  ...
  GLint result;
  gl.getQueryObjectiv(_tile_occlusion_query, GL_QUERY_RESULT_AVAILABLE,
                      &result);
...
bool TileRender::fillSamplers(MapChunk *chunk, unsigned chunk_index,
                              unsigned int draw_call_index) {
  MapTileDrawCall &draw_call = _draw_calls[draw_call_index];

  _chunk_instance_data[chunk_index]
      .ChunkHoles_DrawImpass_TexLayerCount_CantPaint[2] =
      static_cast<int>(chunk->texture_set->num());
  ...
  for (int k = 0; k < chunk->texture_set->num(); ++k) {
    ...
    // :(. Extremely infrequent for terrain. Never for Blizzard terrain as their
    // tilesets use uniform BLP format per map.
    if (sampler_id < 0) [[unlikely]] {
      return false;
    }

    _chunk_instance_data[chunk_index].ChunkTextureSamplers[k] = sampler_id;
    _chunk_instance_data[chunk_index].ChunkTextureArrayIDs[k] =
        (*chunk->texture_set->getTextures())[k]->is_specular() ? tex_index
                                                               : -tex_index;
  }
...
void TileRender::initChunkData(MapChunk *chunk) {
  auto &chunk_render_instance =
      _chunk_instance_data[chunk->px * 16 + chunk->py];

  chunk_render_instance.ChunkHoles_DrawImpass_TexLayerCount_CantPaint[0] =
      chunk->holes;
  chunk_render_instance.ChunkHoles_DrawImpass_TexLayerCount_CantPaint[1] =
      chunk->header_flags.flags.impass;
  chunk_render_instance.ChunkHoles_DrawImpass_TexLayerCount_CantPaint[2] =
      static_cast<int>(chunk->texture_set->num());
