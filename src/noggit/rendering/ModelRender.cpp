...
ModelRender::ModelRender(Model *model) : _model(model) {}
...
void ModelRender::upload() {
  ...
  if (_model->animBones) {
    ...
    OpenGL::Scoped::buffer_binder<GL_TEXTURE_BUFFER> const binder(
        _bone_matrices_buffer);
    gl.bufferData(GL_TEXTURE_BUFFER,
                  _model->bone_matrices.size() * sizeof(glm::mat4x4), nullptr,
                  GL_STREAM_DRAW);
...

  {
    ...
    gl.bufferData(GL_ARRAY_BUFFER,
                  _model->_vertices.size() * sizeof(ModelVertex),
                  _model->_vertices.data(), GL_STATIC_DRAW);
  }

  {
    OpenGL::Scoped::buffer_binder<GL_ARRAY_BUFFER> const binder(_box_vbo);
    gl.bufferData(GL_ARRAY_BUFFER,
                  _vertex_box_points.size() * sizeof(glm::vec3),
  ...
  gl.bufferData(GL_ELEMENT_ARRAY_BUFFER,
                _model->_indices.size() * sizeof(uint16_t),
                _model->_indices.data(), GL_STATIC_DRAW);

  OpenGL::Scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> box_indices_binder(
      _box_indices_buffer);
  gl.bufferData(GL_ELEMENT_ARRAY_BUFFER, _box_indices.size() * sizeof(uint16_t),
                _box_indices.data(), GL_STATIC_DRAW);
...
void ModelRender::draw(glm::mat4x4 const &model_view, ModelInstance &instance,
                       OpenGL::Scoped::use_program &m2_shader,
                       OpenGL::M2RenderState &model_render_state,
                       math::frustum const &frustum, const float &cull_distance,
                       const glm::vec3 &camera, int animtime,
                       display_mode display, bool no_cull) {
  if (!_model->finishedLoading() || _model->loading_failed()) {
    return;
  }

  if (!no_cull && !instance.isInFrustum(frustum) &&
      !instance.isInRenderDist(cull_distance, camera, display)) {
    return;
  }
  ...
  if (_model->animated &&
      (!_model->animcalc || _model->_per_instance_animation)) {
...

  {
    ...
    m2_shader.attrib("bones_indices", 4, GL_UNSIGNED_BYTE, GL_FALSE,
                     sizeof(ModelVertex),
                     reinterpret_cast<void *>(sizeof(::glm::vec3) + 4));
    m2_shader.attrib("normal", 3, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
                     reinterpret_cast<void *>(sizeof(::glm::vec3) + 8));
    m2_shader.attrib("texcoord1", 2, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
                     reinterpret_cast<void *>(sizeof(::glm::vec3) * 2 + 8));
    m2_shader.attrib("texcoord2", 2, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
                     reinterpret_cast<void *>(sizeof(::glm::vec3) * 2 + 8 +
                                              sizeof(glm::vec2)));
  }
...
void ModelRender::draw(
    glm::mat4x4 const &model_view, std::vector<glm::mat4x4> const &instances,
    OpenGL::Scoped::use_program &m2_shader,
    OpenGL::M2RenderState &model_render_state, math::frustum const &frustum,
    const float &cull_distance, const glm::vec3 &camera, int animtime,
    bool all_boxes,
...
void ModelRender::drawParticles(glm::mat4x4 const &model_view,
  ...
  for (auto &p : _model->_particles) {
    p.draw(model_view, particles_shader, _transform_buffer,
...
void ModelRender::drawBox(OpenGL::Scoped::use_program &m2_box_shader,
  ...
  gl.drawElementsInstanced(
      GL_LINE_STRIP, static_cast<GLsizei>(_box_indices.size()),
      GL_UNSIGNED_SHORT, nullptr, static_cast<GLsizei>(box_count));
}
...
void ModelRender::setupVAO(OpenGL::Scoped::use_program &m2_shader) {
...

  {
    OpenGL::Scoped::buffer_binder<GL_ARRAY_BUFFER> const binder(
        _vertices_buffer);
    m2_shader.attrib("pos", 3, GL_FLOAT, GL_FALSE, sizeof(ModelVertex), 0);
    m2_shader.attribi("bones_weight", 4, GL_UNSIGNED_BYTE, sizeof(ModelVertex),
                      reinterpret_cast<void *>(sizeof(::glm::vec3)));
    m2_shader.attribi("bones_indices", 4, GL_UNSIGNED_BYTE, sizeof(ModelVertex),
                      reinterpret_cast<void *>(sizeof(::glm::vec3) + 4));
    m2_shader.attrib("normal", 3, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
                     reinterpret_cast<void *>(sizeof(::glm::vec3) + 8));
    m2_shader.attrib("texcoord1", 2, GL_FLOAT, GL_FALSE, sizeof(ModelVertex),
...

  {
    OpenGL::Scoped::buffer_binder<GL_ARRAY_BUFFER> const transform_binder(
        _transform_buffer);
    gl.bufferData(GL_ARRAY_BUFFER, 10 * sizeof(::glm::mat4x4), nullptr,
                  GL_DYNAMIC_DRAW);
    m2_shader.attrib("transform", 0, 1);
  }
...
void ModelRender::fixShaderIdBlendOverride() {
  for (auto &pass : _render_passes) {
    ...
    if (pass.texture_coord_combo_index + pass.texture_count - 1 >=
        _model->_texture_unit_lookup.size()) {
    ...
    if (!blend_mode_override) {
      uint16_t texture_unit_lookup =
          _model->_texture_unit_lookup[pass.texture_coord_combo_index];
    ...
    for (auto &pass : _render_passes) {
      ...
      if (pass.material_layer == 0) {
        if (pass.texture_count >= 1 &&
            _model->_render_flags[pass.renderflag_index].blend == 0) {
      ...
      bool xor_unlit =
          ((_model->_render_flags[pass.renderflag_index].flags.unlit ^
            _model->_render_flags[first_pass->renderflag_index].flags.unlit) &
           1) == 0;

      if ((some_flags & 0xFF) == 1) {
        if ((_model->_render_flags[pass.renderflag_index].blend == 1 ||
             _model->_render_flags[pass.renderflag_index].blend == 2) &&
            pass.texture_count == 1 && xor_unlit &&
            pass.texture_combo_index == first_pass->texture_combo_index) {
          if (_model->_transparency_lookup[pass.transparency_combo_index] ==
              _model->_transparency_lookup[first_pass
      ...
      if ((some_flags & 0xFF) < 2) {
        if ((_model->_render_flags[pass.renderflag_index].blend == 0) &&
      ...
      if ((some_flags >> 8) != 0) {
        if ((some_flags >> 8) == 1) {
          if (((_model->_render_flags[pass.renderflag_index].blend != 4) &&
               (_model->_render_flags[pass.renderflag_index].blend != 6)) ||
              (pass.texture_count != 1) || (texture_unit_lookup >= 0)) {
            some_flags &= 0xFF00;
          }
          // tod
          else if ((_model->_transparency_lookup.size() >
                        pass.transparency_combo_index &&
                    _model->_transparency_lookup.size() >
                        first_pass->transparency_combo_index) &&
                   _model->_transparency_lookup
                           [pass.transparency_combo_index] ==
                       _model->_transparency_lookup
                           [first_pass->transparency_combo_index]) {
            pass.shader_id = 0x8000;
            first_pass->shader_id =
                _model->_render_flags[pass.renderflag_index].blend != 4
                    ? 0xE
                    : 0x8002;
          ...
          if (((_model->_render_flags[pass.renderflag_index].blend != 2) &&
               (_model->_render_flags[pass.renderflag_index].blend != 1)) ||
              (pass.texture_count != 1) || xor_unlit ||
              ((pass.texture_combo_index & 0xff) !=
               (first_pass->texture_combo_index & 0xff))) {
            some_flags &= 0xFF00;
          } else if (_model->_transparency_lookup
                         [pass.transparency_combo_index] ==
                     _model->_transparency_lookup
                         [first_pass->transparency_combo_index]) {
            ...
            pass.shader_id = 0x8000;
            first_pass->shader_id =
                ((first_pass->shader_id == 0x8002 ? 2 : 0) - 0x7FFF) & 0xFFFF;
...
void ModelRender::initRenderPasses(ModelView const *view,
                                   ModelTexUnit const *tex_unit,
                                   ModelGeoset const *model_geosets) {
...
void ModelRender::updateBoneMatrices() {
  {
    OpenGL::Scoped::buffer_binder<GL_TEXTURE_BUFFER> const binder(
        _bone_matrices_buffer);
    gl.bufferSubData(GL_TEXTURE_BUFFER, 0,
                     _model->bone_matrices.size() * sizeof(glm::mat4x4),
                     _model->bone_matrices.data());
  }
}
...
bool ModelRenderPass::prepareDraw(OpenGL::Scoped::use_program &m2_shader,
                                  Model *m,
                                  OpenGL::M2RenderState &model_render_state) {
  ...
  if (color_index != -1 && m->_colors[color_index].color.uses(0)) {
    ::glm::vec3 c(m->_colors[color_index].color.getValue(0, m->_anim_time,
                                                         m->_global_animtime));
  ...
  if (transparency_combo_index != 0xFFFF &&
      transparency_combo_index < m->_transparency_lookup.size()) {
    auto &transparency(
        m->_transparency[m->_transparency_lookup[transparency_combo_index]]
            .trans);
    if (transparency.uses(0)) {
      mesh_color.w = mesh_color.w * transparency.getValue(0, m->_anim_time,
                                                          m->_global_animtime);
    }
  }
...
void ModelRenderPass::bindTexture(size_t index, Model *m,
                                  OpenGL::M2RenderState &model_render_state,
                                  OpenGL::Scoped::use_program &m2_shader) {
