...
WorldRender::WorldRender(World *world)
    : BaseRender(), _world(world), _liquid_texture_manager(world->_context),
      _view_distance(
          world->_settings->value("view_distance", 1000.f).toFloat()),
      _cull_distance(0.f) {}

void WorldRender::draw(
    glm::mat4x4 const &model_view, glm::mat4x4 const &projection,
    glm::vec3 const &cursor_pos, float cursorRotation,
    glm::vec4 const &cursor_color, CursorType cursor_type, float brush_radius,
    bool show_unpaintable_chunks, bool draw_only_inside_light_sphere,
    bool draw_wireframe_light_sphere, float alpha_light_sphere,
    float inner_radius_ratio, glm::vec3 const &ref_pos, float angle,
    float orientation, bool use_ref_pos, bool angled_mode,
    bool draw_paintability_overlay, editing_mode terrainMode,
    glm::vec3 const &camera_pos, bool camera_moved, bool draw_mfbo,
    bool draw_terrain, bool draw_wmo, bool draw_water, bool draw_wmo_doodads,
    bool draw_models, bool draw_model_animations, bool draw_models_with_box,
    bool draw_hidden_models, MinimapRenderSettings *minimap_render_settings,
    bool draw_fog, eTerrainType ground_editing_brush, int water_layer,
    display_mode display, bool draw_occlusion_boxes, bool minimap_render,
  ...
  if (!minimap_render && display == display_mode::in_3D) {
          ...
          [&](WMOInstance &wmo) {
            if (wmo.wmo->finishedLoading() && wmo.wmo->skybox) {
              ...
              hadSky = wmo.wmo->renderer()->drawSkybox(
                  ...
                  _world->animtime, draw_model_animations, wmo.extents[0],
                  wmo.extents[1], wmo.group_extents);
            }
          },
          [&]() { return hadSky; });
    }
    ...
    if (!hadSky) {
      _skies->draw(model_view, projection, camera_pos, m2_shader, frustum,
                   _cull_distance, _world->animtime, _outdoor_light_stats);
    }
  }
  ...
  if (!_world->mapIndex.hasAGlobalWMO() && draw_fog && draw_terrain) {
    ...
    _horizon_render->draw(model_view, projection, &_world->mapIndex,
                          _skies->color_set[FOG_COLOR], _cull_distance, frustum,
                          camera_pos, display);
  }
  ...
  if (draw_terrain) {
...

    {
      ...
      for (auto &pair : _world->_loaded_tiles_buffer) {
        ...
        tile->renderer()->draw(mcnk_shader, camera_pos, show_unpaintable_chunks,
  ...
  if (terrainMode == editing_mode::object &&
    ...
    _sphere_render.draw(mvp, _world->_multi_select_pivot.value(), cursor_color,
  ...
  for (auto &pair : _world->_loaded_tiles_buffer) {
    ...
    for (auto &pair : tile->getObjectInstances()) {
      if (pair.second[0]->which() == eMODEL) {
        if (!draw_models &&
            !(minimap_render && minimap_render_settings->use_filters))
          continue;
        ...
        for (auto &instance : pair.second) {
          ...
          if (tile->renderer()->objectsFrustumCullTest() > 1 ||
              frustum.intersects(wmo_instance->extents[1],
                                 wmo_instance->extents[0])) {
      ...
      for (auto &instance : wmos_to_draw) {
        ...
        if (draw_hidden_models || !is_hidden) {
          instance->draw(
              ...
              camera_pos, is_hidden, draw_wmo_doodads, draw_fog,
              _world->current_selection(), _world->animtime, _skies->hasSkies(),
              display, disable_cull, draw_wmo_exterior
        ...
        for (auto &pair : models_to_draw) {
          ...
          if (draw_hidden_models || !pair.first->is_hidden()) {
            pair.first->renderer()->draw(
                ...
                draw_models_with_box, model_boxes_to_draw, display);
    ...
    for (auto &selection : _world->current_selection()) {
      if (selection.index() == eEntry_Object) {
        ...
        if (model->isInFrustum(frustum) &&
            model->isInRenderDist(_cull_distance, camera_pos, display)) {
          bool is_selected = false;
          /*
          auto id = model->uid;
          bool const is_selected = _world->current_selection().size() > 0 &&
              std::find_if(_world->current_selection().begin(),
          _world->current_selection().end(), [id](selection_type type)
                  {
                      return var_type(type) == typeid(selected_object_type)
                          && std::get<selected_object_type>(type)->which() ==
          SceneObjectTypes::eMODEL
                          &&
          static_cast<ModelInstance*>(std::get<selected_object_type>(type))->uid
          == id;
                  }) != _world->current_selection().end();*/

          model->draw_box(model_view, projection,
                          is_selected); // make optional!
        }
      }
    }
  }
  ...
  if (draw_water) {
    ...
    for (auto &pair : _world->_loaded_tiles_buffer) {
      ...
      tile->Water.renderer()->draw(frustum, camera_pos, camera_moved,
                                   water_shader, _world->animtime, water_layer,
                                   display, &_liquid_texture_manager);
    }
  ...
  if (terrainMode == editing_mode::light) {
    ...
    for (Sky &sky : skies()->skies) {
          ...
          _cull_distance) // TODO: frustum cull here
      {
        ...
        _sphere_render.draw(mvp, sky.pos, ambient, sky.r1, 32, 18,
                            alpha_light_sphere, false,
                            draw_wireframe_light_sphere);
    ...
    if (CurrentSky && draw_only_inside_light_sphere) {
      glm::vec4 diffuse = {
          CurrentSky->colorFor(LIGHT_GLOBAL_DIFFUSE, CurrenTime), 1.f};
      glm::vec4 ambient = {
          CurrentSky->colorFor(LIGHT_GLOBAL_AMBIENT, CurrenTime), 1.f};

      _sphere_render.draw(mvp, CurrentSky->pos, ambient, CurrentSky->r1, 32, 18,
                          alpha_light_sphere, false,
                          draw_wireframe_light_sphere);
      _sphere_render.draw(mvp, CurrentSky->pos, diffuse, CurrentSky->r2, 32, 18,
                          alpha_light_sphere, false,
                          draw_wireframe_light_sphere);
    }
  }
...
void WorldRender::drawMinimap(MapTile *tile, glm::mat4x4 const &model_view,
  ...
  draw(model_view, projection, glm::vec3(), 0, glm::vec4(), CursorType::NONE,
       0.f, false, false, false, 0.3f, 0.f, glm::vec3(), 0.f, 0.f, false, false,
       false, editing_mode::minimap, camera_pos, true, false, true,
       settings->draw_wmo, settings->draw_water, false, settings->draw_m2,
       false, false, true, settings, false, eTerrainType::eTerrainType_Linear,
       0, display_mode::in_3D, false, true);
...
bool WorldRender::saveMinimap(TileIndex const &tile_idx,
  ...
  if (mTile) {
    ...
    std::string map_name =
        gMapDB.getByID(_world->mapIndex._map_id).getString(MapDB::InternalName);

    auto sstream = std::stringstream();
    sstream << map_name << "/map" << std::setfill('0') << std::setw(2)
            << tile_idx.x << "_" << std::setfill('0') << std::setw(2)
            << tile_idx.z << ".blp";
