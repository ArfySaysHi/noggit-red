...
class Grid {
public:
  void draw(glm::mat4x4 const &mvp, glm::vec3 const &pos,
...
public:
  void draw(glm::mat4x4 const &mvp, glm::vec3 const &pos,
            glm::vec4 const &color, float radius, int longitude = 32,
            int latitude = 18, float alpha = 1.f, bool wireframe = false,
            bool both = false);
  void unload();
...
public:
  void draw(glm::mat4x4 const &mvp, glm::vec3 const &pos,
            float radius // radius of the biggest circle fitting inside the
                         // square drawn
            ,
...
{
public:
    void draw(glm::mat4x4 const& mvp, glm::vec3 const& pos, const glm::vec4
color, float radius, int precision, World* world, int height = 10); void
unload();
...
public:
  void initSpline();
  void draw(glm::mat4x4 const &mvp, std::vector<glm::vec3> const points,
...
private:
  ...
  glm::vec3 interpolate(float t, glm::vec3 p0, glm::vec3 p1, glm::vec3 m0,
                        glm::vec3 m1);

  int _indice_count = 0;

  void setup_shader(std::vector<glm::vec3> vertices,
