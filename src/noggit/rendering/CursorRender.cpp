...
namespace Noggit {
void CursorRender::draw(Mode cursor_mode, glm::mat4x4 const &mvp,
                        glm::vec4 color, glm::vec3 const &pos, float radius,
  ...
  gl.drawElements(GL_LINES, _indices_count[cursor_mode], GL_UNSIGNED_SHORT,
                  nullptr);
...
void CursorRender::create_circle_buffer(OpenGL::Scoped::use_program &shader) {
  ...
  gl.bufferData<GL_ARRAY_BUFFER>(_vbos[id * 2],
...
void CursorRender::create_square_buffer(OpenGL::Scoped::use_program &shader) {
  std::vector<glm::vec3> vertices = {{-0.5f, 0.f, -0.5f},
                                     {0.5f, 0.f, -0.5f},
                                     {0.5f, 0.f, 0.5f},
                                     {-0.5f, 0.f, 0.5f}};
...
void CursorRender::create_cube_buffer(OpenGL::Scoped::use_program &shader) {
  ...
  std::vector<std::uint16_t> indices = {0, 1, 1, 2, 2, 3, 3, 0, 0, 4, 1, 5,
