...
void WireBox::draw(glm::mat4x4 const &model_view, glm::mat4x4 const &projection,
                   glm::mat4x4 const &transform, glm::vec4 const &color,
                   glm::vec3 const &min_point, glm::vec3 const &max_point) {
...
void WireBox::setup_buffers() {
  _program.reset(new OpenGL::program(
      {{GL_VERTEX_SHADER, OpenGL::shader::src_from_qrc("wire_box_vs")},
  ...
  OpenGL::Scoped::buffer_binder<GL_ELEMENT_ARRAY_BUFFER> const index_buffer(
      _indices);
  gl.bufferData(GL_ELEMENT_ARRAY_BUFFER,
                indices.size() * sizeof(*indices.data()), indices.data(),
                GL_STATIC_DRAW);
...
void Grid::draw(glm::mat4x4 const &mvp, glm::vec3 const &pos,
                glm::vec4 const &color, float radius) {
  if (!_buffers_are_setup) {
    setup_buffers();
  }
  ...
  OpenGL::Scoped::vao_binder const _(_vao[0]);
  gl.drawElements(GL_LINES, _indices_vbo, _indice_count, GL_UNSIGNED_SHORT,
                  nullptr);
}
...
void Grid::setup_buffers() {
  ...
  _program.reset(new OpenGL::program(
      {{GL_VERTEX_SHADER, OpenGL::shader::src_from_qrc("grid_vs")},
       {GL_FRAGMENT_SHADER, OpenGL::shader::src_from_qrc("grid_fs")}}));
...
void Sphere::draw(glm::mat4x4 const &mvp, glm::vec3 const &pos,
                  glm::vec4 const &color, float radius, int longitude,
                  int latitude, float alpha, bool wireframe, bool drawBoth) {
  if (!_buffers_are_setup) {
  ...
  OpenGL::Scoped::vao_binder const _(_vao[0]);
  if (drawBoth) {
    gl.drawElements(GL_TRIANGLES, _indices_vbo, _indice_count,
                    GL_UNSIGNED_SHORT, nullptr);
    sphere_shader.uniform("color", glm::vec4(1.f, 1.f, 1.f, 1.f));
    gl.drawElements(GL_LINE_STRIP, _indices_vbo, _indice_count,
                    GL_UNSIGNED_SHORT, nullptr);
    return;
  }
...
void Sphere::setup_buffers(int longitude, int latitude) {
  ...
  _program.reset(new OpenGL::program(
      {{GL_VERTEX_SHADER, OpenGL::shader::src_from_qrc("sphere_vs")},
       {GL_FRAGMENT_SHADER, OpenGL::shader::src_from_qrc("sphere_fs")}}));
...
void Square::draw(glm::mat4x4 const &mvp, glm::vec3 const &pos, float radius,
...
void Square::setup_buffers() {
  ...
  _program.reset(new OpenGL::program(
      {{GL_VERTEX_SHADER, OpenGL::shader::src_from_qrc("square_vs")},
       {GL_FRAGMENT_SHADER, OpenGL::shader::src_from_qrc("square_fs")}}));
...
/*void Cylinder::draw(glm::mat4x4 const& mvp, glm::vec3 const& pos, const
glm::vec4 color, float radius, int precision, World* world, int height)
{
    if (!_buffers_are_setup)
    {
...
void Cylinder::setup_buffers(int precision, World* world, int height)
{
    ...
    for (int i = 0; i < 1; i++)
    {
        for (int j = 0; j < precision; j++)
        {
            ...
            indices[6 * (i * precision + j) + 2] = (i + 1) * (precision + 1) +
j; indices[6 * (i * precision + j) + 3] = i * (precision + 1) + j + 1; indices[6
* (i * precision + j) + 4] = (i + 1) * (precision + 1) + j + 1; indices[6 * (i *
precision + j) + 5] = (i + 1) * (precision + 1) + j;
        }
    }
    ...
    gl.bufferData<GL_ARRAY_BUFFER, glm::vec3> (_vertices_vbo, vertices,
GL_STATIC_DRAW); gl.bufferData<GL_ELEMENT_ARRAY_BUFFER, std::uint16_t>
(_indices_vbo, indices, GL_STATIC_DRAW);
...

    {
        OpenGL::Scoped::vao_binder const _ (_vao[0]);
        OpenGL::Scoped::buffer_binder<GL_ARRAY_BUFFER> const vertices_binder
(_vertices_vbo); shader.attrib("position", 3, GL_FLOAT, GL_FALSE, 0, 0);
        indices_binder.bind();
    }
...
void Line::initSpline() {
  draw(glm::mat4x4{}, std::vector<glm::vec3>{{}, {}}, glm::vec4{}, false);
}

void Line::draw(glm::mat4x4 const &mvp, std::vector<glm::vec3> const points,
                glm::vec4 const &color, bool spline) {
  if (points.size() < 2)
    return;
  ...
  OpenGL::Scoped::vao_binder const _(_vao[0]);
  gl.drawElements(GL_LINE_STRIP, _indices_vbo, _indice_count, GL_UNSIGNED_SHORT,
                  nullptr);
}
...
void Line::setup_buffers_interpolated(std::vector<glm::vec3> const points) {
  ...
  for (int i = 1; i < tempPoints.size() - 2; i++) {
    ...
    glm::vec3 m1((p2.x - p0.x) / s, (p2.y - p0.y) / s, (p2.z - p0.z) / s);

    glm::vec3 m2((p3.x - p1.x) / s, (p3.y - p1.y) / s, (p3.z - p1.z) / s);
...
glm::vec3 Line::interpolate(float t, glm::vec3 p0, glm::vec3 p1, glm::vec3 m0,
                            glm::vec3 m1) {
...
void Line::setup_shader(std::vector<glm::vec3> vertices,
                        std::vector<std::uint16_t> indices) {
  _indice_count = (int)indices.size();
  _program.reset(new OpenGL::program(
