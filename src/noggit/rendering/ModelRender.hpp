...
enum class texture_unit_lookup : int { environment, t1, t2, none };
...
struct ModelRenderPass : ModelTexUnit {
  ...
  void afterDraw();
  void bindTexture(size_t index, Model *m,
                   OpenGL::M2RenderState &model_render_state,
  ...
  bool operator<(const ModelRenderPass &m) const {
    if (priority_plane < m.priority_plane) {
      ...
      return blend_mode == m.blend_mode ? (ordering_thingy < m.ordering_thingy)
...
public:
  ...
  void draw(glm::mat4x4 const &model_view, ModelInstance &instance,
            OpenGL::Scoped::use_program &m2_shader,
            OpenGL::M2RenderState &model_render_state,
            math::frustum const &frustum, const float &cull_distance,
            const glm::vec3 &camera, int animtime, display_mode display,
            bool no_cull = false);
  ...
  void draw(glm::mat4x4 const &model_view,
            ...
            OpenGL::M2RenderState &model_render_state,
            math::frustum const &frustum, const float &cull_distance,
            const glm::vec3 &camera, int animtime, bool all_boxes,
            std::unordered_map<Model *, std::size_t> &model_boxes_to_draw,
            display_mode display, bool no_cull = false);
  ...
  void drawBox(OpenGL::Scoped::use_program &m2_box_shader,
  ...
  void initRenderPasses(ModelView const *view, ModelTexUnit const *tex_unit,
...
private:
  ...
  std::vector<uint16_t> const _box_indices = {5, 7, 3, 2, 0, 1, 3, 1,
                                              5, 4, 0, 4, 6, 2, 6, 7};
