...
WMOInstance::WMOInstance(BlizzardArchive::Listfile::FileKey const &file_key,
    ...
    : SceneObject(SceneObjectTypes::eWMO, context), wmo(file_key, context),
      mFlags(d->flags), mUnknown(d->unknown), mNameset(d->nameSet),
...
WMOInstance::WMOInstance(BlizzardArchive::Listfile::FileKey const &file_key,
                         Noggit::NoggitRenderContext context)
    : SceneObject(SceneObjectTypes::eWMO, context), wmo(file_key, context),
      mFlags(0), mUnknown(0), mNameset(0), _doodadset(0) {
  change_doodadset(_doodadset);
...
void WMOInstance::draw(OpenGL::Scoped::use_program &wmo_shader,
                       ...
                       bool draw_doodads, bool draw_fog,
                       std::vector<selection_type> selection, int animtime,
                       bool world_has_skies, display_mode display, bool no_cull,
                       bool draw_exterior) {
