...
class frustum {
  ...
  public:
    ...
    plane(glm::vec4 const &vec)
        : _normal(glm::vec3(vec.x, vec.y, vec.z)), _distance(vec.w) {
      // normalize();
    }
...
public:
  ...
  bool intersects(const glm::vec3 &v1, const glm::vec3 &v2) const;
  bool intersectsSphere(const glm::vec3 &position, const float &radius) const;
};
} // namespace math
