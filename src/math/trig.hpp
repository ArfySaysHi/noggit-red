...
struct degrees {
  ...
  inline degrees operator+(const degrees &v) const { return degrees(_ + v._); }

  inline degrees operator-(const degrees &v) const { return degrees(_ - v._); }

  inline degrees operator-() const { return degrees(-_); }

  inline degrees &operator+=(const degrees &v) { return *this = *this + v; }
...
inline bool is_inside_of(const glm::vec3 &pos, const glm::vec3 &a,
                         const glm::vec3 &b) {
  return a.x < pos.x && b.x > pos.x && a.y < pos.y && b.y > pos.y &&
         a.z < pos.z && b.z > pos.z;
}
} // namespace math
