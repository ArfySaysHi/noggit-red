...
bool frustum::intersects(
    const std::array<glm::vec3, 8> &intersect_points) const {
  for (auto const &plane : _planes) {
...
bool frustum::intersects(const glm::vec3 &v1, const glm::vec3 &v2) const {
  for (auto const &plane : _planes) {
...
bool frustum::intersectsSphere(const glm::vec3 &position,
  ...
  for (auto const &plane : _planes) {
    const float distance =
        glm::dot(plane.normal(), position + plane.distance());
    if (distance < -radius) {
      return false;
