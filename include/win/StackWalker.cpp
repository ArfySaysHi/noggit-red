...
public:
  ...
  BOOL Init(LPCSTR szSymPath) {
    ...
    if (m_hDbhHelp ==
        nullptr) // if not already loaded, try to load a default-one
    ...
    if (pSC == nullptr || pSFTA == nullptr || pSGMB == nullptr ||
        pSGMI == nullptr || pSGO == nullptr || pSGSFA == nullptr ||
        pSI == nullptr || pSSO == nullptr || pSW == nullptr ||
...
#pragma pack(pop)
  ...
  BOOL GetModuleListPSAPI(HANDLE hProcess) {
    ...
    if ((pEPM == nullptr) || (pGMFNE == nullptr) || (pGMBN == nullptr) ||
        (pGMI == nullptr)) {
...
#error "Platform not supported!"
  ...
  for (frameNum = 0;; ++frameNum) {
    ...
    if (s.AddrPC.Offset != 0) {
      ...
      if (this->m_sw->pSGLFA !=
          nullptr) { // yes, we have SymGetLineFromAddr64()
...
void StackWalker::OnOutput(LPCSTR buffer) { LogError << buffer << std::endl; }
